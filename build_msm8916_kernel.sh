...
if [ "$BUILD_COMMAND" == "a3u_eur" ]; then
...
elif [ "$BUILD_COMMAND" == "e5_chnzt" ]; then
	PRODUCT_NAME=e5ltezt;
	SIGN_MODEL=SM-E500YZ_SEA_ZT_ROOT0
	...
	SIGN_MODEL=
elif [ "$BUILD_COMMAND" == "milletvewifi_eur" ]; then
	PRODUCT_NAME=milletvewifi;
	SIGN_MODEL=
elif [ "$BUILD_COMMAND" == "matissevewifi_eur" ]; then
...
SECFUNC_PRINT_HELP()
{
	...
	echo "	for MILLETVEWIFI_EUR_OPEN use milletvewifi_eur"
	echo "	for MATISSEVEWIFI_EUR_OPEN use matissevewifi_eur"
	echo "  \$2 : "
