...
#include <linux/clk.h>/* [BUFFIX]-Mod- by TCTNB.XQJ, PR-804255, 2014/10/29,add nfc clock control*/
...
#endif
/* [BUFFIX]-Add-bgein  by TCTNB.XQJ, PR-804255, 2014/10/29,add nfc clock control*/
      struct	clk		*s_clk;
       bool			clk_run;
...
{
//[BUGFIX]-Add by TCTSH.wenjing.xi,01/22/2015,PR-896123,[NFC]Can't do card emulation if you hotplug the Type B/A+B NFC SIM card
#if 0
	struct i2c_client *client;
	int ret;
	int i;
...
//[BUGFIX]-Add by TCTSH.wenjing.xi,01/22/2015,PR-896123,[NFC]Can't do card emulation if you hotplug the Type B/A+B NFC SIM card
		...
		if ((bcm2079x_dev->client->flags & I2C_CLIENT_TEN) != I2C_CLIENT_TEN && bcm2079x_dev->error_write == 0) {
			set_client_addr(bcm2079x_dev, 0x1FF);
			ret = i2c_master_send(bcm2079x_dev->client, tmp, count);
            if (ret != count) {
...
/* [BUFFIX]-Add-bgein  by TCTNB.XQJ, PR-804255, 2014/10/29,add nfc clock control*/
/*
	Routine to Select clocks,code refer to Nfc-nci.c
*/
static int bcm2079x_clock_select(struct bcm2079x_dev *bcm2079x_dev)
{
	int r = 0;

	if (!strcmp(bcm2079x_dev->clk_src_name, "BB_CLK2")) {//TO now is,code is only for this clock,
		bcm2079x_dev->s_clk  =
			clk_get(&bcm2079x_dev->client->dev, "ref_clk");
		if (bcm2079x_dev->s_clk == NULL)
			goto err_invalid_dis_gpio;
	} else {
		bcm2079x_dev->s_clk = NULL;
		goto err_invalid_dis_gpio;
	}
	if (bcm2079x_dev->clk_run == false) {
		/* Set clock rate */
		r = clk_prepare_enable(bcm2079x_dev->s_clk);
		if (r)
...
static int bcm2079x_clock_deselect(struct bcm2079x_dev *bcm2079x_dev)
{
	int r = -1;
	if (bcm2079x_dev->s_clk != NULL) {
		if (bcm2079x_dev->clk_run == true) {
...
/* [BUFFIX]-Add-bgein  by TCTNB.XQJ, PR-804255, 2014/10/29,add nfc clock control*/
		gpio_set_value(bcm2079x_dev->en_gpio, arg);
              if(arg==1)
              {
                  r = bcm2079x_clock_select(bcm2079x_dev);
                  return r;
              }
             else
              {
                   r = bcm2079x_clock_deselect(bcm2079x_dev);
                   return r;
              }
/* [BUFFIX]-Add-end by XQJ */
		break;
...
static int nfc_bcm_parse_dt(struct device *dev, struct bcm2079x_platform_data *pdata)
{	
      	const char *clk_src_name;
       unsigned int clkreq_gpio;
	struct regulator *uim_vdd;  
       int r =0;
	struct device_node *np = dev->of_node;

       uim_vdd = regulator_get(dev, "uim");	
	if (!IS_ERR(uim_vdd)) {
		r = regulator_enable(uim_vdd);
		if (r)
			printk("NFC: uim_vdd-regulator could not enable  !\n");
		else
			printk("NFC: uim_vdd-regulator enable  !\n");
	}

	pdata->irq_gpio = of_get_named_gpio(np, "nfc,irq-gpio", 0);
	if ((!gpio_is_valid(pdata->irq_gpio)))
		return -EINVAL;
	pdata->wake_gpio = of_get_named_gpio(np, "nfc,wake_gpio", 0);
	if ((!gpio_is_valid(pdata->wake_gpio)))
		return -EINVAL;

	pdata->en_gpio = of_get_named_gpio(np, "nfc,en-gpio", 0);
	if ((!gpio_is_valid(pdata->en_gpio)))
		return -EINVAL;
       r = of_property_read_string(np, "qcom,clk-src", &pdata->clk_src_name);/* [BUFFIX]-mod-  by TCTNB.XQJ, PR-804255, 2014/10/29,add nfc clock control*/
	clk_src_name= pdata->clk_src_name;
	printk("[NFC]:clk_src_name:%s\n", pdata->clk_src_name);
       if (strcmp(clk_src_name, "GPCLK2")) {
		clkreq_gpio = of_get_named_gpio(np, "nfc,clk-gpio", 0);
		printk("[NFC]:clk_src_name:%d .\n", clkreq_gpio);
		if (gpio_is_valid(clkreq_gpio)) {
			r = gpio_request(clkreq_gpio,	"nfc_clkreq_gpio");
			if (r) {
				printk("NFC:unable to request gpio [%d]\n",clkreq_gpio);				
			}
			r = gpio_direction_input(clkreq_gpio);
			if (r) {
				printk("NFC:unable to set direction for gpio [%d]\n",	clkreq_gpio);				
			}
		} else {
			printk("NFC:clkreq gpio not provided\n");			
		}		
	}	  
       printk("JCH:nfc_bcm_parse_dt irq_gpio:%d, en_gpio:%d, wake_gpio:%d ,clkreq_gpio=%d. \n",pdata->irq_gpio,pdata->en_gpio,pdata->wake_gpio,clkreq_gpio);
	return 0;
}
			   ...
			   const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		ret = nfc_bcm_parse_dt(&client->dev, platform_data);
		if (ret)
			return ret;
	} else {
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
       ...
       printk("JCH:NFC irq_gpio:%d, en_gpio:%d, wake_gpio:%d. \n",platform_data->irq_gpio,platform_data->en_gpio,platform_data->wake_gpio);
...
/* [BUFFIX]-Mod-bgein  by TCTNB.XQJ, PR-804255, 2014/10/29,add nfc clock control*/
        bcm2079x_dev->clk_run = false;
	...
	return 0;
err_clk:
       bcm2079x_clock_deselect(bcm2079x_dev);
err_irq_clk:
...
static int __init bcm2079x_dev_init(void)
{
	...
	if (err) {
		printk("[NFC]:JCH:bcm2079x driver add failed "
		       "(errno = %d)\n", err);
	} else {
		printk( "[NFC]:JCH:Successfully added driver %s\n",
		          bcm2079x_driver.driver.name);
	}
	return err;
}
