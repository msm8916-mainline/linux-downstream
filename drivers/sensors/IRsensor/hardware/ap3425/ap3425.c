...
static int ap3425_light_hw_set_persistence(uint8_t persistence)
{
	...
	/* read Light Sensor Persistence state */
	data_buf = i2c_read_reg_u8(g_i2c_client, AP3425_REG_ALS_PERSIS);
	if (data_buf < 0) {
		err("ap3425_light_hw_set_persistence read ERROR\n");
		return data_buf;
	}
	dbg("ap3425_light_hw_set_persistence read (0x%02X) \n", data_buf);

	data_origin = data_buf;
	data_buf = persistence;

	ret = i2c_write_reg_u8(g_i2c_client, AP3425_REG_ALS_PERSIS, data_buf);
	if (ret < 0) {
		err("ap3425_light_hw_set_persistence write ERROR \n");
		return ret;
	} else {
		log("ap3425_light_hw_set_persistence write (0x%X -> 0x%X) \n", data_origin, data_buf);
	}
...
static struct IRsensor_hw IRsensor_hw_ap3425 = {	
	...
	.proximity_hw_set_led_current = ap3425_proximity_hw_set_led_current,
	.proximity_hw_set_led_duty_ratio = NULL,
	.proximity_hw_set_persistence = ap3425_proximity_hw_set_persistence,
	.proximity_hw_set_integration = ap3425_proximity_hw_set_integration,
	...
	.light_hw_set_persistence = ap3425_light_hw_set_persistence,
	.light_hw_set_integration = ap3425_light_hw_set_integration,
	...
	.proximity_max_led_current = AP3425_SYS_LED_MAX,
	.proximity_max_led_duty_ratio = INT_NULL,
	.proximity_max_persistence = AP3425_PS_PERSIS_MAX,
