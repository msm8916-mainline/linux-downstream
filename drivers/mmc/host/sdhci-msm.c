				...
				struct sdhci_host *host)
{
    ...
    //fgy add
        pdata->switch_en = of_get_named_gpio(np,"en-gpios",0);
        if(gpio_is_valid(pdata->switch_en)){
		if (gpio_request(pdata->switch_en, "en-gpios")) {
			dev_err(dev, "can't request en-gpios gpio\n");
		goto out;
		}
		if (gpio_direction_output(pdata->switch_en,1)) {
			dev_err(dev, "can't enable en-gpios\n");
		goto out;
		}
	}
    //fgy add end
