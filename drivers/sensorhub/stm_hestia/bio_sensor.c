...
#define EVENT_TYPE_NOTIFY	REL_DIAL
...
static void set_initialize(struct bio_data *data)
{
	data->bgm_state = 0;
	data->notify_cmd = 0;
}

static void enable_strip_irq(struct bio_data *data)
{
	/*
	if (data->strip_irq_state == 0) {
		enable_irq(data->strip_irq);
		data->strip_irq_state = 1;
	}*/
}

static void disable_strip_irq(struct bio_data *data)
{
	/*
	if (data->strip_irq_state == 1) {
		disable_irq(data->strip_irq);
		data->strip_irq_state = 0;
	}*/
}

static void report_state(struct bio_data *data,
	int strip_value, int indication_value)
{
	int state, strip, indi;
	strip = (strip_value << 8) & 0xff00;
	indi = (indication_value << 9) & 0xff00;

	state = (data->bgm_state + 1) | strip | indi;
...
static void indication_func(struct work_struct *work)
{
	...
	pr_info("[BGM] : strip=%d, indication=%d, bgm_state=%d\n",
		strip_value, indi_value, data->bgm_state);

	if (data->bgm_state == 0 && indi_value == 0 && strip_value == 1) {
		/* strip inserted Startup check OK */
		pr_info("[BGM] : Initial State\n");
		data->bgm_state = 0;
		//report_state(data, strip_value, indi_value);
	} else if (data->bgm_state == 0 && indi_value == 1 && strip_value == 0) {
		/* strip inserted Startup check OK */
		pr_info("[BGM] : Start-up check OK\n");
		data->bgm_state = 1;
		disable_strip_irq(data);
		report_state(data, strip_value, indi_value);
	} else if (data->bgm_state == 1 && indi_value == 0) {
		/* Waiting sample */
		pr_info("[BGM] : Waiting sample\n");
		data->bgm_state = 2;
		report_state(data, strip_value, indi_value);
	} else if (data->bgm_state == 2 && indi_value == 1) {
		/* Sample applied */
		pr_info("[BGM] : Sample applied\n");
		data->bgm_state = 3;
		report_state(data, strip_value, indi_value);
	} else if (data->bgm_state == 3 && indi_value == 1
		&& (data->notify_cmd == 1 || data->notify_cmd == 10)) {
		/* Check complete*/
		pr_info("[BGM] : check complete\n");
		data->bgm_state = 4;
		report_state(data, strip_value, indi_value);
	} else if (data->bgm_state == 4 && indi_value == 0) {
		/* Send sleep packet */
		pr_info("[BGM] : Sleep OK\n");
		data->bgm_state = 5;
		report_state(data, strip_value, indi_value);
		set_initialize(data);
	}
...
static irqreturn_t bio_strip_irq_handler(int irq, void *dev_id)
{
	...
	strip_status = gpio_get_value(data->strip_status);
	pr_info("[BGM] %s : IRQ_HANDLED. strip %s\n", __func__,
		(strip_status) ? "removed" : "inserted");
	return IRQ_HANDLED;
}
#endif
...
static int bio_parse_dt(struct device *dev, struct bio_data *data)
{
	...
	if (data->indication < 0) {
		...
		if (errorno)
			...
			if (errorno)
				pr_err("[BGM] %s : failed to set indication" \
					" PIN as output\n", __func__);
		}
	}
	...
	if (data->strip_status < 0) {
		...
		if (errorno)
			...
			if (errorno)
				pr_err("[BGM] %s : failed to set strip_status" \
					" PIN as output\n", __func__);
		}
	}
...
/*
		errorno = request_threaded_irq(irq, NULL,
	...
	data->ecg_enable = of_get_named_gpio(np, "bio,ecg_enable", 0);
	if (data->ecg_enable < 0) {
		...
		if (errorno)
			...
			if (errorno)
				pr_err("[ecg] %s: failed to set ecg_enable" \
					" PIN as output\n", __func__);
		}
	}

	/* ECG BOOT0 PIN */
	pr_info("[ecg] %s : bio,ecg_boot0\n", __func__);
	data->ecg_boot0 = of_get_named_gpio(np, "bio,ecg_boot0", 0);
	if (data->ecg_boot0 < 0) {
		errorno = data->ecg_boot0;
		pr_err("[ecg] %s: bio,ecg_boot0\n", __func__);
	} else {
		errorno = gpio_request(data->ecg_boot0, "bio_ECG_BOOT");
		if (errorno)
			pr_err("[ecg] %s: failed to request ecg_boot0 PIN\n",
				__func__);
		else {
			errorno = gpio_direction_output(data->ecg_boot0, 0);
			if (errorno)
				pr_err("[ecg] %s: failed to set ecg_boot0" \
					" PIN as output\n", __func__);
		}
	}

	/* ECG RESET PIN */
	pr_info("[ecg] %s : bio,ecg_reset\n", __func__);
	data->ecg_reset = of_get_named_gpio(np, "bio,ecg_reset", 0);
	if (data->ecg_reset < 0) {
		errorno = data->ecg_reset;
		pr_err("[ecg] %s: bio,ecg_reset\n", __func__);
	} else {
		errorno = gpio_request(data->ecg_reset, "bio_ECG_RESET");
		if (errorno) {
			pr_err("[ecg] %s: failed to request ecg_reset PIN\n",
				__func__);
		} else {
			errorno = gpio_direction_output(data->ecg_reset, 1);
			if (errorno)
				pr_err("[ecg] %s: failed to set ecg_reset" \
					" PIN as output\n", __func__);
		}
	}
	...
	if (data->bgm_enable < 0) {
		...
		if (errorno)
			...
			if (errorno)
				pr_err("[BGM] %s: failed to set bgm_enable" \
					" PIN as output\n", __func__);
		}
	}
			...
			const char *buf, size_t size)
{
	...
	if (new_value && !data->enabled) {
		pr_info("[BGM] %s : bgm enable, polling start, sensor on (%d)\n",
			__func__, gpio_get_value(data->bgm_enable));
		set_initialize(data);
		data->enabled = 1;
			...
			const char *buf, size_t size)
{
	...
	if (noti_cmd == 1)
		data->notify_cmd = noti_cmd;
	else { /* error or test state */
		input_report_rel(data->input, EVENT_TYPE_NOTIFY,
				noti_cmd + 1);
		input_sync(data->input);
	}

	if (noti_cmd == 10) /* error */
		set_initialize(data);
...
/* ECG BOOT0 PIN */
static ssize_t ecg_boot0_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct bio_data *data = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "%d\n",
			gpio_get_value(data->ecg_boot0));
}

static ssize_t ecg_boot0_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t size)
{
	int err;
	...
	err = kstrtol(buf, 10, &boot0_gpio);

	pr_info("[ecg] %s : %s = %d\n", __func__, buf, (int)boot0_gpio);
	...
	if (data->ecg_boot0 > 0) {
		if (boot0_gpio == 1)
			gpio_direction_output(data->ecg_boot0, 1);
		else
			gpio_direction_output(data->ecg_boot0, 0);
	} else {
		pr_err("[ecg] %s: ecg_boot0 gpio is NULL\n", __func__);
		return -EIO;
	}
...
/* ECG RESET PIN */
static ssize_t ecg_reset_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct bio_data *data = dev_get_drvdata(dev);

	return snprintf(buf, PAGE_SIZE, "%d\n",
			gpio_get_value(data->ecg_reset));
}

static ssize_t ecg_reset_store(struct device *dev,
			struct device_attribute *attr,
			const char *buf, size_t size)
{
	int err;
	...
	err = kstrtol(buf, 10, &reset_gpio);

	pr_info("[ecg] %s : %s = %d\n", __func__, buf, (int)reset_gpio);
	...
	if (data->ecg_reset > 0) {
		if (reset_gpio == 1)
			gpio_direction_output(data->ecg_reset, 1);
		else
			gpio_direction_output(data->ecg_reset, 0);
	} else {
		pr_err("[ecg] %s: ecg_reset gpio is NULL\n", __func__);
		return -EIO;
	}
			...
			struct device_attribute *attr, char *buf)
{
	...
	if (IS_ERR(version_filp)) {
		...
		if (err != -ENOENT)
			pr_err("[ECG] %s: Can't open version file\n",
				__func__);
		set_fs(old_fs);
		version = -1;
		return snprintf(buf, PAGE_SIZE, "%d, %s\n", version, ((version > 0) ? "OK" : "FAIL"));
	}
			...
			const char *buf, size_t size)
{
	...
	err = version_filp->f_op->write(version_filp,
		(char *)&version, sizeof(int), &version_filp->f_pos);
	if (err != sizeof(int)) {
		pr_err("[ECG] %s: Can't write the version data to file\n",
			__func__);
		filp_close(version_filp, NULL);
		set_fs(old_fs);
		return size;
...
static DEVICE_ATTR(boot0, 0666, ecg_boot0_show, ecg_boot0_store);
static DEVICE_ATTR(reset, 0666, ecg_reset_show, ecg_reset_store);
...
static int bio_probe(struct platform_device *dev)
{
	...
	if (!data->input) {
		pr_err("[BGM] %s : could not allocate" \
			" mlx90615 input device\n",
			__func__);
	...
	input_set_capability(data->input, EV_REL, EVENT_TYPE_NOTIFY);
	...
	return 0;
//wake_lock_destroy(&data->bgm_wake_lock);
