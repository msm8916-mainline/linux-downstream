...
static int mlx90615_power_parse_dt(struct device *dev, struct mlx90615_data *data)
{
	...
	if (errorno) {
		pr_err("[BODYTEMP] %s: failed to request ENABLE PIN for MLX90615\n",
			__func__);
		goto dt_exit;
	}
...
static void mlx90615_set_power(struct mlx90615_data *mlx90615, int onoff)
{
	...
	if (onoff) {
		...
		if (mlx90615->power > 0)
			...
			mlx90615_power_parse_dt(&mlx90615->i2c_client->dev,mlx90615);
		}
		udelay(100);
		setscl(mlx90615, 0);
		msleep(50);
	...
	int *tar, int *taf, int *tor, int *tof)
{
	...
	if (ret >= 0 && ret < 40000)
		...
		pr_info("%s : ambient temp read fail %d\n", __func__, ret);
	...
	if (ret >= 0 && ret < 40000)
		...
		pr_info("%s : object temp read fail %d\n", __func__, ret);

	mlx90615_convert(object, tor, tof);

	pr_info("%s : %d %d\n", __func__, ambient, object);
...
static void mlx90615_work_func(struct work_struct *work)
{
	...
	mlx90615->ambient_temp = tar * 100 + taf;
	mlx90615->object_temp = tor * 100 + tof;
	...
	pr_info("%s: ambient = %d.%02d C object = %d.%02d C\n", __func__,
		tar, taf, tor, tof);
}
			...
			const struct i2c_device_id *id)
{
	...
	if (!mlx90615) {
		pr_err("[BODYTEMP] %s: failed to alloc memory for mlx90615_data\n", __func__);
		return -ENOMEM;
	}
	...
	if (!mlx90615->input) {
		pr_err("[BODYTEMP] %s: could not allocate mlx90615 input device\n", __func__);
	...
	if (ret) {
		pr_err("[BODYTEMP] %s: cound not register prox sensor device(%d).\n",
			__func__, ret);
		goto err_sysfs_create_symlink;
	}
	...
	//dev_set_drvdata(mlx90615->mlx90615_dev, mlx90615);
