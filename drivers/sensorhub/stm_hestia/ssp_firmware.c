...
#define SSP_FIRMWARE_REVISION_HESTIA		14073100
#define SSP_FIRMWARE_REVISION_HESTIA_REV02	14101500
#define SSP_FIRMWARE_REVISION_HESTIA_REV03	14122300
...
#define BL_FW_NAME_HESTIA_REV02		"ssp_stm_hestia_rev02.fw"
#define BL_FW_NAME_HESTIA_REV03		"ssp_stm_hestia_rev03.fw"
...
unsigned int get_module_rev(struct ssp_data *data)
{
	...
	if (data->ap_rev <= 8)
		return SSP_FIRMWARE_REVISION_HESTIA_REV02;
	else
		return SSP_FIRMWARE_REVISION_HESTIA_REV03;
}
	 ...
	 /* HW request: I2C line is reversed */
		if (data->ap_rev <= 5)
			iRet = load_kernel_fw_bootmode(data->spi,
					BL_FW_NAME_HESTIA);
		else if (data->ap_rev <= 8)
			iRet = load_kernel_fw_bootmode(data->spi,
					BL_FW_NAME_HESTIA_REV02);
		else
			iRet = load_kernel_fw_bootmode(data->spi,
					BL_FW_NAME_HESTIA_REV03);
		break;
...
#ifdef CONFIG_SENSORS_SSP_STM_HESTIA
...
int check_fwbl(struct ssp_data *data)
{
	...
	if (data->ap_rev <= 5)
		...
		fw_revision = SSP_FIRMWARE_REVISION_HESTIA_REV02;
	else
		fw_revision = SSP_FIRMWARE_REVISION_HESTIA_REV03;
