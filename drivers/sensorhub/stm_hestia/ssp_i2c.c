...
static int do_transfer(struct ssp_data *data, struct ssp_msg *msg,
	...
	gpio_set_value(data->ap_int, 0);
	while (gpio_get_value(data->mcu_int2)) {
		mdelay(3);
		if ((ssp_down = data->bSspShutdown) || iDelaycnt++ > 500) {
			pr_err("[SSP] %s exit1 - Time out!!\n", __func__);
			gpio_set_value(data->ap_int, 1);
			status = -1;
			goto exit;
		}
	}
...
retries:
	...
	if (buffer != DEVICE_ID && iReties++ < 2) {
		mdelay(5);
		pr_err("[SSP] %s - get chip ID(0x%x) retry\n", __func__, buffer);
		goto retries;
	}
	...
	pr_err("[SSP] %s - get chip ID(0x%x) failed %d\n", __func__, buffer, iRet);
	return ERROR;
}
