...
{
	/* this func enable 1.8V & 3.3V */
	if(onoff) {
		gpio_set_value_cansleep(data->vdd_1p8_en, onoff);
		msleep(100);
	}
	gpio_set_value_cansleep(data->led_3p3_en, onoff);
	if(!onoff) {
		msleep(100);
		gpio_set_value_cansleep(data->vdd_1p8_en, onoff);
	}
...
#if defined(CONFIG_SENSORS_SSP_STM_HESTIA)
	max86900_led_ldo_onoff(data, regulator_onoff);
#else
	...
	struct device *dev)
{
...
#if defined(CONFIG_SENSORS_SSP_STM_HESTIA)
	data->hrm_int = of_get_named_gpio(dNode,
		"max86900,hrm_int-gpio", 0);
	...
	data->led_3p3_en = of_get_named_gpio(dNode,
		"max86900,led_en-gpio", 0);
	gpio_request(data->led_3p3_en, "HRM_LDO_EN");
	gpio_direction_output(data->led_3p3_en, 0);

	data->vdd_1p8_en = of_get_named_gpio(dNode,
		"max86900,hrm_vdd-gpio", 0);
	gpio_request(data->vdd_1p8_en, "HRM_VDD_EN");
	gpio_direction_output(data->vdd_1p8_en, 0);
#else
...
int max86900_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	err = sensors_register(data->dev, data, uv_sensor_attrs,
			MODULE_NAME_UV);
	if (err) {
		pr_err("[SENSOR] %s - cound not register uv_sensor(%d).\n",
			__func__, err);
