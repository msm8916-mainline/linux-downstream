...
#if 1
static int mcu_pinctrl_dt(struct device *dev)
{
	int ret = 0;
	struct pinctrl *mcu_pinctrl;
	struct pinctrl_state *mcu_suspend;
	struct pinctrl_state *mcu_active;

	pr_info("[SSP] %s\n", __func__);

	/* Get pinctrl if target uses pinctrl */
	mcu_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(mcu_pinctrl)) {
		pr_debug("Target does not use pinctrl\n");
		mcu_pinctrl = NULL;
	} else {
		mcu_suspend = pinctrl_lookup_state(mcu_pinctrl, "mcu_suspend");
		mcu_active = pinctrl_lookup_state(mcu_pinctrl, "mcu_active");
		if (IS_ERR(mcu_suspend)) {
			pr_info("%s fail to suspend lookup_state\n", __func__);
			goto err_exit;
		}
		if (IS_ERR(mcu_active)) {
			pr_info("%s fail to active lookup_state\n", __func__);
			goto err_exit;
		}
		ret = pinctrl_select_state(mcu_pinctrl, mcu_suspend);
		if (ret != 0) {
			pr_err("%s: fail to select_state suspend\n", __func__);
			goto err_exit;
		}
		ret = pinctrl_select_state(mcu_pinctrl, mcu_active);
		if (ret != 0) {
			pr_err("%s: fail to select_state active\n", __func__);
			goto err_exit;
		}
		devm_pinctrl_put(mcu_pinctrl);
	}
	...
	struct ssp_data *data)
{
	...
	errorno = gpio_request(data->mcu_int1, "MCU_INT1");
	if (errorno) {
...
#if 0
	data->reg_hub = regulator_get(NULL, "8916_l5");
	//devm_regulator_get(dev, "hub_vreg");
	...
	if (of_property_read_string(np, "ssp,mag-supply-name", supply_name)) {
		pr_err("[%s]: Unable to read ssp,mag-supply-name\n", __func__);
		supply_name = "8916_l17";
	}
#endif

	data->reg_sns = regulator_get(NULL, "8916_l17");
	//devm_regulator_get(dev, "psns_vreg");
	if (IS_ERR(data->reg_hub)) {
		pr_err("[SSP] could not get psns_vreg, %ld\n",
			PTR_ERR(data->reg_sns));
	} else
		errorno = regulator_enable(data->reg_sns);
