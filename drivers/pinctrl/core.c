...
//[BUG_FIX]-Add-BEGIN by TCTSZ. Zkx, 2014/07/01, fixed repeat pinctrl_select_state should relese the gpio first, this may not good, but it
//is only to release the gpio.need do more test.
 ...
 * @state: the state handle to select/activate/program
 */
int pinctrl_select_state_camera(struct pinctrl *p, struct pinctrl_state *state)
{
       struct pinctrl_setting *setting, *setting2;
       struct pinctrl_state *old_state = p->state;
       int ret;
...
EXPORT_SYMBOL_GPL(pinctrl_select_state_camera);
#endif
