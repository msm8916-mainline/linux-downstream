...
//yxw add for test es705 fw
int escore_request_firmware(const struct firmware **fw, const char *file,	struct device *device)
{	
	struct firmware *firmware;	
	struct file *filp = NULL;	
	long size, err = 0;	char *buf;	
	int i;	
	char *path = __getname();	
	...
	*fw = firmware = kzalloc(sizeof(*firmware), GFP_KERNEL);	
	...
	do {		
		for (i = 0; i < ARRAY_SIZE(fw_path); i++) {/* skip the unset customized path */			
			if (!fw_path[i][0])				
				continue;			
			snprintf(path, PATH_MAX, "%s/%s", fw_path[i], file);
			printk(KERN_ERR "yxw test0705 ++++++++%s+++++++++\n",path);
