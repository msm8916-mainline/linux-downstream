...
struct i2c_msg *read_msg;
 ...
 //add for Novatek
 //only called by synaptics_rmi4_probe once;
 //only when TP is Novatek ,synaptics_rmi4_i2c_read_test return success, then exit synaptics_rmi4_probe;
 static int synaptics_rmi4_i2c_read_test(struct i2c_client *client, uint8_t address, uint8_t *buf, uint8_t len)
  {
			...
			&& (true == rmi4_data->sleep_gesture_flag))
		{
			if( false == easy_wakeup_process )
			{
				...
				schedule_delayed_work(&rmi4_data->delay_work, msecs_to_jiffies(GENSTRUE_WORK_DELAY));
		                ...
		                synaptics_rmi4_i2c_read(rmi4_data, rmi4_data->f01_data_base_addr + 1, intr_status, rmi4_data->num_of_intr_regs);
                		msleep(100);
                		tp_log_info("%s: dummy interrupt...\n",__func__);
			}
		}
		else
		{
...
static int synaptics_rmi4_parse_dt(struct device *dev, struct synaptics_dsx_platform_data *pdata)
{
	...
	*/
	...
	pdata->tp_power_by_lcd = (bool)get_of_u32_val(np, "synaptics,vdd_ctrl_by_lcd", 0);
		...
		const struct i2c_device_id *dev_id)
{
	...
	/*This is not the normal code of Synaptics driver, 8Youth use Synaptics and Novatek TP,
		they have the same i2c slave addr 0x70, and Novtek have another addr 0x1,
		to avoid detect Novatek TP as Synaptics, wo try slave addr 0x1 first, if device
		response the TP should be Novatek, else is Synaptics*/
	novatek_err = synaptics_rmi4_i2c_read_test(client, 0x01, novatek_productid, 3);
	if (novatek_err < 0) {
		dev_info(&client->dev, "maybe is synaptics tp and we need go on\n");
	} else {
		dev_info(&client->dev, "the tp is novatek and we need go out\n");
		goto err_i2c_test;
	}
	//end add for Novatek
...
static int synaptics_rmi4_rt_suspend(struct device *dev)
{
	...
	if (rmi4_data->board->tp_power_by_lcd){
		if(get_lcd_power_status() == 0) {
			tp_log_info("%s: lcd_tp_power_off\n", __func__);
			return 0;
		}
	}
	if (!rmi4_data->sensor_sleep) {
