...
/* sysfs store for amp */
static ssize_t qpnp_vib_amp_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct timed_output_dev *timed_dev = dev_get_drvdata(dev);
	struct qpnp_vib *vib = container_of(timed_dev, struct qpnp_vib,
					 timed_dev);
	u8 reg = 0;
	int rc, value;
	...
	if (value < QPNP_VIB_MIN_LEVEL)
		value = QPNP_VIB_MIN_LEVEL;
	else if (value > QPNP_VIB_MAX_LEVEL)
	...
	reg &= ~QPNP_VIB_VTG_SET_MASK;
	reg |= (value & QPNP_VIB_VTG_SET_MASK);
...
/* sysfs show for amp */
static ssize_t qpnp_vib_amp_show(struct device *dev,
...
static int qpnp_vibrator_probe(struct spmi_device *spmi)
{
	...
	for (i = 0; i < ARRAY_SIZE(qpnp_vib_attrs); i++) {
		rc = sysfs_create_file(&vib->timed_dev.dev->kobj,
...
sysfs_fail:
	for (i--; i >= 0; i--)
		sysfs_remove_file(&vib->timed_dev.dev->kobj,
				&qpnp_vib_attrs[i].attr);
