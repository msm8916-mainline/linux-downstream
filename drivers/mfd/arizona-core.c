...
struct pinctrl *arizona_pinctrl;
static int arizona_pinctrl_configure(bool active)
{
	...
	pr_info("%s - active = %d\n", __func__, active);
	if (active) {
		set_state = pinctrl_lookup_state(arizona_pinctrl, "arizona_gpio_active");
		if (IS_ERR(set_state)) {
			pr_err("%s: cannot get arizona pinctrl active state\n", __func__);
			return PTR_ERR(set_state);
		}
	} else {
		set_state = pinctrl_lookup_state(arizona_pinctrl, "arizona_gpio_suspend");
		if (IS_ERR(set_state)) {
			pr_err("%s: cannot get arizona pinctrl sleep state\n", __func__);
			return PTR_ERR(set_state);
		}
	}
	retval = pinctrl_select_state(arizona_pinctrl, set_state);
	if (retval) {
		pr_err("%s: cannot set arizona pinctrl active state\n", __func__);
		return retval;
	}
	return 0;
}
...
int arizona_dev_init(struct arizona *arizona)
{
	...
	pr_info("%s - get the gpio control\n", __func__);
	arizona_pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(arizona_pinctrl)) {
			pr_err("%s: cannot get arizona pinctrl\n", __func__);
			return -EINVAL;
	}

	if (arizona_pinctrl) {
		ret = arizona_pinctrl_configure(true);
		if (ret) {
			pr_err("%s: cannot set arizona pinctrl active state\n", __func__);
		}
	}
