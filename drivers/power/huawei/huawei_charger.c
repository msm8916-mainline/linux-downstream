...
int huawei_charger_get_ilimit_voltage(void)
{
    ...
    if (rc)
    {
        pr_err("Unable to read ilimit voltage rc=%d\n", rc);
        return -1;
    }
...
/*===========================================
FUNCTION: huawei_lbc_is_in_vin_min_loop
...
=============================================*/
int huawei_lbc_is_in_vin_min_loop(void)
{
    u8 reg_val = 0;
    int rc;
    struct charge_device_info *di = NULL;
...
int hw_get_battery_status(void)
{
    ...
    if(NULL == di)
    {
        pr_err("hw charge_device is not ready!\n");
        return 0;
    }
    ...
    {
        pr_err("hw ops charge_device is not ready!\n");
        return 0;
    }
}
...
/* 8939 platfrom no  JEITA define*/
#ifdef USE_JEITA_SPEC
...
static irqreturn_t huawei_charger_usbin_valid_irq_handler(int irq, void *data)
{
    ...
    if (di->ops && di->ops->usbin_valid_irq)
    {
         di->ops->usbin_valid_irq(usb_present);
    }
#ifdef CONFIG_LOG_JANK
...
static int charge_probe(struct spmi_device *pdev)
{
    ...
    if (di->ops && di->ops->usbin_valid_irq)
    {
        ...
        if (di->usb_present)
        {
            pr_info("usb present, do usbin_valid_irq.\n");
            di->ops->usbin_valid_irq(di->usb_present);
        }
    }
