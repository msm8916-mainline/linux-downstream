...
static int battery_monitor_capacity_changed(struct bq27510_device_info *di)
{
    ...
    pr_info("bat_exist value is %d,curr_capacity is %d,prev_capacity is %d\n",bat_exist,curr_capacity,di->prev_capacity);
    ...
    /*dump bq registers and adc values, and notify to dsm server */
    if(!normal_soc_cal_flag){
        get_current_time(&start_tm_sec);
        di->saved_soc = curr_capacity;
...
void bq27510_dump_regs(struct dsm_client *dclient)
{
    /*dump BQ27510 registers*/
    int ret = 0;
    u8 reg = 0;
    dsm_client_record(dclient, "[BQ27510] regs:\n");
    pr_info("[BQ27510] regs:\n");
    for(reg = BQ27510_REG_CTRL; reg < BQ27510_DESIGN_CAPACITY; reg=reg+2){
        if(bq27510_is_accessible()){
            if(g_battery_measure_by_bq27510_device){
                if(INTERNAL_TEMPERATURE == reg )
                    continue;
                ret = bq27510_i2c_read_word(g_battery_measure_by_bq27510_device, reg);
                dsm_client_record(dclient, "0x%x, 0x%x\n", reg, ret);
                pr_info("0x%x, 0x%x\n", reg, ret);
            }
        }else{
            pr_info("bq27510 isn't accessible,It's updating firmware!\n");
       }
    }
}
#endif
