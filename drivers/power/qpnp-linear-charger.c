...
#define QPNP_LBC_DEBUG_REG(x) {#x, x}
struct debug_reg {
	char *name;
	u8 reg;
};
static struct debug_reg qpnp_lbc_debug_regs[] = {
	QPNP_LBC_DEBUG_REG(CHG_STATUS_REG),
	QPNP_LBC_DEBUG_REG(INT_RT_STS_REG),
	QPNP_LBC_DEBUG_REG(CHG_CTRL_REG),
	QPNP_LBC_DEBUG_REG(CHG_TCHG_MAX_REG),
};
#endif
...
#ifdef CONFIG_LGE_PM_FAC_FACTORY_CABLE_SET
...
#ifdef CONFIG_LGE_PM_FAC_FACTORY_CABLE_SET
#define QPNP_LBC_IBATMAX_FACTORY 1500
#endif
/*
...
static int qpnp_lbc_ibatmax_set(struct qpnp_lbc_chip *chip, int chg_current)
{
	...
	if (is_factory_cable()) {
		...
		chg_current = QPNP_LBC_IBATMAX_FACTORY;
	}
#endif
...
static int get_prop_batt_health(struct qpnp_lbc_chip *chip)
{
	...
	battemp = ret.intval/10;

	if(chip->usb_present && (lgcc_is_probed == 1)){
...
static int get_prop_batt_status(struct qpnp_lbc_chip *chip)
{
	...
	spin_lock_irqsave(&chip->chg_sts_chk, flags);
...
static void qpnp_lbc_set_appropriate_current(struct qpnp_lbc_chip *chip)
{
	...
	if(lgcc_charging_current < chip->usb_psy_ma)
		chg_current = lgcc_charging_current;
	else
		chg_current = chip->usb_psy_ma;
...
static irqreturn_t qpnp_lbc_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (!usb_present) {
			...
			qpnp_lbc_charger_enable(chip, CURRENT, 0);
#ifdef CONFIG_LGE_PM
			spin_lock_irqsave(&chip->chg_sts_chk, chg_flags);
			if (wake_lock_active(&chip->chg_wake_lock))
				wake_unlock(&chip->chg_wake_lock);
			spin_unlock_irqrestore(&chip->chg_sts_chk, chg_flags);
#endif
			spin_lock_irqsave(&chip->ibat_change_lock, flags);
			chip->usb_psy_ma = QPNP_CHG_I_MAX_MIN_90;
			qpnp_lbc_set_appropriate_current(chip);
...
int lgcc_is_charger_present(void){
	...
	int charger = qpnp_lbc_is_usb_chg_plugged_in(qpnp_chg);
...
int lgcc_set_ibat_current(int chg_current){
	...
	ret = qpnp_lbc_ibatmax_set(qpnp_chg, chg_current);
...
int lgcc_set_charging_enable(int enable){
	...
	int ret = 0;
	ret = qpnp_lbc_charger_enable(qpnp_chg, LGCC, enable);
...
void lgcc_charger_reginfo(void)
{
	int rc = 0;
	int i = 0;
	int cnt = ARRAY_SIZE(qpnp_lbc_debug_regs);
	u8 reg_val[cnt];
	struct qpnp_vadc_result results[2];

	for (i = 0; i < cnt; i++) {
		rc = qpnp_lbc_read(qpnp_chg,
				qpnp_chg->chgr_base + qpnp_lbc_debug_regs[i].reg,
				&reg_val[i], 1);
		if (rc) {
			pr_err("spmi read failed: addr = %03X, rc = %d\n",
					qpnp_chg->chgr_base + qpnp_lbc_debug_regs[i].reg, rc);
		}
	}
	rc = qpnp_vadc_read(qpnp_chg->vadc_dev, VBAT_SNS, &results[0]);
	if (rc) {
		pr_err("Unable to read vbat rc=%d\n", rc);
	}
	rc = qpnp_vadc_read(qpnp_chg->vadc_dev, LR_MUX1_BATT_THERM, &results[1]);
	if (rc) {
		pr_err("Unable to read batt temperature rc=%d\n", rc);
	}

	pr_info("%s=0x%02X %s=0x%02X %s=0x%02X %s=0x%02X\n",
			qpnp_lbc_debug_regs[0].name, reg_val[0],
			qpnp_lbc_debug_regs[1].name, reg_val[1],
			qpnp_lbc_debug_regs[2].name, reg_val[2],
			qpnp_lbc_debug_regs[3].name, reg_val[3]);
...
static int qpnp_lbc_main_probe(struct spmi_device *spmi)
{
	...
	wake_lock_init(&chip->uevent_wake_lock,
			WAKE_LOCK_SUSPEND, "qpnp_lbc_uevent");
#endif
