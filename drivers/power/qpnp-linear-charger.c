...
struct wake_lock charger_remove;
struct vddtrim_map {
	int			trim_uv;
	int			trim_val;
};
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-814317, 2014/11/12, update power state */
#ifdef CONFIG_TCT_8X16_ALTO5_VDF
	struct delayed_work			power_supply_change_work;
/* [PLATFORM]-Add-END by TCTNB.FLF */
#endif
#ifdef CONFIG_TCT_8X16_COMMON
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-888729, 2015/02/02, add power off condition */
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-888729, 2015/02/02, add power off condition */
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-928695, 2015/02/10, add power off condition */
		#ifdef FEATURE_TCTNB_MMITEST
		...
		if (unlikely(val->intval <= 3100000 && recheck_flag == true))
		{
			recheck_flag = false;
			val->intval = 3101000;
...
static void qpnp_lbc_jeita_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	if (state == ADC_TM_WARM_STATE) {
		if (temp >= chip->cfg_overheat_bat_decidegc + 30) {
			...
			bat_overheat = true;
			chip->adc_param.low_temp = chip->cfg_overheat_bat_decidegc - 50;
			chip->adc_param.high_temp = chip->cfg_overheat_bat_decidegc + 30;
			chip->adc_param.state_request =
...
static irqreturn_t qpnp_lbc_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		if (!usb_present) {
			...
			alarm_cancel(&chip->eoc_delay_work_alarm); //if charger remove,the eoc timer need cancel.
		} else {
			/*
		...
		wake_lock_timeout(&charger_remove,msecs_to_jiffies(2000));
...
{
	struct qpnp_lbc_chip *chip = container_of(work,struct qpnp_lbc_chip, low_voltage_detect_work.work);
	power_supply_changed(&chip->batt_psy);
}
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
...
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-807254, 2014/10/31, read battery id */
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-814317, 2014/11/12, update power state */
#ifdef CONFIG_TCT_8X16_ALTO5_VDF
#define POEWR_SUPPLY_CHANGE_PERIOD_MS	5000
#define VOLTAGE_CHANGE_LIMIT_UV			10000
static void power_supply_change_work_fn(struct work_struct *work)
{
	...
	if (abs(voltage_now - voltage_pre) >= VOLTAGE_CHANGE_LIMIT_UV) {
		pr_err("voltage_pre = %d, voltage_now = %d\n", voltage_pre, voltage_now);
		power_supply_changed(&chip->batt_psy);
		}
...
static int qpnp_lbc_probe(struct spmi_device *spmi)
{
	...
	struct power_supply *usb_psy;
	int rc = 0;
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, FR-807254, 2014/10/31, disable charging for incorrect battery id */
...
/* [PLATFORM]-Add-BEGIN by TCTNB.FLF, PR-814317, 2014/11/12, update power state */
#ifdef CONFIG_TCT_8X16_ALTO5_VDF
	INIT_DELAYED_WORK(&chip->power_supply_change_work, power_supply_change_work_fn);
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
...
#if (!(defined FEATURE_TCTNB_MMITEST)) && (defined CONFIG_TCT_8X16_ALTO5_VDF)
...
#endif
/* [PLATFORM]-Add-END by TCTNB.FLF */
	wake_lock_init(&charger_remove,0,"charger_remove");
