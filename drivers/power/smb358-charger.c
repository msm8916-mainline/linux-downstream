...
#define THERM_A_SWITCHING_FREQUENCY_BIT        BIT(7)	//mzy add 1.5Mhz
#define THERM_A_SWITCHING_FREQUENCY_MASK       BIT(7)	//mzy add 1.5Mhz
...
struct smb358_charger {
	...
	struct wake_lock 		chg_wake_lock;			//mzy add 
};
...
static int chg_current[] = {
	...
	300, 500, 700, 900, 1000, 1200, 1500, 1800,		//mzy modified 20150530 for smb348 input current
};

static int fast_chg_current[] = {
	//200, 450, 600, 900, 1300, 1500, 1800, 2000,
	100, 200, 375, 450, 600, 800, 1300, 1500,		//mzy modified 20150530 for smb348 fast-charge current
};
...
static int pre_temp;	//mzy
extern int interrupt_flag;

static void dump_regs(struct smb358_charger *chip);

void dump_smb358(struct power_supply *psy)
{
...
{
	int rc;
	//printk(KERN_ERR "mzy >>>> %s, reg = 0x%x, temp = 0x%x\n", __func__, reg, val);
	mutex_lock(&chip->read_write_lock);
...
static int __smb358_charging_disable(struct smb358_charger *chip, bool disable)
{
	...
	printk(KERN_ERR "mzy >>>> %s, 0x30, disable = %d\n", __func__, disable);
						...
						int reason, int disable)
{
	...
	pr_debug("mzy >>>> %s, reason = %d requested_disable = %d disabled_status = %d\n",
						__func__, reason, disable, disabled);
...
static int smb358_hw_init(struct smb358_charger *chip)
{
	...
	reg = STATUS_INT_CHG_TIMEOUT_BIT | //STATUS_INT_OTG_DETECT_BIT |		//mzy delete 
		STATUS_INT_BATT_OV_BIT | STATUS_INT_CHGING_BIT |
		//STATUS_INT_CHG_INHI_BIT | 	//mzy delete
		STATUS_INT_INOK_BIT |
	...
	if (rc) {
        ...
        rc = smb358_masked_write(chip, THERM_A_CTRL_REG,
        	...
        	THERM_A_SOFT_HOT_LIM_MASK | THERM_A_SWITCHING_FREQUENCY_MASK,
        	THERM_A_THERM_MONITOR_EN_DIS |THERM_A_SOFT_COLD_LIM_BIT |
        	THERM_A_SOFT_HOT_LIM_BIT | THERM_A_SWITCHING_FREQUENCY_BIT);
	if (rc) {
	...
	if (chip->charging_disabled) {
		//printk(KERN_ERR "mzy >>>> %s, before smb358_charging_disable(chip, USER, 1);\n", __func__);
		 ...
		 */
		//printk(KERN_ERR "mzy >>>> %s, before __smb358_charging_disable(chip, 0);\n", __func__);
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		case POWER_SUPPLY_STATUS_CHARGING:
			//printk(KERN_ERR "mzy >>>> %s, POWER_SUPPLY_STATUS_CHARGING, before smb358_charging_disable(chip, SOC, true);\n", __func__);
			rc = smb358_charging_disable(chip, SOC, false);
			if (rc < 0) {
				dev_err(chip->dev,
		...
		break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		//printk(KERN_ERR "mzy >>>> %s, POWER_SUPPLY_PROP_CHARGING_ENABLED, before smb358_charging_disable(chip, SOC, true);\n", __func__);
...
static void smb_chg_adc_notification(enum qpnp_tm_state state, void *ctx)
{
	...
	if (state == ADC_TM_WARM_STATE) {
		if (temp > chip->hot_bat_decidegc) {
			...
			chip->adc_param.high_temp = chip->cold_bat_decidegc + 15;
	...
	pr_err("mzy >>>>%s, hot %d, cold %d, warm %d, cool %d, jeita supported %d, missing %d, low = %d deciDegC, high = %d deciDegC\n",
		__func__, chip->batt_hot, chip->batt_cold, chip->batt_warm,
...
static irqreturn_t smb358_chg_stat_handler(int irq, void *dev_id)
{
	...
	wake_lock_timeout(&chip->chg_wake_lock,	2*HZ);
...
static void smb358_external_power_changed(struct power_supply *psy)
{
	...
	printk(KERN_ERR "zou_hao_current_limit = %d, temp = %d\n", current_limit, temp);

	if(!interrupt_flag)		//mzy add for adc irq is not triggerred
	{
		if(temp > pre_temp)
		{
				...
				const struct i2c_device_id *id)
{
	...
	rc = smb358_masked_write(chip, CMD_A_REG, CMD_A_CHG_SUSP_EN_MASK, 0x00);
	if (rc) {
	...
	//STAT_AND_TIMER_CTRL_REG dedult value 0x18
	rc = __smb358_write_reg(chip, STAT_AND_TIMER_CTRL_REG, 0x34);
	if (rc) {
		dev_err(chip->dev,
			"smb358_write Failed: reg=%03X, rc=%d\n", STAT_AND_TIMER_CTRL_REG, rc);
	...
	if (chip->using_pmic_therm) {
		if (!chip->jeita_supported) {
			...
			chip->adc_param.low_temp = chip->cool_bat_decidegc - 10;
			chip->adc_param.high_temp = chip->warm_bat_decidegc;
		}
		chip->adc_param.timer_interval = ADC_MEAS2_INTERVAL_1S;
		chip->adc_param.state_request = ADC_TM_HIGH_LOW_THR_ENABLE;
		chip->adc_param.btm_ctx = chip;
		chip->adc_param.threshold_notification = smb_chg_adc_notification;
	...
	wake_lock_init(&chip->chg_wake_lock, WAKE_LOCK_SUSPEND, "stat_handle_wake_lock");
...
static int smb358_suspend(struct device *dev)
{
	if(0)
	{
		...
		rc = smb358_write_reg(chip, STATUS_INT_REG,
				...
				//|STATUS_INT_OTG_DETECT_BIT | STATUS_INT_CHG_INHI_BIT	//mzy delete 20150622
				);
		if (rc < 0)
			dev_err(chip->dev,
