...
static bool reported_soc_log = false;
...
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
#define SOC_RESCALING_FACTOR 96
#define SOC_BUF_SIZE 10
static int rescale_vm_bms_soc(struct qpnp_bms_chip *chip)
{
	static int soc_buf[SOC_BUF_SIZE];
	static int soc_buf_idx = 0;
	...
	if (chip->last_soc < 0) {
		pr_err("last_soc_is not valid, last_soc = %d\n", chip->last_soc);
		return chip->last_soc;
	}

	charging = is_battery_charging(chip);

	//Round and rescale SOC
	recalc_soc = ((chip->last_soc)*100+50)/SOC_RESCALING_FACTOR;
	recalc_soc = bound_soc(recalc_soc);

	if (first_check_flag == false) {
		for (idx = 0; idx < SOC_BUF_SIZE; idx++) {
			soc_buf[idx] = recalc_soc;
		}
		chip->rescaled_soc = recalc_soc;
		first_check_flag = true;
	}
	else {
		if (soc_buf_idx >= SOC_BUF_SIZE) {
			soc_buf_idx = 0;
		}
		soc_buf[soc_buf_idx] = recalc_soc;
		soc_buf_idx++;
	}
	...
	//Round SOC
	avg_soc = (sum_soc + 5) / SOC_BUF_SIZE;

	//SOC should not be increased if discharing battery
...
#ifdef CONFIG_LGE_PM_FACTORY_TESTMODE
#define FACTORY_TESTMODE_SOC 98
#endif
...
static int report_vm_bms_soc(struct qpnp_bms_chip *chip)
{
	...
	if(reported_soc_log){
	pr_info("Rescaled SOC = %d, last_soc = %d\n", chip->rescaled_soc, chip->last_soc);
		reported_soc_log = false;
	}
	return chip->rescaled_soc;
#else
...
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
	static int prev_rescaled_soc = 0;
#endif
	...
	if (!is_battery_present(chip)) {
		...
		if (chip->dt.cfg_use_voltage_soc) {
			...
			if (chip->calculated_soc != new_soc) {
				...
				report_vm_bms_soc(chip);
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
				if (chip->rescaled_soc != prev_rescaled_soc) {
					pr_debug("update bms_psy\n");
					reported_soc_log = true;
					power_supply_changed(&chip->bms_psy);
				}
...
#ifdef CONFIG_LGE_PM_BATTERY_SOC_RESCALING
			prev_rescaled_soc = chip->rescaled_soc;
#endif
		}
...
static int qpnp_vm_bms_probe(struct spmi_device *spmi)
{
	...
	if(!lge_get_laf_mode())
		...
		pr_info("LAF mode!!! Skip voltage_soc_timeout_work.\n");
