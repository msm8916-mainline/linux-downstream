...
#define  BMS_CAPACITY_SLEEP_DETECT      //when the phone is sleepding,system will wake up periodly to detect the capacity
...
#ifdef   BMS_CAPACITY_SLEEP_DETECT
...
struct qpnp_bms_chip {
	...
	struct alarm			sleepdetect_alarm;
	struct work_struct		sleepdetect_work;
#endif
};
...
static int lookup_soc_ocv(struct qpnp_bms_chip *chip, int ocv_uv, int batt_temp)
{
	...
	if (rc < 0) {
		...
		if( (soc_final==0) && (vbat_uv > 3280000) )
		{
			//printk(KERN_ERR"soc_final =%d, if(vbat_uv =%d > 3280000) soc_final will be set 1 \n",soc_final, vbat_uv);
	 		soc_final = 1;
		}
		if( (soc_final!=0) && (vbat_uv < 3280000) )
		{
		     // printk("soc_final =%d, if(vbat_uv =%d < 3280000) soc_final will be set 0 \n",soc_final, vbat_uv);
		      chk_num++;
			if (chk_num >= 2)
			{
			      // printk(KERN_ERR">>>>>>>>>>>>>>>> chk_num = %d , soc_final = %d\n",chk_num,soc_final);
				soc_final = 0;
			}
		}else
				chk_num = 0;
	...
	pr_info("soc_final=%d soc_ocv=%d soc_cutoff=%d ocv_uv=%u batt_temp=%d\n",
      ...
      //printk(KERN_ERR"chk_num = %d,report_cap=%d,vbat_uv=%d\n",chk_num,soc_final,vbat_uv);
	return soc_final;
}
...
static void check_eoc_condition(struct qpnp_bms_chip *chip)
{
	...
	if (chip->ocv_at_100 == -EINVAL) {
		...
		if (chip->last_ocv_uv >= chip->ocv_at_100) {
			 ...
			 * below 100.
			 */
			if (chip->reported_soc_in_use && chip->reported_soc == 100) {
...
static void calculate_reported_soc(struct qpnp_bms_chip *chip)
{
	...
	if (chip->reported_soc > chip->last_soc) {
		...
		if (chip->reported_soc == 100) {
			ret.intval = POWER_SUPPLY_STATUS_DISCHARGING;
			chip->batt_psy->set_property(chip->batt_psy,POWER_SUPPLY_PROP_STATUS, &ret);
		...
		*/
		...
		if (chip->reported_soc_high_current)
			...
			chip->reported_soc = chip->last_soc + chip->reported_soc_delta;
...
extern void dump_smb358(struct power_supply *psy);
static void monitor_soc_work(struct work_struct *work)
{
	...
	if (!is_battery_present(chip)) {
		...
		if (chip->dt.cfg_use_voltage_soc) {
			...
			if (chip->calculated_soc != new_soc) {
				...
				if(chip->calculated_soc > 98 )
					dump_smb358(chip->batt_psy);/* battery capacity > 98 output regs val*/
	...
	if (chip->reported_soc_in_use && chip->charger_removed_since_full && !chip->charger_reinserted) {
		...
		if (chip->reported_soc_change_sec > UI_SOC_CATCHUP_TIME) {
			if(chip->last_soc < 0)
			{
				report_vm_bms_soc(chip);	//if last_soc err then recalculated last_soc
			}
...
static void reported_soc_check_status(struct qpnp_bms_chip *chip)
{
	...
	if ((chip->reported_soc - chip->last_soc) > (100 - chip->dt.cfg_soc_resume_limit + HIGH_CURRENT_TH)) {
...
#ifdef BMS_CAPACITY_SLEEP_DETECT
#define SLEEPDETECT_PERIOD_INIT_NS      (3600LL * NSEC_PER_SEC)
#define SLEEPDETECT_PERIOD_BATTERY_LOW_NS			(900LL * NSEC_PER_SEC)
#define SLEEPDETECT_PERIOD_BATTERY_MID_NS			(3600LL * NSEC_PER_SEC)
#define SLEEPDETECT_PERIOD_BATTERY_HIGH_NS			(3600LL * NSEC_PER_SEC)


static void qpnp_sleepdetect_work_fn(struct work_struct *work)
{
	ktime_t kt;
	int capacity = 0;
	struct qpnp_bms_chip *chip = container_of(work, struct qpnp_bms_chip,
						sleepdetect_work);
	capacity = get_prop_bms_capacity(chip);
	if(capacity> 10){
		kt = ns_to_ktime(SLEEPDETECT_PERIOD_BATTERY_HIGH_NS);
	} else if (capacity> 5){
		kt = ns_to_ktime(SLEEPDETECT_PERIOD_BATTERY_MID_NS);
	} else {
		kt = ns_to_ktime(SLEEPDETECT_PERIOD_BATTERY_LOW_NS);
	}


	alarm_start_relative(&chip->sleepdetect_alarm, kt);
	printk("sleep detect end     capacity=%d\n",capacity);
	pm_relax(chip->dev);
}
static enum alarmtimer_restart sleepdetect_RTC_callback(struct alarm *alarm,
					ktime_t now)
{
	struct qpnp_bms_chip *chip = container_of(alarm, struct qpnp_bms_chip,sleepdetect_alarm);
	wake_lock_timeout(&chip->wake_lock,3*HZ);
	pm_stay_awake(chip->dev);
	schedule_work(&chip->sleepdetect_work);
	printk("sleep detect start  \n");
...
#ifdef BMS_CAPACITY_SLEEP_DETECT
	wake_lock_init(&chip->wake_lock, WAKE_LOCK_SUSPEND, "battery_wake_lock");
	INIT_WORK(&chip->sleepdetect_work, qpnp_sleepdetect_work_fn);
	alarm_init(&chip->sleepdetect_alarm, ALARM_REALTIME, sleepdetect_RTC_callback);
	kt = ns_to_ktime(SLEEPDETECT_PERIOD_INIT_NS);
	alarm_start_relative(&chip->sleepdetect_alarm, kt);
#endif
