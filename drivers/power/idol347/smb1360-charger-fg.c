...
#define CHARGING_TEMPERATURE_STEP1		390
#define CHARGING_TEMPERATURE_STEP2		420
int temp_above_step4 = 0;
int temp_above_step3 = 0;
int temp_above_step2 = 0;
int temp_above_step1 = 0;
int temp_above_step4_valid = 0;
int temp_above_step3_valid = 0;
int temp_above_step2_valid = 0;
int temp_above_step1_valid = 0;
...
/* JRD BSP START eric porting from NB, add power off condition */
#ifdef CONFIG_TCT_8X16_COMMON
	struct delayed_work         low_voltage_detect_work;
#endif
/* JRD BSP END eric porting from NB, add power off condition*/
	...
	bool				otp_rslow_config;//JRD BSP eric.gong "[Qualcomm]smb1360: Modify the rslow via OTP backup"
...
static int smb1360_enable_fg_access(struct smb1360_chip *chip)
{
	...
	while (retry < FG_ACCESS_RETRY_COUNT) {
		...
		if (count < FG_ACCESS_MAX_WAIT_COUNT) {
			pr_err("FG access granted at %d times\n", count);
 			break;
		} else {
			pr_err("FG access timeout, reset FG to request again, retry = %d\n",
							retry);
...
static int cold_hard_handler(struct smb1360_chip *chip, u8 rt_stat)
{
 ...
 /*
 * smb1360_batt_check_temp check current temperature 
 * different temperature range corresponding to different current/voltage
 * The function is required as idol3-4.7 battery charging policy
*/
static void smb1360_batt_check_temp(int temp)
{
	pr_err("temp==%d\n",temp);
	 //raise (39~41);fall(39~37)
	 if((temp > 380) && (temp < 420)){//39-41
		 temp_above_step1 = 1;
	 }
	 if(1 == temp_above_step1){//39-37
		 if(temp < 390){
			 temp_above_step1 = 0;
			 temp_above_step1_valid = 1;
		 }
	 }
	 //raise (42~44);fall(42~40)
	 if((temp > 410) && (temp < 450)){//42-44
		 temp_above_step2 = 1;
	 }
	 if(1 == temp_above_step2){//42-40
		 if(temp < 420){
			 temp_above_step2 = 0;
			 temp_above_step2_valid = 1;
		 }
	 }
	 //raise (45~54);fall(50~44)
	 if((temp > 440) && (temp < 550)){//45-55
		 temp_above_step3 = 1;
	 }
	 if(1 == temp_above_step3){//50-44
		 if(temp < 450){
			 temp_above_step3 = 0;
			 temp_above_step3_valid = 1;
		 }
	 }
	//raise 55+;fall (51-54)
	 if(temp > 540){//>=55
	 ...
	 if(1 == temp_above_step4){//51-54
		 if(temp < 550){
			 temp_above_step4 = 0;
			 temp_above_step4_valid = 1;
		 }
	 }
	pr_err("step1: %d, step2: %d, step3: %d,step1_valid: %d,step2_valid: %d,step3_valid: %d step4_valid:%d\n",
	temp_above_step1, temp_above_step2, temp_above_step3,
	temp_above_step1_valid, temp_above_step2_valid,
	temp_above_step3_valid,temp_above_step4_valid);
}
/*
...
static void smb1360_jeita_work_fn(struct work_struct *work)
{
	...
	smb1360_batt_check_temp(temp);
	if ((temp > chip->hot_bat_decidegc) || (1 == temp_above_step4_valid)) {// > 550
		...
		if(1 == temp_above_step4_valid){
			//chip->charging_disabled = true;
			pr_err("Do not disable charge for safety\n");//remove this way to disable charge required by HW-Gujianliang "2015-04-10"
			temp_above_step4_valid = 0;
		}
	} else if (((temp > chip->warm_bat_decidegc ||
		(temp == chip->warm_bat_decidegc && !!chip->soft_hot_rt_stat)) && (1 == temp_above_step3))||
		(1 == temp_above_step3_valid) || 
		(temp == chip->warm_bat_decidegc && !chip->soft_hot_rt_stat )) { // > 450
		...
		if(1 == temp_above_step3){
			rc = smb1360_set_soft_jeita_threshold(chip,
				chip->warm_bat_decidegc, chip->hot_bat_decidegc);//raise: soft cold:45 soft hot:55
			if (rc) {
				dev_err(chip->dev, "Couldn't set jeita threshold\n");
				goto end;
			}
		}
		if((1 == temp_above_step3_valid) || (temp == chip->warm_bat_decidegc && !chip->soft_hot_rt_stat )){
			rc = smb1360_set_soft_jeita_threshold(chip,
				CHARGING_TEMPERATURE_STEP2,chip->warm_bat_decidegc);//fall: soft cold:42 soft hot:45
			if (rc) {
				dev_err(chip->dev, "Couldn't set jeita threshold\n");
				goto end;
			}
			if(1 == temp_above_step3_valid)
			{
				temp_above_step3_valid = 0;
			}
		}
	} else if (((temp > chip->cool_bat_decidegc ||
		(temp == chip->cool_bat_decidegc && !chip->soft_cold_rt_stat)) &&
		((0 == temp_above_step1) || (0 == temp_above_step2) ||(0 == temp_above_step3))) ||
		(1 == temp_above_step2_valid) || (1 == temp_above_step1_valid)) {// > 100
		/* battery status is good, do the normal charging */
		chip->batt_warm = false;
		chip->batt_cool = false;
		if((temp < CHARGING_TEMPERATURE_STEP1) && (0 == temp_above_step1) && (0 == temp_above_step1_valid))//100-380/360-100
		{
			...
			if (rc)
				pr_err("Couldn't set temp current\n");
			rc = smb1360_set_soft_jeita_threshold(chip,
				chip->cool_bat_decidegc,CHARGING_TEMPERATURE_STEP1);//soft cold:10 soft hot:39
			if (rc) {
				dev_err(chip->dev, "Couldn't set jeita threshold\n");
				goto end;
			}
		}
		else if (((temp < CHARGING_TEMPERATURE_STEP2) && (0 == temp_above_step2) && (0 == temp_above_step2_valid) )
		||(1 == temp_above_step1_valid))//390-410/390-370
		{
			rc = smb1360_float_voltage_set(chip, chip->cool_bat_mv);
			if (rc) {
				dev_err(chip->dev, "Couldn't set float voltage\n");
			...
			rc = smb1360_system_temp_level_set(chip, THERMAL_MITIGATION_850);
			if (rc)
				pr_err("Couldn't set temp current\n");
			if(0 == temp_above_step2){
				rc = smb1360_set_soft_jeita_threshold(chip,
					CHARGING_TEMPERATURE_STEP1,CHARGING_TEMPERATURE_STEP2);//raise: soft cold:39 soft hot:42
				if (rc) {
					dev_err(chip->dev, "Couldn't set jeita threshold\n");
					goto end;
				}
			}
			if(1 == temp_above_step1_valid || (temp == CHARGING_TEMPERATURE_STEP1 && chip->soft_cold_rt_stat )){
				rc = smb1360_set_soft_jeita_threshold(chip,
					chip->cool_bat_decidegc,CHARGING_TEMPERATURE_STEP1);//fall: soft cold:10 soft hot:39
				if (rc) {
					dev_err(chip->dev, "Couldn't set jeita threshold\n");
					goto end;
				}
				if(1 == temp_above_step1_valid)
				{
					temp_above_step1_valid = 0;
				}
			}
		}
		else if(((temp < chip->warm_bat_decidegc) && (0 == temp_above_step3) && (0 == temp_above_step3_valid))
		|| (1 == temp_above_step2_valid))//420-440/430-400
		{
			rc = smb1360_float_voltage_set(chip, chip->cool_bat_mv);
			if (rc) {
				dev_err(chip->dev, "Couldn't set float voltage\n");
			...
			if (rc)
				pr_err("Couldn't set temp current\n");
			if(0 == temp_above_step3){
				rc = smb1360_set_soft_jeita_threshold(chip,
					CHARGING_TEMPERATURE_STEP2,chip->warm_bat_decidegc);//raise: soft cold:42 soft hot:45
				if (rc) {
					dev_err(chip->dev, "Couldn't set jeita threshold\n");
					goto end;
				}
			}
			if(1 == temp_above_step2_valid || (temp == CHARGING_TEMPERATURE_STEP2 && chip->soft_cold_rt_stat )){
				rc = smb1360_set_soft_jeita_threshold(chip,
					CHARGING_TEMPERATURE_STEP1,CHARGING_TEMPERATURE_STEP2);//fall: soft cold:39 soft hot:42
				if (rc) {
					dev_err(chip->dev, "Couldn't set jeita threshold\n");
					goto end;
				}
				if(1 == temp_above_step2_valid)
				{
					temp_above_step2_valid = 0;
				}
			}
		}
		...
		rc = smb1360_system_temp_level_set(chip, THERMAL_MITIGATION_550);
		if (rc)
			pr_err("Couldn't set temp current\n");
		rc = smb1360_set_soft_jeita_threshold(chip,
...
//JRD BSP START eric.gong "[Qualcomm]smb1360: Modify the rslow via OTP backup"
static int smb1360_otp_rslow_cfg(struct smb1360_chip *chip)
{
	int rc = 0, i;
	int64_t batt_id_uv;
	struct qpnp_vadc_result result;
	u8 reg_val_mapping[10][2]={{0,0}};
	...
	pr_err("current batt_id_uv=%lld\n",batt_id_uv);
	if(batt_id_uv>500000 && batt_id_uv<700000){//606000
		reg_val_mapping[0][0]=0xE0;reg_val_mapping[0][1]=0x56;
		reg_val_mapping[1][0]=0xE1;reg_val_mapping[1][1]=0x29;
		reg_val_mapping[2][0]=0xE2;reg_val_mapping[2][1]=0x57;
		reg_val_mapping[3][0]=0xE3;reg_val_mapping[3][1]=0x78;
		reg_val_mapping[4][0]=0xE4;reg_val_mapping[4][1]=0x54;
		reg_val_mapping[5][0]=0xE5;reg_val_mapping[5][1]=0x71;
		reg_val_mapping[6][0]=0xE6;reg_val_mapping[6][1]=0x55;
		reg_val_mapping[7][0]=0xE7;reg_val_mapping[7][1]=0x73;
		reg_val_mapping[8][0]=0xF0;reg_val_mapping[8][1]=0xAA;
		reg_val_mapping[9][0]=0xF1;reg_val_mapping[9][1]=0x00;
	}
	else if(batt_id_uv>0 && batt_id_uv<100000){//32000
		reg_val_mapping[0][0]=0xE0;reg_val_mapping[0][1]=0x54;
		reg_val_mapping[1][0]=0xE1;reg_val_mapping[1][1]=0xDA;
		reg_val_mapping[2][0]=0xE2;reg_val_mapping[2][1]=0x55;
		reg_val_mapping[3][0]=0xE3;reg_val_mapping[3][1]=0x71;
		reg_val_mapping[4][0]=0xE4;reg_val_mapping[4][1]=0x56;
		reg_val_mapping[5][0]=0xE5;reg_val_mapping[5][1]=0x34;
		reg_val_mapping[6][0]=0xE6;reg_val_mapping[6][1]=0x57;
		reg_val_mapping[7][0]=0xE7;reg_val_mapping[7][1]=0x71;
		reg_val_mapping[8][0]=0xF0;reg_val_mapping[8][1]=0xAA;
		reg_val_mapping[9][0]=0xF1;reg_val_mapping[9][1]=0x00;
	}
	else{
		pr_err("Unknown batt_id_uv=%lld\n",batt_id_uv);
		return 0;
	}
...
//JRD BSP END eric.gong "[Qualcomm]smb1360: Modify the rslow via OTP backup"
...
static int smb1360_check_batt_profile(struct smb1360_chip *chip)
{
	...
	for (i = 0; i < BATTERY_PROFILE_MAX; i++) {
		pr_debug("profile=%d profile_rid=%d connected_rid=%d\n", i,
						chip->profile_rid[i],
						chip->connected_rid);
		if (abs(chip->profile_rid[i] - chip->connected_rid) <=  //lfg 2015.12.23 task:1210386
...
/*JRD BSP START eric.gong  port from qcom method via NB site */
			//reg = DIV_ROUND_UP(chip->delta_soc * MAX_8_BITS, 100);
                        reg = abs(((chip->delta_soc * MAX_8_BITS) / 100) - 1);
/*JRD BSP END eric.gong  port from qcom method via NB site*/
	...
	if (chip->batt_capacity_mah != -EINVAL
		...
		if (chip->batt_capacity_mah != -EINVAL) {
			...
			if (fcc_mah == chip->batt_capacity_mah) {
				pr_debug("battery capacity correct = %d \n", fcc_mah);
			} else {
			    pr_debug("read default battery capacity = %d \n", fcc_mah);
...
static int smb1360_hw_init(struct smb1360_chip *chip)
{
	...
	if (chip->rsense_10mohm || chip->otp_rslow_config) {//JRD BSP eric.gong "[Qualcomm]smb1360: Modify the rslow via OTP backup"
	...
	//JRD BSP START Eric.Gong "fix discharge issue if temp>45 & vol>4.1"
	/* disabling batt_ov end charge protection. */
	rc = smb1360_masked_write(chip, 0x07, BIT(4), 0);
	if (rc) {
		dev_err(chip->dev, "Couldn't set batt_ov_end_charge rc = %d\n", rc);
	}
	//JRD BSP END Eric.Gong "fix discharge issue if temp>45 & vol>4.1"

	/* battery missing detection */
	rc = smb1360_masked_write(chip, CFG_BATT_MISSING_REG,
				BATT_MISSING_SRC_THERM_BIT | BATT_MISSING_SRC_BMD_BIT, //JRD BSP eric.gong@tcl.com "[idol3-4.7][charger]porting smb1360" 
...
/* JRD BSP START eric.gong "porting from NB", change irq mode*/
//			| CHG_STAT_ACTIVE_HIGH_BIT
/* JRD BSP END eric.gong "porting from NB" */
...
static int smb_parse_batt_id(struct smb1360_chip *chip)
{
	...
	for(i=0; i<RETRY_TIMES; i++)
	{
		...
		batt_id_uv = result.physical;
		if(!(batt_id_uv>500000 && batt_id_uv<700000)//606000-->TLP020K2
			&& !(batt_id_uv>0 && batt_id_uv<100000)//32000-->TLK020KJ
			&& !(batt_id_uv>1700000 && batt_id_uv<1900000)//no id-->1799000/1800000
		  )
		{
			pr_err("[%d]: batt_id(%lld) not in ranges, disable charging! \n", i, batt_id_uv);
		}
		else
		{
			pr_debug("[%d]: batt_id(%lld) in valid range ! \n", i, batt_id_uv);
			break;
		}
	}

	if(RETRY_TIMES==i){
		pr_err("10 times bad id!\n");
		//chip->charging_disabled = true;
	}
...
static int smb_parse_dt(struct smb1360_chip *chip)
{
	...
	chip->otp_rslow_config = of_property_read_bool(node,"qcom,otp-rslow-cfg");//JRD BSP eric.gong "[Qualcomm]smb1360: Modify the rslow via OTP backup"
...
/* JRD BSP END eric porting from NB, add power off condition */
	...
	dev_err(chip->dev, "lijian---SMB1360 revision=0x%x probe success! batt=%d usb=%d soc=%d\n",
			chip->revision,
...
/* JRD BSP START eric.gong "porting from NB"*/
                    | IRQ3_SOC_CHANGE_BIT
/* JRD BSP END eric.gong "porting from NB"*/
					| IRQ3_SOC_MIN_BIT
					| IRQ3_SOC_EMPTY_BIT);
