...
#define	ICE40_INFRARED_MAJOR		153	/* assigned */
...
ice40_infrared_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SPI_IOC_WR_MAX_SPEED_HZ:
		...
		if (retval == 0) {
			...
			spi->max_speed_hz = 960000;         // when testing . disable 10K set from apk  by allan
			retval = spi_setup(spi);
			if (retval < 0)
...
static int ice40_infrared_probe(struct spi_device *spi)
{
	 ...
	 */
	mutex_lock(&device_list_lock);
	minor = find_first_zero_bit(minors, ICE40_INFRARED_MINORS);
	if (minor < ICE40_INFRARED_MINORS) {
		struct device *dev;

		spidev->devt = MKDEV(ICE40_INFRARED_MAJOR, minor);
		dev = device_create(ice40_infrared_class, &spi->dev, spidev->devt,
					spidev, INFRARED_SYSDEV_NAME);
...
static struct of_device_id ice40_infrared_of_match_table[] = {
	{ .compatible = "infrared,ice40-spi-infrared", },
	{},
};

static struct spi_driver ice40_infrared_driver = {
	.driver = {
		.name =		"ice40_spi_infrared",
		.owner =	THIS_MODULE,
		.of_match_table = ice40_infrared_of_match_table,
	},
...
{
	spi_unregister_driver(&ice40_infrared_driver);
	class_destroy(ice40_infrared_class);
	unregister_chrdev(ICE40_INFRARED_MAJOR, ice40_infrared_driver.driver.name);
}
module_exit(ice40_infrared_exit);

module_spi_driver(ice40_infrared_driver);
