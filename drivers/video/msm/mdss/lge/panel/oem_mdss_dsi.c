						...
						struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->lge_pan_data->bkltex_en_gpio = of_get_named_gpio(ctrl_pdev->dev.of_node,
            "qcom,platform-bklightex-en-gpio", 0);
	if (!gpio_is_valid(ctrl_pdata->lge_pan_data->bkltex_en_gpio))
		pr_err("%s:%d, bkltex_en_gpio(%d) not specified\n",
				__func__, __LINE__, ctrl_pdata->lge_pan_data->bkltex_en_gpio);

	pr_info("%s tpvdd_en_gpio = %d, bkltex_en_gpio = %d\n", __func__,
            ctrl_pdata->lge_pan_data->tpvdd_en_gpio, ctrl_pdata->lge_pan_data->bkltex_en_gpio);
