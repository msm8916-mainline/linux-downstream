...
static struct panel_list supp_panels[] = {
	{"LGD SiW LG4895 INCELL 720p video mode dsi panel", LGD_LG4895_HD_DSI_VID_PANEL},
...
int pre_mdss_dsi_panel_power_ctrl(struct mdss_panel_data *pdata, int enable)
{
	int rc = 0;
	switch (panel_id) {
	case LGD_LG4895_HD_DSI_VID_PANEL:
		rc = lgd_lg4895_hd_video_pre_mdss_dsi_panel_power_ctrl(pdata, enable);
        break;
...
int lge_msm_dss_enable_vreg(struct mdss_dsi_ctrl_pdata *ctrl_pdata, int enable)
{
	...
	switch (panel_id) {
	case LGD_LG4895_HD_DSI_VID_PANEL:
		rc = lgd_lg4895_hd_video_msm_dss_enable_vreg(ctrl_pdata, enable);
        break;
	case LGD_LG4945_QHD_DUALDSI_CMD_PANEL:
...
int lge_mdss_dsi_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	switch (panel_id) {
	case LGD_LG4895_HD_DSI_VID_PANEL:
		rc = lgd_lg4895_hd_video_mdss_dsi_request_gpios(ctrl_pdata);
		break;
	case LGD_LG4945_QHD_DUALDSI_CMD_PANEL:
...
int lge_mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
    ...
    if ((pinfo->lge_pan_info.lge_panel_send_off_cmd==false) ||
        (pinfo->lge_pan_info.lge_panel_send_on_cmd==false))
        return rc;
#endif
	switch (panel_id) {
	case LGD_LG4895_HD_DSI_VID_PANEL:
		rc = lgd_lg4895_hd_video_mdss_dsi_panel_reset(pdata, enable);
		break;
	case LGD_LG4945_QHD_DUALDSI_CMD_PANEL:
...
int lge_dsi_panel_device_register(struct device_node *pan_node, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	switch (panel_id) {
	case LGD_LG4895_HD_DSI_VID_PANEL:
		rc = lgd_lg4895_hd_video_dsi_panel_device_register(pan_node, ctrl_pdata);
		break;
	case LGD_LG4945_QHD_DUALDSI_CMD_PANEL:
...
int lge_mdss_panel_parse_dt(struct device_node *pan_node, struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	switch (panel_id) {
	case LGD_LG4895_HD_DSI_VID_PANEL:
		rc = lgd_lg4895_hd_video_mdss_panel_parse_dt(pan_node, ctrl_pdata);
		break;
	case LGD_LG4945_QHD_DUALDSI_CMD_PANEL:
...
int lge_mdss_dsi_panel_bl_ctrl(struct mdss_panel_data *pdata, u32 bl_level)
{
	...
	switch (panel_id) {
	case LGD_LG4895_HD_DSI_VID_PANEL:
		rc = lgd_lg4895_hd_video_mdss_dsi_panel_bl_ctrl(pdata, bl_level);
		break;
	case LGD_LG4945_QHD_DUALDSI_CMD_PANEL:
...
//	panel_name = of_get_property(dsi_pan_node, "qcom,mdss-dsi-panel-name", NULL);
	panel_name = "LGD SiW LG4895 INCELL 720p video mode dsi panel";
