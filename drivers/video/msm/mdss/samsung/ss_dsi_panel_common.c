...
int mdss_samsung_panel_on_post(struct mdss_panel_data *pdata)
{
	...
	if (unlikely(vdd->init_bl_level)) {
		mutex_lock(&vdd->mfd_dsi[DISPLAY_1]->bl_lock);
		if (!vdd->mfd_dsi[DISPLAY_1]->unset_bl_level)
			mdss_fb_set_backlight(vdd->mfd_dsi[DISPLAY_1], vdd->init_bl_level);
...
int mdss_samsung_panel_off_post(struct mdss_panel_data *pdata)
{
	...
	if (vdd->mfd_dsi[DISPLAY_1]->bl_level && !vdd->init_bl_level) {
		mutex_lock(&vdd->mfd_dsi[DISPLAY_1]->bl_lock);
...
*
**************************************************************/
unsigned long kvaddr_to_paddr(unsigned long vaddr)
{
	pgd_t *pgd;
	pud_t *pud;
	pmd_t *pmd;
...
void mdss_samsung_dump_regs(void)
{
	...
	for(loop = 0; loop < mdata->nvig_pipes ; loop++) {
		...
		pr_err("=============%s 0x%08zx ==============\n", name,
			(size_t)kvaddr_to_paddr((unsigned long)mdata->vig_pipes[loop].base));
	...
	for(loop = 0; loop < mdata->nrgb_pipes ; loop++) {
		...
		pr_err("=============%s 0x%08zx ==============\n", name,
			(size_t)kvaddr_to_paddr((unsigned long)mdata->rgb_pipes[loop].base));
	...
	for(loop = 0; loop < mdata->ndma_pipes ; loop++) {
		...
		pr_err("=============%s 0x%08zx ==============\n", name,
			(size_t)kvaddr_to_paddr((unsigned long)mdata->dma_pipes[loop].base));
	...
	for(loop = 0; loop < mdata->nmixers_intf ; loop++) {
		...
		pr_err("=============%s 0x%08zx ==============\n", name,
			(size_t)kvaddr_to_paddr((unsigned long)mdata->mixer_intf[loop].base));
	...
	for(loop = 0; loop < mdata->nmixers_wb ; loop++) {
		...
		pr_err("=============%s 0x%08zx ==============\n", name,
			(size_t)kvaddr_to_paddr((unsigned long)mdata->mixer_wb[loop].base));
	...
	for(loop = 0; loop < mdata->nmixers_intf ; loop++) {
		...
		pr_err("=============%s 0x%08zx ==============\n", name,
			(size_t)kvaddr_to_paddr((unsigned long)mdata->mixer_intf[loop].pingpong_base));
...
void mdss_samsung_dsi_dump_regs(int dsi_num)
{
	...
	pr_err("=============%s 0x%08zx ==============\n", name,
		(size_t)kvaddr_to_paddr((unsigned long)dsi_ctrl[dsi_num]->ctrl_io.base));
	mdss_dump_reg((char *)dsi_ctrl[dsi_num]->ctrl_io.base, dsi_ctrl[dsi_num]->ctrl_io.len);

	snprintf(name, sizeof(name), "DSI%d PHY", dsi_num);
	pr_err("=============%s 0x%08zx ==============\n", name,
		(size_t)kvaddr_to_paddr((unsigned long)dsi_ctrl[dsi_num]->phy_io.base));
	...
	pr_err("=============%s 0x%08zx ==============\n", name,
		(size_t)kvaddr_to_paddr((unsigned long)vdd_data.dump_info[dsi_num].dsi_pll.virtual_addr));
