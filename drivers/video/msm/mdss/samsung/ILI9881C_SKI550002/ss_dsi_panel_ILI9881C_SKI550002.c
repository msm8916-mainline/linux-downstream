...
#include "ss_dsi_mdnie_ILI9881C_SKI550002.h"
/*
#if defined (CONFIG_PANEL_ILI9881C_SKI550002_HD_5_0)
#include "ss_dsi_mdnie_ILI9881C_SKI550002_5_0.h"
#elif defined (CONFIG_PANEL_ILI9881C_SKI550002_HD_5_5)
#include "ss_dsi_mdnie_ILI9881C_SKI550002_5_5.h"
#else
...
static struct dsi_panel_cmds * mdss_brightness_tft_pwm(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	bl_map = (unsigned char)vdd->scaled_level;
	if(bl_map == 0)
	{
		vdd->dtsi_data[ctrl->ndx].tft_pwm_tx_cmds->cmds->payload[1] = bl_map;
		vdd->dtsi_data[ctrl->ndx].tft_pwm_tx_cmds->cmds->payload[2] = bl_map;
	...
	vdd->dtsi_data[ctrl->ndx].tft_pwm_tx_cmds->cmds->payload[1] = bl_map>>4;
	vdd->dtsi_data[ctrl->ndx].tft_pwm_tx_cmds->cmds->payload[2] = (bl_map<<4)|0xF;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_ILI9881C_SKI550002_HD";
