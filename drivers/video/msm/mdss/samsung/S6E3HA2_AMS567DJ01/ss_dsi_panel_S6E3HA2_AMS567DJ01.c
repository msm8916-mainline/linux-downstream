...
/*temp_joann*/
	...
	else if (mdss_panel_id2_get(ctrl) == 0x8)
...
static struct dsi_panel_cmds * mdss_gamma_hmt(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
...
static struct dsi_panel_cmds *mdss_aid_hmt(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
	int cmd_idx = 0;
...
static struct dsi_panel_cmds *mdss_elvss_hmt(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
...
static struct dsi_panel_cmds *mdss_vint_hmt(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct samsung_display_driver_data *vdd = check_valid_ctrl(ctrl);
...
static void mdss_make_sdimconf_hmt(struct mdss_dsi_ctrl_pdata *ctrl, struct samsung_display_driver_data *vdd) {
	...
	mdss_samsung_panel_data_read(ctrl,
				&(vdd->dtsi_data[ctrl->ndx].smart_dimming_mtp_rx_cmds[vdd->panel_revision]),
				vdd->smart_dimming_dsi_hmt[ctrl->ndx]->mtp_buffer, PANEL_LEVE2_KEY);


	/* Read mtp (B4h 2nd ~ 31th) for HMT 150cd */
	mdss_samsung_panel_data_read(ctrl,
				&(vdd->dtsi_data[ctrl->ndx].hmt_150cd_rx_cmds[vdd->panel_revision]),
				mtp_buffer_for_150cd, PANEL_LEVE2_KEY);
	...
	if (vdd->smart_dimming_dsi_hmt[ctrl->ndx]->set_para_for_150cd)
		vdd->smart_dimming_dsi_hmt[ctrl->ndx]->set_para_for_150cd(mtp_buffer_for_150cd, 30);
	/* Just a safety check to ensure smart dimming data is initialised well */
...
static int mdss_samart_dimming_init_hmt(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (IS_ERR_OR_NULL(vdd->smart_dimming_dsi_hmt[ctrl->ndx])) {
		...
		vdd->hmt_stat.hmt_low_persistence = 1;
...
static void  mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_S6E3HA2_AMS567DJ01;
	...
	vdd->panel_func.samsung_smart_get_conf_hmt = smart_get_conf_S6E3HA2_AMS567DJ01_hmt;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_S6E3HA2_AMS567DJ01_WQHD";
