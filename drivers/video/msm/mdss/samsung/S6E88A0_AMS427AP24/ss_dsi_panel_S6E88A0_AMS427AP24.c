...
static int mdss_panel_on_post(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	vdd->display_ststus_dsi[0].wait_disp_on = false;
	vdd->dtsi_data[ctrl->ndx].avc_on_tx_cmds[vdd->panel_revision].link_state = DSI_HS_MODE;

	avc_ctrl = ctrl;
	INIT_DELAYED_WORK(&avc_work, delayed_avc_func);
	schedule_delayed_work(&avc_work, msecs_to_jiffies(150));
