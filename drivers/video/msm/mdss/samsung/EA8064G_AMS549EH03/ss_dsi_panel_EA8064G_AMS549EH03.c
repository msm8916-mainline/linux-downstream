 ...
 *
*/
#include "ss_dsi_panel_EA8064G_AMS549EH03.h"
#include "ss_dsi_mdnie_EA8064G_AMS549EH03.h"
...
static struct dsi_panel_cmds elvss_cmd1;
static struct dsi_panel_cmds * mdss_elvss(struct mdss_dsi_ctrl_pdata *ctrl, int *level_key)
{
	...
	if (vdd->acl_status || vdd->siop_status) {
		...
		elvss_cmd1.cmds = &(vdd->dtsi_data[ctrl->ndx].smart_acl_elvss_tx_cmds[vdd->panel_revision].cmds[cmd_idx]);
	} else {
		if (vdd->temperature > 0)
		...
		elvss_cmd1.cmds = &(vdd->dtsi_data[ctrl->ndx].elvss_tx_cmds[vdd->panel_revision].cmds[cmd_idx]);
	}

	elvss_cmd1.cmd_cnt = 1;
...
static void  mdss_panel_init(struct samsung_display_driver_data *vdd)
{
	...
	vdd->panel_func.samsung_smart_get_conf = smart_get_conf_EA8064G_AMS549EH03;
...
static int __init samsung_panel_init(void)
{
	...
	char panel_string[] = "ss_dsi_panel_EA8064G_AMS549EH03_FHD";
