...
#ifdef CONFIG_LGE_DISPLAY_POWER_SEQUENCE
	if (lge_mdss_dsi.pre_mdss_dsi_panel_reset)
		ret = lge_mdss_dsi.pre_mdss_dsi_panel_reset(pdata, 0);
	else
...
#ifdef CONFIG_LGE_DISPLAY_POWER_SEQUENCE
	if (lge_mdss_dsi.post_mdss_dsi_panel_reset)
		lge_mdss_dsi.post_mdss_dsi_panel_reset(pdata, 0);
...
static int mdss_dsi_off(struct mdss_panel_data *pdata, int power_state)
{
	...
	if (lge_mdss_dsi.pre_mdss_dsi_panel_power_ctrl)
		ret = lge_mdss_dsi.pre_mdss_dsi_panel_power_ctrl(pdata, 0);
#endif
...
panel_power_ctrl:
	...
	if (lge_mdss_dsi.post_mdss_dsi_panel_power_ctrl)
		ret = lge_mdss_dsi.post_mdss_dsi_panel_power_ctrl(pdata, 0);
#endif
	pr_info("%s: -\n", __func__);
...
#ifdef CONFIG_LGE_DISPLAY_POWER_SEQUENCE
	if (lge_mdss_dsi.lge_mdss_dsi_lane_config)
		ret = lge_mdss_dsi.lge_mdss_dsi_lane_config(pdata, 0);
#endif
...
void mdss_dsi_dump_reg(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	u32 val;
	mdss_dsi_clk_ctrl(ctrl_pdata, DSI_ALL_CLKS, 1);
	val = MIPI_INP((ctrl_pdata->ctrl_base) + 0x008);
	pr_err("DSI STATUS REG: 0x%x\n", val);
	mdss_dsi_clk_ctrl(ctrl_pdata, DSI_ALL_CLKS, 0);
}
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_DSI_PANEL_STATUS:
		...
		break;
	case MDSS_EVENT_DSI_DUMP_REG:
		mdss_dsi_dump_reg(ctrl_pdata);
		break;
	default:
				...
				struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	ctrl_pdata->knock_on_enabled = of_property_read_bool(ctrl_pdev->dev.of_node,
...
#ifdef CONFIG_LGE_DISPLAY_POWER_SEQUENCE
	if (lge_mdss_dsi.lge_panel_device_create)
		lge_mdss_dsi.lge_panel_device_create(pan_node, ctrl_pdata);

	if (lge_mdss_dsi.lge_mdss_panel_parse_dt)
		lge_mdss_dsi.lge_mdss_panel_parse_dt(pan_node, ctrl_pdata);
