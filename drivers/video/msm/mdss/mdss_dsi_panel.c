...
#if defined(CONFIG_LGD_INCELL_VIDEO_FWVGA_PT_PANEL) || defined(CONFIG_LGE_DUALITY_JDI_LGD)
...
bklt_en_gpio_err:
	return rc;
#elif defined(CONFIG_JDI_INCELL_VIDEO_HD_PANEL)|| defined(CONFIG_LGD_INCELL_PHASE3_VIDEO_HD_PT_PANEL) || defined(CONFIG_JDI_INCELL_VIDEO_FHD_PANEL)  || defined(CONFIG_LGE_DUALITY_JDI_LGD)
mode_gpio_err:
	if (gpio_is_valid(ctrl_pdata->bklt_en_gpio))
		gpio_free(ctrl_pdata->bklt_en_gpio);
bklt_en_gpio_err:
...
static int mdss_dsi_panel_off(struct mdss_panel_data *pdata)
{
	...
	if (ctrl->off_cmds.cmd_cnt) {
		...
		else if (pinfo->display_id == SECONDARY_MODULE) {
			mdss_dsi_panel_cmds_send(ctrl, &ctrl->off_cmds);
			mdelay(120);
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	else if (pinfo->display_id == PRIMARY_MODULE) {
		mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->off_cmds,
			"qcom,mdss-dsi-off-command", "qcom,mdss-dsi-off-command-state");
	}
	else {
		pr_err("Can't find display id\n");
		return -EINVAL;
	}
#else
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->off_cmds,
