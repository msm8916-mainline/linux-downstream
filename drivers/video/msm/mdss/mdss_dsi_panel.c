...
//lm add
static int mdss_dsi_parse_panel_status_command_seq(struct device_node *np,
		u32 esd_value_array[ESD_STATUS_VALUE_MAX_LEN], u32 *esd_value_len,
		const char *name)
{
	int num = 0, i;
	int rc;
	struct property *data;
	u32 tmp[ESD_STATUS_VALUE_MAX_LEN];
	*esd_value_len = 0;
	data = of_find_property(np, name, &num);
	num /= sizeof(u32);
	pr_err("%s, num = %d\n", __func__, num);
	if (!data || !num || num > ESD_STATUS_VALUE_MAX_LEN) {
...
static int mdss_dsi_gen_read_status_array(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int i;

	if(ctrl_pdata->status_value_len1)	//if has another cmd for esd check
	{
		for(i = 0; i < ctrl_pdata->status_value_len1; i++)
		{ 
			if (ctrl_pdata->status_buf1.data[i] !=
							ctrl_pdata->status_value_array1[i]) {
				pr_err(" %s: Read back value from panel is incorrect, ctrl_pdata->status_buf1.data[%d] = 0x%x, ctrl_pdata->status_value_array1[%d] = 0x%x\n",
									__func__, i, ctrl_pdata->status_buf1.data[i], i, ctrl_pdata->status_value_array1[i]);
				return -EINVAL;
			} 
		}
	}
	if (ctrl_pdata->status_buf.data[0] !=
					ctrl_pdata->status_value) {
		pr_err(" %s: Read back value from panel is incorrect, ctrl_pdata->status_buf.data[%d] = 0x%x, ctrl_pdata->status_value_array[%d] = 0x%x\n",
							__func__, i, ctrl_pdata->status_buf.data[i], i, ctrl_pdata->status_value_array[i]);
		return -EINVAL;
	} 
	//printk("liumiao the data is %x,%x,%x,%x\n",ctrl_pdata->status_buf1.data[0],ctrl_pdata->status_buf1.data[1],ctrl_pdata->status_buf1.data[2],ctrl_pdata->status_buf.data[0]);
	return 1;
}
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	mdss_dsi_parse_dcs_cmds(np, &ctrl_pdata->status_cmds1,
			"qcom,mdss-dsi-panel-status-command1",
				"qcom,mdss-dsi-panel-status-command-state");
      // mdss_dsi_parse_panel_status_command_seq(np, ctrl_pdata->status_value_array, &(ctrl_pdata->status_value_len),
             //  "qcom,mdss-dsi-panel-status-value");
	mdss_dsi_parse_panel_status_command_seq(np, ctrl_pdata->status_value_array1, &(ctrl_pdata->status_value_len1),
	...
	if (!rc) {
		if (!strcmp(data, "bta_check")) {
			...
			ctrl_pdata->check_read_status =
						mdss_dsi_gen_read_status_array;
			if(ctrl_pdata->status_cmds1.blen)
				ctrl_pdata->status_cmds_rlen1 = 4;
