...
void mdp3_dmap_dump_reg(void)
{
	u32 val, i = 0x90000;

	pr_err("DMAP REG DUMP\n");
	while (i < 0x9009D) {
		val = MDP3_REG_READ(i);
		pr_err("REG 0x%x VAL 0x%x\n", i , val);
		i = i + 4;
	}
	pr_err("SYNC REG DUMP\n");
	for (i = 0x200; i < 0x220; i = i + 4) {
		val = MDP3_REG_READ(i);
		pr_err("REG 0x%x VAL 0x%x\n", i , val);
	}
	for (i = 0x300; i < 0x332; i = i + 4) {
		val = MDP3_REG_READ(i);
		pr_err("REG 0x%x VAL 0x%x\n", i , val);
	}
}
	...
	struct mdp3_dma_source *source_config, struct mdp3_tear_check *te)
{
	...
	MDP3_REG_WRITE(MDP3_REG_DMA_P_LINE_IRQ, total_lines);
	...
	pr_err("MDP3_REG_SYNC_CONFIG_0 + 0x%x\n", cfg);
