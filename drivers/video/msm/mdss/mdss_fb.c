		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret;
	char *str = "hx8394d";
	if(strncmp(pinfo->panel_name, str, 7))
		ret = scnprintf(buf, PAGE_SIZE,"Novatek BOE 1280*720\n");
	else
		ret = scnprintf(buf, PAGE_SIZE,"Himax DSBJ 1280*720\n");
...
static DEVICE_ATTR(msm_panel_name, S_IRUGO, mdss_fb_get_panel_name,NULL);
...
static int mdss_fb_unblank_sub(struct msm_fb_data_type *mfd)
{
	...
	if(mfd->esd_recovery)			//lm for esd
		mfd->esd_recovery = 0;
