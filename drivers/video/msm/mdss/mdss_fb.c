...
#endif
#if defined(CONFIG_TIANMA_ILI9488_HVGA_CMD_PANEL) || defined (CONFIG_LGE_DISPLAY_LCD_OFF_DIMMING)
#include <soc/qcom/lge/lge_boot_mode.h>
#endif
#if defined(CONFIG_TIANMA_ILI9488_HVGA_CMD_PANEL)
...
#ifdef CONFIG_LGE_DISPLAY_LCD_OFF_DIMMING
	if((lge_get_bootreason() == FOTA_REBOOT_OUT_LCDOFF || lge_get_bootreason() == FOTA_REBOOT_LCDOFF )&& !fb_blank_called){
...
static int mdss_fb_probe(struct platform_device *pdev)
{
	...
	if (lge_get_bootreason() == RECOVERY_MODE || lge_get_bootreason() == FOTA_REBOOT || lge_fota_boot() == LGE_FOTA_BOOT ||
#ifdef CONFIG_LGE_DISPLAY_LCD_OFF_DIMMING
			lge_get_bootreason() == FOTA_REBOOT_LCDOFF ||
#endif
			lge_get_laf_mode() == LGE_LAF_MODE_LAF || lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO)
	{
		mfd->fb_imgType = MDP_RGBA_8888;
		pr_info("%s: selecting RGBA_8888\n", __func__);
	} else {
		mfd->fb_imgType = MDP_RGB_565;
		pr_info("%s: selecting RGB_565\n", __func__);
	}
#else
	mfd->fb_imgType = MDP_RGBA_8888;
#endif
