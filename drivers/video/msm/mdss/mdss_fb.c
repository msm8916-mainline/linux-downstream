...
bool flag_knock_on_ctrl_during_panel_on=true;
#endif
...
#if defined(CONFIG_LGD_PH1DONGBU_INCELL_VIDEO_HD_PANEL)
	if ((bkl_lvl != 0) && (!flag_knock_on_ctrl_during_panel_on)){
		flag_knock_on_ctrl_during_panel_on = true;
		pr_info("%s: Display sequence end, knock_on_ctrl_flag=%d\n", __func__, flag_knock_on_ctrl_during_panel_on);
	}
#endif
