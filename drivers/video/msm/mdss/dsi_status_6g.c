...
void mdss_check_dsi_ctrl_status(struct work_struct *work, uint32_t interval)
{
	...
	pr_debug("%s: ret = %d, underrun_tcnt=%d\n", __func__, ret, ctl->underrun_tcnt);
	...
	if ((pstatus_data->mfd->panel_power_state == MDSS_PANEL_POWER_ON)) {
		if( ctl->underrun_tcnt > 5){
				pr_err("liumiao the underrun\n");
				pstatus_data->mfd->esd_recovery = 1;   //lm for underrupt
				mdss_report_panel_dead(pstatus_data);
				ctl->underrun_tcnt  = 0;}
		else if (ret > 0)
			schedule_delayed_work(&pstatus_data->check_status,
			...
			pstatus_data->mfd->esd_recovery = 1;   //lm for esd check
			mdss_report_panel_dead(pstatus_data);
			}
	}
}
