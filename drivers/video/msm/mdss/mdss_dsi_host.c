...
static int mdss_dsi_read_status(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if(ctrl->status_cmds1.blen)			//lm for two comds
	{
		memset(&cmdreq, 0, sizeof(cmdreq));
		cmdreq.cmds = ctrl->status_cmds1.cmds;
		cmdreq.cmds_cnt = ctrl->status_cmds1.cmd_cnt;
		cmdreq.flags = CMD_REQ_COMMIT | CMD_CLK_CTRL | CMD_REQ_RX;
		cmdreq.rlen = ctrl->status_cmds_rlen1;
		cmdreq.cb = NULL;
		cmdreq.rbuf = ctrl->status_buf1.data;
