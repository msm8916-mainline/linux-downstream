...
if (!size || !args)
		return 0;
#ifdef SMS_TS_RECORD
	smscore_ts_file_write(coredev, ts_buffer, size);
#endif
...
#ifdef SMS_TS_RECORD
		smscore_ts_file_open(smsdev->coredev, "/data/out.ts", O_RDWR | O_CREAT,0644);
#endif
...
#ifdef SMS_TS_RECORD
	smscore_ts_file_close(g_smsi2c_device->coredev);
#endif
