...
#ifdef SMS_LARGE_MEM_MALLOC_ONCE
#define COMMOM_SIZE (MAX_BUF_SIZE*MAX_BUF_NUMBER)
void *gCELLBuffAddr=NULL;
#endif
...
#define MDTV_NAME "SMS4470"
static struct class *class_mdtvinfo = NULL;
...
static int smschar_onresponse(void *context, struct smscore_buffer_t *cb)
{
	...
	if (dev->pending_count > SMS_CHR_MAX_Q_LEN) {
		spin_unlock_irqrestore(&dev->lock, flags);
		sms_err("pending_count overflow!");
		return -EBUSY;
	}
...
#if 0 //wood, for test	
	{
		struct SmsMsgHdr_S *phdr = (struct SmsMsgHdr_S *) ((u8 *) cb->p
			+ cb->offset);
		sms_debug("msgType:%d, cb:0x%x", phdr->msgType, cb);
...
#define OPEN_COUNT_INIT 1
static atomic_t open_count = ATOMIC_INIT(OPEN_COUNT_INIT);
#endif
static int smschar_open(struct inode *inode, struct file *file)
{
	...
	if (dev->cdev.dev == MKDEV(smschar_major, smschar_minor)) { //not all device needed, just "mdtvctrl"
		sms_debug("count:%d, index=%d\n", atomic_read(&open_count), dev->device_index);
		if ((OPEN_COUNT_INIT + 1) == atomic_inc_return(&open_count)) {
			//sms_warn("open_count:%d, dev->coredev:%#x\n", atomic_read(&open_count), dev->coredev);
			if (!(dev->coredev)) {
				sms_chip_plug(1);
				if (!dev->coredev) {
					...
					sms_debug("sms chip wait for plug in times:%d", times);
				}
			}
		}
	} else {
...
static int smschar_release(struct inode *inode, struct file *file)
{
	...
	if (dev->cdev.dev == MKDEV(smschar_major, smschar_minor)) { //not all device needed, just "mdtvctrl"
		int times = 100;
		sms_debug("count:%d, index=%d\n", atomic_read(&open_count), dev->device_index);
		if (OPEN_COUNT_INIT == atomic_dec_return(&open_count)) {
			//sms_warn("open_count:%d, dev->coredev:%#x\n", atomic_read(&open_count), dev->coredev);
			sms_chip_plug(0);
			if (dev->coredev) {
				while (times-- && dev->coredev){
...
if (!copy_from_user(msg_buffer, buf, count)) {
	int rc;
	rc = smsclient_sendrequest(dev->smsclient,
				  msg_buffer, count);
	if (rc < 0)
		sms_err("*****smsclient_sendrequest() error: %d\n", rc );
}
...
#if 0 //wood, for test	
	{
		struct smscore_buffer_t *cb = dev->currentcb;
		
		struct SmsMsgHdr_S *phdr = (struct SmsMsgHdr_S *) ((u8 *) cb->p
			+ cb->offset);
		sms_debug("msgType:%d, cb:0x%x, offset:%d, size:%d", phdr->msgType, cb, touser->offset, \
			touser->size);
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	case SMSCHAR_SEND_FW_FILE_COMPAT:
		{
	...
	default:
			sms_info("forword to smschar_ioctl()\n");
...
/********************* add for deviceInfo **********************/
static ssize_t class_mdtv_info_show(struct class *class,
...
static CLASS_ATTR(mdtv_info, 0660, class_mdtv_info_show, NULL);
/********************* end ck-changjun.li **********************/
int smschar_register(void)
{
	...
	class_mdtvinfo = class_create(THIS_MODULE,"mdtv");
	if (IS_ERR(class_mdtvinfo))
		return PTR_ERR(class_mdtvinfo );
		err = class_create_file(class_mdtvinfo,&class_attr_mdtv_info);
	if (err)
		sms_err("create sys file failed \n");
...
#ifdef SMS_LARGE_MEM_MALLOC_ONCE
		gCELLBuffAddr = kmalloc_node(COMMOM_SIZE, GFP_ATOMIC, NUMA_NO_NODE);
		if (gCELLBuffAddr){
			printk("[SMS] gCELLBuffAddr:0x%p\r\n",gCELLBuffAddr);
		}else{
			printk("[SMS] gCELLBuffAddr dma_alloc_coherent fail !!!\r\n");
		}
#endif
...
void smschar_unregister(void)
{
	...
	class_remove_file(class_mdtvinfo,&class_attr_mdtv_info);
	class_destroy(class_mdtvinfo);

#ifdef SMS_LARGE_MEM_MALLOC_ONCE
	if (gCELLBuffAddr){
		printk("smsmdtv free gCELLBuffAddr\n");
		kfree(gCELLBuffAddr);
		gCELLBuffAddr = NULL;
