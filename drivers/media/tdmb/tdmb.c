...
#endif
	gpio_direction_output(dt_pdata->tdmb_en, 0);
	gpio_direction_input(dt_pdata->tdmb_en);
	usleep_range(1000, 1000);
	if (dt_pdata->tdmb_use_rst) {
		gpio_direction_output(dt_pdata->tdmb_rst, 0);
		gpio_direction_input(dt_pdata->tdmb_rst);
	}
	if (dt_pdata->tdmb_use_irq)
		gpio_direction_input(dt_pdata->tdmb_irq);
...
static struct tdmb_dt_platform_data *get_tdmb_dt_pdata(struct device *dev)
{
	...
	pdata->gpio_init = pinctrl_lookup_state(pdata->tdmb_pinctrl, "tdmb_gpio_init");
	if(IS_ERR(pdata->gpio_init)) {
		DPRINTK("%s : could not get pins tdmb_gpio_init state (%li)\n",
...
static int tdmb_probe(struct platform_device *pdev)
{
	...
	if (pinctrl_select_state(dt_pdata->tdmb_pinctrl, dt_pdata->gpio_init))
