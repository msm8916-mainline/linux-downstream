	...
	struct msm_camera_csiphy_params *csiphy_params)
{
	...
	while (lane_mask & 0x1f) {
		...
		if(csiphy_dev->pdev->id == 0){	//main camera
			...
			#if defined (CONFIG_HI707) || defined (CONFIG_MSM8909_CF)
			if(csiphy_dev->hw_version >= CSIPHY_VERSION_V30) {
				msm_camera_io_w(0xC, csiphybase + csiphy_dev->ctrl_reg->csiphy_reg.
				mipi_csiphy_lnn_cfg4_addr + 0x40*j);
			}
...
/* This is workaround code that remove error log after send start stream to hi351*/
#if defined(CONFIG_MSM8909_CF) || defined(CONFIG_HI351)
		break;
#else
