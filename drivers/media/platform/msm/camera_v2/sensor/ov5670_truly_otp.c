...
#define RG_RATIO_TYPICAL  304
#define BG_RATIO_TYPICAL  300
    ...
    uint32_t index, struct ov5670_otp_struct *otp_ptr)
{
    ...
    (*otp_ptr).rg_ratio = (otp_i2c_read(s_ctrl, start_addr) << 2) + ((temp >> 6) & 0x03);
    (*otp_ptr).bg_ratio = (otp_i2c_read(s_ctrl, (start_addr + 1)) << 2) + ((temp >> 4) & 0x03);
    ...
    rg_gld = (otp_i2c_read(s_ctrl, (start_addr + 4)) << 2) + ((temp >> 6) & 0x03);
    bg_gld = (otp_i2c_read(s_ctrl, (start_addr + 5)) << 2) + ((temp >> 4) & 0x03);
    ...
    uint32_t R_gain, uint32_t G_gain, uint32_t B_gain)
{
    if (R_gain > 0x400) {
    	...
    	otp_i2c_write(s_ctrl, 0x5033, (R_gain & 0x00ff));
    }
    if (G_gain > 0x400) {
    	otp_i2c_write(s_ctrl, 0x5034, (G_gain >> 8));
    	otp_i2c_write(s_ctrl, 0x5035, (G_gain & 0x00ff));
    }
    if (B_gain > 0x400) {
    	otp_i2c_write(s_ctrl, 0x5036, (B_gain >> 8));
    	otp_i2c_write(s_ctrl, 0x5037, (B_gain & 0x00ff));
    }
    return 0;
}

int32_t ov5670_truly_otp_config(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if (otp_index >= 1 && otp_index <= 3) {
        ...
        otp_update_awb(s_ctrl, R_gain, G_gain, B_gain);
    }
...
uint16_t ov5670_truly_otp_get_vcm_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	struct ov5670_otp_struct current_otp = {0};
	uint32_t otp_index = 0;
