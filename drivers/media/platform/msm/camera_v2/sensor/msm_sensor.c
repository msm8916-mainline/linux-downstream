...
/* This is workaround code that sending start stream command to hi351 for capture to avoid sof freeze */
#if defined(CONFIG_HI351) || defined(CONFIG_MSM8909_CF)
		if(s_ctrl->sensordata->sensor_info->position==0){
	struct sensorb_cfg_data cfg;
		cfg.cfgtype = CFG_SET_START_STREAM;
		s_ctrl->func_tbl->sensor_config(s_ctrl, (void*)&cfg);
		pr_err("start stream recommand at sof freeze\n");
			}
#endif
		return 0;
	default:
