...
extern int ov13850_qtech_update_otp(struct msm_sensor_ctrl_t *s_ctrl);
extern int ov13850_match_module_id(struct msm_sensor_ctrl_t *s_ctrl);
extern unsigned int g_hi545_module_id;
...
int msm_sensor_match_id(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if (!strcmp(sensor_name, "ov13850")) {
		rc=ov13850_match_module_id(s_ctrl);
		if (rc==0x06) {
	...
	if (!strcmp(sensor_name, "hi545") || !strcmp(sensor_name, "hi545_avc")) {
		printk("%s:%d: hi545 module_id: 0x%04x \n", __func__, __LINE__, g_hi545_module_id);
		if (!(g_hi545_module_id == 0x07 && !strcmp(sensor_name, "hi545")) &&
			!(g_hi545_module_id == 0x04 && !strcmp(sensor_name, "hi545_avc"))) {
			return -ENODEV;
		}
	}

	if (!strcmp(sensor_name, "ov13850")) {
		camera_info_set(ODMM_HD_INFO_CAMERA_REAR,
				"REAR CAMERA", "ov13850", "Sunny", "13M");
	}else if(!strcmp(sensor_name, "ov13850_qtech")) {
		camera_info_set(ODMM_HD_INFO_CAMERA_REAR,
			"REAR CAMERA", "ov13850", "Qtech", "13M");
	} else if (!strcmp(sensor_name, "hi545")) {
		camera_info_set(ODMM_HD_INFO_CAMERA_FRONT,
				"FRONT CAMERA", "hi545", "O-film", "5M:");
	} else if (!strcmp(sensor_name, "hi545_avc")) {
		camera_info_set(ODMM_HD_INFO_CAMERA_FRONT,
				"FRONT CAMERA", "hi545", "AVC", "5M:");
	} else {
		pr_err("not 1st or 2nd source camera\n");
	}
	...
	void __user *argp)
{
	...
	case CFG_ODM_UPDATE_OTP: {
		...
		} else if (!strcmp(s_ctrl->sensordata->sensor_name, "ov13850_qtech")) {
			ov13850_qtech_update_otp(s_ctrl);
		} else {
