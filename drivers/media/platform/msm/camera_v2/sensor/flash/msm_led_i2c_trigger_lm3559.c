...
static int set_led_status(void *data, u64 val)
{
	...
	int rc = -1;
	pr_info("set_led_status: Enter val: %llu", val);
	if (!fctrl) {
	...
	if (val == 0) {
		pr_info("set_led_status: val is disable");
		rc = msm_flash_led_off(fctrl);
	} else if (val == 2) {
		pr_info("set_led_status: led_high is enable");
		rc = msm_flash_led_high(fctrl);
	} else {
		pr_info("set_led_status: led_low is enable");
