...
static struct msm_camera_i2c_reg_conf t4k35_sxga_settings[] = {
	{0x3612, 0xa9,},
	{0x3614, 0x50,},
	{0x3618, 0x00,},
	{0x3034, 0x18,},
...
static struct msm_camera_i2c_reg_conf t4k35_full_settings[] = {
	{0x3612, 0xab,},
	{0x3614, 0x50,},
	{0x3618, 0x04,},
	{0x3034, 0x18,},
...
static struct msm_camera_i2c_reg_conf t4k35_1080P_settings[] = {
	{0x3612, 0xab,},
	{0x3614, 0x50,},
	{0x3618, 0x04,},
	{0x3034, 0x18,},
...
static struct msm_camera_i2c_reg_conf t4k35_enable_aec_settings[] = {
	{0x3503, 0x00,},
	{0x3406, 0x00,},
};

static struct msm_camera_i2c_reg_conf t4k35_disable_aec_settings[] = {
	{0x3503, 0x07,},
	{0x3406, 0x01,},
};
...
static const struct of_device_id t4k35_dt_match[] = {
	{.compatible = "toshiba,t4k35", .data = &t4k35_s_ctrl},
	{}
};
...
static int __init t4k35_init_module(void)
{
	int32_t rc;
	pr_err("%s:%d\n", __func__, __LINE__);
	rc = platform_driver_register(&t4k35_platform_driver);
	if (!rc)
		return rc;
...
int32_t t4k35_sensor_config(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
	...
	case CFG_SET_INIT_SETTING:
		...
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, t4k35_recommend_settings,
			ARRAY_SIZE(t4k35_recommend_settings),
	...
	/*copy from user the desired resoltuion*/
		...
		if (res == MSM_SENSOR_RES_FULL) {
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
				i2c_write_conf_tbl(
				s_ctrl->sensor_i2c_client, t4k35_full_settings,
				ARRAY_SIZE(t4k35_full_settings),
				MSM_CAMERA_I2C_BYTE_DATA);
				pr_err("%s:%d res =%d\n t4k35_full_settings ",
				__func__, __LINE__, res);
			...
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
				i2c_write_conf_tbl(
				s_ctrl->sensor_i2c_client, t4k35_sxga_settings,
				ARRAY_SIZE(t4k35_sxga_settings),
				MSM_CAMERA_I2C_BYTE_DATA);
			pr_err("%s:%d res =%d t4k35_sxga_settings\n",
				 __func__, __LINE__, res);
		} else if (res == MSM_SENSOR_RES_2) {
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
				i2c_write_conf_tbl(
				s_ctrl->sensor_i2c_client,
				t4k35_1080P_settings,
				ARRAY_SIZE(t4k35_1080P_settings),
				MSM_CAMERA_I2C_BYTE_DATA);
			pr_err("%s:%d res =%d t4k35_1080P_settings\n",
				 __func__, __LINE__, res);
		} else {
	...
	case CFG_SET_STOP_STREAM:
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, t4k35_stop_settings,
			ARRAY_SIZE(t4k35_stop_settings),
			MSM_CAMERA_I2C_BYTE_DATA);
		break;
	case CFG_SET_START_STREAM:
		if (s_ctrl->camera_stream_type != MSM_CAMERA_STREAM_SNAPSHOT) {
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
				i2c_write_conf_tbl(
				s_ctrl->sensor_i2c_client,
				t4k35_enable_aec_settings,
				ARRAY_SIZE(t4k35_enable_aec_settings),
				MSM_CAMERA_I2C_BYTE_DATA);
		} else {
			rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
				i2c_write_conf_tbl(
				s_ctrl->sensor_i2c_client,
				t4k35_disable_aec_settings,
				ARRAY_SIZE(t4k35_disable_aec_settings),
				MSM_CAMERA_I2C_BYTE_DATA);
		}
		if (rc) {
		...
		rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->
			i2c_write_conf_tbl(
			s_ctrl->sensor_i2c_client, t4k35_start_settings,
...
#ifdef CONFIG_COMPAT
int32_t t4k35_sensor_config32(struct msm_sensor_ctrl_t *s_ctrl,
	void __user *argp)
{
...
static struct msm_sensor_fn_t t4k35_sensor_func_tbl = {
	.sensor_config = t4k35_sensor_config,
#ifdef CONFIG_COMPAT
	.sensor_config32 = t4k35_sensor_config32,
#endif
...
static struct msm_sensor_ctrl_t t4k35_s_ctrl = {
	...
	.func_tbl = &t4k35_sensor_func_tbl,
};
