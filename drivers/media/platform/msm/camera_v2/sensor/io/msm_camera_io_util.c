...
#ifdef CONFIG_MFD_RT5033_RESET_WA
		if (system_rev >= CRITERION_REV && !strncmp(cam_vreg->reg_name, "cam_vana", 8)){
			CDBG("%s : CRITERION_REV = %d, system_rev = %d\n",__func__, CRITERION_REV, system_rev);
