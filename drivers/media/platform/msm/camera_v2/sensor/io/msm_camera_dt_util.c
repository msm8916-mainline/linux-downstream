	...
	uint16_t gpio_array_size)
{
	...
	// added byyangyongfeng to add check pin value of camera switch  (qw620) 2015-06-12 begin
	rc = of_property_read_u32(of_node, "qcom,gpio-mipiswitch", &val);
	if (rc != -EINVAL) {
		if (rc < 0) {
			pr_err("%s:%d read qcom,gpio-cameraid failed rc %d\n",
				__func__, __LINE__, rc);
		...
		gconf->gpio_num_info->gpio_num[SENSOR_GPIO_MIPI_SWITCH] =
			gpio_array[val];
		gconf->gpio_num_info->valid[SENSOR_GPIO_MIPI_SWITCH] = 1;
		CDBG("%s qcom,gpio-cameraid %d\n", __func__,
			gconf->gpio_num_info->gpio_num[SENSOR_GPIO_MIPI_SWITCH]);
	} else {
		rc = 0;
