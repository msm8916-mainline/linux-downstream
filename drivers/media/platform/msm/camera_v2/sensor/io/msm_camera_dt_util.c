    ...
    struct msm_camera_i2c_client *sensor_i2c_client)
{
    ...
    if(strstr(ctrl->dev->driver->name, "hm2056")){
        CDBG("%s:%d msm_camera_request_gpio_table", __func__, __LINE__);
    ...
    else if(strstr(ctrl->dev->driver->name, "s5k5e2")){
        CDBG("%s:%d msm_camera_request_gpio_table", __func__, __LINE__);

        if(firstbootrear == 1)
        {

            rc = msm_camera_request_gpio_table_without_init(
                ctrl->gpio_conf->cam_gpio_req_tbl,
                ctrl->gpio_conf->cam_gpio_req_tbl_size, 1);
            if (rc < 0)
                no_gpio = rc;
    ...
    for (index = 0; index < ctrl->power_setting_size; index++) {
        ...
        case SENSOR_GPIO:
            ...
            CDBG("%s:%d gpio set val %d\n", __func__, __LINE__,
                ...
                [power_setting->seq_val]);
		if(strstr(ctrl->dev->driver->name, "s5k5e2"))
		{
    ...
    struct msm_camera_i2c_client *sensor_i2c_client)
{
    ...
    ctrl->cam_pinctrl_status = 0;
    if(strstr(ctrl->dev->driver->name, "s5k5e2") || strstr(ctrl->dev->driver->name, "hm2056"))
    {
	//do dothing.
    }
    else
