...
static int RearCamera_proc_show(struct seq_file *m, void *v) {
  seq_printf(m, "s5k5e2\n");
  return 0;
}

static int RearCamera_proc_open(struct inode *inode, struct  file *file) {
  return single_open(file, RearCamera_proc_show, NULL);
}

static const struct file_operations RearCamera_proc_fops = {
  .owner = THIS_MODULE,
  .open = RearCamera_proc_open,
  .read = seq_read,
  .llseek = seq_lseek,
...
static int32_t s5k5e2_platform_probe(struct platform_device *pdev)
{
    ...
    if(rc==0){
        pr_err("Create Rear Camera Name\n");
        proc_create("RearCamera", 0, NULL, &RearCamera_proc_fops);
    }
    return rc;
}
...
static void __exit s5k5e2_exit_module(void)
{
    ...
    if (s5k5e2_s_ctrl.pdev) {
        ...
        remove_proc_entry("RearCamera", NULL);
    } else
...
static struct msm_sensor_ctrl_t s5k5e2_s_ctrl = {
    ...
    .power_setting_array.power_down_setting = s5k5e2_power_down_setting,
