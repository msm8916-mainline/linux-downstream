...
typedef enum COWELL_8MP_EEPROM_MAP {
	...
	DATA_END_ADDR_13M         = 0x0B7F,//add 13M

	//AF CHECKSUM : IMX258
	AF_CHECKSUM_START_ADDR = 0x070B,
	AF_CHECKSUM_END_ADDR   = 0x070C,
...
int32_t msm_eeprom_checksum_cowell_imx258(struct msm_eeprom_ctrl_t *e_ctrl)
{
	...
	//CheckSum
	checkSum_Total  = shiftedSum(e_ctrl, TOTAL_CHECKSUM_START_ADDR, TOTAL_CHECKSUM_END_ADDR, BigEndian);
	checkSum_Total  += shiftedSum(e_ctrl, AF_CHECKSUM_START_ADDR, AF_CHECKSUM_END_ADDR, BigEndian);
