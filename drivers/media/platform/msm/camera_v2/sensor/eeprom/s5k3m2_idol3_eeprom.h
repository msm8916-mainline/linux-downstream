#ifndef S5K3M2_IDOL3_OTP
#define S5K3M2_IDOL3_OTP

#include "msm_cci.h"

#define S5K3M2_IDOL3_OTP_PAGE_FULL_SIZE             32
#define S5K3M2_IDOL3_OTP_BASE_ADD                    0x0A04
#define S5K3M2_IDOL3_OTP_WB_FLAG_ADD                0x0A04
#define S5K3M2_IDOL3_OTP_WB_DATA_ADD                0x0A0F
#define S5K3M2_IDOL3_OTP_WB_GOLDEN_DATA_ADD        0x0A15
#define S5K3M2_IDOL3_OTP_MID_AWB_CHECKSUM_NUM        24
#define S5K3M2_IDOL3_OTP_GAIN_DEFAULT                0x100

#define S5K3M2_IDOL3_OTP_AF_DATA_ADD            0x0A34

uint16_t S5K3M2_IDOL3_AF_MACRO = 315;
uint16_t S5K3M2_IDOL3_AF_INF = 99;

struct S5K3M2MIPI_idol3_otp_struct {
    uint32_t mid;
    uint32_t lens_id;
    uint32_t rg_ratio;
    uint32_t bg_ratio;
...
static int32_t s5k3m2_idol3_otp_i2c_write(struct msm_eeprom_ctrl_t *s_ctrl,
...
static uint16_t s5k3m2_idol3_otp_i2c_read(struct msm_eeprom_ctrl_t *s_ctrl,
    uint32_t addr)
{
    int rc = 0;
    uint16_t data = 0;
...
static int s5k3m2_idol3_otp_LSC_on(struct msm_eeprom_ctrl_t * s_ctrl)
{
    int rc = 0;
    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6028,0x2000,MSM_CAMERA_I2C_WORD_DATA);
    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x602A,0x14FA,MSM_CAMERA_I2C_WORD_DATA);
    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6F12,0x0F,  MSM_CAMERA_I2C_BYTE_DATA);

    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6028,0x4000,MSM_CAMERA_I2C_WORD_DATA);
    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x602A,0x306A,MSM_CAMERA_I2C_WORD_DATA);
    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6F12,0x0068,MSM_CAMERA_I2C_WORD_DATA);

    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x602A,0x0B00,MSM_CAMERA_I2C_WORD_DATA);
    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6F12,0x01,  MSM_CAMERA_I2C_BYTE_DATA);

    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x602A,0x3058,MSM_CAMERA_I2C_WORD_DATA);
    rc |= s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6F12,0x0900,MSM_CAMERA_I2C_WORD_DATA);

    return rc;
}

static int s5k3m2_idol3_otp_stream_on(struct msm_eeprom_ctrl_t * s_ctrl)
{
    int rc = 0;
...
#define IDOL3_AF_OFFSET_L2 0x36
#define IDOL3_AF_OFFSET_L1 0x30

void idol3_otp_AF_on(uint8_t *buffer)
{
    int offset = 0;
    ...
    if ((buffer[IDOL3_AF_OFFSET_L2] & 0xc0)>>6 == 1 || buffer[IDOL3_AF_OFFSET_L2] == 1)
    {
        offset = IDOL3_AF_OFFSET_L2;
    }
    else if ((buffer[IDOL3_AF_OFFSET_L1] & 0xc0)>>6 == 1 || buffer[IDOL3_AF_OFFSET_L1] == 1)
    {
        offset = IDOL3_AF_OFFSET_L1;
    }

    idol3_af_inf   = (uint16_t)(((buffer[offset + 4]) << 8) | (buffer[offset + 3]));
...
static void s5k3m2_idol3_otp_AWB_on(struct msm_eeprom_ctrl_t * s_ctrl,uint8_t *buffer, uint16_t size)
{
   ...
   if (buffer[0] >> 6 == valid)
	...
	s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6f12,R_test,MSM_CAMERA_I2C_WORD_DATA);
	s5k3m2_idol3_otp_i2c_write(s_ctrl,0x602a,0x0212,MSM_CAMERA_I2C_WORD_DATA);
	s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6f12,B_test,MSM_CAMERA_I2C_WORD_DATA);
	s5k3m2_idol3_otp_i2c_write(s_ctrl,0x602a,0x0214,MSM_CAMERA_I2C_WORD_DATA);
	s5k3m2_idol3_otp_i2c_write(s_ctrl,0x6f12,Gb_test,MSM_CAMERA_I2C_WORD_DATA);	
}

static int s5k3m2_idol3_read_otp(struct msm_eeprom_ctrl_t * s_ctrl,uint8_t page, uint16_t address, uint8_t *buffer, uint16_t size)
{
    uint8_t  reVal = 0;
    uint16_t i = 0;
    int nCheckSum = 0;
    ...
    s5k3m2_idol3_otp_i2c_write(s_ctrl,0x0A02, page,MSM_CAMERA_I2C_BYTE_DATA);//select page
    ...
    while (i < size){
        int addr = address + i;
        reVal = s5k3m2_idol3_otp_i2c_read(s_ctrl,addr);
    ...
    s5k3m2_idol3_otp_AWB_on(s_ctrl,buffer,size);
