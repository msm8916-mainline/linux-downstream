...
static uint16_t isFirstStream = FALSE; /*                                                                     */
...
static struct msm_camera_i2c_reg_conf hi707_cowell_recommend_settings[] = {
	...
	//최종 업데이트 : 20130315
		...
		{0x20, 0x04},//05}, //03},	//SHARP_Negative //for resolution , 131119
		{0x21, 0x04},//05}, //04}, //03},	//SHARP_Positive 131105
		...
		{0x90, 0x03},//05}, //04},	//SHARP2D_SLOPE  //for resolution , 131119
		{0x91, 0x04},//06}, //05},	//SHARP2D_DIFF_CTL //for resolution , 131119
...
{0x12, 0x30},
{0x40, 0x80},
{0x41, 0x24},//16}, //DYOFS 02 // for SKT SPEC 0622
{0x48, 0x80},
{0x50, 0x80}, //PGA brightness
{0x60, 0x01},
{0x61, 0x00},
{0x62, 0x74}, //80},
{0x63, 0x84},
...
{0x13, 0x19},
{0x14, 0x08},
{0x20, 0x02}, // For SKT SPEC 0622
{0x21, 0x02}, // For SKT Spec 0622
{0x23, 0x25},
{0x24, 0x21},
{0x25, 0x08},
{0x26, 0x40},
...
{0x80, 0x05},
{0x81, 0x07},
{0x90, 0x02},// for SKT SPEC 0622
{0x91, 0x02},// for SKT SPEC 0622
{0x92, 0x00},
{0x93, 0x30},
{0x94, 0x30},
{0x95, 0x10},
...
{0x88, 0x02}, //EXP Max(120Hz) 8.57 fps
{0x89, 0xab},
{0x8a, 0x98},
...
------------------------------------------------------------------------------------------------------------------*/
#if defined(CONFIG_FAST_TUNE_REGISTER)
const static struct msm_camera_i2c_reg_conf *hi707_recommend_settings_tunning;
static struct msm_camera_i2c_reg_conf temp_recommend_settings[6500];

const static struct msm_camera_i2c_reg_conf *hi707_vt_recommend_settings_tunning;
static struct msm_camera_i2c_reg_conf temp_vt_recommend_settings_tuning[6500];


static uint16_t recommend_size, recommend_cnt = 0;
static uint16_t vt_size, vt_cnt = 0;
//static uint16_t snap_size, snap_cnt = 0;

static void hi707_parsing_register(char* buf, int buf_size)
{
	int i = 0;
	unsigned int addr = 0;
	...
	while (i < buf_size) {
	 ...
	 // code delude
		if (buf[i] == '{') {
			...
			switch (subject) {
			 case 'A' : {
				temp_recommend_settings[recommend_cnt].reg_addr= addr;
				temp_recommend_settings[recommend_cnt].reg_data= value;
				temp_recommend_settings[recommend_cnt].dt = MSM_CAMERA_I2C_BYTE_DATA;
			 ...
			 case 'B' : {
				temp_vt_recommend_settings_tuning[vt_cnt].reg_addr = addr;
				temp_vt_recommend_settings_tuning[vt_cnt].reg_data = value;
				temp_vt_recommend_settings_tuning[vt_cnt].dt  = MSM_CAMERA_I2C_BYTE_DATA;
...
/*
			 case 'C' : {
				temp_hi707_snap_settings[snap_cnt].reg_addr = addr;
				temp_hi707_snap_settings[snap_cnt].reg_data = value;
				temp_hi707_snap_settings[snap_cnt].dt  = MSM_CAMERA_I2C_BYTE_DATA;
...
static void hi707_release_parsing_register(void) {
	//Init - recommend
	hi707_recommend_settings_tunning = temp_recommend_settings;
	recommend_size = recommend_cnt;
	pr_err("%s:%d recommend_size = %d\n", __func__, __LINE__, recommend_size);

	// Preview
	hi707_vt_recommend_settings_tunning = temp_vt_recommend_settings_tuning;
	vt_size = vt_cnt;
	pr_err("%s:%d vt_size = %d\n", __func__, __LINE__, vt_size);
...
static void hi707_read_register_from_file1(void)
{
	int fd =0;
	...
	fd = sys_open("/data/hi707_reg1.txt", O_RDONLY|O_LARGEFILE, 0777);

	if (fd < 0) {
		pr_err("%s:%d File from Internal Memory open fail fd = %d\n", __func__, __LINE__, fd);
		fd = sys_open("/storage/external_SD/hi707_reg1.txt", O_RDONLY |O_LARGEFILE, 0777);
	}
	...
	hi707_parsing_register(buf, read_size);
err_read:
	kfree(buf);
err_buf:
	sys_close(fd);
...
static void hi707_read_register_from_file2(void)
{
	int fd;
	mm_segment_t oldfs;
	char* buf;
	...
	fd = sys_open("/data/hi707_reg2.txt", O_RDONLY |O_LARGEFILE, 0777);

	if (fd < 0) {
		pr_err("%s:%d File from Internal Memory open fail\n", __func__, __LINE__);
		fd = sys_open("/storage/external_SD/hi707_reg2.txt", O_RDONLY |O_LARGEFILE, 0777);
	}
	...
	hi707_parsing_register(buf, read_size);
	hi707_release_parsing_register();
...
static void hi707_update_register(struct msm_sensor_ctrl_t *s_ctrl)
{
	pr_err("%s START to Read the file from internal or SDcard\n", __func__);
	hi707_read_register_from_file1();
	msleep(1);
	hi707_read_register_from_file2();
	msleep(500);
	...
	case CFG_SET_INIT_SETTING:{
			 ...
			 if(TUNING_REGISTER)
				hi707_i2c_write_table(s_ctrl,
							(struct msm_camera_i2c_reg_conf *) hi707_recommend_settings_tunning, recommend_size);
			 else
				hi707_i2c_write_table(s_ctrl,
...
/*                                                                                          */
	...
	case CFG_SET_BESTSHOT_MODE:
		CDBG("%s: Do not support this config\n", __func__);
		break;
...
/*                                                                                         */
	case CFG_SET_INIT_SETTING_VT:
		...
		pr_err("%s - vt init\n", __func__);
			 ...
			 if(TUNING_REGISTER)
				hi707_i2c_write_table(s_ctrl,
						(struct msm_camera_i2c_reg_conf *) hi707_vt_recommend_settings_tunning, vt_size);
			 else
					hi707_i2c_write_table(s_ctrl,
