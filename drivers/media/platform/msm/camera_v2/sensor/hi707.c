...
#if defined(CONFIG_MACH_MSM8916_C50N_GLOBAL_COM) || \
...
static struct msm_sensor_power_setting hi707_power_setting_a[] = {
	{
...
#elif defined(CONFIG_MACH_MSM8916_C50_TMO_US) || \
...
#else //CONFIG_MACH_MSM8916_C50C_VZW
static struct msm_sensor_power_setting hi707_power_setting[] = {
	{
		.seq_type = SENSOR_GPIO,
		.seq_val = SENSOR_GPIO_RESET,
...
static struct msm_camera_i2c_reg_conf hi707_recommend_settings[] = {
	...
	//최종 업데이트 : 20130315
		...
		{0x84, 0x22},
		{0x85, 0xa0},
		{0x86, 0x00}, //EXPMin 7462.50 fps
		{0x87, 0xc8},
		{0x88, 0x02}, //EXP Max 60hz 10.00 fps
		{0x89, 0x45},
		{0x8a, 0x40},
		{0xa0, 0x02}, //EXP Max 50hz 10.00 fps
		{0xa1, 0x46},
		{0xa2, 0x08},
		{0x8B, 0x3a}, //EXP100
		{0x8C, 0x34},
		{0x8D, 0x30}, //EXP120
		{0x8E, 0x70},
		{0x9c, 0x04}, //EXP Limit 1243.75 fps
		{0x9d, 0xb0},
		{0x9e, 0x00}, //EXP Unit
		{0x9f, 0xc8},
	  ...
	  //{0x10, 0x05},			//05},
		...
		{0x1a, 0x00},				 //00},
		{0x1b, 0x35},				 //00},
		{0x1c, 0x02},//03}, // HSP_LPX	 //02},
		{0x1d, 0x04},//05}, // HSN_LPX	 //04},
...
static int __init hi707_init_module(void)
{
	...
	switch(lge_get_board_revno()) {
		case HW_REV_A:
			...
			hi707_s_ctrl.power_setting_array.power_setting = hi707_power_setting_a;
			hi707_s_ctrl.power_setting_array.size = ARRAY_SIZE(hi707_power_setting_a);
			break;
		case HW_REV_B:
			...
			break;
		default:
			pr_err("%s: Sensor power is set as default\n", __func__);
			hi707_s_ctrl.power_setting_array.power_setting = hi707_power_setting;
			hi707_s_ctrl.power_setting_array.size = ARRAY_SIZE(hi707_power_setting);
			break;
	}
