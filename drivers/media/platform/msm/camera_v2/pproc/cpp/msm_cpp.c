	...
	uint8_t put_buf)
{
	...
	if (frame_qcmd) {
		...
		//LGE_CHANGE to avoid double free(case#01940151) yt.jeon@lge.com 2015-03-18
		//kfree(frame_qcmd);
		event_qcmd = kzalloc(sizeof(struct msm_queue_cmd), GFP_ATOMIC);
		if (!event_qcmd) {
			pr_err("Insufficient memory\n");
			kfree(frame_qcmd);//LGE_CHANGE to avoid double free(case#01940151) yt.jeon@lge.com 2015-03-18
			return -ENOMEM;
		}
