...
static void msm_otg_chg_check_timer_func(unsigned long data)
{
	...
	else if ((readl_relaxed(USB_PORTSC) & PORTSC_LS) == PORTSC_LS) {
		dev_dbg(otg->phy->dev, "DCP is detected as SDP\n");
		set_bit(B_FALSE_SDP, &motg->inputs);
		queue_work(system_nrt_wq, &motg->sm_work);
	}
	else{
		printk("%s recheck recheck_time = %d\n",__func__,recheck_time);
		recheck_time ++;
		if(recheck_time < 6 ){
		 mod_timer(&motg->chg_check_timer,
					CHG_RECHECK_DELAY);
		}
		else if(recheck_time >= 6){
			dev_dbg(otg->phy->dev, "DCP is detected as SDP\n");
			set_bit(B_FALSE_SDP, &motg->inputs);
			queue_work(system_nrt_wq, &motg->sm_work);
			recheck_time =0;
...
/*fgy add for usb notifier start*/
/*For any user that care about USB udc events, for example the charger*/
static BLOCKING_NOTIFIER_HEAD(usb_charger_notifier_list);
static void call_charger_notifier(struct msm_otg *motg)
{
	blocking_notifier_call_chain(&usb_charger_notifier_list,
				motg->chg_type,NULL);
}
int usb_charger_register(struct notifier_block *nb)
{
	struct msm_otg *motg  = the_msm_otg;
	int ret = 0;
	...
	ret = blocking_notifier_chain_register(&usb_charger_notifier_list, nb);
	if (ret)
		return ret;

	if (motg->chg_type){
		printk("%s motg->chg_type =%d\n",__func__,motg->chg_type);
		call_charger_notifier(motg);
		}

	return 0;
}
EXPORT_SYMBOL(usb_charger_register);

int usb_charger_unregister(struct notifier_block *nb)
{
	return blocking_notifier_chain_unregister(&usb_charger_notifier_list, nb);
}
EXPORT_SYMBOL(usb_charger_unregister);
/*fgy add for usb notifier end*/
	...
	case OTG_STATE_B_IDLE:
		if (test_bit(MHL, &motg->inputs)) {
			...
			case USB_CHG_STATE_UNDEFINED:
				...
				break;
			case USB_CHG_STATE_DETECTED:
				call_charger_notifier(motg);//fgy add for usb notifier
				printk("%s motg->chg_type = %d\n",__func__,motg->chg_type);
