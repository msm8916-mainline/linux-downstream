...
#endif
#ifdef CONFIG_USB_G_LGE_REBOOT_FACTORY_CABLE
...
#if defined (CONFIG_USB_G_LGE_REBOOT_FACTORY_CABLE)
	...
	* E7IILTE models  : embedded type
	...
	if (uevent_envp == connected) {
		if (lge_pm_get_cable_type() == CABLE_56K &&
			...
			kernel_restart(NULL);
		} else if (lge_pm_get_cable_type() == CABLE_910K && lge_get_sbl_cable_type() != 11) {
			usb_gadget_disconnect(cdev->gadget);
			usb_ep_dequeue(cdev->gadget->ep0, cdev->req);
			pr_info("[FACTORY] reset due to 910K cable, pm:%d\n",
					lge_pm_get_cable_type());
		...
		struct usb_composite_dev *cdev)
{
	...
	config->fsg.nluns = 1;
	config->fsg.luns[config->fsg.nluns-1].removable = 1;
	/* cdrom(read only) flag */
	config->fsg.luns[config->fsg.nluns-1].cdrom = 1;
