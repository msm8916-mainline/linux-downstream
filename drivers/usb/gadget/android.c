...
static char function_buff_backup[256];
...
static struct usb_device_descriptor device_desc = {
	...
	.bcdDevice            = __constant_cpu_to_le16(0x0225),//[BUGFIX]-Mod by TCTNB.93391,10/30/2014,675779,Change bcdDevice from 0xffff to 0x0225
...
bind_config:
/* [PLATFORM]-Add-BEGIN by TCTNB.YQJ, 2014/12/26, PR-864571 8916 only 8 endpoit so when set to "diag,serial,rmnet,adb",only 1 ports can be enabled */
	if(!strcmp(serial_transports, "smd")&&!strcmp(function_buff_backup, "diag,serial,rmnet,adb"))
		ports = 1;
/* [PLATFORM]-Add-END by TCTNB.YQJ, 2014/12/26 */
	for (i = 0; i < ports; i++) {
					...
					struct usb_composite_dev *cdev)
{
	...
	if (dev->pdata && dev->pdata->cdrom) {
		...
		config->fsg.nluns++;
		//[BUGFIX]-Add-BEGIN by TCTNB.93391,10/30/2014,821919,USB driver autoinstall
		#if defined(CONFIG_JRD_CD_ROM_EMUM_EJECT) && !defined (FEATURE_TCTNB_MMITEST)
		is_cd_rom_inited = TRUE;
		printk("lun0\n");
		#endif
		//[BUGFIX]-Add-END by TCTNB.93391,10/30/2014,821919,USB driver autoinstall
	}
			       ...
			       const char *buff, size_t size)
{
	...
	strlcpy(function_buff_backup, buff, sizeof(function_buff_backup));
	...
	if(!remove_cd_rom_flag && !is_power_off_charging && is_cd_rom_inited)
	{
		if(strstr(buff,"rndis"))
		{
			...
			strlcpy(buf, buff, sizeof(buf));
               ...
               //[BUGFIX]-Add-BEGIN by TCTNB.93391,10/30/2014,653319, USB Accessory Test case fail
		else if(strstr(buff,"accessory")) //CTS Accessory Test in Ubuntu
		{
			...
			strlcpy(buf, buff, sizeof(buf));
               ...
               //[BUGFIX]-Add-END by TCTNB.93391,10/30/2014,653319, USB Accessory Test case fail
