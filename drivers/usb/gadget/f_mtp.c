...
unsigned int mtp_skip_vfs_write = false;
module_param(mtp_skip_vfs_write, uint, S_IRUGO | S_IWUSR);

unsigned int mtp_skip_vfs_read = false;
module_param(mtp_skip_vfs_read, uint, S_IRUGO | S_IWUSR);
#endif /*                      */
...
#ifdef CONFIG_LGE_USB_G_MTP
	mreq = kzalloc(sizeof(*mreq), GFP_KERNEL);
	if (!mreq) {
...
static void mtp_request_free(struct usb_request *req, struct usb_ep *ep)
{
	...
	if (req) {
#ifdef CONFIG_LGE_USB_G_MTP
		mreq = (struct mtp_request *)req->context;
		kfree(mreq);
#endif
		kfree(req->buf);
		usb_ep_free_request(ep, req);
...
#ifdef CONFIG_LGE_USB_G_MTP
		mreq = list_first_entry(head, struct mtp_request, list);
		req = mreq->req;
		list_del(&mreq->list);
#else
...
#ifdef CONFIG_LGE_USB_G_MTP
	unsigned long flags;
	struct mtp_request *mreq = (struct mtp_request *)req->context;
...
static void mtp_ep_yield_req(struct mtp_dev	*dev)
{
	...
	list_for_each_entry(mreq, &dev->tx_idle, list) {
		lge_usb_ep_yield_request(dev->ep_in, mreq->req);
	}

	list_for_each_entry(mreq, &dev->rx_idle, list) {
		lge_usb_ep_yield_request(dev->ep_out, mreq->req);
	}

	list_for_each_entry(mreq, &dev->intr_idle, list) {
		lge_usb_ep_yield_request(dev->ep_intr, mreq->req);
	}
#else
	struct usb_request *req;
	int i;
...
requeue_req:
	...
	req->length = len;
	mtp_req_put(dev, &dev->rx_queue, req);
#else
	req = dev->rx_req[0];
	req->length = len;
	dev->rx_done = 0;
	...
	DBG(cdev, "usb_ep_queue\n");
	ret = usb_ep_queue(dev->ep_out, req, GFP_KERNEL);
	if (ret < 0) {
		r = -EIO;
#ifdef CONFIG_LGE_USB_G_MTP
		req = mtp_req_get(dev, &dev->rx_queue);
		mtp_req_put(dev, &dev->rx_idle, req);
#endif
		goto done;
	} else {
	...
	if (dev->state == STATE_CANCELED) {
		r = -ECANCELED;
#ifdef CONFIG_LGE_USB_G_MTP
		mreq = list_first_entry(&dev->rx_queue, struct mtp_request, list);
		if (mreq) {
			usb_ep_dequeue(dev->ep_out, mreq->req);
		}
#else
		if (!dev->rx_done)
...
static void receive_file_work(struct work_struct *data)
{
	...
	while (count > 0) {
		...
		if (mtp_skip_vfs_write)
			...
			ret = vfs_write(filp, req->buf, req->actual, &offset);
	...
	while (!list_empty(&dev->rx_queue)) {
		...
		if (dev->state == STATE_OFFLINE) {
			list_del(&mreq->list);
			mtp_req_put(dev, &dev->rx_idle, mreq->req);
		}
	}
#else
