...
static int sm5418_get_charging_status(struct i2c_client *client)
{
	...
	if((int2 & SM5414_INT2_DONE) || (int2 & SM5414_INT2_TOPOFF)) {
		...
		nCHG = gpio_get_value(charger->pdata->chg_en_gpio);
				...
				struct i2c_client *client)
{
	...
	if (charger->cable_type == POWER_SUPPLY_TYPE_BATTERY) {
		...
		gpio_direction_output((charger->pdata->chg_en_gpio), 1);
		sm5418_set_toggle_charger(client, 0);
	} else {
		...
		if (value.intval > 0) {
			...
			ctrl = 0x44;
			sm5418_i2c_write(client, SM5414_CTRL, &ctrl);
			msleep(20);
		...
		if (!(chg_en & CHARGE_EN)) {
			...
			gpio_direction_output((charger->pdata->chg_en_gpio), 0);
...
static int sm5418_charger_parse_dt(struct sec_charger_info *charger)
{
	...
	if (np == NULL) {
		...
		ret = of_get_named_gpio(np, "battery,chg_en_gpio", 0);
		if (ret > 0) {
			pdata->chg_en_gpio = ret;
			pr_info("%s reading chg_en_gpio = %d\n", __func__, ret);
		} else {
			pdata->chg_en_gpio = 0;
			pr_info("%s chg_en_gpio read fail\n", __func__);
		}
