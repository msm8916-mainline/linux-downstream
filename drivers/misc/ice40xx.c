			...
			struct irda_ice40_platform_data *pdata)
{
	...
	// HSS_TEMP
	pr_err("[%s]: %d %d %d %d %d %d %d \n", __func__, gpio_ir_led_en, pdata->rst_n, pdata->spi_clk, pdata->spi_si, pdata->irda_irq, pdata->cdone, pdata->cresetb);
	return 0;
}
#else
static int irda_ice40_parse_dt(struct device *dev,
...
static int ice40_fpga_firmware_update_start(const u8 *data, int len)
{
	...
	printk("%s : [after] irda_irq = %d", __func__, gpio_get_value(g_pdata->irda_irq)); // HSS_TEMP
	if(g_pdata->cdone == NO_PIN_DETECTED)
		...
		const char *buf, size_t size)
{
	...
	if (rc)
		pr_err("%s: spi_si error : %d\n", __func__, rc);
	gpio_direction_output(g_pdata->spi_clk, 1);
	if (rc)
		pr_err("%s: spi_clk error : %d\n", __func__, rc);	
