...
//#define MMSS_CC_PWM_SET		0xFD8C3420 //[WX project] The value will be from device tree. gp_cmd_rcgr
...
struct timed_irrc_data {
	...
	struct pinctrl_state *active_state, *suspended_state;
...
static void android_irrc_disable_pwm(struct work_struct *work)
{
	...
	if(gpio_high_flag == 1){
		gpio_direction_output(irrc->pwm_gpio, 0);
...
#ifdef CONFIG_COMPAT
static long IRRC_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	return android_irrc_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
}
#endif
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = IRRC_compat_ioctl,
#endif
...
static void android_irrc_parse_dt(struct device *dev, struct timed_irrc_data *data)
{
	...
	data->clk_name = of_get_property(np, "clock-names", &len);
	of_property_read_u32(np, "lge,clk-rate", &data->clk_rate);

	INFO_MSG("rcgr:%x, gpio:%d, clk name: %s, clk rate: %u\n",
			data->gp_cmd_rcgr, data->pwm_gpio, data->clk_name, data->clk_rate);
}
