...
static void lis3dh_acc_report_values(struct lis3dh_acc_data *acc,
					int *xyz)
{
	input_report_abs(acc->input_dev, ABS_X, xyz[0] + acc->accel_cali[0]);
	input_report_abs(acc->input_dev, ABS_Y, xyz[1] + acc->accel_cali[1]);
	input_report_abs(acc->input_dev, ABS_Z, xyz[2] + acc->accel_cali[2]);
...
static int lis3dh_strtok(const char *buf,
...
static ssize_t lis3dh_accel_get_cali(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	...
	calibration[0] = acceld->accel_cali[acceld->pdata->axis_map_x];
	calibration[1] = acceld->accel_cali[acceld->pdata->axis_map_y];
	calibration[2] = acceld->accel_cali[acceld->pdata->axis_map_z];
...
static ssize_t lis3dh_accel_set_cali(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	...
	if (lis3dh_strtok(buf, count, buf2, cali_count) < 0) {
		...
		acceld->accel_cali[0] = (int)calibration[0];
		acceld->accel_cali[1] = (int)calibration[1];
		acceld->accel_cali[2] = (int)calibration[2];
...
static DEVICE_ATTR(cali, S_IRUGO | S_IWUGO, lis3dh_accel_get_cali, lis3dh_accel_set_cali);
		...
		const struct i2c_device_id *id)
{
	...
	device_create_file(&acc->input_dev->dev, &dev_attr_cali);
