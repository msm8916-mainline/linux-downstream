...
#ifdef CONFIG_OF
	if ((!this->get_nirq_low) || this->get_nirq_low(this->irq_gpio)) {
#else
...
int sx86XX_init(psx86XX_t this)
{
	...
	err = request_threaded_irq(this->irq, NULL, sx86XX_interrupt_thread,
				    IRQF_TRIGGER_FALLING | IRQF_ONESHOT, this->pdev->driver->name,  //Riven Enable threaded_irq
				    this);
#else
...
#endif
	if (err) {
	    dev_err(this->pdev, "request_irq(%d) return %d\n", this->irq, err);
