...
//+++ ASUS_BSP Alian_Shen "enable interrupt report event"
...
#define FLICK_INPUT_DEV_NAME  "shake"
...
#define FLICK_STAGE_1_REPEAT_MAX_NUM           10
#define FLICK_STAGE_2_REPEAT_MAX_NUM           4
#define FLICK_STAGE_3_REPEAT_MAX_NUM           10
#define FLICK_STAGE_4_REPEAT_MAX_NUM           3

#define FLICK_STAGE_Z_1_REPEAT_MAX_NUM        8
#define FLICK_STAGE_Z_2_REPEAT_MAX_NUM        4
#define FLICK_STAGE_Z_3_REPEAT_MAX_NUM        6
#define FLICK_STAGE_Z_4_REPEAT_MAX_NUM        3
#define Z_SHAKE_NUM                                              2

//#define FLICK_G_DATA_MAX                                    1900
#define FLICK_STAGW_1_G_DATA_MAX                   1024
#define FLICK_STAGW_2_G_DATA_MAX                   1024
#define FLICK_STAGW_3_G_DATA_MAX                   1024
#define FLICK_STAGW_4_G_DATA_MAX                   512

#define FLICK_STAGW_Z_1_G_DATA_MAX               1024
#define FLICK_STAGW_Z_2_G_DATA_MAX               1024
#define FLICK_STAGW_Z_3_G_DATA_MAX               1024
#define FLICK_STAGW_Z_4_G_DATA_MAX               512

#define BMA2x2_SLOPE_DUR                                    (1)     
#define BMA2x2_SLOPE_THR_mg                             (500)
#define RDY_DATA_BUF_NUM                                   (2)

static struct workqueue_struct *bma2x2_wakeup_work = NULL;
static struct workqueue_struct *bma2x2_read_work = NULL;

struct ShakeNumDef_t {
    int xP, xN;
    int yP, yN;
    int zP, zN;
};
struct bma2x2_wake_up_work_t {
    struct bma2x2_data      *bma2x2;
    struct timer_list       timer;
    struct timer_list       poll_timer;
//  int                     shake_num;   
    struct ShakeNumDef_t    tShakeNum;
    int                     shake_num[3];
    int                     reverse_num[3];
    ...
    int                     xyz;
    int                     flickDataNum;
} bma2x2_wake_up_data, bma2x2_read_data;

struct bma2x2_data *g_bma2x2Data;
static bool g_bIsStop = false;
static bool g_bIsFirstMotion = true;
//--- ASUS_BSP Alian_Shen "add shake-shake gesture "

//+++ ASUS_BSP Alian_Shen "change input event name"
...
//+++ ASUS Alian_Shen "add 4G for above android 5.0 version"
#define BMA2X2_RANGE_SET            5  /* +/- 4G */
//--- ASUS Alian_Shen "add 4G for above android 5.0 version"
//#define BMA2X2_BW_SET               12 /* 125HZ  */
#define BMA2X2_BW_SET                BMA2X2_BW_125HZ          //+++ Alian_Shen
...
//+++ ASUS_BSP Alian_Shen "Add shake-shake function"
#define BMA2x2_SLOPE_INT1_MASK                  (0x1 << 2)
#define BMA2x2_SLOPE_INT1_EN                       (0x1 << 2)
#define BMA2x2_SLOPE_INT_EN_X_MASK          (0x1)
#define BMA2x2_SLOPE_INT_EN_X                     (0x1)
#define BMA2x2_SLOPE_INT_EN_Y_MASK          (0x1 << 1)
#define BMA2x2_SLOPE_INT_EN_Y                     (0x1 << 1)
#define BMA2x2_SLOPE_INT_EN_Z_MASK          (0x1 << 2)
#define BMA2x2_SLOPE_INT_EN_Z                     (0x1 << 2)

#define BMA2x2_SLOPE_INT_STATUS_MASK      (0x1 << 2)
#define BMA2x2_SLOPE_SIGN_MASK                   (0x1 << 3)
#define BMA2x2_SLOPE_FIRST_X_MASK             (0x1 << 0)
...
//+++ ASUS Alian_Shen "add function for 'shake-shake'"
    struct input_dev *flick_interrupt;
//--- ASUS Alian_Shen "add function for 'shake-shake'"  
...
//+++ ASUS_BSP Alian_Shen "add sensor class define "
    ...
    while (read_count++ < CHECK_CHIP_ID_TIME_MAX) {
        ...
        if (i < bma2x2_sensor_type_count)
            ...
            if (read_count == CHECK_CHIP_ID_TIME_MAX) {
                printk("Failed! Bosch Sensortec Device"
                        ...
                        unsigned char enabled_mode)
{
    ...
    if (mode < 6) {
        ...
        case BMA2X2_MODE_SUSPEND:
            ...
            if (ret < 0)
                printk("Error switching to suspend");
            break;
        ...
        const char *buf, size_t count)
{
...
#ifdef CONFIG_SENSORS_BMI058
    printk("diff y is %d,value1 is %d, value2 is %d\n", diff,
                value1, value2);
    test_result_branch = 2;
#else
    printk("diff x is %d,value1 is %d, value2 is %d\n", diff,
                value1, value2);
    ...
    printk("diff z is %d,value1 is %d, value2 is %d\n", diff,
            value1, value2);
        ...
        signed char sensor_type, struct bma2x2acc *acc)
{
    ...
    //printk("[bma] newX=%d, newY=%d, newZ=%d\n", data[0]&0x1, data[2]&0x1, data[4]&0x1);
...
static void bma2x2_work_func(struct work_struct *work)
{
    ...
    //+++ ASUS_BSP Alian_Shen "dispose the cts fail"	
    static struct bma2x2acc acc_last;
    //--- ASUS_BSP Alian_Shen "dispose the cts fail"
    ...
    //+++ ASUS_BSP Alian_Shen "dispose the cts fail"
    acc.x = (acc.x == acc_last.x) ? (acc.x + 1) : acc.x;
    acc.y = (acc.x == acc_last.y) ? (acc.y + 1) : acc.y;
    acc.z = (acc.x == acc_last.z) ? (acc.z + 1) : acc.z;
    acc_last = acc;
    //--- ASUS_BSP Alian_Shen "dispose the cts fail"
        ...
        const char *buf, size_t count)
{
    ...
    printk("[bma2x2][alian][L%d] bma2x2_delay_store(%s)\n", __LINE__, buf);
    error = kstrtoul(buf, 10, &data);
    if (error)
        return error;
    ...
    printk("[bma2x2][L%d] bma2x2->delay=%d\n", __LINE__, bma2x2->delay.counter);
...
//+++ ASUS_BSP Alian_Shen "Add sensor class node"
        ...
        const char *buf, size_t count)
{
    ...
    do {
        ...
        /*printk("wait 2ms cal ready flag is %d\n", tmp); */
        timeout++;
        if (timeout == 50) {
            printk("get fast calibration ready error\n");
            return -EINVAL;
        };
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (data > MAX_FIFO_F_LEVEL) {
        ...
        if (BMA2X2_MODE_NORMAL == mode) {
            printk("bma2x2 fifo_count: %d abnormal, op_mode: %d",
                    data, mode);
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    if (bma2x2->bma_mode_enabled == 0) {
        printk("bma2x2_fifo_data, bma_mode_enabled 0");
        /*return -EINVAL;*/
    }
...
static void bma2x2_tap_timeout_handle(unsigned long data)
{
    ...
    printk("tap interrupt handle, timeout\n");
...
static void bma2x2_high_g_interrupt_handle(struct bma2x2_data *bma2x2)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        printk("High G interrupt happened,exis is %d,"
...
static void bma2x2_slope_interrupt_handle(struct bma2x2_data *bma2x2)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (first_value == 1) {
            ...
            if (sign_value == 1) {
                if (i == 0) {
                    bma2x2_wake_up_data.tShakeNum.xN++;
                    printk("[BMA2x2] slope X -  **************\n");
                }
                if (i == 1) {
                    bma2x2_wake_up_data.tShakeNum.yN++;
                    printk("[BMA2x2] slope Y -  **************\n");
                }
                if (i == 2) {
                    bma2x2_wake_up_data.tShakeNum.zN++;
                    printk("[BMA2x2] slope Z -  **************\n");
                }
            } else {
                if (i == 0) {
                    bma2x2_wake_up_data.tShakeNum.xP++;
                    printk("[BMA2x2] slope X +  **************\n");
                }
                if (i == 1) {
                    bma2x2_wake_up_data.tShakeNum.yP++;
                    printk("[BMA2x2] slope Y +  **************\n");
                }
                if (i == 2) {
                    bma2x2_wake_up_data.tShakeNum.zP++;
                    printk("[BMA2x2] slope Z +  **************\n");
                }
            }
            
//+++ ASUS_BSP Alian_Shen "Add function for 'shake=shake' "
            //bma2x2_wake_up_data.shake_num++;  
            bma2x2_wake_up_data.bma2x2 = bma2x2;
            //queue_work(bma2x2_wakeup_work, &(bma2x2_wake_up_data.wakeup_work));
            if (g_bIsFirstMotion) {
                queue_work(bma2x2_wakeup_work, &(bma2x2_wake_up_data.wakeup_work));
            }
//--- ASUS_BSP Alian_Shen "Add function for 'shake=shake' "     
        ...
        //printk("Slop interrupt happened,exis is %d,"
        //          "first is %d,sign is %d\n", i,
        //              first_value, sign_value);
    }
}
...
#define CHANGE_NEXT_STAGE(num, max, nowStage, nextstage, backStage, bIsZ)   \
            ( ( ((num) >=   ((bIsZ) ? g_Z_repeatNumMax[nowStage]: g_repeatNumMax[nowStage]) ) )  \
               ? (nextstage) : (backStage))
...
static enum STEP_t   flickCheckStage(s16 data, bool bIsFirst, bool * bIsErrp, bool bIsZ)
{
    ...
    enum STEP_t  tNextStep =  bIsZ ? STEP_OK : STEP_4;
    ...
    *bIsErrp = false;
    ...
    switch(tStep) {
        case STEP_1:
            ...
            //printk("[bma2x2] stage =%d, data=%d, s_dataMax=%d\n", tStep+1, data, s_dataMax);
            
            if (cOrien == s_cOrien) {
                s_repeatNum = ADD_REPEAT_NUM(data_abs, GET_STEP_DATA_MAX(tStep, bIsZ), s_repeatNum);
            } else {
                //printk("flickCheckStage: step%d, data=%i, repeat=%d, max=%d, z=%d\n", tStep+1, data, s_repeatNum, s_dataMax, bIsZ);

                tStep = CHANGE_NEXT_STAGE(s_repeatNum, s_dataMax, STEP_1, STEP_2, STEP_1, bIsZ);
                s_dataMax = 0;
                s_repeatNum = 0;
                s_cOrien = cOrien;
        ...
        case STEP_2:
            ...
            if (cOrien == s_cOrien) {
                ...
                tStep = CHANGE_NEXT_STAGE(s_repeatNum, s_dataMax, STEP_2, STEP_3, STEP_1, bIsZ);
                //printk("next stage: %d\n", tStep);
                if (STEP_1 == tStep) {
                    *bIsErrp = true;
                }
        ...
        case STEP_3:            
            ...
            if (cOrien == s_cOrien) {
                s_repeatNum = ADD_REPEAT_NUM(data_abs, GET_STEP_DATA_MAX(tStep, bIsZ), s_repeatNum);
                //if (bIsZ) {
                    //tStep = CHANGE_NEXT_STAGE(s_repeatNum, s_dataMax, STEP_3, tNextStep, STEP_3, bIsZ);
                    tStep = CHANGE_NEXT_STAGE(s_repeatNum, s_dataMax, STEP_3, STEP_OK, STEP_3, bIsZ);
                    //printk("step next = %d s_repeatNum=%d\n", tStep, s_repeatNum);
                    s_dataMax = 0;
                //}
            } else {
                ...
                tStep = CHANGE_NEXT_STAGE(s_repeatNum, s_dataMax, STEP_3, tNextStep, STEP_1, bIsZ);
                s_dataMax = 0;
                s_repeatNum = 0;
                s_cOrien = cOrien;
        ...
        case STEP_4:
            ...
            if (cOrien == s_cOrien) {
                ...
                tStep = CHANGE_NEXT_STAGE(s_repeatNum, s_dataMax, STEP_4, STEP_OK, STEP_4, bIsZ);
                s_dataMax = 0;
            } else {
                //printk("flickCheckStage: step%d, data=%i, repeat=%d, max=%d, z=%d\n", tStep+1, data, s_repeatNum, s_dataMax, bIsZ);
                ...
                tStep = CHANGE_NEXT_STAGE(s_repeatNum, s_dataMax, STEP_4, STEP_OK, STEP_1, bIsZ);
                s_dataMax = 0;
                s_repeatNum = 0;
                s_cOrien = cOrien;
...
static void bma2x2_timer_callback(unsigned long data)
{   
    int i = 0;
    int j = 0;
    int xyz = 0;
    ...
    enum STEP_t tStep;
/*  
    if ((2 == bma2x2_wake_up_data.tShakeNum.xP) ||
        (2 == bma2x2_wake_up_data.tShakeNum.xN) ||
        (2 == bma2x2_wake_up_data.tShakeNum.zP) ||
        (2 == bma2x2_wake_up_data.tShakeNum.zN)) {
        printk("shake-shake occur+++++++++++++\n");
        
        input_report_rel(bma2x2_wake_up_data.bma2x2->flick_interrupt, REL_DIAL, 2);
        input_sync(bma2x2_wake_up_data.bma2x2->flick_interrupt);    
    }   
    
    bma2x2_wake_up_data.tShakeNum.xP = 0;
    bma2x2_wake_up_data.tShakeNum.yP = 0;
    bma2x2_wake_up_data.tShakeNum.zP = 0;
    bma2x2_wake_up_data.tShakeNum.xN = 0;
    bma2x2_wake_up_data.tShakeNum.yN = 0;
    bma2x2_wake_up_data.tShakeNum.zN = 0;

    bma2x2_wake_up_data.shake_num = 0;
    */
    g_bIsStop = true;
    g_bIsFirstMotion = true;

    printk("bma2x2_timer_callback: flick data = %d\n",  bma2x2_read_data.flickDataNum);
/*
    printk("[bma2x2] flick data: ");
    //+++  test
    for (i = 0; i < bma2x2_read_data.flickDataNum; i++) {
        printk("[bma2x2] x y z = %6i %6i %6i\n", g_cFlickDataBuff[i][ORIEN_X],
...
*/
    for(xyz = 0; xyz < 3; xyz++) {
        ...
        if (ORIEN_Y == xyz)
            continue;
        if (ORIEN_Z == xyz)
            bIsZ =  true;//(bma2x2_wake_up_data.tShakeNum.zN+bma2x2_wake_up_data.tShakeNum.zP) >=1;
        else 
            bIsZ = false;

        printk("[bma2x2] check stage: %d\n", xyz);
        
        for (i = 0, j =0; i < bma2x2_read_data.flickDataNum; i++, j++) {
            ...
            //printk("[bma2x2] bIsErr=%d stepErrNum=%d j=%d\n", bIsErr, stepErrNum, j);
            ...
            if (bIsErr) {
                ...
                if (step2Pos > 0) {
                    j = step2Pos-1;
                    if (j >= bma2x2_read_data.flickDataNum - 4)
                        break;
                    step2Pos = 0;
                    i =  j;
                }
        ...
        //printk("[kxtj9] check stage for xyz=%d step=%d i = %d num=%d ++++++\n", xyz, tStep+1, i,  bma2x2_read_data.flickDataNum);

        if (STEP_OK == tStep)
            break;      
    }
    bma2x2_read_data.flickDataNum = 0;
    ...
    bma2x2_wake_up_data.shake_num[0] = 0;
    bma2x2_wake_up_data.shake_num[1] = 0;
    bma2x2_wake_up_data.shake_num[2] = 0;

    bma2x2_wake_up_data.reverse_num[0] = 0;
    bma2x2_wake_up_data.reverse_num[1] = 0;
    bma2x2_wake_up_data.reverse_num[2] = 0;
    bma2x2_wake_up_data.reverse_num[2] = 0;

    
}

static void bma2x2_timer_poll_callback(unsigned long data)
{           
    struct bma2x2_data  *bma2x2 = g_bma2x2Data;
    if (!g_bIsStop) {
        bma2x2_read_data.bma2x2 = bma2x2;
        queue_work(bma2x2_read_work, &(bma2x2_read_data.wakeup_work));
            
        mod_timer(&(bma2x2_wake_up_data.poll_timer), jiffies + msecs_to_jiffies(TIME_MOTION_DETECT_POLL_TIME_ms));  
    }   
}


//char g_cFlickOrientationBuff[50];
static void bma2x2_wakeup_work_handler(struct work_struct *p_work)
{
    int ret;
//  int iNum = 0;
    //static struct bma2x2acc acc;
    struct bma2x2_wake_up_work_t *wakeup_work = container_of(p_work, struct bma2x2_wake_up_work_t, wakeup_work);
...
/*
    ...
    bma2x2_read_accel_xyz(wakeup_work->bma2x2->bma2x2_client,wakeup_work->bma2x2->sensor_type, &acc);
    
    g_cFlickDataBuff[bma2x2_read_data.flickDataNum][ORIEN_X]= x;
...
*/
    ...
    //printk("[bma223] bma2x2_wakeup_work_handler\n");
/*
    if (1 == wakeup_work->shake_num) {
        ret = mod_timer(&(wakeup_work->timer), jiffies + msecs_to_jiffies(TIME_WAIT_SECOND_SHAKE_MS));  
    }
*/  
}

static void bma2x2_read_work_handler(struct work_struct *p_work)
{
//  int ret;
    struct bma2x2_wake_up_work_t *wakeup_work = container_of(p_work, struct bma2x2_wake_up_work_t, wakeup_work);
    ...
    //printk("[bam2x2] acc: %d %d %d\n", acc.x, acc.y, acc.z);

    g_cFlickDataBuff[bma2x2_read_data.flickDataNum][ORIEN_X]= acc.x;
    g_cFlickDataBuff[bma2x2_read_data.flickDataNum][ORIEN_Y]= acc.y;
    g_cFlickDataBuff[bma2x2_read_data.flickDataNum][ORIEN_Z]= acc.z;    
    bma2x2_read_data.flickDataNum++;    
}
//--- ASUS_BSP Alian_Shen "add shake-shake function"
...
static void bma2x2_irq_work_func(struct work_struct *work)
{
    ...
    //printk("[alian][L%d] bma2x2_irq_work_func  interruptstatus2=%X\n", __LINE__, status); 

    if ((status&0x80) == 0x80) {    
        /* printk("New data interrupt happened\n");*/
        bma2x2_read_accel_xyz(bma2x2->bma2x2_client,
                    bma2x2->sensor_type, &acc);
        //+++ ASUS_BSP Alian_Shen "input: sensors: send boot time alone with sensor events"
        input_event(bma2x2->input_dev, EV_SYN, SYN_TIME_SEC,
            ktime_to_timespec(timestamp).tv_sec);
        input_event(bma2x2->input_dev, EV_SYN, SYN_TIME_NSEC,
            ktime_to_timespec(timestamp).tv_nsec);
    //--- ASUS_BSP Alian_Shen    
        input_report_abs(bma2x2->input, ABS_X, acc.x);
        input_report_abs(bma2x2->input, ABS_Y, acc.y);
    ...
    //printk("bma2x2_irq_work_func, status = 0x%x\n", status);

#ifdef CONFIG_SIG_MOTION
    if (status & 0x04)  {
        if (atomic_read(&bma2x2->en_sig_motion) == 1) {
            printk("Significant motion interrupt happened\n");
            /* close sig sensor,
    ...
    if (status & 0x20) {
        if (atomic_read(&bma2x2->en_double_tap) == 1) {
            printk("single tap interrupt happened\n");
    ...
    case 0x01:
        printk("Low G interrupt happened\n");
    ...
    case 0x08:
        printk("slow/ no motion interrupt happened\n");
        input_report_rel(bma2x2->dev_interrupt,
    ...
    case 0x10:
        printk("double tap interrupt happened\n");
        input_report_rel(bma2x2->dev_interrupt,
            DOUBLE_TAP_INTERRUPT,
            DOUBLE_TAP_INTERRUPT_HAPPENED);
        break;
    ...
    case 0x80:
        bma2x2_get_orient_flat_status(bma2x2->bma2x2_client,
                    &sign_value);
        printk("flat interrupt happened,flat status is %d\n",
                    sign_value);
        if (sign_value == 1) {
            input_report_abs(bma2x2->dev_interrupt,
                FLAT_INTERRUPT,
...
/*
    //+++ ASUS_BSP Alian_Shen "add function for 'shake-shake' "
    //printk("[alian][L%d] bma2x2_irq_handler\n", __LINE__);
    bma2x2_smbus_read_byte(data->bma2x2_client, BMA2X2_STATUS1_REG, &reg_data);
    if (reg_data & BMA2x2_SLOPE_INT_STATUS_MASK)  {
        bma2x2_smbus_read_byte(data->bma2x2_client, BMA2X2_STATUS_TAP_SLOPE_REG, &reg_data);
        if (reg_data & BMA2x2_SLOPE_FIRST_X_MASK) {
            orientation = (reg_data & BMA2x2_SLOPE_SIGN_MASK) ? '-' : '+';
        }

        printk("[bam2x2] slope had happen. orientation=%c  reg=%d\n", orientation, reg_data);
    }
    //+++ ASUS_BSP Alian_Shen "add function for 'shake-shake' "
*/
...
// ASUS_BSP +++ guochang_qiu "[ZC550KL][Sensor][NA][Spec] support I2C stress test"
#ifdef CONFIG_I2C_STRESS_TEST
static int TestBMASensorI2C (struct i2c_client *apClient)
{
    int ret=0;
...
//  i2c_log_in_test_case("%s +\n",__func__);
    ...
    ret = bma2x2_set_Int_Mode(apClient, 1);
    if(ret < 0){
        i2c_log_in_test_case("%s: set Int mode failed\n",__func__);
        lnResult = -1;
        goto exit;
    }
    ...
    ret = bma2x2_get_Int_Mode(apClient, &data);
    if(ret < 0){
        i2c_log_in_test_case("%s: get Int mode failed\n",__func__);
        lnResult = -1;
        goto exit;
    }
//  i2c_log_in_test_case("%s -\n",__func__);
...
{
     __I2C_STRESS_TEST_CASE_ATTR(TestBMASensorI2C),
};
#endif
// ASUS_BSP --- Guochang_qiu "[ZC550KL][Sensor][NA][Spec] support I2C stress test"
...
//--- ASUS_BSP Alian_Shen "add shake-shake function"
    ...
    if (bma2x2_soft_reset(client) < 0) {
        printk("i2c bus write error, pls check HW connection\n");
        err = -EINVAL;
        goto kfree_exit;
    }
    mdelay(5);
...
#if defined(BMA2X2_ENABLE_INT1) || defined(BMA2X2_ENABLE_INT2)
    printk("[alian][L%d] bma2x2_probe \n", __LINE__);
    pdata = client->dev.platform_data;
    if (pdata) {
        if (pdata->irq_gpio_cfg && (pdata->irq_gpio_cfg() < 0)) {
            printk("IRQ GPIO conf. error %d\n",
                client->irq);
        }
    }
    ...
    //+++ ASUS_BSP Alian_Shen "add shake-shake function"
    //- define the slope_dur
    bma2x2_smbus_read_byte(client, BMA2X2_SLOPE_DURN_REG, &reg_data);
    reg_data &= ~BMA2X2_SLOPE_DUR__MSK;
    reg_data |=   BMA2x2_SLOPE_DUR;
    bma2x2_smbus_write_byte(client, BMA2X2_SLOPE_DURN_REG,  &reg_data);

    //-define the slope_th
    reg_data =   (BMA2x2_SLOPE_THR_mg*100) /781 ; 
    bma2x2_smbus_write_byte(client, BMA2X2_SLOPE_THRES_REG,  &reg_data);

    //-define the int1_slope
    bma2x2_smbus_read_byte(client, BMA2X2_INT1_PAD_SEL_REG, &reg_data);
    reg_data &= ~BMA2x2_SLOPE_INT1_MASK;  
    reg_data |=   BMA2x2_SLOPE_INT1_EN;
    bma2x2_smbus_write_byte(client, BMA2X2_INT1_PAD_SEL_REG,  &reg_data);

    //-enable slope_en_x
    bma2x2_smbus_read_byte(client, BMA2X2_INT_ENABLE1_REG, &reg_data);
    reg_data &= ~(BMA2x2_SLOPE_INT_EN_X_MASK|BMA2x2_SLOPE_INT_EN_Z_MASK);  
    reg_data |=   BMA2x2_SLOPE_INT_EN_X|BMA2x2_SLOPE_INT_EN_Z;       
    bma2x2_smbus_write_byte(client, BMA2X2_INT_ENABLE1_REG,  &reg_data);

    //- init timer
    setup_timer(&bma2x2_wake_up_data.timer, bma2x2_timer_callback, 0);
    setup_timer(&bma2x2_wake_up_data.poll_timer, bma2x2_timer_poll_callback, 0);

    bma2x2_wakeup_work = create_workqueue("bma2x2_wakeup_work");
    if (!bma2x2_wakeup_work) {
        printk("create workqueue for 'bma2x2_wakeup_work' ERROR \n");
        return -ENOMEM;
    }
    INIT_WORK(& (bma2x2_wake_up_data.wakeup_work), bma2x2_wakeup_work_handler); 
    
    bma2x2_read_work = create_workqueue("bma2x2_read_work");
    if (!bma2x2_read_work) {
        printk("create workqueue for 'bma2x2_read_work' ERROR \n");
        return -ENOMEM;
    }
    INIT_WORK(& (bma2x2_read_data.wakeup_work), bma2x2_read_work_handler);  
...
//+++ ASUS_BSP Alian_Shen "add shake-shake function"
    flick_interrupt = input_allocate_device();
    if (!flick_interrupt) {
        kfree(data);
        input_free_device(dev); /*free the successful dev and return*/
        return -ENOMEM;
...
//+++ ASUS_BSP Alian_Shen "add shake-shake function"
    flick_interrupt->name = FLICK_INPUT_DEV_NAME;
    flick_interrupt->id.bustype = BUS_I2C;
    input_set_capability(flick_interrupt, EV_REL, REL_DIAL);

    err = input_register_device(flick_interrupt);
    if (err < 0)
        goto err_register_input_device_interrupt;

    data->flick_interrupt = flick_interrupt;
//--- ASUS_BSP Alian_Shen "add shake-shake function"
...
printk("[alian][L%d] bma2x2_probe \n", __LINE__);
    ...
    if (IS_ERR(data->g_sensor_class)) {
        ...
        printk("could not allocate g_sensor_class\n");
    ...
    if (unlikely(IS_ERR(data->g_sensor_dev))) {
        ...
        printk("could not allocate g_sensor_dev\n");
...
printk("[alian][L%d] bma2x2_probe \n", __LINE__);
    ...
    if (IS_ERR(data->g_sensor_class_doubletap)) {
        ...
        printk("could not allocate g_sensor_class_doubletap\n");
        goto err_create_class;
    }
    ...
    if (unlikely(IS_ERR(data->g_sensor_dev_doubletap))) {
        ...
        printk("could not allocate g_sensor_dev_doubletap\n");
...
/***
    ...
    printk("555555555555555555555555\n");
    ...
    printk("666666666666666666666666\n");
...
// ASUS_BSP --- guochang_qiu "[ZC550KL][Sensor][NA][Spec] support I2C stress test"
    ...
    if (bma2x2_set_mode(client, BMA2X2_MODE_SUSPEND, BMA_ENABLED_ALL) < 0)
        return -EINVAL;
    printk("BMA2x2 driver probe successfully");
...
//+++ ASUS_BSP Alian_Shen "add sensor class define "
	...
	//data->cdev.resolution = sensor_type_map[data->sensor_type].resolution;
	//if (pdata->int_en)
...
//+++ ASUS Alian_Shen "add function for 'shake-shake' "
    input_unregister_device(data->flick_interrupt);
//--- ASUS Alian_Shen "add function for 'shake-shake' "
#endif
...
//+++ ASUS Alian_Shen "add function for 'shake-shake' " 
    input_free_device(flick_interrupt);
//--- ASUS Alian_Shen "add function for 'shake-shake' "
    input_unregister_device(data->input);
