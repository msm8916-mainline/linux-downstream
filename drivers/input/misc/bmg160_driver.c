...
//TK(20140610) Add gyro sensor driver to Qsensortest - BMG160 start
#include <linux/sensors.h>
//TK(20140610) Add gyro sensor driver to Qsensortest - BMG160 end
...
//#define SENSOR_NAME	"bmg160-gyroscope"
...
//TK(20140610) Add gyro sensor driver to Qsensortest - BMG160 start
struct sensors_classdev bmg_dev = {
//	.name = SENSOR_NAME,
	.name = "bmg160-gyro",
	.vendor = "bosch",
	.version = 157,
...
//	.type = SENSOR_TYPE_GYROSCOPE_UNCALIBRATED,
	...
	.resolution = "0.06",
	.sensor_power = "0.2",
	.min_delay = BMG_DELAY_MIN * 1000, /* in microseconds */
	.delay_msec = BMG_DELAY_DEFAULT, /* in millisecond */
	.sensors_enable = NULL,
...
struct bmg160_data_t ggyro_data_high;
struct bmg160_data_t ggyro_data_low;
...
static void bmg_work_func(struct work_struct *work)
{
	...
	dev_notice(&bmg_client->dev,"ABS_RX=%5d, ABS_RY=%5d, ABS_RZ=%5d",
		gyro_data.datax, gyro_data.datay, gyro_data.dataz);

	if( ggyro_data_high.datax < gyro_data.datax )
	    ggyro_data_high.datax = gyro_data.datax;
	if( ggyro_data_high.datay < gyro_data.datay )
	    ggyro_data_high.datay = gyro_data.datay;
	if( ggyro_data_high.dataz < gyro_data.dataz )
	    ggyro_data_high.dataz = gyro_data.dataz;

	if( ggyro_data_low.datax > gyro_data.datax )
	    ggyro_data_low.datax = gyro_data.datax;
	if( ggyro_data_low.datay > gyro_data.datay )
	    ggyro_data_low.datay = gyro_data.datay;
	if( ggyro_data_low.dataz > gyro_data.dataz )
	    ggyro_data_low.dataz = gyro_data.dataz;

	dev_notice(&bmg_client->dev,"MAX_X=%5d, MAX_Y=%5d, MAX_Z=%5d",
		ggyro_data_high.datax, ggyro_data_high.datay, ggyro_data_high.dataz);

	dev_notice(&bmg_client->dev,"MIN_X=%5d, MIN_Y=%5d, MIN_Z=%5d",
		ggyro_data_low.datax, ggyro_data_low.datay, ggyro_data_low.dataz);

	dev_notice(&bmg_client->dev,"MAX_X=%5d.%02d, MAX_Y=%5d.%02d, MAX_Z=%5d.%02d",
		ggyro_data_high.datax >> 4, (ggyro_data_high.datax & 0x0F) *6,
		ggyro_data_high.datay >> 4, (ggyro_data_high.datay & 0x0F) *6,
		ggyro_data_high.dataz >> 4, (ggyro_data_high.dataz & 0x0F) *6);

	dev_notice(&bmg_client->dev,"MIN_X=%5d.%02d, MIN_Y=%5d.%02d, MIN_Z=%5d.%02d",
		ggyro_data_low.datax >> 4, (ggyro_data_low.datax & 0x0F) *6,
                ggyro_data_low.datay >> 4, (ggyro_data_low.datay & 0x0F) *6,
                ggyro_data_low.dataz >> 4, (ggyro_data_low.dataz & 0x0F) *6);		
	...
	dev_notice(&bmg_client->dev,"%s(%d)schedule_delayed_work:%ld",__func__,__LINE__, delay);
		...
		const char *buf, size_t count)
{
	...
	if (data != client_data->enable) {
		if (data) {
			dev_notice(&bmg_client->dev,"%s(%d)schedule_delayed_work:%ld",
				__func__,__LINE__,
				msecs_to_jiffies(atomic_read(&client_data->delay)));
			...
			dev_notice(&bmg_client->dev,"cancel_delayed_work_sync");
...
//TK(20140610) Add gyro sensor driver to Qsensortest - BMG160 start
static int bmg_cdev_poll_delay(struct sensors_classdev *bmg_dev,
	unsigned int delay_ms)
{
	dev_notice(&bmg_client->dev,"bmg_cdev_poll_delay");	
//	schedule_delayed_work(&bmg_client->work,
//                                msecs_to_jiffies(atomic_read(&client_data->delay)));
	return 0;
}

static void bmg_set_mode(struct bmg_client_data *client_data, u8 op_mode)
{
	int err;
	dev_notice(&bmg_client->dev,"bmg_set_mode");
	mutex_lock(&client_data->mutex_op_mode);
	err = BMG_CALL_API(set_mode)(op_mode);
	mutex_unlock(&client_data->mutex_op_mode);
}

static void bmg_set_enable(struct bmg_client_data *client_data, int enable)
{
	u8 pre_enable = client_data->enable;
	mutex_lock(&client_data->mutex_enable);
	dev_notice(&bmg_client->dev,"bmg_set_enable");
	if (enable) {
		dev_notice(&bmg_client->dev,"bmg_set_enable:Enable");
		if (pre_enable == 0) {
			bmg_set_mode(client_data, BMG_VAL_NAME(MODE_NORMAL));
		...
		dev_notice(&bmg_client->dev,"bmg_set_enable:Disable");
		if (pre_enable == 1) {
			bmg_set_mode(client_data, BMG_VAL_NAME(MODE_SUSPEND));
			dev_notice(&bmg_client->dev,"cancel_delayed_work_sync");
			cancel_delayed_work_sync(&client_data->work);
...
static int bmg_cdev_enable(struct sensors_classdev *bmg_dev,
	unsigned int enable)
{
	struct bmg_client_data *client_data = container_of(bmg_dev, struct bmg_client_data, bmg_cdev);
	dev_notice(&bmg_client->dev,"bmg_cdev_enable");
	bmg_set_enable(client_data, enable);
...
//	mdelay(5);
...
#if 0
        err = BMG_CALL_API(get_bw)(&bandwidth);

        if( err < 0 )
            dev_notice(&client->dev, "get_bw fail");
        else
            dev_notice(&client->dev, "get bandwidth = %x", (int)bandwidth);

        bandwidth = 0x05;

        err = BMG_CALL_API(set_bw)(bandwidth);
        if( err < 0 )
            dev_notice(&client->dev, "set_bw fail");
        else
            dev_notice(&client->dev, "set bandwidth = %x", (int)bandwidth);

        bandwidth = 0x00;

        err = BMG_CALL_API(get_bw)(&bandwidth);
        if( err < 0 )
            dev_notice(&client->dev, "get_bw again fail");
        else
            dev_notice(&client->dev, "get bandwidth = %x", (int)bandwidth);
	...
	err = BMG_CALL_API(get_range_reg)(&range);

	if( err < 0 )
	    dev_notice(&client->dev, "get_range_reg  fail");
	else
	    dev_notice(&client->dev, "get range = %x", (int)range);

//	dev_notice(&client->dev, "bmg_set_soft_reset again");
//	err = bmg_set_soft_reset(client);

//	mdelay(30);
#endif
            ggyro_data_high.datax = 0;
            ggyro_data_high.datay = 0;
            ggyro_data_high.dataz = 0;

            ggyro_data_low.datax = 0;
            ggyro_data_low.datay = 0;
            ggyro_data_low.dataz = 0;
	...
	{
		dev_err(&client->dev,
			"sysfs node creation fail:%d",err);
		goto exit_err_sysfs;
	}
...
while( bandwidth == 0x00 )
{
	...
	dev_notice(&client->dev, "set_mode:MODE_SUSPEND");
...
//TK(20140610) Add gyro sensor driver to Qsensortest - BMG160 start
	client_data->bmg_cdev = bmg_dev;
	client_data->bmg_cdev.sensors_enable = bmg_cdev_enable;
	client_data->bmg_cdev.sensors_poll_delay = bmg_cdev_poll_delay;
	dev_notice(&client->dev, "sensors_classdev_register bmg\n");
	err = sensors_classdev_register(&client->dev, &client_data->bmg_cdev);
 	if (err) {
...
#if 0
	mutex_lock(&client_data->mutex_enable);
	dev_notice(&client->dev, "client_data->enable = %u\n", client_data->enable);
	if(!client_data->enable){
		client_data->enable = 1;
		schedule_delayed_work(&client_data->work, 0);
	}
	mutex_unlock(&client_data->mutex_enable);
#endif
