			     ...
			     struct axis_data *data)
{
	...
	accel_fir.raw[accel_fir.idx][0] = be16_to_cpu(buffer[0]);
	accel_fir.raw[accel_fir.idx][1] = be16_to_cpu(buffer[1]);
	accel_fir.raw[accel_fir.idx][2] = be16_to_cpu(buffer[2]);

	if(accel_fir.idx >= accel_fir.firlen-1)
	{
		accel_fir.idx = 0;
	}else
	{
	...
	if(accel_fir.num < accel_fir.firlen)
	{
		accel_fir.num++;
	}
	...
	for(k=0;k<accel_fir.num;k++)
	{
		accel_fir.sum[0] += accel_fir.raw[k][0];
		accel_fir.sum[1] += accel_fir.raw[k][1];
		accel_fir.sum[2] += accel_fir.raw[k][2];
	}

	data->rx = accel_fir.sum[0] /accel_fir.num;
	data->ry = accel_fir.sum[1] /accel_fir.num;
	data->rz = accel_fir.sum[2] /accel_fir.num;
#endif
				   ...
				   const struct i2c_device_id *id)
{
	...
	gyro_fir.firlen = 12;
	memset(&accel_fir, 0, sizeof(accel_fir));
	accel_fir.firlen = 12;
#endif
