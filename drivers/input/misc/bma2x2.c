...
#define DRIVER_INFO
#ifdef DRIVER_INFO
# define LOGI(fmt, args...) printk(KERN_INFO "[Shengwei] %s: " fmt, __FUNCTION__, ## args)
#else 
 ...
 * Sensor config file
 */
#define SENSOR_CONF_FILE "/info2/sensors/acc_offset"
/* QCI End: ALT Sensor Daniel Wang 20140709 */
...
#endif
	/* QCI Begin: ALT Sensor Daniel Wang 20140709 */
	bool offset_enalbed; 							/** offset flag */
	int offset_x; 									/** offset x */
	int offset_y; 									/** offset y */
	int offset_z; 									/** offset z */
	int calibration_result;							/** calibration result */
	/* QCI End: ALT Sensor Daniel Wang 20140709 */
};
...
static struct bma2x2_data* sensors_bma2x2_data;
		...
		signed char sensor_type, struct bma2x2acc *acc)
{
	 ...
	 * Offset acc
 	 */
 	acc->x = acc->x - sensors_bma2x2_data->offset_x;
	acc->y = acc->y - sensors_bma2x2_data->offset_y;
	acc->z = acc->z + sensors_bma2x2_data->offset_z;
	/* QCI End: ALT Sensor Daniel Wang 20140709 */
LOGD("OFFSET X = %d, Y = %d, Z = %d\n", client_data->offset_x, client_data->offset_y, client_data->offset_z);
LOGD("acc->x = %d, y = %d, z = %d\n", acc->x, acc->y, acc->z);
...
//	struct bma2x2_data *bma2x2 = container_of((struct delayed_work *)work,
//			struct bma2x2_data, work);
	LOGI("Open file(%s).", SENSOR_CONF_FILE);

	file = filp_open(SENSOR_CONF_FILE, O_RDONLY, 0);
	error = (int)IS_ERR(file);
	if(error)
	{
		LOGI("Failed to open file(%s) with error(%d)!", SENSOR_CONF_FILE, error);
	}
	else
	{
		LOGI("Read file(%s).", SENSOR_CONF_FILE);
		fs = get_fs();
		set_fs(get_ds());
		vfs_read(file, content, sizeof(content), &position);
		set_fs(fs);
		LOGI("Sscanf offset from content(%s).", content);
		error = sscanf(content, "%d %d %d", &sensors_bma2x2_data->offset_x, &sensors_bma2x2_data->offset_y, &sensors_bma2x2_data->offset_z);
		if(error != 3)
		{
			LOGD("Failed to sscanf offset!");
		}
		LOGI("Sscanfed offset(%d,%d,%d).", sensors_bma2x2_data->offset_x, sensors_bma2x2_data->offset_y, sensors_bma2x2_data->offset_z);
		LOGI("Close file(%s).", SENSOR_CONF_FILE);
...
/**
  ...
  *
  * Description
  * Start bma2x2 calibration.
  *
  * Dependencies
  * None
  *
  * Arguments
  * device: sys/class/sensors/gsensor device
  ...
  */
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
static ssize_t bma2x2_calibration_show(struct device* device, struct device_attribute* device_attribute, char* buffer)
{
	/**
	 * Init variable
	 */
	 ...
	 * Check variable
	 */
	if(!sensors_bma2x2_data)
	{
		LOGD("Failed for null-point bma2x2 data!");
		error = snprintf(buffer, PAGE_SIZE, "fail\n");
		goto out;
	}
	 ...
	 */
	if(sensors_bma2x2_data->calibration_result)
	{
		LOGD("Failed for %d!", sensors_bma2x2_data->calibration_result);
		error = snprintf(buffer, PAGE_SIZE, "FAIL\n");
		goto out;
	}

	error = snprintf(buffer, PAGE_SIZE, "PASS\n");
...
/**
  ...
  *
  * Arguments
  * device: sys/class/sensors/gsensor sensor
  * device_attribute: Device attribute
  * buffer: Echo buffer
  *
  * Return value
  ...
  */
/*@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@*/
static ssize_t bma2x2_calibration_store(struct device* device, struct device_attribute* device_attribute, const char* buffer, size_t count)
{
	/**
	 * Init variables
	 */
	...
	loff_t position = 0;
	mm_segment_t fs = {0};
	struct bma2x2acc acc = {0,0,0};
	struct bma2x2acc diff = {0,0,0};
	struct file* file = NULL;
	 ...
	 * Read acc
	 */
	bma2x2_smbus_read_byte_block(sensors_bma2x2_data->bma2x2_client, BMA2X2_ACC_X12_LSB__REG, value, 6);

	acc.x = (value[1]<<8)|value[0];
	acc.y = (value[3]<<8)|value[2];
	acc.z = (value[5]<<8)|value[4];

	bitwidth = bma2x2_sensor_bitwidth[sensors_bma2x2_data->sensor_type];

	acc.x = (acc.x >> (16 - bitwidth));
	acc.y = (acc.y >> (16 - bitwidth));
	acc.z = (acc.z >> (16 - bitwidth));

	bma2x2_remap_sensor_data(&acc, sensors_bma2x2_data);
	
	LOGD("Read acc(%d,%d,%d).", acc.x, acc.y, acc.z);

	sensors_bma2x2_data->offset_x = acc.x - 0;
	sensors_bma2x2_data->offset_y = acc.y - 0;
	sensors_bma2x2_data->offset_z = acc.z * -1 - 1024;

	/**
	 * Verify the calibration
	 */
	LOGD("Verify the calibration.");
	msleep(1000);
	acc.x = 0;
	acc.y = 0;
	acc.z = 0;
	bma2x2_read_accel_xyz(sensors_bma2x2_data->bma2x2_client, sensors_bma2x2_data->sensor_type, &acc);
	diff.x = abs(acc.x);
	diff.y = abs(acc.y);
	diff.z = abs(acc.z - 1024);
#if 0	
	if(diff.x > 50 || diff.y > 50 || diff.z > 50)
	{
		LOGD("Failed to verify the calibration with diff(%d,%d,%d)!", diff.x, diff.y, diff.z);
		error = -ERANGE;
		goto out;
	}
#endif
	...
	/**
	 * Save offsets to gsensor.conf file
	 */
	LOGD("Save offsets(%d,%d,%d) to file(%s).", sensors_bma2x2_data->offset_x, sensors_bma2x2_data->offset_y, sensors_bma2x2_data->offset_z, SENSOR_CONF_FILE);
	LOGD("Open file(%s).\n", SENSOR_CONF_FILE);
	file = filp_open(SENSOR_CONF_FILE, O_CREAT | O_WRONLY, S_IRWXUGO);
	error = IS_ERR(file);
	if(error)
	{
		LOGD("Failed to open file(%s) with error(%d)!", SENSOR_CONF_FILE, error);
		error = -ENOENT;
		goto out;
	}

	snprintf(content, sizeof(content), "%d %d %d", sensors_bma2x2_data->offset_x, sensors_bma2x2_data->offset_y, sensors_bma2x2_data->offset_z);

	LOGD("Write content(%s) to file(%s).", content, SENSOR_CONF_FILE);
	fs = get_fs();
	set_fs(get_ds());
	vfs_write(file, content, sizeof(content), &position);
	set_fs(fs);

	LOGD("Close file(%s).", SENSOR_CONF_FILE);
	filp_close(file, NULL);

out:
	sensors_bma2x2_data->calibration_result = error;
	return count;
}
/* QCI End: ALT Sensor Daniel Wang 20140709 */
...
static DEVICE_ATTR(place, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_place_show, bma2x2_place_store);
#ifdef CONFIG_SIG_MOTION
...
#endif
/* QCI Begin: ALT Sensor Daniel Wang 20140709 */
static DEVICE_ATTR(calibration, S_IRUGO|S_IWUSR|S_IWGRP, bma2x2_calibration_show, bma2x2_calibration_store);
/* QCI End: ALT Sensor Daniel Wang 20140709 */
		...
		const struct i2c_device_id *id)
{
...
#endif
	INIT_DELAYED_WORK(&data->restore_sensordata_work, do_restore_sensordata_work);
	schedule_delayed_work(&data->restore_sensordata_work, 10*HZ);
	...
	/**
	 * Save bma2x2 data for callback
 	 */
 	sensors_bma2x2_data = data;
	/* QCI End: ALT Sensor Daniel Wang 20140709 */
...
//	schedule_delayed_work(&data->restore_sensordata_work, 2*HZ);
	mutex_lock(&data->enable_mutex);
	if (atomic_read(&data->enable) == 1) {
		bma2x2_set_mode(data->bma2x2_client, BMA2X2_MODE_NORMAL);
#ifndef CONFIG_BMA_ENABLE_NEWDATA_INT
