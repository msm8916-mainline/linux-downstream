...
struct kionix_accel_platform_data default_kionix_accel_platform_data = {
	.rot_matrix = {
		{1, 0, 0},
		{0, 1, 0},
		{0, 0, 1},
...
static void kionix_swap(int *x,int *y)
{
	int temp = 0;
	temp = *x;
	*x = -*y;
...
/*add by yin*/
static ssize_t kionix_acc_get_data(struct kionix_accel_driver *acceld, int32_t *xyz)
{
	int32_t i, err = -1, hw_d[3] = {0};
	uint8_t acc_data[6];
	err =kionix_i2c_read(acceld->client,ACCEL_GRP4_XOUT_L, acc_data, 6);
		if(err < 0)
			return err;
	hw_d[0] = (short)(acc_data[1] << 8 | acc_data[0]);
	hw_d[1] = (short)(acc_data[3] << 8 | acc_data[2]);
	hw_d[2] = (short)(acc_data[5] << 8 | acc_data[4]);
	kionix_swap(&hw_d[0],&hw_d[1]);
	mutex_lock(&acceld->lock);
	for (i = 0; i < 3; i++) {
		xyz[i] = (int32_t)acceld->accel_pdata.rot_matrix[0][i] * hw_d[0] +
				(int32_t)acceld->accel_pdata.rot_matrix[1][i] * hw_d[1] +
				(int32_t)acceld->accel_pdata.rot_matrix[2][i] * hw_d[2];
		xyz[i] = (int32_t)(xyz[i] * GRAVITY_HAL_UNIT)/GRAVITY_REG_UNIT;
	}
	xyz[2] = -xyz[2];
...
static void kionix_accel_grp4_report_accel_data(struct kionix_accel_driver *acceld)
{
	...
	if (atomic_read(&acceld->accel_enabled) > 0) {
		if (atomic_read(&acceld->accel_enable_resume) > 0)
		{
			...
			while (loop) {
				mutex_lock(&input_dev->mutex);
				err= kionix_acc_get_data(acceld,acc_data);
				if (err < 0){
					loop--;
					mdelay(KIONIX_I2C_RETRY_TIMEOUT);
				} else
			...
			if (err < 0) {
				...
				mutex_unlock(&input_dev->mutex);
				if (atomic_read(&acceld->accel_input_event) > 0) {
					input_report_abs(acceld->input_dev, ABS_X, acc_data[0]+acceld->accel_cali[0]);
					input_report_abs(acceld->input_dev, ABS_Y, acc_data[1]+acceld->accel_cali[1]);
					input_report_abs(acceld->input_dev, ABS_Z, acc_data[2]+acceld->accel_cali[2]);
					input_sync(acceld->input_dev);
				}
				write_unlock(&acceld->rwlock_accel_data);
			}
...
static int kionix_cdev_set_enable(struct sensors_classdev *sensors_cdev,
				unsigned int enable)
{
	struct kionix_accel_driver *acceld = container_of(sensors_cdev, struct kionix_accel_driver, cdev);
	struct i2c_client *client = acceld->client;

	if ((enable != 0) && (enable != 1)) {
		pr_err("%s: invalid value(%d)\n", __func__, enable);
		return -EINVAL;
	}
	dev_info(&client->dev, "kionix enable:%d\n", enable);
	if (enable)
		return kionix_accel_enable(acceld);
	else
		return kionix_accel_disable(acceld);
}
...
static int kionix_cdev_set_poll_delay(struct sensors_classdev *sensors_cdev,
							unsigned int msecs)
{
	struct kionix_accel_driver *acceld = container_of(sensors_cdev, struct kionix_accel_driver, cdev);
	struct input_dev *input_dev = acceld->input_dev;
	...
	if (msecs != acceld->poll_interval) {
		acceld->poll_interval = msecs;
		acceld->poll_delay = msecs_to_jiffies(acceld->poll_interval);
		err = acceld->kionix_accel_update_odr(acceld, acceld->poll_interval);
	}
	if (acceld->accel_drdy == 1)
...
static DEVICE_ATTR(poll_delay, S_IRUGO|S_IWUGO, kionix_accel_get_delay, kionix_accel_set_delay);
/*modify by longyu for cts bugid:67748*/
static DEVICE_ATTR(direct, S_IRUGO, kionix_accel_get_direct, kionix_accel_set_direct);
...
#endif /* CONFIG_PM */
#ifdef CONFIG_OF
static int kxcjk_parse_dt(struct device *dev,
				struct kionix_accel_platform_data *accel_pdata)
{
	...
	rc =  of_property_read_u16_array(np,"rot-matrix",vect, ARRAY_SIZE(vect) );
	if (rc >= 0){
		for( j= 0;j < 3; j ++)
			for(i = 0;i < 3; i++){
				accel_pdata->rot_matrix[i][j]=
							(short)vect[3 * j + i];
				}
	} else {
		for (j = 0; j < 3; j++) {
				for (i = 0; i < 3; i++) {
					accel_pdata->rot_matrix[i][j] =
					default_kionix_accel_platform_data.rot_matrix[i][j];
				}
			}
	}
...
static int kionix_cdev_calibrate(struct sensors_classdev *sensors_cdev,
		int axis, int apply_now)
{
	int ret = 0;
	struct kionix_accel_driver *acceld= container_of(sensors_cdev, struct kionix_accel_driver,
			cdev);
	int acc_data[3];
	memset(acc_data, 0, sizeof(acc_data));
	ret = kionix_acc_get_data(acceld,acc_data);
		if(ret < 0) {
			dev_err(&acceld->client->dev, "get acc data error!\n");
			return ret;
		}
	acceld->accel_cali[0] = -acc_data[0];
	acceld->accel_cali[1] = -acc_data[1];
	acceld->accel_cali[2] = GRAVITY_HAL_UNIT - acc_data[2];
	dev_dbg(&acceld->client->dev, "kxcjx calibrate data, x:%d, y:%d, z:%d\n",
				acceld->accel_cali[0], acceld->accel_cali[1],acceld->accel_cali[2]);
	if (sensors_cdev->params == NULL) {
		sensors_cdev->params = devm_kzalloc(&acceld->client->dev, 64, GFP_KERNEL);
		if (sensors_cdev->params == NULL) {
			dev_err(&acceld->client->dev,
					"kionix_accel_driver sensor_cdev allocate memory error!\n");
			return -EBUSY;
		}
	}
			snprintf(sensors_cdev->params, 64, "%d,%d,%d", acceld->accel_cali[0],
			acceld->accel_cali[1], acceld->accel_cali[2]);
	return 0;
}

static int kionix_cdev_write_cal_params(struct sensors_classdev
		*sensors_cdev, struct cal_result_t *cal_result)
{
	...
	acceld->accel_cali[0] = cal_result->offset[0];
	acceld->accel_cali[1] = cal_result->offset[1];
	acceld->accel_cali[2] = cal_result->offset[2];
				 ...
				 const struct i2c_device_id *id)
{
	...
	acceld->register_early_suspend(&acceld->early_suspend);
#endif /* CONFIG_HAS_EARLYSUSPEND */
	acceld->cdev = sensors_accel_cdev;
	acceld->cdev.sensors_calibrate=kionix_cdev_calibrate;
	acceld->cdev.sensors_write_cal_params = kionix_cdev_write_cal_params;
	acceld->cdev.sensors_enable = kionix_cdev_set_enable;
	acceld->cdev.sensors_poll_delay = kionix_cdev_set_poll_delay;
...
static struct of_device_id kxcjk_match_table[] = {
	{ .compatible = "kionix,accelerometer", },
	{ },
};
