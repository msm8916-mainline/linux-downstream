...
#if 0
		//mask ps_state_last to resolve psensor don't report the first near event when enabled, 
		//due to previously disconnect the call, the face near to phone   
			...
			/*
                     if (data->ps_state == 1 && data->dynamic_noise > 20 && psdata < (data->dynamic_noise - 50) ) {
                        ...
                        if(psdata < 50){
                            ...
                            data->platform_data->prox_hsyteresis_threshold = psdata*19/10;
                        }else if(psdata < 100){
                            ...
                            data->platform_data->prox_hsyteresis_threshold = psdata*14/10;
                        }else if(psdata < 200){
...
static ssize_t ltr559_show_ps_data(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	int psdata;
	struct ltr559_data *data = dev_get_drvdata(dev);
			...
			struct device_attribute *attr, char *buf)
{
	int ret;
	struct ltr559_data *data = dev_get_drvdata(dev);
	ret = sprintf(buf, "[PS][ltr559]PS Hi/Low THD ps_close_thd_set = 0x%04x(%d), ps_away_thd_set = 0x%04x(%d)\n", data->platform_data->prox_threshold, data->platform_data->prox_threshold, data->platform_data->prox_hsyteresis_threshold, data->platform_data->prox_hsyteresis_threshold);
	return ret;	
}
				...
				const char *buf, size_t count)
{
	...
	data->platform_data->prox_threshold = code1;//near	
	data->platform_data->prox_hsyteresis_threshold = code2;//away
	
	ltr559_set_ps_threshold(client, LTR559_PS_THRES_UP_0, data->platform_data->prox_threshold);
	ltr559_set_ps_threshold(client, LTR559_PS_THRES_LOW_0, data->platform_data->prox_hsyteresis_threshold);

	printk("[PS][ltr559]%s: ps_close_thd_set = 0x%04x(%d), ps_away_thd_set = 0x%04x(%d)\n", __func__, data->platform_data->prox_threshold, data->platform_data->prox_threshold, data->platform_data->prox_hsyteresis_threshold, data->platform_data->prox_hsyteresis_threshold);
...
static DEVICE_ATTR(ps_thd, S_IRUGO | S_IWUGO, ps_thd_show, ps_thd_store);
...
static DEVICE_ATTR(ps_data, S_IRUGO, ltr559_show_ps_data, NULL);
#ifdef LTR559_PS_CALIBERATE
...
int ltr559_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_info(&client->dev,"probe succece\n");
	return 0;
...
exit_kfree_data:
	kfree(data);
exit:
	dev_info(&client->dev,"probe fail ret:%d\n",ret);
	return ret;
}
