...
#define TAOS_THD_H_OFFSET 320	// mod by yuzhong.li for opimize p-sensor pr877930 2014-12-22
#define TAOS_THD_L_OFFSET 120	// mod by yuzhong.li for opimize p-sensor pr877930 2014-12-22
...
/*mod by yuzhong.li,pr876392,2015-1-5,disable light senosr auto adjust,its done in framework*/
...
#define TAOS_ALS_THD_BRIGHT	25	
#define TAOS_ALS_THD_DARK	25
...
static int als_level[] = {15, 36, 80, 120, 192, 275, 390, 500, 845, 1136, 1545, 2364, 4655, 6982};
static int als_value[] = {0, 40, 85, 110, 225, 300, 410, 550, 930, 1250, 1700, 2600, 5120, 7680};
...
/*Add by liyuzhong,PR-876392, 2014/12/31,adjust light sensor value*/
#if defined CONFIG_TCT_8X16_ALTO5
             lux =lux*100/365;
#elif defined CONFIG_TCT_8X16_ALTO5_VDF
              lux =lux*10/52;
#endif
		...
		if(idx >=14)
			idx = 13;
		lux = als_value[idx];
		printk(KERN_ALERT "report_als, after tunning, the lux = %d\n",lux);
        #ifdef ALS_TUNE_AUTOBRIGHTNESS
...
static irqreturn_t taos_irq(int irq, void *handle)
{
	...
	struct device *dev = &chip->client->dev;
    printk(KERN_INFO "taos_irq in_suspend=%d",chip->in_suspend);
	...
	if (on) {
			...
			return rc;
        {
    	    ...
    	    chip->prx_inf.detected =
    			(d && (chip->prx_inf.raw > chip->params.prox_th_min)) ||
    			(!d && (chip->prx_inf.raw > chip->params.prox_th_max));
/* [BUFFIX]-Mod-Begin by TCTNB.XQJ,PR-841025, 2014/11/14,aovid data can't report to upper,because of report abs,-1 will be filterd by hal*/
	...
	struct device_attribute *attr, char *buf)
{
    ...
    d = chip->prx_inf.detected;
    chip->prx_inf.raw =  prx_raw_hi << 8 | prx_raw_lo ;;
      //pr_err("raw value=%d,params.prox_th_min=%d,max=%d, chip->prx_inf.detected=%d\n",chip->prx_inf.raw, chip->params.prox_th_min,chip->params.prox_th_max, chip->prx_inf.detected);	
      chip->prx_inf.detected =
            (d && (chip->prx_inf.raw > chip->params.prox_th_min)) ||
			(!d && (chip->prx_inf.raw > chip->params.prox_th_max));
...
static int taos_suspend(struct device *dev)
{
	...
	dev_info(dev, "%s\n", __func__);
	//mutex_lock(&chip->lock);/* [BUGFIX]-Del- by TCTNB.XQJ, 2014/09/28, BUG 799932 ,for remove unncecssary mutex lock*/
	/*if (chip->p_idev && chip->p_idev->users) {
	...
	chip->prx_enable_pre = chip->prx_enabled;
	taos_prox_enable(chip, 0);    
    chip->als_enable_pre = chip->als_enabled;
	taos_als_enable(chip, 0);
    chip->in_suspend = 1;
	//mutex_unlock(&chip->lock);/* [BUGFIX]-Del- by TCTNB.XQJ, 2014/09/28, BUG 799932 ,for remove unncecssary mutex lock*/
...
//err_power:*/
    dev_info(dev, "%s\n", __func__);
    chip->in_suspend = 0;
    taos_prox_enable(chip, chip->prx_enable_pre);
    if (chip->irq_pending) {
		dev_dbg(dev, "%s: pending interrupt\n", __func__);
		chip->irq_pending = 0;
		//(void)taos_check_and_report(chip);
    ...
    taos_als_enable(chip, chip->als_enable_pre);
