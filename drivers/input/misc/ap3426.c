...
static int ap3426_get_ir_value(struct i2c_client *client)
{
    ...
    i2c_smbus_write_byte_data(client, AP3426_REG_PS_INTEGR, 0x02);//ptime = 1t
...
//modify by yanfei for ap3426  ps enable  20150212 begin
    ...
    if(enable==0)
		...
		cancel_delayed_work(&ps_data->work_light);
...
//modify by yanfei for ps-sensor threshold 20150318 begin
	...
	for(i=0;i<40;i++){
		...
		printk(KERN_ERR"yanfei test tmp==%d read_count=%d\n",tmp,read_count);
		msleep(50);
	}
//add by yanfei for calibration fail
	if(read_count == 0){
//modify by yanfei for SW00110845  20150204
		data->crosstalk =-1;
...
//modify by yanfei for ap3426  ps enable  20150212 begin
	...
	if(als_data->rels_enable == 1){ 
		...
		cancel_delayed_work(&als_data->work_light);
...
//modify for ps-sensor config by yanfei 20150318 begin	
    ...
    i2c_smbus_write_byte_data(client, AP3426_REG_PS_MEAN, 0x03);//ps mean time = 3
    i2c_smbus_write_byte_data(client, AP3426_REG_PS_INTEGR, 0x02);
    i2c_smbus_write_byte_data(client, AP3426_REG_PS_PERSIS, 0x04);
...
//add by yanfei for mutex lock 20150112 begin
    ...
    mutex_unlock(&data->lock);
//modify for distinguish "C" light and "D" "A" light 20150204 begin
 	if(value_ir <15)
	{
	value = value * 19159/10000;
	}
	else
...
//delete by yanfei for mutex 20150206 begin
 ...
 //   printk(KERN_ERR"yanfei int_stat=%d \n",int_stat);
