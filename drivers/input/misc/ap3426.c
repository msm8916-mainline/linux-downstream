...
#define LSC_DBG 0
#if LSC_DBG
#define LDBG(s,args...)	{printk("zhaoxh: func [%s], line [%d], ",__func__,__LINE__); printk(s,## args);}
#else
#define LDBG(s,args...) {}
#endif
			...
			struct device_attribute *attr, char *buf)
{
    ...
    printk("Now in %s: g_ps_cal_2cm = 0x%x", __func__, g_ps_cal_2cm);
...
{
	struct ap3426_data *data = data_info;
	int ret;
	//unsigned long val;
    	g_ps_cal_2cm = simple_strtol(buf, NULL, 0);
    
	printk("Now in %s: g_ps_cal_2cm = 0x%x", __func__, g_ps_cal_2cm);
    	data->ps_close_thd_set =  g_ps_cal_2cm ;
	ret = ap3426_set_phthres(data->client, data->ps_close_thd_set);
   	 if (ret < 0)
		return ret;
			...
			struct device_attribute *attr, char *buf)
{
    ...
    printk("Now in %s: g_ps_cal_4cm = 0x%x", __func__, g_ps_cal_4cm);
				...
				const char *buf, size_t count)
{
   ...
   // unsigned long val;
	...
	printk("Now in %s: g_ps_cal_4cm = 0x%x", __func__, g_ps_cal_4cm);
	data->ps_away_thd_set =  g_ps_cal_4cm;
    ret = ap3426_set_plthres(data->client, data->ps_away_thd_set);
    if (ret < 0)
	return ret;
...
{
    	int32_t ret;
    printk("///yhj add  %s, misc_ls_opened = %d, enable=%d\n", __func__, misc_ls_opened, enable);
	...
	struct device_attribute *attr, char *buf)
{
   ...
   lsb= i2c_smbus_read_byte_data(data->client,AP3426_REG_PS_THDL_L );
   msb = i2c_smbus_read_byte_data(data->client,AP3426_REG_PS_THDL_H );
   printk("//////yhj add in ap3426 %d\n", (msb <<8|lsb));
   return sprintf(buf, "%d\n", (msb <<8|lsb));
}
...
static DEVICE_ATTR(plthres, 0664,
	...
	struct device_attribute *attr, char *buf)
{
  ...
  lsb= i2c_smbus_read_byte_data(data->client,AP3426_REG_PS_THDH_L );
  msb = i2c_smbus_read_byte_data(data->client,AP3426_REG_PS_THDH_H );
  return sprintf(buf, "%d\n", (msb << 8|lsb));
}
...
static DEVICE_ATTR(phthres, 0664,
...
static int ap3426_init_client(struct i2c_client *client)
{
   // struct ap3426_data *data = i2c_get_clientdata(client);
	...
	//hight
    i2c_smbus_write_byte_data(client, 0x2C, 0xC0);
...
#endif	
    /* set defaults */
    ap3426_set_range(client, AP3426_ALS_RANGE_2);
	...
	const struct i2c_device_id *id)
{
    ...
    struct ap3426_data *data;
    int err = 0;
    //char *l_info=l_sensor_info;
   // char *p_info=p_sensor_info;
	...
	//yhj add 0909
	data_info = data;
		data->ap3426_class = class_create(THIS_MODULE, "capella_sensors");
	...
	if (unlikely(IS_ERR(data->ps_dev ))) {
	  ...
	  err = device_create_file(data->ps_dev, &dev_attr_ps_cal_2cm);
   // if (err)
       // goto err_create_ps_device_file;
            
  	  err = device_create_file(data->ps_dev, &dev_attr_ps_cal_4cm);
    //if (err)
        //goto err_create_ps_device_file;
   ...
   if (err)
    ...
    //  l_info += sprintf(l_info,"ap3426_light");    
//	    p_info += sprintf(p_info,"ap3426_proximity");  
          //end
