...
/* IO Used for NIRQ */
#define GPIO_SX9500_NIRQ    956//114
...
#define FILTER_NUM          2
#define PATH_CAPSENSOR_CAL  "/persist/sensors/capsensor_cal.dat"
#define PATH_CAPSENSOR_CAL2 "/persist/sensors/capsensor_cal2.dat"
#define PATH_CAL_BACKUP     "/persist/sensors/capsensor_cal_backup.dat"
#define SCANPERIOD_CAL      0x0
#define PATH_XO_THERM       "/sys/class/hwmon/hwmon0/device/xo_therm"
...
/*start pin ctrl*/
		...
		pplatData->ts_pinctrl = devm_pinctrl_get(&(client->dev));
		if (IS_ERR(pplatData->ts_pinctrl)) {
			if (PTR_ERR(pplatData->ts_pinctrl) == -EPROBE_DEFER) {
				printk("ts_pinctrl ==  -EPROBE_DEFER\n");
				return -EPROBE_DEFER;
			}
			printk("Target does not use pinctrl(pplatData->ts_pinctrl == NULL) \n");
			pplatData->ts_pinctrl = NULL;
		}

		if (pplatData->ts_pinctrl) {
			pplatData->ts_pinset_state_active = pinctrl_lookup_state(pplatData->ts_pinctrl, "default");
			if (IS_ERR(pplatData->ts_pinset_state_active))
				printk("cannot get ts pinctrl active state\n");

			pplatData->ts_pinset_state_suspend = pinctrl_lookup_state(pplatData->ts_pinctrl, "sleep");
			if (IS_ERR(pplatData->ts_pinset_state_suspend))
				printk("cannot get ts pinctrl active state\n");
/*

			if (pplatData->ts_pinset_state_active) {
				err= pinctrl_select_state(pplatData->ts_pinctrl, pplatData->ts_pinset_state_active);
				if (err)
    ...
    if (this) {
        ...
        * (e.q. if suspend reset device) */
		...
		/*
		if (pplatData->ts_pinset_state_active) {
			err= pinctrl_select_state(pplatData->ts_pinctrl, pplatData->ts_pinset_state_suspend);
			if (err)
				printk("cannot set ts pinctrl active state \n");
		} else {
			printk("pinctrl active == NULL \n");
		...
		*/
		printk(KERN_INFO "sx9500 Probe success..");
		return  0;
	}
