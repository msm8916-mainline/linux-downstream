...
#ifdef DEBUG
	printk(KERN_WARNING "k303c_acc1 = %2x,%2x,%2x,%2x,%2x,%2x\n",
...
ST_EXIT:
	...
	if (en_state) lsm303c_acc_enable(stat);
...
static int lsm303c_parse_dts(struct lsm303c_acc_status *stat)
{
	...
	stat->vddio_reg = regulator_get(&stat->client->dev, "st,vddio_i2c");
	if (IS_ERR(stat->vddio_reg)) {
		rc = PTR_ERR(stat->vddio_reg);
		dev_err(&stat->client->dev,
			"Regulator get failed vcc_ana rc=%d\n", rc);
		return rc;
	}

	if (regulator_count_voltages(stat->vddio_reg) > 0) {
		rc = regulator_set_voltage(stat->vddio_reg,
					   1800000,
					   1800000);
	}
	...
	regulator_enable(stat->vddio_reg);
	regulator_enable(stat->vdd_reg);
		...
		const struct i2c_device_id *id)
{
	...
	if (client->dev.platform_data == NULL) {
		...
		memcpy(stat->pdata, &default_lsm303c_acc_pdata,
							sizeof(*stat->pdata));
	...
	if (stat->pdata->gpio_int1 >= 0) {
		...
		pr_info("%s: %s has set irq1 to irq: %d, mapped on gpio:%d\n",
			LSM303C_ACC_DEV_NAME, __func__, stat->irq1,
