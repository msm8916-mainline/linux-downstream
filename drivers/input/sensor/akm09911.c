	...
	struct input_dev **input)
{
	...
	*input = input_allocate_device();
	...
	/* Magnetic field uncalibrated  (limited to 16bit) */
...
/* Compile Error */
#if 1
	/* Bias of  Magnetic field uncalibrated  (limited to 32bit) */
/*
...
static int akm_compass_power_set(struct akm_compass_data *data, bool on)
{
	...
	if (!on) {
		...
		data->vdd = regulator_get(&data->i2c->dev, "sensor_vdd");
		if (IS_ERR(data->vdd)) {
			rc = PTR_ERR(data->vdd);
			dev_err(&data->i2c->dev,
				"Regulator get failed sensor_vdd rc=%d\n", rc);
			return rc;
		}
		...
		if (regulator_count_voltages(data->vdd) > 0) {
			...
			if (rc) {
				dev_err(&data->i2c->dev,
					"Regulator set failed sensor_vdd rc=%d\n",
					rc);
				goto reg_vdd_put;
			}
		}
		...
		if (rc) {
			dev_err(&data->i2c->dev,
				"Regulator enable sensor_vdd failed rc=%d\n", rc);
			goto reg_vdd_put;
		}
		data->vio = regulator_get(&data->i2c->dev, "sensor_vcc_i2c");
		if (IS_ERR(data->vio)) {
			rc = PTR_ERR(data->vio);
			dev_err(&data->i2c->dev,
				"Regulator get failed sensor_vcc rc=%d\n", rc);
			goto reg_vdd_set;
		}
		...
		if (regulator_count_voltages(data->vio) > 0) {
			...
			if (rc) {
				dev_err(&data->i2c->dev,
				"Regulator set failed sensor_vcc rc=%d\n", rc);
				goto reg_vio_put;
			}
		}
		...
		if (rc) {
				dev_err(&data->i2c->dev,
				"Regulator enable sensor_vcc failed rc=%d\n", rc);
				goto reg_vio_put;
		}
	}
...
static int akm_pinctrl_init(struct akm_compass_data *s_akm)
{
	...
	s_akm->pin_default = pinctrl_lookup_state(s_akm->pinctrl, "akm09911_active");
	if (IS_ERR_OR_NULL(s_akm->pin_default)) {
		dev_err(&client->dev, "Failed to look up default state\n");
		return PTR_ERR(s_akm->pin_default);
	}

	s_akm->pin_sleep = pinctrl_lookup_state(s_akm->pinctrl, "akm09911_suspend");
...
#ifndef USE_DEFAULT_PINCTRL_NAME /* Using pinctrl-names value is "default" in DTSi file */
	/* initialize pinctrl */
