 ...
 */
int FirmwareUpgrade(struct synaptics_ts_data *ts, struct firmware *fw_img)
{
	...
	if(fw_img == NULL) {
		TOUCH_ERR_MSG("fw_img load fail .\n");
		goto exit_upgrade;
	}
	else {
		fw->image_size = (unsigned long) fw_img->size;
		fw->image_bin = (unsigned char *) fw_img->data;
		TOUCH_INFO_MSG("fw_img load success, image_size: %lu .\n", fw->image_size);
	}
