...
int mip_run_test(struct mit_data *ts, u8 test_type, int *dsize, int *rot, int *si)
{
	int busy_cnt = 50;
	int wait_cnt = 50;
	u8 wbuf[8];
...
//	mutex_unlock(&info->lock);
    ...
    case MIP_TEST_TYPE_CM_ABS:
		...
		case MIP_TEST_TYPE_SHORT:
			...
			//sprintf(info->print_buf, "\n=== Short Test ===\n\n");
			break;
		case MIP_TEST_TYPE_MUX_SHORT:
			printk("=== Mux Short Test ===\n");
			//sprintf(info->print_buf, "\n=== Short Test ===\n\n");
			break;
		case MIP_TEST_TYPE_LPWG_RAWDATA_DIFF:
			printk("=== LPWG RawData Diff Test ===\n");
			//sprintf(info->print_buf, "\n=== Short Test ===\n\n");
			break;
		default:
	...
	if(wait_cnt <= 0){
		dev_err(&ts->client->dev, "%s [ERROR] Wait timeout   0   \n", __func__);
		goto ERROR;
	}
	...
	dev_dbg(&ts->client->dev, "%s - set test type\n", __func__);
	 ...
	 if(test_type == MIP_TEST_TYPE_CM_JITTER){
  	 mdelay(1000);
  }
  //wait ready status
  wait_cnt = 40;
	...
	if(wait_cnt <= 0){
		dev_err(&ts->client->dev, "%s [ERROR] Wait timeout            1\n", __func__);
		goto ERROR;
	}
...
static int  print_openshort_data(struct mit_data *ts, char *buf, int type)
{
	...
	int row = 0;
	int ret = 0;
	if (type == MIP_TEST_TYPE_CM_DELTA){
		ts->d_min = ts->mit_data[0][0];
		ts->d_max = ts->mit_data[0][0];
	} else if (type == MIP_TEST_TYPE_CM_JITTER){
		ts->j_min = ts->mit_data[0][0];
		ts->j_max = ts->mit_data[0][0];
	} else if (type == MIP_TEST_TYPE_SHORT){
		ts->o_min = ts->mit_data[0][0];
		ts->o_max = ts->mit_data[0][0];
	} else if (type == MIP_TEST_TYPE_MUX_SHORT){
		ts->m_min = ts->mit_data[0][0];
		ts->m_max = ts->mit_data[0][0];
	}

	for (row = 0 ; row < ts->dev.row_num ; row++) {
		if (type == MIP_TEST_TYPE_CM_DELTA || type == MIP_TEST_TYPE_CM_JITTER || type == MIP_TEST_TYPE_SHORT || type == MIP_TEST_TYPE_MUX_SHORT) {
			printk("[Touch] [%2d]  ", row);
			ret += sprintf(buf + ret,"[%2d]  ", row);
		}
		...
		for (col = 0 ; col < ts->dev.col_num ; col++) {
			...
			if (type == MIP_TEST_TYPE_CM_DELTA){
				...
				ts->m_min = (ts->m_min > ts->mit_data[row][col]) ? ts->mit_data[row][col] : ts->m_min;
				ts->m_max = (ts->m_max < ts->mit_data[row][col]) ? ts->mit_data[row][col] : ts->m_max;
			}
	...
	if (type == MIP_TEST_TYPE_CM_DELTA || type == MIP_TEST_TYPE_CM_JITTER || type == MIP_TEST_TYPE_SHORT || type == MIP_TEST_TYPE_MUX_SHORT) {
		...
		if(type == MIP_TEST_TYPE_CM_DELTA){
			...
			TOUCH_INFO_MSG("MAX : %d,  MIN : %d  (MAX - MIN = %d)\n\n",ts->d_max , ts->d_min, ts->d_max - ts->d_min);
			TOUCH_INFO_MSG("OPEN / CM_DELTA TEST SPEC(MIN : %d, MAX : %d)\n", ts->pdata->limit->cm_delta_min, ts->pdata->limit->cm_delta_max);
			ret += sprintf(buf + ret,"OPEN / CM_DELTA TEST SPEC(MIN : %d, MAX : %d)\n", ts->pdata->limit->cm_delta_min, ts->pdata->limit->cm_delta_max);
		} else if(type == MIP_TEST_TYPE_CM_JITTER){
			ret += sprintf(buf + ret,"MAX = %d,  MIN = %d  (MAX - MIN = %d)\n\n",ts->j_max , ts->j_min, ts->j_max - ts->j_min);
			TOUCH_INFO_MSG("MAX : %d,  MIN : %d  (MAX - MIN = %d)\n\n",ts->j_max , ts->j_min, ts->j_max - ts->j_min);
			TOUCH_INFO_MSG("OPEN / JITTER TEST SPEC(MIN : %d, MAX : %d)\n", ts->pdata->limit->jitter_min, ts->pdata->limit->jitter_max);
			ret += sprintf(buf + ret,"OPEN / JITTER TEST SPEC(MIN : %d, MAX : %d)\n", ts->pdata->limit->jitter_min, ts->pdata->limit->jitter_max);
		} else if(type == MIP_TEST_TYPE_SHORT){
			...
			ret += sprintf(buf + ret,"MAX = %d,  MIN = %d  (MAX - MIN = %d)\n\n",ts->m_max , ts->m_min, ts->m_max - ts->m_min);
			TOUCH_INFO_MSG("MAX : %d,  MIN : %d  (MAX - MIN = %d)\n\n",ts->m_max , ts->m_min, ts->m_max - ts->m_min);
			TOUCH_INFO_MSG("MUX / SHORT TEST SPEC(MIN : %d, MAX : %d)\n", ts->pdata->limit->mux_short_min, ts->pdata->limit->mux_short_max);
		...
		if (type == MIP_TEST_TYPE_CM_DELTA) {
			for (row = 0 ; row < ts->dev.row_num ; row++) {
				...
				ret += sprintf(buf + ret,"[%2d]  ",row);
				for (col = 0 ; col < ts->dev.col_num ; col++) {
					if (ts->mit_data[row][col] >= ts->pdata->limit->cm_delta_min && ts->mit_data[row][col] <= ts->pdata->limit->cm_delta_max) {
						printk(" ,");
						ret += sprintf(buf + ret," ,");
					}else{
						printk("X,");
			...
			for (row = 0 ; row < ts->dev.row_num ; row++) {
				...
				ret += sprintf(buf + ret,"[%2d]  ",row);
				for (col = 0 ; col < ts->dev.col_num ; col++) {
					if (ts->mit_data[row][col] >= ts->pdata->limit->jitter_min && ts->mit_data[row][col] <= ts->pdata->limit->jitter_max) {
						printk(" ,");
						ret += sprintf(buf + ret," ,");
					}else{
						printk("X,");
						ret += sprintf(buf + ret,"X,");
						ts->pdata->selfdiagnostic_state[SD_JITTER] = 0;
					}
				}
				printk("\n");
				ret += sprintf(buf + ret,"\n");
			}
			if(ts->pdata->selfdiagnostic_state[SD_JITTER] ==  1) {
				TOUCH_INFO_MSG("Jitter Test : Pass\n\n");
				ret += sprintf(buf + ret,"Jitter Test : PASS\n\n");
			} else {
				TOUCH_INFO_MSG("Jitter Test : Fail\n\n");
				ret += sprintf(buf + ret,"Jitter Test : FAIL\n\n");
			}
		} else if (type == MIP_TEST_TYPE_SHORT) {
			for (row = 0 ; row < ts->dev.row_num ; row++) {
				printk("[Touch] [%2d]  ",row);
			...
			for (row = 0 ; row < ts->dev.row_num ; row++) {
				...
				ret += sprintf(buf + ret,"[%2d]  ",row);
				for (col = 0 ; col < ts->dev.col_num ; col++) {
					if (ts->mit_data[row][col] >= ts->pdata->limit->mux_short_min && ts->mit_data[row][col] <= ts->pdata->limit->mux_short_max) {
						printk(" ,");
						ret += sprintf(buf + ret," ,");
					}else{
						printk("X,");
						ret += sprintf(buf + ret,"X,");
						ts->pdata->selfdiagnostic_state[SD_MUXSHORT] = 0;
					}
				}
				printk("\n");
				ret += sprintf(buf + ret,"\n");
			}
			if(ts->pdata->selfdiagnostic_state[SD_MUXSHORT] ==  1) {
				TOUCH_INFO_MSG("MuxShort Test : Pass\n\n");
				ret += sprintf(buf + ret,"MuxShort Test : PASS\n\n");
			} else {
				TOUCH_INFO_MSG("MuxShort Test : Fail\n\n");
				ret += sprintf(buf + ret,"MuxShort Test : FAIL\n\n");
			}
		} else {
			//TOUCH_INFO_MSG("OpenShort Test : Pass\n\n");
			//ret += sprintf(buf + ret,"OpenShort Test : PASS\n\n");
...
ssize_t mit_get_test_result(struct i2c_client *client, char *buf, int type)
{
	...
	if(OPENSHORT == type || OPENSHORT_STORE == type ) {
	   ...
	   while(retry_count++ < retry_max){
		   if(mip_run_test(ts, MIP_TEST_TYPE_CM_DELTA,&dsize, &rot, &si)){
			   ...
			   mdelay(100);
				...
				if (ts->pdata->check_openshort == 1)
					ret = print_openshort_data(ts, buf, MIP_TEST_TYPE_CM_DELTA);
				if (ret < 0) {
	   ...
	   retry_count = 0;
	   while(retry_count++ < retry_max){
		   if(mip_run_test(ts, MIP_TEST_TYPE_CM_JITTER,&dsize, &rot, &si)){
			   ...
			   mdelay(100);
				...
				if (ts->pdata->check_openshort == 1)
					ret = print_openshort_data(ts, buf, MIP_TEST_TYPE_CM_JITTER);
				if (ret < 0) {
					TOUCH_ERR_MSG("fail to print open short data\n");
					ts->pdata->selfdiagnostic_state[SD_JITTER] = 0;
					goto error;
	   ...
	   retry_count = 0;
	   while(retry_count++ < retry_max){
		   if(mip_run_test(ts, MIP_TEST_TYPE_SHORT,&dsize, &rot, &si )){
			   ...
			   mdelay(100);
				...
				if (ts->pdata->check_openshort == 1)
					ret = print_openshort_data(ts, buf, MIP_TEST_TYPE_SHORT);
				if (ret < 0) {
					TOUCH_ERR_MSG("fail to print open short data\n");
					ts->pdata->selfdiagnostic_state[SD_OPENSHORT] = 0;
					goto error;
	   ...
	   retry_count = 0;
	   while(retry_count++ < retry_max){
		   if(mip_run_test(ts, MIP_TEST_TYPE_MUX_SHORT,&dsize, &rot, &si )){
			   dev_err(&ts->client->dev, "%s [ERROR] mip_run_test MIP_TEST_TYPE_MUX_SHORT\n", __func__);
			   ret = snprintf(buf, PAGE_SIZE, "%s\n", "ERROR");
			   TOUCH_INFO_MSG("%s retry (%d/%d) \n", __func__, retry_count, retry_max);
			   mit_power_reset(ts);
			   mdelay(100);
				...
				if (ts->pdata->check_openshort == 1)
					ret = print_openshort_data(ts, buf, MIP_TEST_TYPE_MUX_SHORT);
				if (ret < 0) {
					TOUCH_ERR_MSG("fail to print open short data\n");
					ts->pdata->selfdiagnostic_state[SD_MUXSHORT] = 0;
					goto error;
...
*/
int lpwg_debug_enable(struct i2c_client* client, int enable)
{

	u8 wbuf[4];
	struct mit_data *ts = (struct mit_data *) get_touch_handle_(client);
	TOUCH_INFO_MSG("%s [START]\n", __func__);
	ts->pdata->lpwg_debug_enable = enable;
...
int lpwg_start(struct i2c_client *client)
{
	u8 wbuf[4];
...
int mit_lpwg_test(struct i2c_client *client)
{
	struct mit_data *ts = (struct mit_data *) get_touch_handle_(client);
	int retry_max = 3;
	int retry_count = 0;
	...
	int i_y = 0;
	int i_x = 0;
	int test_success = 1; // 1 success, 0 : fail
  ...
  lpwg_start(client);
	...
	while(retry_count++ < retry_max)
	{
	  if(mip_run_test(ts, MIP_TEST_TYPE_CM_JITTER, &dsize, &rot, &si)){
		   dev_err(&ts->client->dev, "%s [ERROR] mip_run_test MIP_TEST_TYPE_CM_JITTER\n", __func__);
		   ret = snprintf(temp_buf, PAGE_SIZE, "%s\n", "ERROR");
		   TOUCH_INFO_MSG("%s retry (%d/%d) \n", __func__, retry_count, retry_max);
  ...
  /////////////////////////////////////////////////////////////////////////////////
  //print cm jitter table
  printk(KERN_INFO "%s  cm jitter table \n",__func__);
  printk(KERN_INFO "dsize : %d rot : %d, si : %d\n", dsize, rot, si);
  lim_x = ts->dev.col_num;
  lim_y = ts->dev.row_num;
  ...
  printk("[%2d]", i_y);
       ...
       for(i_x = 0; i_x < lim_x; i_x++){
          printk(" %4d", ts->mit_data[i_y][i_x]);
          if( ts->mit_data[i_y][i_x] > ts->pdata->limit->lpwg_jitter_max || ts->mit_data[i_y][i_x] < ts->pdata->limit->lpwg_jitter_min )
	  {
	        printk(KERN_INFO "CM jitter NG\n");
		test_success = 0;
	  }
	}
	printk("\n");
  ...
  /////////////////////////////////////////////////////////////////////////////////
	...
	while(retry_count++ < retry_max)
	{
	    if(mip_run_test(ts, MIP_TEST_TYPE_CM_ABS, &dsize, &rot, &si )){
		    dev_err(&ts->client->dev, "%s [ERROR] mip_run_test MIP_TEST_TYPE_CM_ABS\n", __func__);
		    ret = snprintf(temp_buf, PAGE_SIZE, "%s\n", "ERROR");
		    TOUCH_INFO_MSG("%s retry (%d/%d) \n", __func__, retry_count, retry_max);
  ...
  /////////////////////////////////////////////////////////////////////////////////
  //print cm abs table
  printk(KERN_INFO "%s  cm abs table \n",__func__);
  printk(KERN_INFO "dsize : %d rot : %d, si : %d\n", dsize, rot, si);
  ...
  for(i_y = 0; i_y < lim_y; i_y++){
      ...
      for(i_x = 0; i_x < lim_x; i_x++){
          printk(" %4d", ts->mit_data[i_y][i_x]);
          if( ts->mit_data[i_y][i_x] > ts->pdata->limit->lpwg_raw_data_max || ts->mit_data[i_y][i_x] < ts->pdata->limit->lpwg_raw_data_min )
          {
		printk(KERN_INFO "CM ABS NG\n");
		test_success = 0;
          }
       }
       printk("\n");
  ...
  /////////////////////////////////////////////////////////////////////////////////
  /////////LPWG_RawData Diff_START
	mdelay(20);
	retry_count = 0;
	while(retry_count++ < retry_max)
	{
	    if(mip_run_test(ts, MIP_TEST_TYPE_LPWG_RAWDATA_DIFF, &dsize, &rot, &si )){
		    dev_err(&ts->client->dev, "%s [ERROR] mip_run_test MIP_TEST_TYPE_LPWG_RAWDATA_DIFF\n", __func__);
		    ret = snprintf(temp_buf, PAGE_SIZE, "%s\n", "ERROR");
		    TOUCH_INFO_MSG("%s retry (%d/%d) \n", __func__, retry_count, retry_max);
  ...
  /////////////////////////////////////////////////////////////////////////////////
  //print MIP_TEST_TYPE_LPWG_RAWDATA_DIFF
  printk(KERN_INFO "%s  lpwg rawdata diff table \n",__func__);
  printk(KERN_INFO "dsize : %d rot : %d, si : %d\n", dsize, rot, si);
  ...
  for(i_y = 0; i_y < lim_y; i_y++){
      ...
      for(i_x = 0; i_x < lim_x; i_x++){
          printk(" %4d", ts->mit_data[i_y][i_x]);
          if( ts->mit_data[i_y][i_x] > ts->pdata->limit->lpwg_rawdata_diff_max|| ts->mit_data[i_y][i_x] < ts->pdata->limit->lpwg_rawdata_diff_min)
          {
		printk(KERN_INFO "LPWG_RAWDATA_DIFF NG\n");
		test_success = 0;
          }
       }
       printk("\n");
  }
/////////LPWG_RawData Diff_END
  ////////////////////////////////////////////////////////////////////////////////

  lpwg_debug_enable(client, 0);
  msleep(10);
  lpwg_start(client);
