 ...
 *
 *
 * mip4_fw_mit200.c : Firmware update functions for MIT200
 *
 *
 * Version : 2015.07.20
 *
...
#define ISC_CMD_READ_STATUS		{0xFB, 0x4A, 0x00, 0xC8, 0x00, 0x00}
...
* Command : Read Page
*/
static int mip_isc_read_page(struct mip_ts_info *info, int offset, u8 *data, int length)
{
...
{
	u8 data[ISC_PAGE_SIZE];
	int ret = 0;

	dev_dbg(&info->client->dev, "%s [START]\n", __func__);

	//Erase = Program 0xFF
	memset(data, 0xFF, ISC_PAGE_SIZE);

	ret = mip_isc_program_page(info, offset, data, ISC_PAGE_SIZE);
	if(ret != 0){
		dev_err(&info->client->dev,"%s [ERROR] mip_isc_program_page\n", __func__);
		goto ERROR;
	}
...
int mip_flash_fw(struct mip_ts_info *info, const u8 *fw_data, size_t fw_size, bool force, bool section)
{
	...
	if(force == true){
		...
		if(retry < 0){
			...
			dev_info(&client->dev, "%s - Full download\n", __func__);
			full_download = true;
		}
		else{
			...
			else if(ver_chip[0] == bin_info->ver_boot){
				if(section == true){
					dev_info(&client->dev, "%s - Section download\n", __func__);
					full_download = false;
				}
				else{
					dev_info(&client->dev, "%s - Full download\n", __func__);
	...
	if(full_download == true){
		...
		offset_start = bin_info->core_start * ISC_PAGE_SIZE;
	}
	...
	while(offset >= offset_start){
		...
		dev_dbg(&client->dev, "%s - mip_isc_read_page : offset[0x%04X]\n", __func__, offset);

#if MIP_FW_UPDATE_DEBUG
		print_hex_dump(KERN_ERR, MIP_DEVICE_NAME " F/W File : ", DUMP_PREFIX_OFFSET, 16, 1, &bin_data[offset], ISC_PAGE_SIZE, false);
		print_hex_dump(KERN_ERR, MIP_DEVICE_NAME " F/W Chip : ", DUMP_PREFIX_OFFSET, 16, 1, rbuf, ISC_PAGE_SIZE, false);
#endif
