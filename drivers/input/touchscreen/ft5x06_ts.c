...
#define FTP_GESTRUE_POINTS 255
#define FTP_GESTRUE_POINTS_HEADER 8
unsigned short coordinate_x[150] = {0};
unsigned short coordinate_y[150] = {0};
#define KEY_DOUBLE_TAP 589
#endif
...
static int charge_status = CHARGE_ORG_STATUS;
static int charge_flag;
extern int usb_charger_unregister(struct notifier_block *nb);
extern int usb_charger_register(struct notifier_block *nb);
static int charge_noise_handle(struct ft5x06_ts_data *data);
...
void fts_get_upgrade_array(struct i2c_client *client)
{
    ...
    if (err < 0) {
		printk("lxh:** get  chip_id fail **\n");
    } else {
		printk("lxh:** %s chip_id = 0x%x **\n", __func__, chip_id);
	}
...
int fts_i2c_Read(struct i2c_client *client, char *writebuf,int writelen, char *readbuf, int readlen)
{
  ...
  return ft5x06_i2c_read(client,  writebuf, writelen,  readbuf, readlen);
}
...
{
	return ft5x0x_write_reg(client, regaddr, regvalue);
}
...
int fts_read_reg(struct i2c_client *client, u8 regaddr, u8 *regvalue)
{
  ...
  return ft5x0x_read_reg(client, regaddr, regvalue);
}
...
#ifdef FTP_GESTRUE
static void check_gesture(int gesture_id, struct ft5x06_ts_data *data)
{
  //  printk("lxh*** gesture_id==0x%x\n ",gesture_id);
    switch(gesture_id)
    {
        case GESTURE_DOUBLECLICK:
    //        printk("lxh:***ENTER THE CASE DOUBLE CLICK");
            input_report_key(data->input_dev, KEY_DOUBLE_TAP, 1);       //KEY_POWER
            input_sync(data->input_dev);
            input_report_key(data->input_dev, KEY_DOUBLE_TAP, 0);
            input_sync(data->input_dev);
...
static int ft5x0x_read_Touchdata(struct ft5x06_ts_data *data)
{
    unsigned char buf[FTP_GESTRUE_POINTS * 4] = { 0 };
    int ret = -1;
    int gestrue_id = 0;
    struct ft5x06_ts_data * ts_data = data;

    buf[0] = 0xd3;
    ret = ft5x06_i2c_read(ts_data->client, buf, 1, buf, FTP_GESTRUE_POINTS_HEADER);
    ...
    if (GESTURE_DOUBLECLICK == buf[0])
    {
        ...
        check_gesture(gestrue_id, ts_data);
        return -1;
    }
    return -1;
}
...
#ifdef FTP_GESTRUE
 	i2c_smbus_read_i2c_block_data(data->client, 0xd0, 1, &state);

	if(state ==1) {

	//	 printk(KERN_ERR"lxh:****   gesture state %d  0602*****\n", state); // test 
...
static int ft5x06_reset_configure(struct ft5x06_ts_data *data, int value)
{
     	int err = 0;
		...
		if (gpio_is_valid(data->pdata->reset_gpio)) {
			...
			err = gpio_direction_output(data->pdata->reset_gpio, value);
			if (err) {
				dev_err(&data->client->dev,
...
static int ft5x06_irq_configure(struct ft5x06_ts_data *data, bool on)
{
	int err = 0;
	  ...
	  if(data->doubletap)
        {
			...
			input_mt_report_pointer_emulation(data->input_dev, false);
			input_sync(data->input_dev);
         //   printk(KERN_ERR"lxh:**going to enable double click **\n"); 

			ft5x0x_write_reg(data->client, 0xd0, 0x01);
			ft5x0x_write_reg(data->client, 0xd1, 0x1f);
			ft5x0x_write_reg(data->client, 0xd2, 0x1f);
			err = irq_set_irq_wake(data->client->irq, 1);
			if(err){
				dev_err(dev, "enable set_irq_wake failed");
				return err;
			} 
	...
	if (!data->suspended) {
		...
		return 0;
			...
			if(data->doubletap)
			{
				...
				ft5x0x_write_reg(data->client,0xD1,0x00);
				ft5x0x_write_reg(data->client,0xD2,0x00);
				err = irq_set_irq_wake(data->client->irq, 0);
				if(err){
					dev_err(dev, "disable set_irq_wake failed");
					return err;
				}
			}
#endif
...
static int usb_notifier_callback(struct notifier_block *nb,
					 unsigned long type, void *chg_event)
{
    
	struct ft5x06_ts_data *ft5x06_data =
		container_of(nb, struct ft5x06_ts_data, usb_notif);
    int ret = 0;
    int i = 0;

//	printk("%s:lxh:****  type is %ld **** \n", __func__, type);
	
    if (type == 0) {
         charge_flag = CHARGE_PULL_OUT;
        do {
          ret = ft5x0x_write_reg(ft5x06_data->client, FT_REG_CHARGE_STATUS,0);
		  i++;
        } while ((ret < 0) && (i < 5));
        if (ret < 0) {
            charge_status = CHARGE_PULL_OUT;
        	//printk("%s:lxh:****ft5306_touch_write 0x8b 0 fail **** \n",  __func__);
        } else {
			charge_status = CHARGE_ORG_STATUS;
        	//printk("%s:lxh: ****ft5306_touch_write 0x8b 0 success ****\n",  __func__);
        }
      } else {
         charge_flag = CHARGE_PLUG_IN;
        do {
      	 ret = ft5x0x_write_reg(ft5x06_data->client, FT_REG_CHARGE_STATUS,1);
		 i++;
        } while ((ret < 0) && (i < 5));
        if (ret < 0) {
             charge_status = CHARGE_PLUG_IN;
           //  printk("%s::****ft5306_touch_write  0x8b 1 fail :****\n",  __func__);
        } else {
	     	charge_status = CHARGE_ORG_STATUS;
	     //	printk("%s::****ft5306_touch_write 0x8b 1 success :****\n",  __func__);
        } 
      }
    return 0;  
}
...
static int charge_noise_handle(struct ft5x06_ts_data *data)
{
    int ret = 0;
    int i = 0;
    struct ft5x06_ts_data *ft5x06_data = data;
#if 0
	if (charge_status != CHARGE_ORG_STATUS) {
        if (charge_status == CHARGE_PLUG_IN)  {
            do {
                ret = ft5x0x_write_reg(ft5x06_data->client, FT_REG_CHARGE_STATUS,1);
                i++;
            } while ((ret < 0) && (i < 5));
          
       } else if (charge_status == CHARGE_PULL_OUT) {
            do {
                ret = ft5x0x_write_reg(ft5x06_data->client, FT_REG_CHARGE_STATUS,0);
                i++;
            } while ((ret < 0) && (i < 5));
...
#endif
  // 0x8b will reset to 0,whenever reset happens.
   if (charge_flag == CHARGE_PLUG_IN) {
    do {
       ret = ft5x0x_write_reg(ft5x06_data->client, FT_REG_CHARGE_STATUS,1);
       i++;
     } while ((ret < 0) && (i < 5));
...
//	return snprintf(buf, FT_FW_NAME_MAX_LEN - 1, "%s\n", data->fw_name);
  ...
  struct ft5x06_ts_data *data = dev_get_drvdata(dev);
	u8 reg_addr = FT_REG_FW_VER, reg_value;
	int err, ret = 0;
	...
	if (err < 0) {
  		ret = snprintf(buf, FT_FW_NAME_MAX_LEN - 1, "%s\n", "fail");
	} else {
        ret = snprintf(buf, FT_FW_NAME_MAX_LEN - 1, "0x%x\n", reg_value);	 	
	} 
...
// lxh:add; add node to provide panel vendor infor; 20150514
static ssize_t ft5x06_vendor_name_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct ft5x06_ts_data *data = dev_get_drvdata(dev);
 	u8 reg_addr = FT_REG_FW_VENDOR_ID, reg_value;
  ...
  //      printk("lxh:**reg_value is 0x%x **\n", reg_value); 
	    ...
	    if ((reg_value == VENDOR_BOE) || (reg_value == VENDOR_BOE_LATER)) {
          ret = snprintf(buf, FT_FW_NAME_MAX_LEN - 1, "%s\n", "BOE");
		} else if((reg_value == VENDOR_DSJM) || (reg_value == VENDOR_DSJM_LATER)){  
		  ret = snprintf(buf, FT_FW_NAME_MAX_LEN - 1, "%s\n", "DSJM");
        } else
          ret = snprintf(buf, FT_FW_NAME_MAX_LEN - 1, "%s\n", "Unknow");
	} 
...
static ssize_t ft5x06_vendor_name_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
  return 0;
...
static DEVICE_ATTR(vendor_name, 0664, ft5x06_vendor_name_show, ft5x06_vendor_name_store);

#ifdef FTP_GESTRUE
static ssize_t ft5x06_double_tap_switch_show(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct ft5x06_ts_data *data = dev_get_drvdata(dev);
	return snprintf(buf, 2, "%d\n", data->doubletap);
...
static ssize_t ft5x06_double_tap_switch_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct ft5x06_ts_data *data = dev_get_drvdata(dev);
	...
	int rc;
        ...
        if(data->suspended)
        {
            dev_info(&data->client->dev, "In suspend state, try again later...\n");
            return size;
        }
...
static DEVICE_ATTR(double_tap_switch, 0664, ft5x06_double_tap_switch_show, ft5x06_double_tap_switch_store);
#endif
			   ...
			   const struct i2c_device_id *id)
{
 ...
 // printk(KERN_ERR"lxh:**ft5x06_ts_probe get in_0429_5**\n");
	...
	printk(KERN_ERR"lxh:** %s;%d **\n",__func__, __LINE__);
    // set reset pin low before power on according to spec..
     
	    ft5x06_reset_configure(data);
   ...
   // err = pdata->power_init(true);
	...
	reg_addr = FT_REG_FW_VENDOR_ID;
		...
		printk(KERN_ERR"lxh:**vendor ID = 0x%x **\n", reg_value);
// -----
	if ((pdata->family_id != reg_value) && (!pdata->ignore_id_check)) {
	 ...
	 ft5x06_irq_configure(data, 1); // lxh:test
	...
	*/
	...
	err = device_create_file(&client->dev, &dev_attr_vendor_name);
	if (err) {
		dev_err(&client->dev, "sys file creation failed\n");
...
#ifdef FTP_GESTRUE
		err = device_create_file(&client->dev, &dev_attr_double_tap_switch);
		if (err) {
			dev_err(&client->dev, "sys file creation failed\n");
			goto free_vendor_name_sys;
		}		
#endif
	...
	register_early_suspend(&data->early_suspend);
  	...
  	data->usb_notif.notifier_call = usb_notifier_callback;
	err = usb_charger_register(&data->usb_notif);

	if (err)
		dev_err(&client->dev, "Unable to register usb_notifier: %d\n",
			err);
    ...
    fts_create_apk_debug_channel(client);
 		  ...
 		  fts_get_upgrade_array(client); 
			...
			ret_auto_upgrade = fts_ctpm_auto_upgrade(client);

			  i++;
				if(ret_auto_upgrade < 0) {
					 printk("lxh:** ctp upgrade fail err = %d **\n",ret_auto_upgrade);
				}
			}
...
//	printk(KERN_ERR"lxh:** FT touch done ok **\n");
...
#ifdef FTP_GESTRUE
free_double_tap_switch:
	device_remove_file(&client->dev, &dev_attr_double_tap_switch);
#endif
      
free_vendor_name_sys:
	device_remove_file(&client->dev, &dev_attr_vendor_name);
...
static int ft5x06_ts_remove(struct i2c_client *client)
{
    ...
    usb_charger_unregister(&data->usb_notif);
