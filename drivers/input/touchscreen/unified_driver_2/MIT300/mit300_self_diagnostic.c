...
#define CM_DELTA_MAX 240
#define CM_DELTA_MIN 100
...
static int  MIT300_PrintData(struct i2c_client *client, char *buf, int* result, int type)
{
	...
	if (*result == TOUCH_FAIL){
		ret += sprintf(buf + ret, "%d errors are detected in test with '!' character\n", error_count);
	}

	//TOUCH_LOG("MAX : %d,  MIN : %d  (MAX - MIN = %d), Spec rower : %d, upper : %d\n\n", max_data , min_data, max_data - min_data, limit_lower, limit_upper);
