...
#defined IRQS_PENDING 0x00000200
#endif
...
static void TouchMaskIrq(void)
{
	...
	if(desc == NULL) {
		TOUCH_ERR("fail to irq_to_desc, it's NULL\n");
		return;
	}
...
void TouchPower(struct i2c_client *client, int isOn )
{
	/* to avoid prequently change of this file */
	TouchPowerModel(client, isOn);
}
...
{
	#if defined ( TOUCH_PLATFORM_MSM8210 ) || defined ( TOUCH_PLATFORM_MSM8916 ) || defined ( TOUCH_PLATFORM_MSM8909 ) 
...
int TouchGetBootMode(void)
{
    ...
    /*                                      */
    /*                                    */      
	#elif defined ( TOUCH_PLATFORM_MSM8210) || defined ( TOUCH_PLATFORM_MSM8909 )
	{
		enum lge_boot_mode_type lge_boot_mode;
    ...
    #elif defined ( TOUCH_PLATFORM_MSM8909 )
    /* TBD - NSM  */
