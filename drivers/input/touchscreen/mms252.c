...
static int mms_flash_fw(struct mms_ts_info *info, bool force_update, int cmd)
{
	...
	if (cmd == UMS) {
		...
		if(info->panel == EELY) {
			dev_err(&client->dev, "requesting tsp_melfas/mms252.fw\n");
			ret = request_firmware(&fw,
					"tsp_melfas/mms252.fw",
					&client->dev);
		} else if (info->panel == ILJIN) {
			ret = request_firmware(&fw,
					"tsp_melfas/melfas_ij.fw",
					&client->dev);
			dev_err(&client->dev, "requesting tsp_melfas/melfas_ij.fw\n");
		}
		else {
			ret = -1;
...
static int is_melfas_vdd_on(struct mms_ts_info *info)
{
	...
	ret2 = gpio_get_value(info->pdata->vdd_en2);
	pr_info("[TSP] %s = %d %d\n", __func__, ret,ret2);
...
static void melfas_request_gpio(struct melfas_tsi_platform_data *pdata)
{
	...
	if(pdata->tkey_led_en >= 0){
        ...
        pr_info("%s: tsp_irq is %d,en_gpio:%d, en2_gpio :%d tsp_vendor1 :%d tsp_vendor2 :%d\n", __func__,pdata->gpio_int,pdata->vdd_en, pdata->vdd_en2, pdata->tsp_vendor1, pdata->tsp_vendor2);
}
...
int melfas_power(struct mms_ts_info *info, int on){
	...
	ret = gpio_direction_output(info->pdata->vdd_en2, on);
                if (ret) {
                        dev_err(&info->client->dev,"[TSP]%s: unable to set_direction for mms_vdd_en [%d]\n",
                                        __func__, info->pdata->vdd_en);
                return 0;
