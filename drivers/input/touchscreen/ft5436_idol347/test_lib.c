...
#define Item_WeakShortTest_CC_Min "WeakShortTest_CC"/*\D7ۺϲ\E2\CA\D4\C8\ED\BC\FE\C3\FC\C3\FB\D3\D0\CE\F3*/
...
΢\B6\CC·\C1\BF\B2\FAFW\B7\BD\C3\E6\CF\E0\B9ؼĴ\E6\C6\F7\BC\B0\B9\A6\C4\DC\C8\E7\CF£\BA
ShortTestEn\A3\A80x07\A3\A9\A3\BA     ΢\B6\CC·\B2\E2\CA\D4ʹ\C4ܼĴ\E6\C6\F7
ValLBuf0 \A3\A80xF4\A3\A9\A3\BA     ValL\CA\FD\BEݼĴ\E6\C6\F70
ValLBuf0 \A3\A80xF5\A3\A9\A3\BA     ValL\CA\FD\BEݼĴ\E6\C6\F71

΢\B6\CC·\C1\BF\B2\FA\B2\E2\CA\D4ʱHost\B6\CB\CB\F9\D0\E8\B2\D9\D7\F7\A3\BA
1. ΢\B6\CC·\B2\E2\CA\D4ʹ\C4\DC
Host:  W 07 01          // \D6\F7\BB\FA\B7\A2\CB\CDʹ\C4ܺ\F3΢\B6\CC·\B2\E2\CA\D4һ\B4Σ\BBȻ\BA\F3\D6\D8\D0»ָ\B4\B5\BD\B9\A4\B3\A7ģʽ

2. \B2\E2\CA\D4\CA\FD\BEݶ\C1ȡ
Host:  W F4  R FF     // \BD\AB\D7Զ̺ͻ\A5\B6̲\E2\CAԵ\C4\CB\F9\D3\D0ValL\CA\FD\BEݶ\C1\B3\F6\A3\BAGroundValL[64]\BA\CDMutualValL[64] 
3. \CA\FD\BE\DD˵\C3\F7\A3\BA

Offset:  0x00~0x01

Ground_ShortTest:
\B5ض\CC·У׼\A3\BA2   Byte          
\B5ض\CC·\CA\FD\BEݣ\BA(TxNum+RxNum)*2   Byte  // Tx\D4\DAǰ\A3\ACRx\D4ں\F3

Mutual_ShortTest:
\BB\A5\B6\CC·У׼\A3\BA2  Byte
\BB\A5\B6\CC·\CA\FD\BEݣ\BA(TxNum+RxNum)*2   Byte  // Tx\D4\DAǰ\A3\ACRx\D4ں\F3
==================================================================================*/
#if 0
static boolean TestItem_WeakShortTest(void)
{
	...
	//TxNum\BCĴ\E6\C6\F7\A3\BA0x02(Read Only)
	//RxNum\BCĴ\E6\C6\F7\A3\BA0x03(Read Only)	
	...
	iMaxTx = iTxNum;
	iAllAdcDataNum = 1 + (1 + iTxNum + iRxNum)*2;//\D7\DCͨ\B5\C0\CA\FD + \B6Ե\D8У׼\CA\FD\BE\DD + ͨ\B5\C0\BC\E4У׼\CA\FD\BE\DD + Offset
	...
	{
		iDsen = iAdcData[i+2];
		if((2047+iDoffset) - iDsen <= 0)//С\D3ڵ\C8\D3\DA0\A3\ACֱ\BD\D3\C5\D0PASS
		{
			continue;
		}

		//\B2\C9\D3\C3\D0µĹ\ABʽ\BD\F8\D0м\C6\CB\E3
		...
		if((iMin_CG > fGShortResistance[i]) || (iDsen - iDoffset < 0))//С\D3ڵ\C8\D3\DA0\A3\ACֱ\BD\D3\C5\D00ŷ\B6\CC·
		{
			if(iCount == 0)
...
static int WeakShort_GetAdcData( int AllAdcDataLen, int *pRevBuffer )
{
	...
	ReCode = WriteReg(0x07, 0x01);// \D6\F7\BB\FA\B7\A2\CB\CDʹ\C4ܺ\F3΢\B6\CC·\B2\E2\CA\D4һ\B4\CE
	if(ReCode < ERROR_CODE_OK)return ReCode;

	focal_msleep(200);

	for(i = 0; i < 40; i++)//׼\B1\B8\BA\C3\CA\FD\BEݺ\F3\A3\ACFW\BD\AB0x07\BCĴ\E6\C6\F7\D6\C3Ϊ0
	{
...
unsigned char GetTxSC_CB(unsigned char index, unsigned char *pcbValue)
{
	...
	if(index<128)//\B5\A5\B8\F6\B6\C1ȡ
	{	
		*pcbValue = 0;
	...
	else//\C1\AC\D0\F8\B6\C1ȡ\A3\AC\B3\A4\B6\C8Ϊindex-128
	{
...
boolean TestItem_SCapCbTest()
{
	...
	{			
		for(i = 0; i < 1; i++)
		{
			...
			ReCode = WriteReg( 0x44, 1 );//\D7\D4\C8ݹ\A4\D7\F7\B7\BDʽѡ\D4\F1:  1\A3\BA\B7\C0ˮ 0:\B7Ƿ\C0ˮ
			StartScan();
			...
			//\B7Ƿ\C0ˮrawdata
			I2C_wBuffer[0] = REG_ScCbBuf0;
			ReCode = WriteReg( 0x44, 0 );//\D7\D4\C8ݹ\A4\D7\F7\B7\BDʽѡ\D4\F1:  1\A3\BA\B7\C0ˮ 0:\B7Ƿ\C0ˮ
			StartScan();
			ReCode = WriteReg( REG_ScCbAddrR, 0 );
	...
	//----------------------------------------------------------\C5жϳ\AC\B9\FD\B7\B6Χ\B5\C4rawData
	...
	{	
		...
		if(g_TestParam_FT5X46.SCapCbTest_SetWaterproof_ON && bFlag)
		{
			...
			////>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\CAռ\AF\B2\E2\CA\D4\CA\FD\BEݣ\AC\B4\E6\C8\EBCSV\CEļ\FE
			/*
			strTemp = "";
			for(int i = 0; i < 2; i++)
			{		
