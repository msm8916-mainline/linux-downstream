...
//    GTP_INFO("FILE HARDWARE INFO:%02x%02x%02x%02x", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
    printk("FILE PID:%s", fw_head->pid);
    printk("FILE VID:%04x", fw_head->vid);
//    GTP_INFO("IC HARDWARE INFO:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
//             update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
    printk("IC PID:%s", update_msg.ic_fw_msg.pid);
    printk("IC VID:%04x", update_msg.ic_fw_msg.vid);
...
//        GTP_ERROR("Don't meet the third condition.");
        printk("File VID <= Ic VID, update aborted!");
    }
    else
    {
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head* fw_head, u8* path)
{
    ...
    if (path)
    {
  //      printk("Update File path:%s, %d", path, strlen(path));
...
//        printk("Firmware actual size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
        ...
        printk("firmware checksum:%x", fw_checksum&0xFFFF);
...
//    printk("Bin firmware actual size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
...
static u8 gup_burn_proc(struct i2c_client *client, u8 *burn_buf, u16 start_addr, u16 total_length)
{
    ...
    printk("Begin burn %dk data to addr 0x%x", (total_length/1024), start_addr);
    while(burn_length < total_length)
    {
        printk("B/T:%04d/%04d", burn_length, total_length);
        ...
        for(retry = 0; retry < MAX_FRAME_CHECK_TIME; retry++)
        {
            ...
            if(memcmp(&wr_buf[GTP_ADDR_LENGTH], &rd_buf[GTP_ADDR_LENGTH], frame_length))
            {
                ...
                printk("write array:");
          //      printk_ARRAY(&wr_buf[GTP_ADDR_LENGTH], frame_length);
                printk("read array:");
            //    printk_ARRAY(&rd_buf[GTP_ADDR_LENGTH], frame_length);
                continue;
            }
            else
            {
                //printk("Check frame data success.");
                break;
            }
        }
...
static u8 gup_recall_check(struct i2c_client *client, u8* chk_src, u16 start_rd_addr, u16 chk_length)
{
    ...
    while(recall_length < chk_length)
    {
        ...
        if(memcmp(&rd_buf[GTP_ADDR_LENGTH], &chk_src[recall_length], frame_length))
        {
            ...
            printk("chk_src array:");
        //    printk_ARRAY(&chk_src[recall_length], frame_length);
            printk("recall array:");
        //    printk_ARRAY(&rd_buf[GTP_ADDR_LENGTH], frame_length);
            return FAIL;
        }
    ...
    printk("Recall check %dk firmware success.", (chk_length/1024));
...
static u8 gup_burn_fw_section(struct i2c_client *client, u8 *fw_section, u16 start_addr, u8 bank_cmd )
{
    ...
    printk("[burn_fw_section]Wait for the burn is complete......");
    do{
        ...
        msleep(10);
        //printk("[burn_fw_section]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]);
...
static u8 gup_burn_dsp_isp(struct i2c_client *client)
{
    ...
    //step1:alloc memory
    printk("[burn_dsp_isp]step1:alloc memory");
    while(retry++ < 5)
    {
    ...
    printk("[burn_dsp_isp]step2:load dsp isp file data");
    ...
    printk("[burn_dsp_isp]step3:disable wdt,clear cache enable");
    ...
    //step4:hold ss51 & dsp
    printk("[burn_dsp_isp]step4:hold ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
    if(ret <= 0)
    {
    ...
    printk("[burn_dsp_isp]step5:set boot from sram");
    ...
    printk("[burn_dsp_isp]step6:software reboot");
    ...
    //step7:select bank2
    printk("[burn_dsp_isp]step7:select bank2");
    ...
    printk("[burn_dsp_isp]step8:enable accessing code");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__MEM_CD_EN, 0x01);
    if(ret <= 0)
    {
    ...
    printk("[burn_dsp_isp]step9:burn 4k dsp_isp");
    ...
    //step10:set scramble
    printk("[burn_dsp_isp]step10:set scramble");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
    if(ret <= 0)
    {
    ...
    printk("[burn_dsp_isp]Burned length:%d", update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_ss51(struct i2c_client *client)
{
    ...
    //step1:alloc memory
    printk("[burn_fw_ss51]step1:alloc memory");
    while(retry++ < 5)
    {
        ...
        {
            printk("[burn_fw_ss51]Alloc %dk byte memory success.", (FW_SECTION_LENGTH / 1024));
            break;
        }
    }
    if(retry >= 5)
...
//    printk("[burn_fw_ss51]step2:load ss51 firmware section 1 file data");
...
//    }
    ...
    printk("[burn_fw_ss51]step2: reset bank0 0xC000~0xD000");
    memset(fw_ss51, 0xFF, FW_SECTION_LENGTH);
    
    //step3:clear control flag
    printk("[burn_fw_ss51]step3:clear control flag");
    ...
    printk("[burn_fw_ss51]step4:burn ss51 firmware section 1");
    ...
    printk("[burn_fw_ss51]step5:load ss51 firmware section 2 file data");
    ...
    printk("[burn_fw_ss51]step6:burn ss51 firmware section 2");
    ...
    printk("[burn_fw_ss51]step7:load ss51 firmware section 3 file data");
    ...
    printk("[burn_fw_ss51]step8:burn ss51 firmware section 3");
    ...
    printk("[burn_fw_ss51]step9:load ss51 firmware section 4 file data");
    ...
    printk("[burn_fw_ss51]step10:burn ss51 firmware section 4");
    ...
    printk("[burn_fw_ss51]Burned length:%d", update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_dsp(struct i2c_client *client)
{
    ...
    //step1:alloc memory
    printk("[burn_fw_dsp]step1:alloc memory");
    while(retry++ < 5)
    {
        ...
        {
            printk("[burn_fw_dsp]Alloc %dk byte memory success.", (FW_SECTION_LENGTH / 1024));
            break;
        }
    }
    if(retry >= 5)
    ...
    printk("[burn_fw_dsp]step2:load firmware dsp");
    ...
    //step3:select bank3
    printk("[burn_fw_dsp]step3:select bank3");
    ...
    //step4:hold ss51 & dsp
    printk("[burn_fw_dsp]step4:hold ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
    if(ret <= 0)
    {
    ...
    //step5:set scramble
    printk("[burn_fw_dsp]step5:set scramble");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
    if(ret <= 0)
    {
    ...
    printk("[burn_fw_dsp]step6:release ss51 & dsp");
    ...
    printk("[burn_fw_dsp]step7:burn 4k dsp firmware");
    ...
    printk("[burn_fw_dsp]step8:send burn cmd to move data to flash from sram");
    ...
    printk("[burn_fw_dsp]Wait for the burn is complete......");
    do{
        ret = gup_get_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
        if(ret <= 0)
        {
        ...
        msleep(10);
        //printk("[burn_fw_dsp]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]);
    }while(rd_buf[GTP_ADDR_LENGTH]);
    
    //step9:recall check 4k dsp firmware
    printk("[burn_fw_dsp]step9:recall check 4k dsp firmware");
    ...
    printk("[burn_fw_dsp]Burned length:%d", update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_boot(struct i2c_client *client)
{
    ...
    //step1:Alloc memory
    printk("[burn_fw_boot]step1:Alloc memory");
    while(retry++ < 5)
    {
        ...
        {
            printk("[burn_fw_boot]Alloc %dk byte memory success.", (FW_BOOT_LENGTH/1024));
            break;
        }
    }
    if(retry >= 5)
    ...
    printk("[burn_fw_boot]step2:load firmware bootloader");
    ...
    //step3:hold ss51 & dsp
    printk("[burn_fw_boot]step3:hold ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
    if(ret <= 0)
    {
    ...
    //step4:set scramble
    printk("[burn_fw_boot]step4:set scramble");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
    if(ret <= 0)
    {
    ...
    printk("[burn_fw_boot]step5:hold ss51 & release dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x04);                 //20121211
    if(ret <= 0)
    {
    ...
    //step6:select bank3
    printk("[burn_fw_boot]step6:select bank3");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x03);
    if(ret <= 0)
    {
    ...
    printk("[burn_fw_boot]step6:burn 2k bootloader firmware");
    ...
    //step7:send burn cmd to move data to flash from sram
    printk("[burn_fw_boot]step7:send burn cmd to move data to flash from sram");
    ...
    printk("[burn_fw_boot]Wait for the burn is complete......");
    do{
        ret = gup_get_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
        if(ret <= 0)
        {
        ...
        msleep(10);
        //printk("[burn_fw_boot]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]);
    }while(rd_buf[GTP_ADDR_LENGTH]);
    
    //step8:recall check 2k bootloader firmware
    printk("[burn_fw_boot]step8:recall check 2k bootloader firmware");
    ...
    printk("[burn_fw_boot]Burned length:%d", update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_boot_isp(struct i2c_client *client)
{
    ...
    {
        printk("No need to upgrade the boot_isp code!");
        return SUCCESS;
    }
    GTP_INFO("[burn_fw_boot_isp]Begin burn boot_isp firmware---->>");
    
    //step1:Alloc memory
    printk("[burn_fw_boot_isp]step1:Alloc memory");
    while(retry++ < 5)
    {
        ...
        {
            printk("[burn_fw_boot_isp]Alloc %dk byte memory success.", (FW_BOOT_ISP_LENGTH/1024));
            break;
        }
    }
    if(retry >= 5)
    ...
    //step2:load firmware bootloader
    printk("[burn_fw_boot_isp]step2:load firmware bootloader isp");
    ...
    //step3:hold ss51 & dsp
    printk("[burn_fw_boot_isp]step3:hold ss51 & dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x0C);
    if(ret <= 0)
    {
    ...
    //step4:set scramble
    printk("[burn_fw_boot_isp]step4:set scramble");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_OPT_B0_, 0x00);
    if(ret <= 0)
    {
    ...
    //step5:hold ss51 & release dsp
    printk("[burn_fw_boot_isp]step5:hold ss51 & release dsp");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__SWRST_B0_, 0x04);                 //20121211
    if(ret <= 0)
    {
    ...
    //step6:select bank3
    printk("[burn_fw_boot_isp]step6:select bank3");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x03);
    if(ret <= 0)
    {
    ...
    printk("[burn_fw_boot_isp]step7:burn 2k bootloader firmware");
    ...
    //step7:send burn cmd to move data to flash from sram
    printk("[burn_fw_boot_isp]step8:send burn cmd to move data to flash from sram");
    ...
    printk("[burn_fw_boot_isp]Wait for the burn is complete......");
    do{
        ret = gup_get_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, rd_buf, 1);
        if(ret <= 0)
        {
        ...
        msleep(10);
        //printk("[burn_fw_boot_isp]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]);
    }while(rd_buf[GTP_ADDR_LENGTH]);
    
    //step8:recall check 2k bootload_isp firmware
    printk("[burn_fw_boot_isp]step9:recall check 2k bootloader firmware");
    ...
    printk("[burn_fw_boot_isp]Burned length:%d", update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_link(struct i2c_client *client)
{
    ...
    if(update_msg.fw_burned_len >= update_msg.fw_total_len)
    {
        printk("No need to upgrade the link code!");
        return SUCCESS;
    }
    GTP_INFO("[burn_fw_link]Begin burn link firmware---->>");
    
    //step1:Alloc memory
    printk("[burn_fw_link]step1:Alloc memory");
    while(retry++ < 5)
    {
        ...
        {
            printk("[burn_fw_link]Alloc %dk byte memory success.", (FW_SECTION_LENGTH/1024));
            break;
        }
    }
    if(retry >= 5)
    ...
    printk("[burn_fw_link]step2:load firmware link section 1");
    ...
    printk("[burn_fw_link]step3:burn link firmware section 1");
    ...
    printk("[burn_fw_link]step4:load link firmware section 2 file data");
    ...
    printk("[burn_fw_link]step4:burn link firmware section 2");
    ...
    printk("[burn_fw_link]Burned length:%d", update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_gwake_section(struct i2c_client *client, u8 *fw_section, u16 start_addr, u32 len, u8 bank_cmd )
{
    ...
    do{
        ...
        msleep(10);
        //printk("[burn_fw_app_section]Get burn state:%d.", rd_buf[GTP_ADDR_LENGTH]);
    }while(rd_buf[GTP_ADDR_LENGTH]);
...
static u8 gup_burn_fw_gwake(struct i2c_client *client)
{
    ...
    if(update_msg.fw_burned_len >= update_msg.fw_total_len)
    {
        printk("No need to upgrade the gwake code!");
        return SUCCESS;
    }
    ...
    //step1:alloc memory
    printk("[burn_fw_gwake]step1:alloc memory");
    while(retry++ < 5)
    {
        ...
        {
            printk("[burn_fw_gwake]Alloc %dk byte memory success.", (FW_SECTION_LENGTH/1024));
            break;
        }
    }
    if(retry >= 5)
    ...
    printk("[burn_fw_gwake]step2:load app_code firmware section 1 file data");
    ...
    printk("[burn_fw_gwake]step3:burn app_code firmware section 1");
    ...
    printk("[burn_fw_gwake]step5:load app_code firmware section 2 file data");
    ...
    printk("[burn_fw_gwake]step6:burn app_code firmware section 2");
    ...
    printk("[burn_fw_gwake]step7:load app_code firmware section 3 file data");
    ...
    printk("[burn_fw_gwake]step8:burn app_code firmware section 3");
    ...
    printk("[burn_fw_gwake]step9:load app_code firmware section 4 file data");
    ...
    printk("[burn_fw_gwake]step10:burn app_code firmware section 4");
    ...
    printk("[burn_fw_gwake]Burned length:%d", update_msg.fw_burned_len);
    ret = SUCCESS;
...
static u8 gup_burn_fw_finish(struct i2c_client *client)
{
    ...
    //step1:alloc memory
    printk("[burn_fw_finish]step1:alloc memory");
    while(retry++ < 5)
    {
        fw_ss51 = (u8*)kzalloc(FW_SECTION_LENGTH, GFP_KERNEL);
        if(fw_ss51 == NULL)
        ...
        {
            printk("[burn_fw_finish]Alloc %dk byte memory success.", (FW_SECTION_LENGTH/1024));
            break;
        }
    }
    if(retry >= 5)
    ...
    printk("[burn_fw_finish]step2: burn ss51 first 8K.");
    ret = gup_load_section_file(fw_ss51, 0, FW_SECTION_LENGTH, SEEK_SET);
    if(FAIL == ret)
    {
    ...
    printk("[burn_fw_finish]step3:clear control flag");
    ret = gup_set_ic_msg(client, _rRW_MISCTL__BOOT_CTL_, 0x00);
    if(ret <= 0)
    {
    ...
    printk("[burn_fw_finish]step4:burn ss51 firmware section 1");
    ret = gup_burn_fw_section(client, fw_ss51, 0xC000, 0x01);
    if(FAIL == ret)
    {
    ...
    printk("[burn_fw_finish]step5:enable download DSP code ");
    ...
    printk("[burn_fw_finish]step6:release ss51 & hold dsp");
...
//    printk("[update_proc]Begin update ......");
...
update_fail:
    printk("[update_proc]leave update mode.");
    ...
    if (SUCCESS == update_ret)
    {
        ...
        {
            printk("[update_proc]send config.");
...
u8 gup_check_fs_mounted(char *path_name)
{
    ...
    if (err)
    {
        printk("%s NOT Mounted: %d", path_name, err);
        return FAIL;
    }
...
static u32 gup_clk_count_get(void)
{
    ...
    printk("Clk_count 0: %2X", buf[0]);
    printk("Clk_count 1: %2X", buf[1]);
    printk("Clk_count 2: %2X", buf[2]);
    printk("Clk_count 3: %2X", buf[3]);
...
s32 gup_clk_calibration(void)
{
    ...
    for (i = INIT_CLK_DAC; i < MAX_CLK_DAC; i++)
    {
        ...
        printk("== time %d, %d, %d", sec, usec, count_ref);
        
        if (count > count_ref)
        {
            printk("== count_diff %d", count - count_ref);
            break;
        }
...
s32 gup_hold_ss51_dsp(struct i2c_client *client)
{
    ...
    while(retry++ < 200)
    {
        ...
        if(ret <= 0)
        {
            printk("Hold ss51 & dsp I2C error,retry:%d", retry);
            continue;
        }
        ...
        if (0x0C == rd_buf[GTP_ADDR_LENGTH])
        {
            printk("[enter_update_mode]Hold ss51 & dsp confirm SUCCESS");
            break;
        }
        printk("Hold ss51 & dsp confirm 0x4180 failed,value:%d", rd_buf[GTP_ADDR_LENGTH]);
    }
    if(retry >= 200)
    {
        GTP_ERROR("Enter update Hold ss51 failed.");
...
static u8 gup_download_fw_dsp(struct i2c_client *client, u8 dwn_mode)
{
    ...
    //step1:select bank2
    printk("[download_fw_dsp]step1:select bank2");
    ret = gup_set_ic_msg(client, _bRW_MISCTL__SRAM_BANK, 0x02);
    if (ret == FAIL)
    {
...
static s32 gup_burn_fw_proc(struct i2c_client *client, u16 start_addr, s32 start_index, s32 burn_len)
{
    ...
    printk("burn firmware: 0x%04X, %d bytes, start_index: 0x%04X", start_addr, burn_len, start_index);
...
static s32 gup_check_and_repair(struct i2c_client *client, u16 start_addr, s32 start_index, s32 chk_len)
{
    ...
    printk("check firmware: start 0x%04X, %d bytes", start_addr, chk_len);
...
static u8 gup_download_fw_ss51(struct i2c_client *client, u8 dwn_mode)
{
    ...
    for (section = 1; section <= 4; section += 2)
    {
        ...
        printk("download firmware ss51: select bank%d", bank);
...
static s32 gup_prepare_fl_fw(char *path, st_fw_head *fw_head)
{
    ...
    if (!memcmp(path, "update", 6))
    {
        ...
        for (i = 0; i < GUP_SEARCH_FILE_TIMES; ++i)
        {
            ...
            printk("Search for %s, %s for fw update.(%d/%d)", FL_UPDATE_PATH, FL_UPDATE_PATH_SD, i+1, GUP_SEARCH_FILE_TIMES);
        ...
        {
            printk("request processing, waiting for accomplishment");
            msleep(1000);
        }
    }
    ...
    update_msg.fw_total_len -= FW_HEAD_LENGTH;
    printk("Fimrware size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len / 1024);
