...
#define GOODIX_PINCTRL_STATE_SLEEP "pmx_ts_suspend"
#define GOODIX_PINCTRL_STATE_DEFAULT "pmx_ts_active"
...
static ssize_t gesture_state = 0;//0--off,1--on
static ssize_t init_gesture = 0;
...
static int goodix_ctp_gesture_report(struct input_dev *dev, u8 value)
{
	struct input_dev *pdev = dev;
	u8 val = value, report_data = 0;
	int ret = 0;

	pr_info("%s, value=%c(0x%02x)\n", __func__, val, val);
	switch (val)
	{
		case 'e':
	...
	input_report_key(pdev, KEY_HOME, 1);
	input_sync(pdev);
	input_report_key(pdev, KEY_HOME, 0);
	input_sync(pdev);
...
#if defined(CONFIG_CT820_COMMON)
				goodix_ctp_gesture_report(ts->input_dev, doze_buf[2]);
#else
                doze_status = DOZE_WAKEUP;
...
static s32 gtp_bak_ref_proc(struct goodix_ts_data *ts, u8 mode)
{
    ...
    ref_filp = filp_open(GTP_BAK_REF_PATH, O_RDWR|O_CREAT, 0660);
    if (IS_ERR(ref_filp))
    {
...
static s32 gtp_main_clk_proc(struct goodix_ts_data *ts)
{
    ...
    clk_filp = filp_open(GTP_MAIN_CLK_PATH, O_RDWR|O_CREAT, 0660);
    if (IS_ERR(clk_filp))
    {
...
int goodix_self_test(void)
{
	int ret = -1;

//	GTP_INFO("%s\n", __func__);
//	ret = gt9xx_short_test(i2c_connect_client);
//	printk("%s, gt9xx_short_test=%d\n", __func__, ret);
	if (SUCCESS == ret)
	{
//		ret = gt9xx_open_test(i2c_connect_client);
	...
	GTP_INFO("%s done, ret=%d\n", __func__, ret);
	return ret;
}
/*
 add sys device node for control gesture mode begin//lgm
*/
//static ssize_t gesture_state = 1;//0--off,1--on
static struct kobject *android_touch_ftobj;
static ssize_t gt6xx_gesture_show(struct device *dev,
					struct device_attribute *attr,
					char *buf)
{
	ssize_t num_read_chars = 0;
	num_read_chars = sprintf(buf, "TP  gesture state::%ld\n", gesture_state);
	return num_read_chars;
}
static ssize_t gt6xx_gesture_store(struct device *dev,
					struct device_attribute *attr,
					const char *buf, size_t count)
{
	...
	if(on_off==0){
	    if(init_gesture == 1){
			...
			gesture_state = on_off;
			init_gesture = 0;
			printk("[%s]: ! TP gesture_state disabled by top test node\n", __func__);
		}else {
			return count;
		}
	}else{
		if(init_gesture == 0){
			...
			gesture_state = on_off;
			init_gesture =1;
			printk("[%s]: TP gesture_state enabled by top test node\n", __func__);
		}else {
			return count;
		}
	}
...
static DEVICE_ATTR(gesture, (S_IWUSR|S_IRUGO), gt6xx_gesture_show,
			gt6xx_gesture_store);
static int gt6xx_touch_sysfs_init(void)
{
	int ret;
	android_touch_ftobj = kobject_create_and_add("android_touch", NULL);
	if (android_touch_ftobj == NULL)
	{
		GTP_INFO(KERN_ERR "TOUCH_ERR: subsystem_register failed\n");
		ret = -ENOMEM;
		return ret;
	}

	ret = sysfs_create_file(android_touch_ftobj, &dev_attr_gesture.attr);
	if (ret) 
	{
		GTP_INFO(KERN_ERR "TOUCH_ERR: create_file rawdata failed\n");
		return ret;
	}
	return 0;
}
static void gt6xx_touch_sysfs_deinit(void)
{
	sysfs_remove_file(android_touch_ftobj, &dev_attr_gesture.attr);
	kobject_del(android_touch_ftobj);
}
/*
 add sys device node for control gesture mode end//lgm
*/
...
//    GTP_INFO("%s:GTP Driver Version: %s",__func__,GTP_DRIVER_VERSION);
//    GTP_INFO("%s:GTP I2C Address: 0x%02x",__func__,client->addr);
...
//    {
 ...
 //   }
   ...
   init_wr_node(client);
#endif
#ifdef SUPPORT_READ_TP_VERSION
	// GTP_INFO("version_info=0x%x, product_id=%s", version_info, pdata->product_id);
	// memset(fw_version, 0, sizeof(fw_version));
	// sprintf(fw_version, "vid:%d, fw:0x%04x, ic:%s(%s)\n", 
		// g_sensor_id, version_info, (char *)pdata->product_id,
#if GTP_AUTO_UPDATE
		"open"
#else
		"close"
	...
	init_tp_fm_info(version_info, fw_version, "Goodix");
#endif
#if GTP_ESD_PROTECT
    gtp_esd_switch(client, SWITCH_ON);
#endif
	...
	ctp_gesture_set_input_device(ts->input_dev);
#endif
//	GTP_INFO("version_info=0x%x", version_info);
    gt6xx_touch_sysfs_init();
	if(version_info == 0x0){
		GTP_INFO("%s:version_info\n",__func__);
	}else{
		sprintf(buf,"%x",version_info);
		hw_info_set(ODMM_HD_INFO_TP,"TP", "GT615", "innolux(QUNCHUANG)", buf, "touch screen mode");
	}
    printk("%s:E\n",__func__);
    return 0;
...
Err_power_init:
Err_pinctrl:
	//if (gt9xx_pctrl.pinctrl)
		//pinctrl_put(gt9xx_pctrl.pinctrl);
	i2c_set_clientdata(client, NULL);
