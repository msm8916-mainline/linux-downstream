...
extern void gtp_create_ctp_proc(void);
#endif
...
extern s32 gesture_event_handler(struct goodix_ts_data *ts);
extern s32 gtp_init_node(void);
extern void gtp_deinit_node(void);
extern void gesture_clear_wakeup_data(void);
extern s32 gtp_send_glove_cfg(struct i2c_client *client);

extern int gesture_enabled;
extern int glove_enabled;
#endif
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    GTP_DEBUG("checksum:%x, cfg_ver:%x", config[ts->gtp_cfg_len], config[GTP_ADDR_LENGTH]);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
   ...
   GTP_DEBUG("step 1: parse dt & power on ic");
   ...
   GTP_DEBUG("step 2: request gpio port");
   ...
   GTP_DEBUG("step 3: test if ic is working");
...
#if GTP_AUTO_UPDATE
	ret = gtp_i2c_read_dbl_check(client, GUP_REG_HW_INFO, &hw_info_buf[GTP_ADDR_LENGTH], 4);
	if (FAIL == ret)
	{
		...
		return -EINVAL;
		 ...
		 GTP_DEBUG("hw_info:%x, %x, %x, %x", hw_info_buf[2], hw_info_buf[3],hw_info_buf[4],hw_info_buf[5]);
		 GTP_DEBUG("need force update");
	}
#else
        GTP_GPIO_FREE(gtp_rst_gpio);
        GTP_GPIO_FREE(gtp_int_gpio);
    ...
    if (ret < 0)
    {
        GTP_INFO("Read version failed.");
    }

	snprintf(tp_info,sizeof(tp_info),"ic:gt9176,ver:0x%x",version_info);
...
static int gtp_fb_notifier_callback(struct notifier_block *noti, unsigned long event, void *data)
{
	...
	if (gesture_enabled && ev_data && ev_data->data
		&& event == FB_EARLY_EVENT_BLANK && ts) {
		blank = ev_data->data;
