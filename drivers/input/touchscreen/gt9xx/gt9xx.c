...
static void switch_report_work_func(struct work_struct *work) 
{
		...
		if(HALL_FAR_AWAY==hall_tp_open)//hall far away
		{
		    ...
		    if(ts->gtp_is_suspend){	//waiting for awake state
                 ...
                 GTP_INFO("shj far away,awake status,switch to normal window");
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	param_switch = val;
	//iqueue_delayed_work(ts->switch_wq, &ts->switch_dwork,msecs_to_jiffies(0));	
...
//added by chenchen for tp crash 20140925 begin
		enable_irq_wake(ts->client->irq);
		gtp_irq_enable(ts);
//added by chenchen for tp crash 20140925 end
            return ret;
        }
        msleep(10);
    }
...
static int gtp_request_irq(struct goodix_ts_data *ts)
{
	...
	GTP_DEBUG("INT trigger type:%x, irq=%d", ts->int_trigger_type,
			ts->client->irq);
        //GTP_INFO("shihj irq_table[ts->int_trigger_type]: %x",irq_table[ts->int_trigger_type]);
