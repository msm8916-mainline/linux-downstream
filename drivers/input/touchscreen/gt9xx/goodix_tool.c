 ...
 *Auther:	zengguang
 *Date	:	2014.6.24
 *Purpose:  goodix firmware update version 2.2
 *
 */
...
static s32 judge_gwake(struct i2c_client *client);
void write_cmd(struct i2c_client *client);
u8 gwake_FW_integrated = 0;//0:gwake FW is not intergrated 1:gwake FW is intergrated
u8 FW_check_time_flag = 0;//gwake FW check flag
...
/*add for judge gesture FW correct by zengguang 2015.01.04 end*/
...
static s32 tool_i2c_read_no_extra(u8* buf, u16 len)
{
    ...
    u8* data;		//modified for rawdata check by zengguang 2014.07.18
    struct i2c_msg msgs[2];
    
	data = (u8*)kmalloc(IIC_MAX_TRANSFER_SIZE, GFP_KERNEL); 	//modified for rawdata check by zengguang 2014.07.18
    ...
    kfree(data); 	//modified for rawdata check by zengguang 2014.07.18
    return ret;
}
...
//modified for transfer data size end by zengguang  2014.07.15
...
{
    s32 ret = 0;
    static u8 FW_integrated_flag = 0;	//add for judge gesture FW correct by zengguang 2015.01.04 
		...
		if(1 != FW_check_time_flag)          
              {
                ...
                #if GTP_ESD_PROTECT
                	gtp_esd_switch(gt_client, SWITCH_OFF);//turn off the ESD check thread
                #endif
                	gtp_irq_disable(i2c_get_clientdata(gt_client));//turn off interrupt
                	FW_integrated_flag = judge_gwake(gt_client);
                	gtp_irq_enable(i2c_get_clientdata(gt_client));//turn on interrupt
                #if GTP_ESD_PROTECT
                	gtp_esd_switch(gt_client, SWITCH_ON);//turn on the ESD check thread
                #endif
                
            		if(FW_integrated_flag <= 0)
             		{
                		GTP_ERROR("gwake FW is not integrated!");
                		return -1;
            		...
            		else 
             		{
               		 //mdelay(100);
               		write_cmd(gt_client);
                		GTP_INFO("gwake FW is integrated!");
                		//GTP_DEBUG("gwake FW is integrated001!");
	     		}
	   	}	
/*add for judge gesture FW correct by zengguang 2015.01.04 end*/
            if (FAIL == comfirm())
...
/*******************************************************
Function:
    Judge gwake FW  function.
Input:
    NULL
Output:
    return gwake_FW_integrated
********************************************************/
...
static s32 judge_gwake(struct i2c_client *client)
{
	...
	//ret = gtp_enter_doze(client);
    s8 retry = 0;
    u8 i2c_control_buf[3] = {(u8)(GTP_REG_SLEEP >> 8), (u8)GTP_REG_SLEEP, 8};
    FW_check_time_flag = 1;

    GTP_DEBUG_FUNC();

    GTP_DEBUG("Entering gesture mode...");
    // spin_lock(&judge_gwake_lock); 
    mdelay(700);
    while(retry++ < 5)
    {
	...
	if (retry >= 5)
    {
        ...
        //spin_unlock(&judge_gwake_lock);
		    return -1;
 
    }

	mdelay(700);
    ret = gtp_i2c_read_dbl_check(client, 0x41E4, &check_flag, 1);
    if (ret == FAIL)
    {
    	GTP_ERROR("gtp_i2c_read_dbl_check from 0x41E4 failed.");
      //spin_unlock(&judge_gwake_lock);
		  return -1;	
    }

	if ((check_flag == 0xFC) || (check_flag == 0xFB))
	{
		gwake_FW_integrated = 0;//gwake FW is not integrated
		FW_check_time_flag = 0;
	}
	else
		{
		gwake_FW_integrated = 1;//gwake FW is  integrated
		}
...
void write_cmd(struct i2c_client *client)
{
	s32 ret = 0;
	s32 retry = 0;
    u8 i2c_control_buf[3] = {0x80, 0x40, 1};
	while(retry++ < 5)
    {
        ...
        if (ret < 0)
        {
            GTP_DEBUG("Failed to write cmd to 0x8040, %d", retry);
            continue;
        }

        else
        {
            GTP_INFO("write cmd success.");
            break;
        }
        msleep(10);
    }
