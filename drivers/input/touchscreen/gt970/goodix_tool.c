...
static void tool_set_proc_name(char * procname)
{
    ...
    //VIVO_TS_LOG_DBG("compile date: %s", date);
    ...
    VIVO_TS_LOG_DBG("procname = %s", procname);
}
...
//        || !strncmp(IC_TYPE, "GT813", 5))
        ...
        && strncmp(IC_TYPE, "GTxxx", 5) && strncmp(IC_TYPE, "GT9XX", 5))
    {
        ...
        VIVO_TS_LOG_DBG("I2C function: with pre and end cmd!");
    }
    else
    {
        tool_i2c_read = tool_i2c_read_no_extra;
        tool_i2c_write = tool_i2c_write_no_extra;
        VIVO_TS_LOG_INF("I2C function: without pre and end cmd!");
    }
}
...
static void unregister_i2c_func(void)
{
    tool_i2c_read = NULL;
    tool_i2c_write = NULL;
    VIVO_TS_LOG_INF("I2C function: unregister i2c transfer function!");
}
...
s32 init_wr_node(struct i2c_client *client)
{
    ...
    {
        VIVO_TS_LOG_ERR("Apply for memory failed.");
        return FAIL;
    }
    ...
    {
        VIVO_TS_LOG_ERR("Couldn't create proc entry!");
        return FAIL;
    }
    else
    {
        VIVO_TS_LOG_INF("Create proc entry success!");
...
static u8 relation(u8 src, u8 dst, u8 rlt)
{
    ...
    case 1:
        ret = (src == dst) ? true : false;
        VIVO_TS_LOG_DBG("equal:src:0x%02x   dst:0x%02x   ret:%d.", src, dst, (s32)ret);
        break;
...
//    memcpy(buf, &cmd_head.flag_addr, cmd_head.addr_len);//Modified by Scott, 2012-02-17
    ...
    for (i = 0; i < cmd_head.times; i++)
    {
        ...
        {
            VIVO_TS_LOG_ERR("Read flag data failed!");
            return FAIL;
        }
        if (true == relation(buf[GTP_ADDR_LENGTH], cmd_head.flag_val, cmd_head.flag_relation))
        {
            VIVO_TS_LOG_DBG("value at flag addr:0x%02x.", buf[GTP_ADDR_LENGTH]);
            VIVO_TS_LOG_DBG("flag value:0x%02x.", cmd_head.flag_val);
            break;
        }
    ...
    if (i >= cmd_head.times)
    {
        VIVO_TS_LOG_ERR("Didn't get the flag to continue!");
        return FAIL;
    }
...
ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
	...
	{
		VIVO_TS_LOG_INF("Goodix_tool_write System suspend");
		if(ts->gtp_rawdiff_mode == true)
    ...
    VIVO_TS_LOG_DBG("[Operation]wr: %02X", cmd_head.wr);
    VIVO_TS_LOG_DBG("[Flag]flag: %02X, addr: %02X%02X, value: %02X, relation: %02X", cmd_head.flag, cmd_head.flag_addr[0], 
                        cmd_head.flag_addr[1], cmd_head.flag_val, cmd_head.flag_relation);
    VIVO_TS_LOG_DBG("[Retry]circle: %d, times: %d, retry: %d, delay: %d", (s32)cmd_head.circle, (s32)cmd_head.times, 
                        (s32)cmd_head.retry, (s32)cmd_head.delay);
    VIVO_TS_LOG_DBG("[Data]data len: %d, addr len: %d, addr: %02X%02X, buffer len: %d, data[0]: %02X", (s32)cmd_head.data_len, 
    ...
    if (1 == cmd_head.wr)
    {
        ...
        if (1 == cmd_head.flag)
        {
            if (FAIL == comfirm())
            {
                VIVO_TS_LOG_ERR("[WRITE]Comfirm fail!");
                return -EPERM;
            }
        }
        ...
        {
            VIVO_TS_LOG_ERR("[WRITE]Write data failed!");
            return -EPERM;
        }
    ...
    else if(17 == cmd_head.wr)
    {
        ...
        if(ret)
        {
            VIVO_TS_LOG_DBG("copy_from_user failed.");
            return -EPERM;
        }
        if(cmd_head.data[GTP_ADDR_LENGTH])
        {
            VIVO_TS_LOG_INF("gtp enter rawdiff.");
            ts->gtp_rawdiff_mode = true;
        }
        else
        {
            ts->gtp_rawdiff_mode = false;
            VIVO_TS_LOG_INF("gtp leave rawdiff.");
        }
    }
#ifdef UPDATE_FUNCTIONS
...
ssize_t goodix_tool_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	if(ts->gtp_is_suspend)
	{
		VIVO_TS_LOG_INF("Goodix_tool_read System suspend");
		if(ts->gtp_rawdiff_mode == true)
 			ts->gtp_rawdiff_mode = false;
 			return -1;
	}
    ...
    else if (!cmd_head.wr)
    {
        ...
        if (1 == cmd_head.flag)
        {
            if (FAIL == comfirm())
            {
                VIVO_TS_LOG_ERR("[READ]Comfirm fail!");
                return -EPERM;
            }
        }
        else if (2 == cmd_head.flag)
        ...
        VIVO_TS_LOG_DBG("[CMD HEAD DATA] ADDR:0x%02x%02x.", cmd_head.data[0], cmd_head.data[1]);
        VIVO_TS_LOG_DBG("[CMD HEAD ADDR] ADDR:0x%02x%02x.", cmd_head.addr[0], cmd_head.addr[1]);
        ...
        while(data_len > 0)
        {
            ...
            {
                VIVO_TS_LOG_ERR("[READ]Read data failed!");
                return -EPERM;
            }
