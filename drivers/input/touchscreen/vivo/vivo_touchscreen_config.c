 ...
 * =====================================================================================
 *
 *       Filename:  vivo_touchscreen_config.c
 *
 *    Description:  the touchscreen configure file.It loads the touchscreen settings for 
 *                  the current product which pre-settled in relactive device tree and
 *                  which for configuring the touchscreen IC the current product used
 *                  and detailing different functions for the product such as double-click,
 *                  anti-esd and so on. 
 *
 *        Version:  1.0
 *        Created:  03/13/2015 10:11:52 AM
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  chenyunzhe,chenyunzhe@vivo.com.cn 
 *        Company:  vivo
 *
 * =====================================================================================
 */
...
static void vivo_touchscreen_config_load_module_id_gpio(struct platform_device *pdev,touchscreen_config *ts_cfg)
{
    ...
    //rc = of_property_read_u32(pdev->dev.of_node, "vivo,ts-module-id1-gpio", &int_val);
	...
	//rc = of_property_read_u32(pdev->dev.of_node, "vivo,ts-module-id2-gpio", &int_val);
...
static int vivo_touchscreen_config_load_device_tree(struct platform_device *pdev,touchscreen_config *ts_cfg)
{
	...
	if(rc < 0)  {
		VIVO_TS_LOG_ERR(" load prj name error. \n");
	...
	if(rc < 0)  {
		VIVO_TS_LOG_ERR(" load touchscreen's vdd mode is error. \n");
		// goto out;
	}
	
	rc = vivo_touchscreen_config_load_use_ic(pdev,ts_cfg);
	if(rc < 0)  {
		VIVO_TS_LOG_ERR(" load use_ic error. \n");
