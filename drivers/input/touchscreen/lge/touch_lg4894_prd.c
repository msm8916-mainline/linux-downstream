...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata)
{
	...
	lg4894_reg_write(dev, rawdata_ctl_write, (u8 *)&wdata, sizeof(wdata));
	TOUCH_I("wdata = %d\n", wdata);
	...
	do {
		...
		if (retry != 1000)
			touch_msleep(5);
		lg4894_reg_read(dev, rawdata_ctl_read,
	...
	lg4894_reg_write(dev, tc_tsp_test_data_offset, (u8 *)&data_offset, sizeof(u32));

	lg4894_reg_read(dev, tc_tsp_data_access_addr, (u8 *)buf, __frame_size);
