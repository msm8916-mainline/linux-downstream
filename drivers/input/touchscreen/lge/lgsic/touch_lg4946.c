...
static void lg4946_get_swipe_info(struct device *dev)
{
	...
	d->swipe.mode = SWIPE_DOWN_BIT;/* | SWIPE_UP_BIT; */
}
		...
		u32 x1, u32 y1, u32 x2, u32 y2)
{
	...
	ret = lg4946_reg_write(dev, ACT_AREA_X1_W,
			&x1, sizeof(x1));
	ret = lg4946_reg_write(dev, ACT_AREA_Y1_W,
			&y1, sizeof(y1));
	ret = lg4946_reg_write(dev, ACT_AREA_X2_W,
			&x2, sizeof(x2));
	ret = lg4946_reg_write(dev, ACT_AREA_Y2_W,
			&y2, sizeof(y2));
	...
	if (d->lcd_mode != LCD_MODE_U3) {
		if (d->lcd_mode == LCD_MODE_U2) {
			if (d->driving_mode == LCD_MODE_U2_UNBLANK) {
				TOUCH_I("U1 -> U2 mode change\n");
				lg4946_watch_init(dev);
				// knockon mode change + swipe enable
				lg4946_tc_driving(dev, LCD_MODE_U2);
				ret = 1;
			} else {
			...
			case LCD_MODE_STOP:
				...
				lg4946_watch_display_off(d->dev);
				ret = 1;
				break;
			case LCD_MODE_U2:
				TOUCH_I("U2 -> U1 mode change\n");
				lg4946_watch_display_off(dev);
				// abs mode change + swipe disable
				lg4946_tc_driving(dev, LCD_MODE_U2_UNBLANK);
				ret = 1;
				break;
			case LCD_MODE_U0:
...
static int lg4946_init(struct device *dev)
{
	...
	if (lg4946_asc_usable(dev)) {	/* ASC */
		if (atomic_read(&ts->state.core) == CORE_UPGRADE)
			lg4946_asc_get_fw_sensitivity(dev);
...
int lg4946_debug_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct lg4946_data *d = to_lg4946_data(dev);
...
static int lg4946_irq_abs_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct lg4946_data *d = to_lg4946_data(dev);
