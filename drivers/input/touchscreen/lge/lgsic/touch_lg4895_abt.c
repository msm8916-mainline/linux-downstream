...
int lg4895_sic_abt_is_set_func(void)
{
...
static uint32_t abt_ksocket_rcv_from_pctool(uint8_t *buf, uint32_t len)
{
	...
	if (abt_comm.recv_packet->hdr.cmd == TCP_REG_READ) {
		ret = lg4895_reg_read(abt_comm.dev,
		...
		ret = lg4895_reg_write(abt_comm.dev,
					abt_comm.recv_packet->hdr.addr,
...
static uint32_t abt_ksocket_rcv_from_abtstudio(uint8_t *buf, uint32_t len)
{
	...
	case CMD_RAW_DATA_COMPRESS_WRITE:
		lg4895_write_value(abt_comm.dev,
...
void lg4895_sic_abt_probe(void)
{
	mutex_init(&abt_comm_lock);
}

void lg4895_sic_abt_remove(void)
{
...
void lg4895_sic_abt_init(struct device *dev)
{
	u32 head_loc[4];
	...
	lg4895_read_value(dev, CMD_ABT_LOC_X_START_READ, &head_loc[0]);
	lg4895_read_value(dev, CMD_ABT_LOC_X_END_READ, &head_loc[1]);
	lg4895_read_value(dev, CMD_ABT_LOC_Y_START_READ, &head_loc[2]);
	lg4895_read_value(dev, CMD_ABT_LOC_Y_END_READ, &head_loc[3]);

	tHeadBuffer.resolution_x = 720;
	tHeadBuffer.resolution_y = 1360;
...
int lg4895_sic_abt_is_debug_mode(void)
{
...
int lg4895_sic_abt_irq_handler(struct device *dev)
{
	struct lg4895_data *d = to_lg4895_data(dev);
	int ret = 0;
	u8 all_data[264];

	int report_mode = lg4895_sic_abt_is_set_func();
	if (lg4895_reg_read(dev, tc_ic_status,
	...
	if (report_mode) {
		...
		lg4895_sic_abt_report_mode(dev, all_data);
	} else {
		if (d->info.wakeup_type == ABS_MODE) {
			if (lg4895_sic_abt_is_debug_mode()) {
				lg4895_sic_abt_onchip_debug(dev, &all_data[8]);
				ret = lg4895_irq_abs(dev);
			} else {
				lg4895_sic_abt_ocd_off(dev);
				ret = lg4895_irq_abs(dev);
			}
		} else {
			ret = lg4895_irq_lpwg(dev);
...
void lg4895_sic_abt_ocd_off(struct device *dev)
{
	u32 wdata = 0;

	if (abt_ocd_off) {
		lg4895_write_value(dev, CMD_ABT_OCD_ON_WRITE, wdata);
		lg4895_read_value(dev, CMD_ABT_OCD_ON_READ, &wdata);
		TOUCH_I("[ABT] onchipdebug off: wdata=%d\n", wdata);
		abt_ocd_off = 0;
	}
}

void lg4895_sic_abt_onchip_debug(struct device *dev, u8 *all_data)
{
	u32			i, j;
	u32			wdata;
	...
	u16 nOffsetValueByteSize, u8 *pData, u32 nSize)
{
	...
	while (nRead > 0) {
		lg4895_reg_write(dev, offset_addr, (u8 *)&nOffset,
			nOffsetValueByteSize);
		nRead = lg4895_reg_read(dev, nMemAddr, pData, nRead);
		if (nRead <= 0)
			return nRead;
...
void lg4895_sic_abt_report_mode(struct device *dev, u8 *all_data)
{
	...
	struct	lg4895_touch_info *t_info = (struct lg4895_touch_info *) all_data;
		...
		if (t_info->data[0].track_id != 15) {
			if (abt_report_point) {
				...
				memcpy(d_data_ptr, &t_info->data[0],
					sizeof(struct lg4895_touch_data) * t_info->touch_cnt);
				d_data_ptr += sizeof(struct lg4895_touch_data)
					* t_info->touch_cnt;
			} else
				packet_ptr->touchCnt = 0;
		}
...
static struct attribute *lg4895_abt_attribute_list[] = {
	&touch_attr_abt_monitor.attr,
	&touch_attr_raw_report.attr,
	NULL,
};

static const struct attribute_group lg4895_abt_attribute_group = {
	.attrs = lg4895_abt_attribute_list,
};

void lg4895_sic_abt_register_sysfs(struct kobject *kobj)
{
	int ret = sysfs_create_group(kobj, &lg4895_abt_attribute_group);
