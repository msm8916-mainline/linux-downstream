...
static int lg4945_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&d->global_reset) == 1) {
		TOUCH_I(">> ABNORMAL, SKIP LPWG_MODE, global_reset = 1\n");
		return 0;
	}
...
static int lg4945_notify(struct device *dev, ulong event, void *data)
{
	...
	case NOTIFY_TOUCH_RESET:
		...
		atomic_set(&d->global_reset, 1);
		break;

	case LCD_EVENT_LCD_MODE:
		TOUCH_I("LCD_EVENT_LCD_MODE!\n");
		atomic_set(&d->global_reset, 0);
...
#if defined(CONFIG_LGD_INCELL_PHASE3_VIDEO_HD_PT_PANEL)
			//if (ts->fw_info.need_rewrite_firmware) /* Not implemented in lg4945 */
				mdss_lcd_lut_update();
#endif
