...
static u32 M2_34ch_specup_check;
...
static int prd_print_rawdata(struct device *dev, char *buf, u8 type)
{
	...
	if (!M2_34ch_specup_check) {
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"\n34CH Rawdata Spec UP ! \n");
	}
...
static int prd_compare_rawdata(struct device *dev, u8 type)
{
	...
	for (i = 0; i < ROW_SIZE; i++) {
		if (i == 33 && !M2_34ch_specup_check 
			&& type == DOZE1_M2_RAWDATA_TEST) {
			snprintf(lower_str, sizeof(lower_str),
					"DOZE1_M2_34Ch_Lower");
			snprintf(upper_str, sizeof(upper_str),
					"DOZE1_M2_34Ch_Upper");
...
static int prd_rawdata_test(struct device *dev, u8 type)
{
	...
	lg4945_reg_read(dev, tc_tsp_test_m2_34ch_raw_data,
				(u8 *)&M2_34ch_specup_check,
				sizeof(M2_34ch_specup_check));
	TOUCH_I("34CH SPEC %s : %d",
		M2_34ch_specup_check ? "DOWN" : "UP", M2_34ch_specup_check);
...
static int prd_print_noise(struct device *dev, char *buf)
{
	int i = 0, j = 0;
	int ret = 0;
	int min = 9999;
	int max = 0;
	u16 *rawdata_buf = M1_Rawdata_buf;
	int col_size = M1_COL_SIZE;
...
static int check_noise_test(struct device *dev)
{
	int result = 0;
	char noise_test_cnt_str[64] = {0, };
	int noise_test_cnt = 0;
	int count = 0;
	char buffer[1024] = {0, };
	int ret = 0;

	snprintf(noise_test_cnt_str, sizeof(noise_test_cnt_str),
			"NOISE_COUNT");

	noise_test_cnt = sic_get_limit(dev, noise_test_cnt_str);
	TOUCH_I("[NOISE_TEST] COUNT SPEC = %d\n", noise_test_cnt);

	lg4945_reg_read(dev, tc_tsp_test_noise_cnt,
			(u8 *)&count, sizeof(count));
	TOUCH_I("[NOISE_TEST] Check count = %d\n", count);

	if (count >= noise_test_cnt) {
		result = 1;
		TOUCH_I("noise_result = %d\n", result);
		/* Test Type Write */
		ret = snprintf(buffer, 1024, "[NOISE_TEST]\n");
		ret += snprintf(buffer + ret, 1024 - ret,
			"SKIP Test - Already Failed, count = %d\n\n", count);
		write_file(dev, buffer, TIME_INFO_SKIP);
	}

	/* return 1 - already fail
	     return 0 - pass */
	return result;
}

static void write_noise_test(struct device *dev)
{
	u32 testmode = 0;
	int retry = 5;
	u32 rdata = 0;

	/* write noise test result fail*/
	testmode = 0xA;
	/* TestType Set */
	lg4945_reg_write(dev, tc_tsp_test_ctl,
				(u8 *)&testmode, sizeof(testmode));
	...
	TOUCH_I("write noise_test FAIL\n");
}

static int prd_noise_test(struct device *dev)
{
	char test_type[32] = {0, };
	u32 testmode = 0;
	u32 rdata = 0x01;
	int retry = 50;
	char noise_th_str[64] = {0, };
	char noise_frame_str[64] = {0, };
	u32 noise_th = 0;
	u32 noise_frame = 0;
	int noise_result = 0;
	int ret = 0;
	...
	/* 1. spec read */
	snprintf(noise_th_str, sizeof(noise_th_str),
			"NOISE_TH");
	snprintf(noise_frame_str, sizeof(noise_frame_str),
			"NOISE_FRAME");

	noise_th = sic_get_limit(dev, noise_th_str);
	noise_frame = sic_get_limit(dev, noise_frame_str);

	lg4945_reg_write(dev, tc_tsp_test_noise_th,
			(u8 *)&noise_th,
			sizeof(noise_th));
	TOUCH_I("noise_th = %d\n", noise_th);

	lg4945_reg_write(dev, tc_tsp_test_noise_frame_cnt,
			(u8 *)&noise_frame,
			sizeof(noise_frame));
	TOUCH_I("noise_frame = %d\n", noise_frame);
	...
	if (rdata != 0xAA) {
		TOUCH_I("ProductionTest Type [NOISE_TEST] Time out\n");
		noise_result = 1;
		return noise_result;
	}
	...
	/* 3. get result */
	lg4945_reg_read(dev, tc_tsp_test_pf_result,
			(u8 *)&noise_result, sizeof(noise_result));
	TOUCH_I("noise_result = %d\n", noise_result);

	/* 4. value print */
	memset(M1_Rawdata_buf, 0, sizeof(M1_Rawdata_buf));
	lg4945_reg_read(dev, tc_tsp_test_noise_p2p,
			(u8 *)&M1_Rawdata_buf, sizeof(M1_Rawdata_buf));
	memset(W_Buf, 0, BUF_SIZE);
	ret = prd_print_noise(dev, W_Buf);

	ret += snprintf(W_Buf + ret, PAGE_SIZE - ret,
			"NOISE_TEST spec : %d frame : %d\n",
			noise_th, noise_frame);
	ret += snprintf(W_Buf + ret, PAGE_SIZE - ret, "NOISE_TEST : %s\n\n",
			noise_result ? "Fail" : "Pass");
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	int ret = 0;
	int mfts_mode = 0;
	u32 noise_test_done = 0;
	int noise_test_needed = 0;
	...
	mfts_mode = touch_mfts_mode_check(dev);
	lg4945_reg_read(dev, tc_tsp_test_noise_done,
			(u8 *)&noise_test_done, sizeof(noise_test_done));

	if ((mfts_mode == MFTS_FLAT) && (noise_test_done != 1))
		noise_test_needed = 1;
	...
	/*
		...
		noise - pass : 0, fail : 1
	*/
	if (noise_test_needed) {
		if (check_noise_test(dev) == 0)
			noise_ret = prd_noise_test(dev);
		else
			noise_ret = 1;
	}
	/*
	...
	*/
	if (noise_test_needed) {
		...
		/* write noise_result when all test item fail */
		if ((noise_ret == 1) && (rawdata_ret == 0) &&
				(openshort_ret == 0)) {
			write_noise_test(dev);
		}
	}
