...
static int rmi_f54_init(struct rmi_function_container *fc)
{
	...
	VIVO_TS_LOG_INF("[%s]:Intializing F54.", __func__);
...
static int rmi_f54_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f54) {
		VIVO_TS_LOG_ERR("[%s]:Failed to allocate rmi_fn_54_data.\n", __func__);
		return -ENOMEM;
	}
	fc->data = f54;
	f54->fc = fc;
...
static int rmi_f54_reset(struct rmi_function_container *fc)
{
	...
	if (driver->restore_irq_mask) {
		VIVO_TS_LOG_DBG("[%s]:Restoring interupts!\n", __func__);
		driver->restore_irq_mask(fc->rmi_dev);
	} else {
		VIVO_TS_LOG_ERR("[%s]:No way to restore interrupts!\n", __func__);
	}
...
static void rmi_f54_remove(struct rmi_function_container *fc)
{
	...
	VIVO_TS_LOG_INF("[%s]:Removing F54.", __func__);
...
static int rmi_f54_create_sysfs(struct rmi_function_container *fc)
{
	int reg_num;
	int retval;
	VIVO_TS_LOG_INF("[%s]:Creating sysfs files.", __func__);
	...
	for (reg_num = 0; reg_num < ARRAY_SIZE(attrs_ctrl_regs); reg_num++) {
		if (attrs_ctrl_regs_exist[reg_num]) {
			if (sysfs_create_group(&fc->dev.kobj,
				...
				VIVO_TS_LOG_ERR("[%s]:Failed to create sysfs file group for reg group %d.", __func__, reg_num);
				return -ENODEV;
			}
		}
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to create sysfs file for F54 data (error = %d).\n", __func__, retval);
		return -ENODEV;
	}
	return 0;
}
...
static int rmi_f54_initialize(struct rmi_function_container *fc)
{
    ...
    init_waitqueue_head(&instance_data->wait);
	atomic_set(&instance_data->data_read_done, 0);
...
int rmi_f54_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	if (data->report_size == 0) {
		VIVO_TS_LOG_ERR("[%s]:Invalid report type set "
	...
	if (data->bufsize < data->report_size) {
		...
		if (!data->report_data) {
			VIVO_TS_LOG_ERR("[%s]:Failed to allocate report_data.\n", __func__);
			error = -ENOMEM;
			data->bufsize = 0;
	...
	VIVO_TS_LOG_DBG("[%s]:F54 Interrupt handler is running.\nSize: %d\n", __func__,
		 data->report_size);
	...
	if (error < 0)
		VIVO_TS_LOG_ERR("[%s]:Failed to write fifo to zero!\n", __func__);
	else
		error = rmi_read_block(fc->rmi_dev,
			fc->fd.data_base_addr + RMI_F54_REPORT_DATA_OFFSET,
			data->report_data, data->report_size);
	if (error < 0)
		VIVO_TS_LOG_ERR("[%s]:F54 data read failed. Code: %d.\n", __func__, error);
	else if (error != data->report_size) {
		VIVO_TS_LOG_ERR("[%s]:F54 data read data size error. Code: %d.\n", __func__, error);
		error = -EINVAL;
		goto error_exit;
	}
#if RAW_HEX
	...
	VIVO_TS_LOG_INF("[%s]:Report data (raw hex):\n", __func__);
	for (l = 0; l < data->report_size; l += 2) {
		//pr_info("%03d: 0x%02x%02x\n", l/2,
		VIVO_TS_LOG_INF("[%s]:%03d: 0x%02x%02x\n", __func__, l/2,
	...
	case F54_FULL_RAW_CAP:
	case F54_FULL_RAW_CAP_RX_COUPLING_COMP:
		VIVO_TS_LOG_INF("[%s]:Report data (Image):\n", __func__);
		int i, j, k;
		char c[2];
		short s;
		k = 0;
		...
		break;
	default:
		VIVO_TS_LOG_ERR("[%s]:Report type %d debug image not supported", __func__,
							data->report_type);
	}
#endif
	error = IDLE;
error_exit:
	...
	atomic_set(&data->data_read_done, 1);
...
static void clear_status_worker(struct work_struct *work)
{
	...
	mutex_lock(&data->status_mutex);
	if (data->status == BUSY) {
		VIVO_TS_LOG_INF("[%s]:F54 Timout Occured: Determining status.\n", __func__);
		result = rmi_read_block(fc->rmi_dev, fc->fd.command_base_addr,
								&command, 1);
		if (result < 0) {
			VIVO_TS_LOG_ERR("[%s]:Could not read get_report register "
				"from 0x%04x\n", __func__, fc->fd.command_base_addr);
			data->status = -ETIMEDOUT;
		} else {
			if (command & GET_REPORT) {
				VIVO_TS_LOG_ERR("[%s]:Report type unsupported!", __func__);
				data->status = -EINVAL;
			} else {
				data->status = -ETIMEDOUT;
			}
...
static int rmi_fn_54_report_type_set(struct rmi_function_container *fc,
								unsigned char val)
{
	int result;
	unsigned char data;
	...
	if (!is_report_type_valid(val)) {
		VIVO_TS_LOG_ERR("[%s]: Report type %d is invalid.\n",
					__func__, (u8) val);
		return -EINVAL;
	}
	...
	if (instance_data->status != BUSY) {
		...
		if (result < 0) {
			VIVO_TS_LOG_ERR("[%s]: Could not write report type to"
				" 0x%x\n", __func__, fc->fd.data_base_addr);
			return result;
		}
		return 0;
	} else {
		VIVO_TS_LOG_ERR("[%s]: Report type cannot be changed in the middle"
				" of command.\n", __func__);
		mutex_unlock(&instance_data->status_mutex);
		return -EINVAL;
	}
...
static ssize_t rmi_fn_54_report_type_store(struct device *dev,
	...
	result = rmi_fn_54_report_type_set(fc, (unsigned char)val);
	if (result < 0) {
		return result;
	}else{
		return count;
...
static int rmi_fn_54_get_report(struct rmi_function_container *fc)
{
	struct rmi_fn_54_data *instance_data;
	...
	if (instance_data->status == -ECONNRESET) {
		VIVO_TS_LOG_ERR("[%s]:Device has been reset force status to idle\n", __func__);
		instance_data->status = IDLE;
	}

	if (instance_data->status != IDLE) {
		if (instance_data->status != BUSY) {
			VIVO_TS_LOG_ERR("[%s]:F54 status is in an abnormal state: 0x%x", __func__,
	 ...
	 */
	VIVO_TS_LOG_DBG("[%s]:Storing and overriding interupts\n", __func__);
	if (driver->store_irq_mask)
		driver->store_irq_mask(fc->rmi_dev,
					fc->irq_mask);
	else
		VIVO_TS_LOG_ERR("[%s]:No way to store interupts!\n", __func__);
...
static ssize_t rmi_fn_54_no_auto_cal_store(struct device *dev,
	...
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write control to 0x%x\n",
...
static ssize_t rmi_fn_54_fifoindex_show(struct device *dev,
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Could not read fifoindex from 0x%04x\n", __func__,
		       fc->fd.data_base_addr + RMI_F54_FIFO_OFFSET);
		return retval;
	}
					...
					size_t count)
{
	...
	if (error < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write fifoindex to 0x%x\n",
...
int rmi_dev_get_fn54_data(struct rmi_function_container *fc, char **dest_data,
					unsigned char function_number) 
{
	...
	error = rmi_fn_54_report_type_set(fc, function_number);
	if (error < 0) {
		VIVO_TS_LOG_ERR("[%s]:Set report type error!\n", __func__);
		return error;
	}

	error = rmi_fn_54_get_report(fc);
	if (error < 0) {
		VIVO_TS_LOG_ERR("[%s]:Get report error!\n", __func__);
		return error;
	}

	timeout = wait_event_interruptible_timeout(instance_data->wait, 
						atomic_read(&instance_data->data_read_done),
						msecs_to_jiffies(150));

	/* only one wait queue no need to check the data_read_done status */
	if (instance_data->status != IDLE) {
		VIVO_TS_LOG_ERR("[%s]:Fn54 status(%d) not idle!\n", __func__, instance_data->status);
		return -1;
	}

	*dest_data = instance_data->report_data;
	return 0;
...
#endif
{
...
#ifdef CONFIG_ARM64
		VIVO_TS_LOG_ERR("[%s]: F54 report size too large for buffer: %ld."
				" Need at least: %d for Report type: %d.\n",
			__func__, count, instance_data->report_size,
			instance_data->report_type);
#else
		VIVO_TS_LOG_ERR("[%s]: F54 report size too large for buffer: %d."
				" Need at least: %d for Report type: %d.\n",
			__func__, count, instance_data->report_size,
			instance_data->report_type);
#endif
	...
	if (instance_data->report_data) {
		...
		mutex_unlock(&instance_data->data_mutex);
		VIVO_TS_LOG_DBG("[%s]: Presumably successful.", __func__);
		return instance_data->report_size;
	} else {
		VIVO_TS_LOG_ERR("[%s]: F54 report_data does not exist!\n", __func__);
		mutex_unlock(&instance_data->data_mutex);
		return -EINVAL;
	}
}
...
static ssize_t rmi_fn_54_burst_count_show(struct device *dev,
	...
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not read control at 0x%x\n"
