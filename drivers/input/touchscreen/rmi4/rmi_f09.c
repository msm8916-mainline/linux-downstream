...
static int rmi_f09_init(struct rmi_function_container *fc)
{
	...
	VIVO_TS_LOG_INF("[%s]:Intializing F09 values.", __func__);
...
static int rmi_f09_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f09) {
		VIVO_TS_LOG_ERR("[%s]:Failed to allocate rmi_fn_09_data.\n", __func__);
		return -ENOMEM;
	}
	fc->data = f09;
...
static int rmi_f09_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	if (data->status == ECONNRESET) {
		VIVO_TS_LOG_ERR("[%s]:RESET occured: %#04x.\n", __func__,
			data->status);
	} else if (data->cmd.run_bist) {
		VIVO_TS_LOG_ERR("[%s]:Command register not cleared: %#04x.\n", __func__,
			data->cmd.run_bist);
	}
	return 0;
}
					  ...
					  const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u", &new_value) != 1) {
		VIVO_TS_LOG_ERR("[%s]: Error - hostTestEnable has an invalid length.\n",
			__func__);
		return -EINVAL;
	}

	if (new_value < 0 || new_value > 1) {
		VIVO_TS_LOG_ERR("[%s]: Invalid hostTestEnable bit %s.",
			__func__, buf);
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.query_base_addr,
		data->query.host_test_enable);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write hostTestEnable to 0x%x\n",
					...
					const char *buf, size_t count)
{
	...
	data = fc->data;
	if (sscanf(buf, "%u", &new_value) != 1) {
		VIVO_TS_LOG_ERR("[%s]: Error - run_bist_store has an "
		"invalid len.\n",
		__func__);
		return -EINVAL;
	}
	...
	if (new_value < 0 || new_value > 1) {
		VIVO_TS_LOG_ERR("[%s]: Invalid run_bist bit %s.", __func__, buf);
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.command_base_addr,
		data->cmd.run_bist);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write run_bist_store to 0x%x\n",
					...
					const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%u %u %u", &new_low, &new_high, &new_diff) != 3) {
		VIVO_TS_LOG_ERR("[%s]: Error - f09_control_test1_store has an "
		"invalid len.\n",
		__func__);
		return -EINVAL;
	}
	...
	if (new_low < 0 || new_low > 1 || new_high < 0 || new_high
			|| new_diff < 0 || new_diff) {
		VIVO_TS_LOG_ERR("[%s]: Invalid f09_control_test1_diff bit %s.",
			__func__, buf);
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test1_limit_low);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write f09_control_test1_limit_low to 0x%x\n",
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test1_limit_high);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write f09_control_test1_limit_high to 0x%x\n",
				__func__, fc->fd.control_base_addr);
		return result;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test1_limit_diff);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write f09_control_test1_limit_diff to 0x%x\n",
				__func__, fc->fd.control_base_addr);
		return result;
	}
					...
					const char *buf, size_t count)
{
	...
	if (new_low < 0 || new_low > 1 || new_high < 0 || new_high > 1 ||
		...
		VIVO_TS_LOG_ERR("[%s]: Invalid f09_control_test2_diff bit %s.",
			__func__, buf);
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test2_limit_low);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write f09_control_test2_limit_low to 0x%x\n",
				__func__, fc->fd.control_base_addr);
		return result;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test2_limit_high);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write f09_control_test2_limit_high to 0x%x\n",
				__func__, fc->fd.control_base_addr);
		return result;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->control.test2_limit_diff);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write f09_control_test2_limit_diff to 0x%x\n",
				__func__, fc->fd.control_base_addr);
		return result;
	}
					...
					const char *buf, size_t count)
{
	...
	data = fc->data;
	if (sscanf(buf, "%u", &new_value) != 1) {
		VIVO_TS_LOG_ERR("[%s]: Error - test_number_control_store has an "
		"invalid len.\n",
		__func__);
		return -EINVAL;
	}
	...
	if (new_value < 0 || new_value > 1) {
		VIVO_TS_LOG_ERR("[%s]: Invalid test_number_control bit %s.",
			__func__, buf);
		return -EINVAL;
	}
	...
	result = rmi_write(fc->rmi_dev, fc->fd.control_base_addr,
		data->data.test_number_control);
	if (result < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write "
