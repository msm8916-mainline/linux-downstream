...
static int rmi_f34_init(struct rmi_function_container *fc)
{
	...
	VIVO_TS_LOG_INF("[%s]:Intializing f34 values.", __func__);
...
static int rmi_f34_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f34) {
		VIVO_TS_LOG_ERR("[%s]:Failed to allocate rmi_fn_34_data.\n", __func__);
		return -ENOMEM;
	}
	fc->data = f34;
...
static int rmi_f34_initialize(struct rmi_function_container *fc)
{
	...
	VIVO_TS_LOG_DBG("[%s]:Initializing F34 values for %s.\n", __func__,
		pdata->sensor_name);
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Could not read bootloaderid from 0x%04x.\n", __func__,
			query_base_addr);
		return retval;
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Could not read block size from 0x%04x, "
			"error=%d.\n", __func__, query_base_addr + BLK_SZ_OFF, retval);
		return retval;
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Couldn't read image block count from 0x%x, "
			"error=%d.\n", __func__, query_base_addr + IMG_BLK_CNT_OFF,
			retval);
		return retval;
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Couldn't read config block count from 0x%x, "
			"error=%d.\n", __func__, query_base_addr + CFG_BLK_CNT_OFF,
			retval);
		return retval;
	}
...
static int rmi_f34_create_sysfs(struct rmi_function_container *fc)
{
	...
	rc = sysfs_create_bin_file(&fc->dev.kobj,
				&dev_attr_data);
	if (rc < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to create sysfs file for F34 data "
		     "(error = %d).\n", __func__, rc);
		return -ENODEV;
	}
...
static int f34_read_status(struct rmi_function_container *fc)
{
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Could not read status from 0x%x\n", __func__,
					...
					size_t count)
{
	...
	if (error < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write bootloader id to 0x%x\n",
				...
				size_t count)
{
	...
	if ((val != F34_ENABLE_FLASH_PROG) && !instance_data->inflashprogmode) {
		VIVO_TS_LOG_ERR("[%s]: CANNOT SEND CMD %d TO SENSOR - "
	...
	case F34_WRITE_CONFIG_BLOCK:
		...
		if (error < 0) {
			VIVO_TS_LOG_ERR("[%s]: Could not write command 0x%02x "
		...
		break;
	default:
		VIVO_TS_LOG_DBG("[%s]: RMI4 function $34 - "
					...
					size_t count)
{
	...
	if (error < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write block number %u to 0x%x\n",
				     ...
				     const char *buf, size_t count)
{
	...
	if (instance_data->cmd != F34_ENABLE_FLASH_PROG) {
		VIVO_TS_LOG_ERR("[%s]: NOT IN FLASH PROG MODE - CAN'T RESCAN PDT.\n"
				, __func__);
		return -EINVAL;
	}
	...
	if (rescan) {
		...
		for (i = PDT_START_SCAN_LOCATION; i >= PDT_END_SCAN_LOCATION;
			...
			if (retval != sizeof(pdt_entry)) {
				VIVO_TS_LOG_ERR("[%s]: err frm rmi_read_block pdt "
					"entry data from PDT, "
		...
		if (!fn01found || !fn34found) {
			VIVO_TS_LOG_ERR("[%s]: failed to find fn$01 or fn$34 trying "
				"to do rescan PDT.\n"
				, __func__);
			return -EINVAL;
		}
...
#endif
{
...
#ifdef CONFIG_ARM64
		VIVO_TS_LOG_ERR("[%s]: Incorrect F34 block size %ld. "
			"Expected size %d.\n",
			__func__, count, instance_data->blocksize);
#else
		VIVO_TS_LOG_ERR("[%s]: Incorrect F34 block size %d. "
			"Expected size %d.\n",
			__func__, count, instance_data->blocksize);
#endif
		return -EINVAL;
	...
	if (error < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not read data from 0x%04x\n",
		       __func__, data_base_addr + BLK_NUM_OFF);
		return error;
	}
...
#endif
{
	...
	if (count) {
		...
		if (error < 0) {
			VIVO_TS_LOG_ERR("[%s]: Could not write block data "
				"to 0x%x\n", __func__,
				data_base_addr + BLK_NUM_OFF);
			return error;
		}
