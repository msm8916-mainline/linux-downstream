				     ...
				     const char *buf, size_t count)
{
	...
	if (reset) {
		...
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]: failed to issue reset command, "
				     ...
				     const char *buf, size_t count)
{
	...
	if (allow_doze) {
		...
		retval = rmi_write_block(fc->rmi_dev, fc->fd.command_base_addr,
				&commands.reg, sizeof(commands.reg));
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]: failed to issue allow_doze command, "
				"error = %d.", __func__, retval);
			return retval;
		}
	}
					 ...
					 size_t count)
{
	...
	if (retval < 0 || !RMI_IS_VALID_SLEEPMODE(new_value)) {
		VIVO_TS_LOG_ERR("[%s]: Invalid sleep mode %s.", __func__, buf);
		return -EINVAL;
	}

	VIVO_TS_LOG_DBG("[%s]:Setting sleep mode to %ld.", __func__, new_value);
	...
	if (retval >= 0)
		...
		VIVO_TS_LOG_ERR("[%s]:Failed to write sleep mode, code %d.\n", __func__, retval);
	return retval;
}
				       ...
				       size_t count)
{
	...
	if (retval < 0 || new_value < 0 || new_value > 1) {
		VIVO_TS_LOG_ERR("[%s]: Invalid nosleep bit %s.", __func__, buf);
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		VIVO_TS_LOG_ERR("[%s]:Failed to write nosleep bit.\n", __func__);
	return retval;
}
				       ...
				       size_t count)
{
	...
	retval = strict_strtoul(buf, 10, &new_value);
	if (retval < 0 || new_value < 0 || new_value > 1) {
		VIVO_TS_LOG_ERR("[%s]: Invalid chargerinput bit %s.", __func__, buf);
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		VIVO_TS_LOG_ERR("[%s]:Failed to write chargerinput bit.\n", __func__);
	return retval;
}
				       ...
				       size_t count)
{
	...
	retval = strict_strtoul(buf, 10, &new_value);
	if (retval < 0 || new_value < 0 || new_value > 1) {
		VIVO_TS_LOG_ERR("[%s]: Invalid reportrate bit %s.", __func__, buf);
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		VIVO_TS_LOG_ERR("[%s]:Failed to write reportrate bit.\n", __func__);
	return retval;
}
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->irq_count; i++) {
		...
		if (len > 0) {
			...
			VIVO_TS_LOG_ERR("[%s]: Failed to build interrupt_enable"
				" buffer, code = %d.\n", __func__, len);
					  ...
					  const char *buf, size_t count)
{
	...
	for (i = 0; i < data->irq_count && *buf != 0;
		...
		if ((result != 1) ||
			...
			VIVO_TS_LOG_ERR("[%s]: Error - interrupt enable[%d]"
	...
	if (irq_count != data->irq_count) {
		VIVO_TS_LOG_ERR("[%s]: Error - interrupt enable count of %d"
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write interrupt_enable_store"
					  ...
					  const char *buf, size_t count)
{
	...
	if (retval < 0 || new_value < 0 || new_value > 255) {
		VIVO_TS_LOG_ERR("[%s]: Invalid doze interval %s.", __func__, buf);
		return -EINVAL;
	}
	...
	ctrl_base_addr = fc->fd.control_base_addr + sizeof(u8) +
			(sizeof(u8)*(data->num_of_irq_regs));
	VIVO_TS_LOG_DBG("[%s]:doze_interval store address %x, value %d", __func__,
	...
	if (retval >= 0)
		...
		VIVO_TS_LOG_ERR("[%s]:Failed to write doze interval.\n", __func__);
	return retval;
					  ...
					  const char *buf, size_t count)
{
	...
	retval = strict_strtoul(buf, 10, &new_value);
	if (retval < 0 || new_value < 0 || new_value > 255) {
		VIVO_TS_LOG_ERR("[%s]: Invalid wakeup threshold %s.", __func__, buf);
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		VIVO_TS_LOG_ERR("[%s]:Failed to write wakeup threshold.\n", __func__);
	return retval;
					  ...
					  const char *buf, size_t count)
{
	...
	retval = strict_strtoul(buf, 10, &new_value);
	if (retval < 0 || new_value < 0 || new_value > 255) {
		VIVO_TS_LOG_ERR("[%s]: Invalid doze holdoff %s.", __func__, buf);
		return -EINVAL;
	}
	...
	if (retval >= 0)
		...
		VIVO_TS_LOG_ERR("[%s]:Failed to write doze holdoff.\n", __func__);
	return retval;
	...
	int num_of_irq_regs)
{
	...
	if (!f01) {
		VIVO_TS_LOG_ERR("[%s]:Failed to allocate fn_01_data.\n", __func__);
		return -ENOMEM;
	}
...
static int rmi_f01_initialize(struct rmi_function_container *fc)
{
	...
	retval = rmi_read_block(rmi_dev, fc->fd.control_base_addr,
			...
			sizeof(data->device_control.ctrl0.reg));
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read F01 control.\n", __func__);
		return retval;
	}
#if 0
	...
	case RMI_F01_NOSLEEP_ON:
		data->device_control.ctrl0.nosleep = 0; /* wyl change from 1 to force nosleep to 0 */
		break;
	}
#endif
	data->device_control.ctrl0.nosleep = 0; /* wyl change from 1 to force nosleep to 0 */
	...
	if (data->device_control.ctrl0.sleep_mode != RMI_SLEEP_MODE_NORMAL) {
		VIVO_TS_LOG_ERR("[%s]:WARNING: Non-zero sleep mode found. Clearing...\n", __func__);
	...
	retval = rmi_write_block(rmi_dev, fc->fd.control_base_addr,
			...
			sizeof(data->device_control.ctrl0.reg));
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to write F01 control.\n", __func__);
		return retval;
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read F01 control interrupt enable register.\n", __func__);
		goto error_exit;
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read Interrupt Status.\n", __func__);
		return retval;
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read device query registers.\n", __func__);
		return retval;
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read product ID.\n", __func__);
		return retval;
	}
	data->product_id[RMI_PRODUCT_ID_LENGTH] = '\0';
	VIVO_TS_LOG_INF("[%s]:found RMI device, manufacturer: %s, product: %s\n", __func__,
	...
	if (data->basic_queries.has_adjustable_doze) {
		...
		if (pdata->power_management.doze_interval) {
			...
			retval = rmi_write(rmi_dev, data->doze_interval_addr,
					data->device_control.doze_interval);
			if (retval < 0) {
				VIVO_TS_LOG_ERR("[%s]:Failed to configure F01 doze interval register.\n", __func__);
				goto error_exit;
			}
		} else {
			retval = rmi_read(rmi_dev, data->doze_interval_addr,
					&data->device_control.doze_interval);
			if (retval < 0) {
				VIVO_TS_LOG_ERR("[%s]:Failed to read F01 doze interval register.\n", __func__);
				goto error_exit;
			}
		}
		...
		if (pdata->power_management.wakeup_threshold) {
			...
			retval = rmi_write(rmi_dev, data->wakeup_threshold_addr,
					data->device_control.wakeup_threshold);
			if (retval < 0) {
				VIVO_TS_LOG_ERR("[%s]:Failed to configure F01 wakeup threshold register.\n", __func__);
				goto error_exit;
			}
		} else {
			retval = rmi_read(rmi_dev, data->wakeup_threshold_addr,
					&data->device_control.wakeup_threshold);
			if (retval < 0) {
				VIVO_TS_LOG_ERR("[%s]:Failed to read F01 wakeup threshold register.\n", __func__);
				goto error_exit;
			}
		}
	}
	...
	if (data->basic_queries.has_adjustable_doze_holdoff) {
		...
		if (pdata->power_management.doze_holdoff) {
			...
			retval = rmi_write(rmi_dev, data->doze_holdoff_addr,
					data->device_control.doze_holdoff);
			if (retval < 0) {
				VIVO_TS_LOG_ERR("[%s]:Failed to configure F01 "
			...
			retval = rmi_read(rmi_dev, data->doze_holdoff_addr,
					&data->device_control.doze_holdoff);
			if (retval < 0) {
				VIVO_TS_LOG_ERR("[%s]:Failed to read F01 doze"
					" holdoff register.\n", __func__);
				goto error_exit;
			}
		}
	...
	if (data->device_status.unconfigured) {
		VIVO_TS_LOG_ERR("[%s]:Device reset during configuration process, status: "
			"%#02x!\n", __func__, data->device_status.status_code);
		retval = -EINVAL;
		goto error_exit;
	}
...
static int rmi_f01_config(struct rmi_function_container *fc)
{
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to write device_control.reg.\n", __func__);
		return retval;
	}
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to write interrupt enable.\n", __func__);
		return retval;
	}
...
int rmi_f01_change_to_sleep_mode(struct rmi_function_container *fc, bool yes)
{
	struct rmi_device *rmi_dev = fc->rmi_dev;
	struct rmi_driver_data *driver_data = rmi_get_driverdata(rmi_dev);
	...
	if (yes) {
		...
		retval = rmi_write_block(rmi_dev,
				...
				sizeof(struct f01_device_control_0));
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to restore normal operation. Code: %d.\n", __func__,
				retval);
		} else {
			retval = 0;
		}
...
static int rmi_f01_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	retval = rmi_read_block(rmi_dev, fc->fd.data_base_addr,
		data->device_status.regs, ARRAY_SIZE(data->device_status.regs));
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read device status, code: %d.\n", __func__,
			retval);
		return retval;
	}
	if (data->device_status.unconfigured) {
		VIVO_TS_LOG_INF("[%s]:Device reset detected.\n", __func__);
