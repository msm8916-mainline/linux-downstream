...
static int f17_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f17->sticks) {
		VIVO_TS_LOG_ERR("[%s]:Failed to allocate per stick data.\n", __func__);
		return -ENOMEM;
	}
...
static int f17_init_stick(struct rmi_device *rmi_dev,
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read stick general query.\n", __func__);
		return retval;
	}
	*next_query_reg += sizeof(stick->query.general.regs);

	VIVO_TS_LOG_DBG("[%s]:Stick %d found\n", __func__, stick->index);
	VIVO_TS_LOG_DBG("[%s]:    Manufacturer: %d.\n", __func__,
				stick->query.general.manufacturer);
	VIVO_TS_LOG_DBG("[%s]:    Resistive:    %d.\n", __func__,
				stick->query.general.resistive);
	VIVO_TS_LOG_DBG("[%s]:    Ballistics:   %d.\n", __func__,
				stick->query.general.ballistics);
	VIVO_TS_LOG_DBG("[%s]:    Manufacturer: %d.\n", __func__,
				stick->query.general.ballistics);
	VIVO_TS_LOG_DBG("[%s]:    Has relative: %d.\n", __func__,
				stick->query.general.has_relative);
	VIVO_TS_LOG_DBG("[%s]:    Has absolute: %d.\n", __func__,
				stick->query.general.has_absolute);
	VIVO_TS_LOG_DBG("[%s]:    Had dribble:  %d.\n", __func__,
				stick->query.general.has_dribble);
	VIVO_TS_LOG_DBG("[%s]:    Has gestures: %d.\n", __func__,
	...
	if (stick->query.general.has_gestures) {
		...
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to read F17 gestures query, code %d.\n", __func__,
				retval);
			return retval;
		}
		*next_query_reg += sizeof(stick->query.gestures.regs);
		VIVO_TS_LOG_DBG("[%s]:        single tap: %d.\n", __func__,
					stick->query.gestures.has_single_tap);
		VIVO_TS_LOG_DBG("[%s]:        tap & hold: %d.\n", __func__,
					stick->query.gestures.has_tap_and_hold);
		VIVO_TS_LOG_DBG("[%s]:        double tap: %d.\n", __func__,
					stick->query.gestures.has_double_tap);
		VIVO_TS_LOG_DBG("[%s]:        early tap:  %d.\n", __func__,
					stick->query.gestures.has_early_tap);
		VIVO_TS_LOG_DBG("[%s]:        press:      %d.\n", __func__,
...
static int f17_initialize(struct rmi_function_container *fc)
{
	...
	VIVO_TS_LOG_INF("[%s]:Found F17 with %d sticks.\n", __func__,
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to initialize F17 control params.\n", __func__);
		return retval;
	}
	...
	for (i = 0; i < f17->query.number_of_sticks + 1; i++) {
		...
		retval = f17_init_stick(rmi_dev, &f17->sticks[i],
					...
					&next_control_reg);
		if (!retval) {
			VIVO_TS_LOG_ERR("[%s]:Failed to init stick %d.\n", __func__, i);
			return retval;
		}
	}
...
//	struct rmi_device *rmi_dev = fc->rmi_dev;
	...
	if (stick->query.general.has_absolute) {
		...
		if (!input_dev) {
			VIVO_TS_LOG_ERR("[%s]:Failed to allocate stick device %d.\n", __func__,
				stick->index);
			return -ENOMEM;
		}
		...
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to register stick device %d.\n", __func__,
				stick->index);
...
static int f17_config(struct rmi_function_container *fc)
{
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]:Could not write stick controls to 0x%04x\n", __func__,
				f17->control_address);
		return retval;
	}
...
static int f17_process_stick(struct rmi_device *rmi_dev,
	...
	if (stick->query.general.has_absolute) {
		...
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to read abs data for stick %d, code %d.\n", __func__,
				stick->index, retval);
			goto error_exit;
		}
	}
	if (stick->query.general.has_relative) {
		...
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to read rel data for stick %d, code %d.\n", __func__,
				stick->index, retval);
			goto error_exit;
		}
		VIVO_TS_LOG_DBG("[%s]:Reporting dX: %d, dy: %d\n", __func__,
	...
	if (stick->query.general.has_gestures) {
		...
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to read gestures for stick %d, code %d.\n", __func__,
				stick->index, retval);
			goto error_exit;
		}
	}
