	...
	struct f19_data *f19)
{
	...
	if (error < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read f19_0d_control_0, code:"
			" %d.\n", __func__, error);
		return error;
	}
	...
	if (button_control->int_enabled_button) {
		...
		if (error < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to read f19_0d_control_2,"
				" code: %d.\n", __func__, error);
			return error;
		}
	...
	if (button_control->sensor_map) {
		...
		if (error < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to read f19_0d_control_3,"
			" code: %d.\n", __func__, error);
			return error;
		}
		ctrl_base_addr = ctrl_base_addr +
	...
	if (button_query->has_sensitivity_adjust) {
		...
		if (error < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to read f19_0d_control_5,"
				" code: %d.\n", __func__, error);
			return error;
		}
		ctrl_base_addr = ctrl_base_addr +
	...
	if (button_query->has_hysteresis_threshold) {
		...
		if (error < 0) {
			VIVO_TS_LOG_ERR("[%s]:Failed to read f19_0d_control_6,"
				" code: %d.\n", __func__, error);
			return error;
		}
		ctrl_base_addr = ctrl_base_addr +
...
static int rmi_f19_alloc_memory(struct rmi_function_container *fc)
{
	...
	if (!f19->button_control.single_button) {
		VIVO_TS_LOG_ERR("[%s]:Failed to allocate"
			" single button.\n", __func__);
		return -ENOMEM;
	}
...
static int rmi_f19_initialize(struct rmi_function_container *fc)
{
	...
	if (rc < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to read command register.\n", __func__);
		return rc;
	}
	...
	if (rc < 0) {
		VIVO_TS_LOG_ERR("[%s]:Failed to initialize F19 control params.\n", __func__);
		return rc;
	}
...
static int rmi_f19_config(struct rmi_function_container *fc)
{
	...
	retval = rmi_write_block(fc->rmi_dev, data->single_button_address,
		...
		data->button_bitmask_size);
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write interrupt_enable_store to"
				" 0x%x\n", __func__,
	...
	if (retval < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not sensor_map_store to 0x%x\n",
	...
	if (button_query->has_sensitivity_adjust) {
		retval = rmi_write_block(fc->rmi_dev,
			...
			sizeof(struct f19_0d_control_5));
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]: Could not sensitivity_adjust_store to"
				" 0x%x\n", __func__,
				data->all_button_sensitivity_address);
			return retval;
		}
	...
	if (!button_query->has_hysteresis_threshold) {
		retval = rmi_write_block(fc->rmi_dev,
			...
			sizeof(struct f19_0d_control_6));
		if (retval < 0) {
			VIVO_TS_LOG_ERR("[%s]: Could not write all_button hysteresis "
				"threshold to 0x%x\n", __func__,
				data->all_button_threshold_address);
			return -EINVAL;
		}
					 ...
					 const char *buf, size_t count)
{
	...
	if (new_value < 0 || new_value > 4) {
		VIVO_TS_LOG_ERR("[%s]: Error - filter_mode_store has an invalid value %d.\n",
			__func__, new_value);
		return -EINVAL;
	}
					 ...
					 const char *buf, size_t count)
{
	...
	data = fc->data;
	if (sscanf(buf, "%u", &new_value) < 1) {
		VIVO_TS_LOG_ERR("[%s]: Error - button_usage_store has an invalid len.\n",
			__func__);
		return -EINVAL;
	}

	if (new_value < 0 || new_value > 4) {
		VIVO_TS_LOG_ERR("[%s]: Error - button_usage_store has an invalid value %d.\n",
			__func__, new_value);
		return -EINVAL;
	}
	...
	if (result < 0) {
		...
		VIVO_TS_LOG_ERR("[%s]: Could not write button_usage_store to 0x%x\n",
			__func__, fc->fd.control_base_addr);
		return result;
	}
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		if (len > 0) {
			...
			VIVO_TS_LOG_ERR("[%s]: Failed to build interrupt button buffer, code = %d.\n",
				__func__, len);
					 ...
					 const char *buf, size_t count)
{
	...
	for (i = 0; i < data->button_count && *buf != 0;
		...
		if ((result != 1) || (button != 0 && button != 1)) {
			VIVO_TS_LOG_ERR("[%s]: Error-int enable button for button %d is not a valid value 0x%x.\n",
				__func__, i, button);
			return -EINVAL;
		}
	...
	if (button_count != data->button_count) {
		VIVO_TS_LOG_ERR("[%s]: Error-int enable button count of %d doesn't match device button count of %d.\n",
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		if (len > 0) {
			...
			VIVO_TS_LOG_ERR("[%s]: Failed to build signle button buffer, code = %d.\n",
				__func__, len);
			return snprintf(buf, PAGE_SIZE, "unknown\n");
		}
	}
					 ...
					 const char *buf, size_t count)
{
	...
	for (i = 0; i < data->button_count && *buf != 0;
		...
		if ((result != 1) || (button != 0 && button != 1)) {
			VIVO_TS_LOG_ERR("[%s]: Error - single button for button %d is not a valid value 0x%x.\n",
				__func__, i, button);
			kfree(new_value);
			return -EINVAL;
		}
	...
	/* Make sure the button count matches */
	if (button_count != data->button_count) {
		VIVO_TS_LOG_ERR("[%s]:Error-single button count of %d doesn't match device button count of %d\n",
			__func__, button_count, data->button_count);
		kfree(new_value);
		return -EINVAL;
	}
					 ...
					 char *buf)
{
	...
	for (i = 0; i < data->button_count; i++) {
		...
		if (len > 0) {
			...
			VIVO_TS_LOG_ERR("[%s]: Failed to build sensor map buffer, code = %d.\n", __func__, len);
			return snprintf(buf, PAGE_SIZE, "unknown\n");
		}
	}
	len = snprintf(current_buf, PAGE_SIZE - total_len, "\n");
					 ...
					 const char *buf, size_t count)
{
	...
	if (!new_value) {
		VIVO_TS_LOG_ERR("[%s]: Error - failed to allocate sensor map.\n", __func__);
		return -ENOMEM;
	}

	if (data->button_query.configurable == 0) {
		VIVO_TS_LOG_ERR("[%s]: Error - sensor map is not configuralbe at run-time", __func__);
		return -EINVAL;
	}
	...
	for (i = 0; i < data->button_count && *p != 0; i++) {
		...
		if (new_value[i] == 0  && p[0] != '0') {
			VIVO_TS_LOG_ERR("[%s]: Error - reading sensor map is invalid\n", __func__);
			return -EINVAL;
		}
		if (new_value[i] > INT_MAX) {
			VIVO_TS_LOG_ERR("[%s]: Error - reading sensor map is out for range\n", __func__);
			return -ERANGE;
		}
		p = tail;
		...
		if (new_value[i] < SENSOR_MAP_MIN ||
			...
			VIVO_TS_LOG_ERR("[%s]: Error - sensor map for button %d is not a valid value 0x%x.\n",
	...
	if (button_count != data->button_count) {
		VIVO_TS_LOG_ERR("[%s]: Error - button map count of %d doesn't match device button count of %d.\n",
			__func__, button_count, data->button_count);
		return -EINVAL;
	}
					 ...
					 const char *buf, size_t count)
{
	...
	if (data->button_query.configurable == 0) {
		VIVO_TS_LOG_ERR("[%s]: Error - sensitivity_adjust is not configuralbe at run-time", __func__);
		return -EINVAL;
	}
	...
	if (len < 0) {
		...
		VIVO_TS_LOG_ERR("[%s]: Could not sensitivity_adjust_store to 0x%x\n", __func__,
			data->all_button_sensitivity_address);
		return len;
	}
					 ...
					 const char *buf, size_t count)
{
	...
	if (new_value < HYSTERESIS_THRESHOLD_MIN ||
		...
		VIVO_TS_LOG_ERR("[%s]: Error - hysteresis_threshold_store has an invalid value %d.\n",
			__func__, new_value);
		return -EINVAL;
	}
	...
	if (len < 0) {
		all_button_hysteresis_threshold->hysteresis_threshold =
								old_value;
		VIVO_TS_LOG_ERR("[%s]: Could not write all_button hysteresis threshold to 0x%x.\n", __func__,
			data->all_button_threshold_address);
		return -EINVAL;
	}
	return count;
					 ...
					 size_t count)
{
	...
	if (new_value != 0 && new_value != 1) {
		VIVO_TS_LOG_ERR("[%s]: Error - rezero is not a valid value 0x%x.\n",
			__func__, new_value);
		return -EINVAL;
	}
	...
	if (len < 0) {
		VIVO_TS_LOG_ERR("[%s]: Could not write rezero to 0x%x\n",
