...
{
#include "fw_data_0x3044_0x5511_AUO.i"
};
...
static int eKTH3260_parse_dt(struct device *dev, struct elan_ktf_ts_data *ts)
{
    ...
    if(ts->rst_gpio < 0)
    {
        printk("[elan] %s: get elan,reset-gpio fail. \n", __func__);
        return ts->rst_gpio;
    }
    ...
    if(ts->intr_gpio < 0)
    {
        printk("[elan] %s: get elan,intr-gpio fail. \n", __func__);
        return ts->intr_gpio;
    }
    ...
    if(ts->power_gpio < 0)
    {
        printk("[elan] %s: get elan,power-gpio fail. \n", __func__);
    ...
    if(ts->lcmid0_gpio < 0)
    {
        printk("[elan] %s: get elan,lcmid0-gpio fail. \n", __func__);
    ...
    if(ts->lcmid2_gpio < 0)
    {
        printk("[elan] %s: get elan,lcmid2-gpio fail. \n", __func__);
...
int eKTH3260_cable_status_handler(int state)
{
    ...
    if(entry_mode==4)
    {
        printk("%s:[%d]: In COS, Skip usb notifier\n", __func__, __LINE__);
        return 0;
    }
    else if(entry_mode==3)
    {
        printk("%s:[%d]: In POS, Skip usb notifier\n", __func__, __LINE__);
        return 0;
    }
    // add by leo for skip COS/POS --
...
static ssize_t eKTH3260_proc_tp_debug_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
    ...
    else if((buf[0] == 'r') && (buf[1] == 'e') && (buf[2] == 's') && (buf[3] == 'e') && (buf[4] == 't'))
    {
                             ...
                             const struct i2c_device_id *id)
{
    ...
    printk("////////////////////		elan_ktf_ts_probe START v34 \n");
    ...
    printk("[elan] %s touch power enable\n", __func__);
    msleep(300);
    ...
    */
    ...
    else
    {
        if(ts->LCM_ID2) // AUO LCM
        {
            ...
            {
                printk("[elan] %s: Skip touch FW update because it's old AUO Panel.\n", __func__);
            ...
            {
                //if((FW_ID!=0x3038)||(FW_VERSION!=0x5514)||(RECOVERY==0x80))
                if((FW_ID != 0x3044) || (FW_VERSION < 0x5511))
                {
                    printk("[elan] %s: Start touch FW update to version 0x3044_0x5511 for new AUO LCM...\n", __func__);
                    //elan_TWO_WIRE_ICE(ts->client);
                    Update_FW_in_Driver(0);
