	...
	for (i = 0; i < TOUCH_PWR_NUM; ++i) {
		if (ts_pwr[i].type == 1 && gpio_is_valid(ts_pwr[i].value)) {
			...
			if (!strncmp(ts_pwr[i].name, "low", strlen("low"))) {
				...
				TOUCH_ERR_MSG("%s ret = %d\n", gpio_request_name, ret);
			} else {
...
static int mms_power(struct i2c_client* client, int power_ctrl)
{
	...
	case POWER_ON:
		...
		do {
			if (ts_pwr[i].type == 1) {
				...
				if (ts->vdd_regulator[i] != NULL && !IS_ERR(ts->vdd_regulator[i])) {
					...
					if (ret) {
						TOUCH_INFO_MSG("power[%d]: ", i);
						TOUCH_INFO_MSG("regulator enable failed ret = %d\n", ret);
					} else {
						TOUCH_INFO_MSG("power[%d]: regulator enabled\n", i);
					}
#endif
		...
		msleep(ts_role->booting_delay*10);
...
#if defined(CONFIG_MACH_MSM8926_VFP_KR) || defined(CONFIG_MACH_MSM8926_VFPV_KR)
	while((ret < 0) && (upgrade_cnt < 2)) {
		if(ts->pdata->ic_type == MMS100A)
			ret = mms_100a_fw_upgrade(ts, info);
		else
			ret = mms_100s_isc(ts, info);
		TOUCH_INFO_MSG("mms_fw_upgrade_again(%d)\n",upgrade_cnt);
		upgrade_cnt++;
	}
#endif
...
static int mms_ic_ctrl(struct i2c_client *client, u32 code, u32 value)
{
	...
	case IC_CTRL_INFO_SHOW:
		...
		buf = (char *) param->v1;
		if(buf) {
			ret += sprintf(buf+ret, "Firmware Version : %d.%02X \n", (ts->ts_section[2].version&0x80?1:0), ts->ts_section[2].version&0x7F);
	...
	case IC_CTRL_TESTMODE_VERSION_SHOW:
		...
		buf = (char *) param->v1;
		if(buf) {
			ret += sprintf(buf+ret, "%d.%02X(0x%X, 0x%X, 0x%X, %s)\n",
...
static ssize_t mms_self_diagnostic_show(struct i2c_client *client, char *buf)
{
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "Firmware Version : %d.%02X \n", (ts->ts_section[2].version&0x80?1:0), ts->ts_section[2].version&0x7F);
