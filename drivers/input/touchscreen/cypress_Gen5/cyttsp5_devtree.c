		...
		struct device_node *core_node)
{
	...
	if (!gpio_is_valid(pdata->irq_gpio)) {
		TS_LOG_ERR( "%s:get cy,irq_gpio failed.\n", __func__);
		rc = -EIO;
		goto fail;
	}
	...
	if (!gpio_is_valid(pdata->rst_gpio)) {
		TS_LOG_ERR("%s:get cy,rst_gpio failed.\n", __func__);
		rc = -EIO;
		goto fail;
	}
	...
	TS_LOG_INFO("%s: irq_gpio:%d rst_gpio:%d hid_desc_register:%d\n"
