		...
		struct cyttsp5_core *core)
{
	...
	if (ret) {
		TS_LOG_ERR( "%s: failed to register device, err %d\n",
			__func__, ret);
		pdev->core = NULL;
	}
	return ret;
...
static void _cyttsp5_unregister_dev(struct cyttsp5_device *pdev)
{
	...
	TS_LOG_DEBUG( "%s: Unregistering device '%s'.\n",
		__func__, dev_name(&pdev->dev));
...
static void _cyttsp5_unregister_core(struct cyttsp5_core *pdev)
{
	...
	TS_LOG_DEBUG( "%s: Unregistering core '%s'.\n",
		__func__, dev_name(&pdev->dev));
	device_unregister(&pdev->dev);
}
...
int cyttsp5_register_device(struct cyttsp5_device_info const *dev_info)
{
	...
	if (ret) {
		TS_LOG_DEBUG("%s: dev_info is invalid\n", __func__);
		goto fail;
	}

	mutex_lock(&core_lock);
	if (find_device(dev_info->name, dev_info->core_id)) {
		TS_LOG_DEBUG("%s: device '%s' with core id '%s' already exists\n",
	...
	if (!dev) {
		TS_LOG_ERR("%s: failed to allocate device '%s'\n",
	...
	TS_LOG_DEBUG("%s: '%s' added to cyttsp5_dev_list\n", __func__, dev->name);
...
int cyttsp5_unregister_device(char const *name, char const *core_id)
{
	...
	if (!dev) {
		TS_LOG_ERR("%s: device '%s' could not be found\n", __func__, name);
		ret = -ENODEV;
		goto fail_unlock;
	}
	_cyttsp5_unregister_dev(dev);
	list_del(&dev->node);
	TS_LOG_DEBUG("%s: '%s' removed from cyttsp5_dev_list\n", __func__,
		dev->name);
	kfree(dev);
fail_unlock:
	mutex_unlock(&core_lock);
...
int cyttsp5_register_core_device(struct cyttsp5_core_info const *core_info)
{
	...
	if (ret) {
		TS_LOG_DEBUG("%s: core_info is invalid\n", __func__);
		goto fail;
	}

	mutex_lock(&core_lock);
	if (find_core(core_info->id)) {
		TS_LOG_DEBUG("%s: core id '%s' already exists\n",
	...
	if (!core) {
		TS_LOG_ERR("%s: failed to allocate core device '%s'\n",
	...
	TS_LOG_DEBUG("%s: '%s' added to core_dev_list\n", __func__, core->name);
	adap = find_adapter(core->adap_id);
	if (adap) {
		TS_LOG_DEBUG("%s: adapter for '%s' is '%s'\n", __func__,
		...
		struct device *parent)
{
	...
	if (!parent) {
		TS_LOG_ERR("%s: need parent for '%s'\n", __func__, id);
		return -EINVAL;
	}
	mutex_lock(&core_lock);
	if (find_adapter(id)) {
		TS_LOG_ERR("%s: adapter '%s' already exists\n",
				__func__, id);
		rc = -EEXIST;
		goto fail;
	}
	...
	if (!a) {
		TS_LOG_ERR("%s: failed to allocate adapter '%s'\n",
				__func__, id);
		rc = -ENOMEM;
		goto fail;
	}
	...
	TS_LOG_DEBUG( "%s: '%s' added to adapter_list\n", __func__, id);
	rescan_cores(a);
fail:
	mutex_unlock(&core_lock);
	return rc;
...
int cyttsp5_del_adapter(char const *id)
{
	...
	if (!adap) {
		TS_LOG_ERR("%s: adapter '%s' does not exist\n",
			__func__, id);
		rc = -ENODEV;
		goto fail;
	}
	...
	list_del(&adap->node);
	kfree(adap);
	TS_LOG_DEBUG("%s: '%s' removed from adapter_list\n", __func__, id);
fail:
	mutex_unlock(&core_lock);
	return rc;
}
...
static int cyttsp5_device_match(struct device *dev, struct device_driver *drv)
{
	...
	match = 0;
exit:
	TS_LOG_DEBUG( "%s: %s matching '%s' driver\n", __func__,
