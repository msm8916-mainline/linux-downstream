		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		/*
		 * Test implementaion is not correct,
		 * will use different logic for now
...
static int _cyttsp5_proximity_enable(struct cyttsp5_proximity_data *pd)
{
	...
	rc = cyttsp5_request_enable_scan_type(ttsp, CY_ST_PROXIMITY);
	if (rc < 0) {
		...
		bool force)
{
	...
	rc = cyttsp5_request_disable_scan_type(ttsp, CY_ST_PROXIMITY);
	if (rc < 0) {
...
static int cyttsp5_proximity_attention(struct cyttsp5_device *ttsp)
{
	...
	if (!pd->is_suspended) {
		...
		TS_LOG_DEBUG( "%s: Ignoring report while suspended\n",
			__func__);
	}
