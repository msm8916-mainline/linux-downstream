...
static unsigned char CTPM_FW_BOE[] = {
	#include "FT3427_BOE.i"
};

static unsigned char CTPM_FW_DSJM[] = {
	#include "FT3427_DSJM.i"
};
...
//	int fw_len = sizeof(CTPM_FW);
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x54)) //here is case...
	{
...
// -- int fts_ctpm_get_project_setting(struct i2c_client *client)
int fts_ctpm_get_project_setting(struct i2c_client *client, u8 *vendor_id)
{	
	u8 reg_val[4] = {0};
	u32 i = 0;
	u8 auc_i2c_write_buf[FTS_SETTING_BUF_LEN];
	int i_ret;
...
//	printk("lxh:**bootloader version = 0x%x**\n", reg_val[0]);
  ...
  //  msleep(5); 
	...
	i_ret = fts_i2c_Read(client, auc_i2c_write_buf, 4, auc_i2c_write_buf, FTS_SETTING_BUF_LEN);
    if (i_ret < 0) {
	    printk("lxh:**[FTS] get vendor id fail **\n");
		return -EIO;
    } else {
       printk("lxh:**vendor_id: 0x%x**\n", auc_i2c_write_buf[4]);
	}
	*vendor_id = auc_i2c_write_buf[4];
#if 0
...
***********************************************************************/
int fts_ctpm_get_i_file_ver(struct i2c_client *client, u8 *host_ver)
{
    ...
    if ((reg_value == VENDOR_BOE) || (reg_value == VENDOR_BOE_LATER))  {
        ...
        CTPM_FW = CTPM_FW_DSJM;
		ui_sz = sizeof(CTPM_FW_DSJM);			
	} else {
	 force_upgrade = true;
     fts_ctpm_get_project_setting(client, &reg_value);
	 if ((reg_value == VENDOR_BOE) || (reg_value == VENDOR_BOE_LATER))  {
		   CTPM_FW = CTPM_FW_BOE;
		   ui_sz = sizeof(CTPM_FW_BOE);
	  } else if ((reg_value == VENDOR_DSJM) || (reg_value == VENDOR_DSJM_LATER)) { 			   
	...
	if (ui_sz > 2)
	{
        if(fts_updateinfo_curr.CHIP_ID==0x36)
          *host_ver = CTPM_FW[0x10a];
	    else
		  *host_ver = CTPM_FW[ui_sz - 2];
		return 0; 
	}  else
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	i_ret = fts_ctpm_get_i_file_ver(client, &uc_host_fm_ver);
    if (i_ret < 0)
       return -EIO;  

 	if ((uc_tp_fm_ver == FTS_REG_FW_VER) ||
 	      (uc_tp_fm_ver < uc_host_fm_ver) || (force_upgrade == true)) 
 	{
	    force_upgrade = false;
		msleep(100);
		printk("lxh:**[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
		...
		//	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
			printk("lxh:***[FTS] upgrade to new version 0x%x\n",
					uc_host_fm_ver);
		} else {
...
//	i_ret = HidI2c_To_StdI2c(client);  // for build error fix 
	i_ret = hid_to_i2c(client);
...
/************************************************************************
* Name: fts_ftsdrv_show
* Brief:  show driver version
* Input: device, device attribute, char buf
* Output: driver version
* Return: num_read_chars
***********************************************************************/
static ssize_t fts_drv_show(struct device *dev,
struct device_attribute *attr, char *buf)
{	
	ssize_t num_read_chars = 0;
	num_read_chars = snprintf(buf, PAGE_SIZE, FTS_DRV_VERSION);
	return num_read_chars;
}
/************************************************************************
* Name: fts_ftsdrv_store
* Brief:  no
* Input: device, device attribute, char buf, char count
* Output: no
* Return: -EPERM
***********************************************************************/
static ssize_t fts_drv_store(struct device *dev,
struct device_attribute *attr,
	const char *buf, size_t count)
{
	return -EPERM;
...
static DEVICE_ATTR(ftstpdrvver, S_IRUGO|S_IWUSR, fts_drv_show, fts_drv_store);
