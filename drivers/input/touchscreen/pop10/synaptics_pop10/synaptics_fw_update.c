...
//[FEATURE]-Add-BEGIN by TCTSZ. weihong.chen,FR-674715 2014/05/29, update firmware info
...
static int read_config_version(void);
static int read_firmware_version(void);
#endif
...
static unsigned int version_extract_uint(const unsigned char *ptr)
 ...
 static int read_firmware_version(void)
 {
   int retval;
   unsigned char firmware_id[4];
   //[BUGFIX]-Add-Begin by TCTSZ. weihong.chen,11/04/2014,fix read firmware version adress wrong,PR829356
   retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   fwu->rmi4_data->f01_query_base_addr + 18,
				   firmware_id,
				sizeof(firmware_id));
   //[BUGFIX]-Add-End by TCTSZ. weihong.chen,11/04/2014,fix read firmware version adress wrong,PR829356
    firmware_id[3] = 0;
 ...
 static int read_config_version(void)
 {
   ...
   unsigned char firmware_id[4];
    //[BUGFIX]-Add-Begin by TCTSZ. weihong.chen,11/04/2014,fix read firmware version adress wrong,PR829356
   retval = fwu->fn_ptr->read(fwu->rmi4_data,
				   fwu->rmi4_data->f34_ctrl_base_addr,
				   firmware_id,
				sizeof(firmware_id));
     //[BUGFIX]-Add-End by TCTSZ. weihong.chen,11/04/2014,fix read firmware version adress wrong,PR829356
...
static enum flash_area fwu_go_nogo(void)
{
    ...
    //[BUGFIX]-Add-BEGIN by TCTSZ. weihong.chen,10/15/2014,remove this judge,we do not need
#ifndef CONFIG_TCT_8X16_POP10
	if (img->is_contain_build_info) {
...
#endif
    //[BUGFIX]-Add-END by TCTSZ. weihong.chen,10/15/2014,remove this judge,we do not need
   /* check firmware size */
   ...
   // printk(KERN_CRIT"fwu->f34_fd.ctrl_base_addr = 0x%x \n",fwu->f34_fd.ctrl_base_addr);

	deviceConfigID =  extract_uint_be(config_id);
    //printk(KERN_CRIT"deviceConfigID = 0x%x \n",deviceConfigID);
	//  printk(KERN_CRIT"Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
...
static int fwu_start_reflash(void)
{
	...
	/* reset device */
    //[BUGFIX]-Add-BEGIN by TCTSZ. weihong.chen,FR-782201 2014/09/04, fix f11_init error
    #ifdef CONFIG_TCT_8X16_POP10
	fwu->rmi4_data->device_is_reseting =true;
	#endif
	fwu_reset_device();
     #ifdef CONFIG_TCT_8X16_POP10
     //[BUGFIX]-Add-BEGIN by TCTSZ. weihong.chen,PR-797848 2014/09/25, update firmware information
     g_tp_cfg_ver =read_config_version();
     g_tp_firmware_ver =read_firmware_version();
     //[BUGFIX]-Add-END by TCTSZ. weihong.chen,PR-797848 2014/09/25, update firmware information
	fwu->rmi4_data->device_is_reseting =false;
	#endif
    //[BUGFIX]-Add-END by TCTSZ. weihong.chen,FR-782201 2014/09/04, fix f11_init error
	/* check device status */
	retval = fwu_read_f01_device_status(&f01_device_status);
	if (retval < 0)
		goto exit;
