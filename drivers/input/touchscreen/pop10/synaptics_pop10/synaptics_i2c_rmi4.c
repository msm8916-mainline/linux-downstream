...
#define F11_2D_DATA38_LPWG_STATUS  0x004E
#define UNICODE_DETECTED   (0x01<<6)
#define DOUBLE_TAP_DETECTED 0x01
#define F11_2D_DATA39  0x004f
#define LETTER_C        0x63
#define LETTER_M        0x6d
#define F01_RMI_DATA0  0x0014
#endif
...
static unsigned int version_extract_uint(const unsigned char *ptr)
 ...
 static int synaptics_read_cfg_version(struct synaptics_rmi4_data *rmi4_data)
 {
   ...
   retval = synaptics_rmi4_i2c_read(rmi4_data,
				   ...
				   firmware_id,
				sizeof(firmware_id));
  // printk(KERN_CRIT"rmi4_data->f34_ctrl_base_addr = 0x%x \n",rmi4_data->f34_ctrl_base_addr);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	//gesture wakeup
	#ifdef SYNAPTICS_GESTURE_WAKE_UP
	//unsigned char detected_gesturess=0;
	unsigned char detected_gestures=0;
	unsigned char letter_state[4];
	struct synaptics_rmi4_f11_extra_data *extra_data;
	#endif
	//palm
	#ifdef SYNAPTICS_PALM_SLEEP
	unsigned char detected_palm;
	#endif
  //[FEATURE]-Add-END by TCTSZ.weihong.chen
	//gw
   ...
   {  detected_palm =0;
	  ...
	  if(detected_palm == 0x02)
	   {    finger_number = synaptics_finger_count(rmi4_data,fhandler);
            ...
            #endif
            if(finger_number >= 2) //add by weihong.chen 2014/08/08 ,only if more than 2 fingers detected light off the scree
            {
    ...
    if (rmi4_data->suspended && g_wakeup_gesture) {
	      retval = synaptics_rmi4_i2c_read(rmi4_data,
				F11_2D_DATA38_LPWG_STATUS,
				&detected_gestures,
				sizeof(detected_gestures));
		if (retval < 0)
			{
			 #ifdef TP_DEBUG
	         STP_DEBUG("read F11_2D_DATA38_LPWG_STATUS faile \n");
             #endif
			return 0;
			}
      #ifdef TP_DEBUG
			STP_DEBUG("address=0x%x,detected_gestures = %d\n",F11_2D_DATA38_LPWG_STATUS,detected_gestures);
			#endif
       if(detected_gestures & DOUBLE_TAP_DETECTED)
       {
	      detected_palm =0;
           ...
           //Add-BEGIN by TCTSZ. weihong.chen 2014/06/18, add double cilick to unlock  screen
       ...
       if(detected_gestures & UNICODE_DETECTED)
          	{
              retval = synaptics_rmi4_i2c_read(rmi4_data,
				        F11_2D_DATA39,
				        letter_state,
				        sizeof(letter_state));
		                 ...
		                 {
		                 #ifdef TP_DEBUG
			             STP_DEBUG("read F11_2D_DATA39 faile\n");
			             #endif
			             return 0;
		                 }
				    #ifdef TP_DEBUG
			      STP_DEBUG("address=0x%x,letter_state[0] =0x%x ,letter_state[1] =0x%x ,letter_state[2] =0x%x ,letter_state[3] =0x%x \n",F11_2D_DATA39, \
			                 letter_state[0],letter_state[1],letter_state[2],letter_state[3]);
			      #endif
				if(letter_state[2] == LETTER_C)
					{
            input_report_key(rmi4_data->input_dev, KEY_LETTER_C, 1);
				    input_sync(rmi4_data->input_dev);
				    input_report_key(rmi4_data->input_dev, KEY_LETTER_C, 0);
				    input_sync(rmi4_data->input_dev);
					#ifdef TP_DEBUG
				    STP_DEBUG("letter[c] detected ,KEY_LETTER_C\n");
				    #endif
					return 0;
				    } else if (letter_state[2] == LETTER_M)
				    {
                    input_report_key(rmi4_data->input_dev, KEY_LETTER_M, 1);
				    input_sync(rmi4_data->input_dev);
				    input_report_key(rmi4_data->input_dev, KEY_LETTER_M, 0);
				    input_sync(rmi4_data->input_dev);
					#ifdef TP_DEBUG
				    STP_DEBUG("letter[m] detected ,KEY_LETTER_M\n");
				    #endif
					return 0;
                    }
             }
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
           ...
           #ifdef TP_DEBUG
			 STP_DEBUG("Finger %d: status = 0x%02x,x = %d ,y = %d\n",finger,finger_status,x, y);
           #endif
...
//[FEATURE]-Add-BEGIN by TCTSZ. weihong.chen,PR-674715 2014/05/26, add new feature
    ...
    input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_LETTER_C);
    input_set_capability(rmi4_data->input_dev, EV_KEY, KEY_LETTER_M); //Add by TCTSZ. weihong.chen 2014/08/04, add double cilick to unlock
#endif
//[FEATURE]-Add-END by TCTSZ.weihong.chen
...
err_pinctrl_select:
    ...
    //[FEATURE]-Add-BEGIN by TCTSZ. weihong.chen,PR-782371 2014/09/03, delete it since quallcom fix it
    //  #ifdef CONFIG_TCT_8X16_POP10
	//		synpatics_rmi4_pinctrl_select(rmi4_data, false);
    //       pinctrl_put(rmi4_data->ts_pinctrl);
    //   #else
    //[FEATURE]-Add-END by TCTSZ. weihong.chen,PR-782371 2014/09/03, delete it since quallcom fix it
		if (IS_ERR_OR_NULL(rmi4_data->pinctrl_state_release)) {
			devm_pinctrl_put(rmi4_data->ts_pinctrl);
			rmi4_data->ts_pinctrl = NULL;
		} else {
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->fw_updating) {
        ...
        //[BUG-FIX]-Add-BEGIN by TCTSZ. weihong.chen,PR-792110 2014/10/28, keep power on(VBUS and VDDIO must both disable or both  enable,since VREG_L6_1P8 can not disable,so keep both on)
        #if 0
        ...
        #endif
        //[BUG-FIX]-Add-END by TCTSZ. weihong.chen,PR-792110 2014/10/28, keep power on(VBUS and VDDIO must both disable or both  enable,since VREG_L6_1P8 can not disable,so keep both on)
	} else {
		dev_err(dev,
