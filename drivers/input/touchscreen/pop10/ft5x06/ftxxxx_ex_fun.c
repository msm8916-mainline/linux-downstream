...
#include "FT5526_POP10_V0F_D88_20140929_app.i"
};
#ifdef CONFIG_TCT_8X16_POP10
extern int g_tp_cfg_ver;
extern u8 update_finish ;
#endif
//extern struct ft5x06_ts_data *g_data;
struct ft5x06_ts_data *g_data =NULL;
...
int HidI2c_To_StdI2c(struct i2c_client * client)
{
	...
	DBG("Change to STDI2cValue,REG1 = 0x%x,REG2 = 0x%x,REG3 = 0x%x, iRet=%d\n",
					reg_val[0], reg_val[1], reg_val[2], iRet);
	//iRet = ft5x06_i2c_read(client, auc_i2c_write_buf, 3, reg_val, 3);
	//DBG("Change to STDI2cValue,REG1 = 0x%x,REG2 = 0x%x,REG3 = 0x%x, iRet=%d\n",
	//				reg_val[0], reg_val[1], reg_val[2], iRet);
...
int FTS_I2c_Read(unsigned char * wBuf, int wLen, unsigned char *rBuf, int rLen)
{
	...
	return ft5x06_i2c_read(G_Client, wBuf, wLen, rBuf, rLen);
}
...
int FTS_I2c_Write(unsigned char * wBuf, int wLen)
{
	...
	return ft5x06_i2c_write(G_Client, wBuf, wLen);
}
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client * client)
{
	...
	*/
	//printk(KERN_CRIT"FT5526 ---> start fts_ctpm_fw_upgrate_with_i_file \n");
...
int fts_ctpm_auto_upgrade(struct i2c_client * client)
{
   ...
   #ifdef CONFIG_TCT_8X16_POP10
   ftxxxx_read_reg(client, FTXXXX_REG_FW_VER, &cfg_ver);
   g_tp_cfg_ver =cfg_ver;
   #endif
	return 0;
}

#if defined(FOCALTECH5526_PWRON_UPGRADE)
 void ft5526_init_update_work(struct work_struct *work)
{
	...
	fts_ctpm_auto_upgrade(ts->client);
    #ifdef CONFIG_TCT_8X16_POP10
    update_finish =1;
    #endif
...
u8 ft5526_init_update_proc(struct ft5x06_ts_data *ts)
{
	dev_dbg(&ts->client->dev, "Ready to run update work.");

	INIT_DELAYED_WORK(&ts->focaltech_update_work, ft5526_init_update_work);
...
int ftxxxxEnterUpgradeMode(struct i2c_client * client, int iAddMs, bool bIsSoft)
{
	...
	if(bIsSoft)//reset by App
	{
		...
		for (i = 0; i < FTS_UPGRADE_LOOP; i++)
		{
			...
			//if(i_ret < 0)
			//DBG("[FTS] failed writing  0x66 to register 0xbc or oxfc! \n");
	...
	else//reset by hardware reset pin
	{
		...
		for (i = 0; i < FTS_UPGRADE_LOOP; i++)
		{
			/*********Step 1:Reset  CTPM *****/
			 if(g_data !=NULL)
              {
                printk(KERN_CRIT"FT5526--->update reset \n");
                if (gpio_is_valid(g_data->pdata->reset_gpio)) {
		        gpio_set_value_cansleep(g_data->pdata->reset_gpio, 0);
		        msleep(10);
		        gpio_set_value_cansleep(g_data->pdata->reset_gpio, 1);
                   }
              }
                  else
                  {
                      printk(KERN_CRIT"FT5526--->update reset fail g_data ==NULL \n");
                  }
            msleep(10+iAddMs);	//time (5~20ms)
	...
	const char *buf, size_t count)
{
	...
	if(2 == num_read_chars)
	{
		...
		if(ftxxxx_read_reg(client, regaddr, &regvalue) < 0)
			printk(KERN_CRIT"Could not read the register(0x%02x)\n", regaddr);
		else
			printk(KERN_CRIT"the register(0x%02x) is 0x%02x\n", regaddr, regvalue);
	}
	else
	{
		regaddr = wmreg>>8;
		...
		if(ftxxxx_write_reg(client, regaddr, regvalue)<0)
			printk(KERN_CRIT"Could not write the register(0x%02x)\n", regaddr);
		else
			printk(KERN_CRIT"Write 0x%02x into register(0x%02x) successful\n", regvalue, regaddr);
	}
error_return:
	mutex_unlock(&g_device_mutex);
	...
	const char *buf, size_t count)
{
	...
	if(ftxxxx_get_testparam_from_ini(cfgname) <0)
		...
		/////////////////Print RawData Start
		for(i = 0;i < iTxNumber;i++)
		{
			printk("\n");
			for(j = 0;j < iRxNumber;j++)
			{
				printk("%5d  ", TestData[i*iRxNumber + j]);
			}
		}
		printk("\n");
		/////////////////Print RawData End
...
static ssize_t  ft5x0x_debug_write(struct file *filp,
...
static const struct file_operations ft5x0x_ops = {
    .owner = THIS_MODULE,
...
{
	ftxxxx_proc_entry =proc_create(PROC_NAME, 0777, NULL, &ft5x0x_ops);
