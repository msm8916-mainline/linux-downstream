		...
		if (status == FT_TOUCH_DOWN || status == FT_TOUCH_CONTACT) 
		{
			if(y >= 900)
			{
				...
				case 80:
					...
					break;
				case 240:
					input_report_key(ip_dev, 172, 1);//tianhongwei modfiy home key 2014/10/14
					break;
				case 400:
			...
			input_report_key(ip_dev, 172, 0);//tianhongwei modfiy home key 2014/10/14
			   ...
			   const struct i2c_device_id *id)
{
	...
	__set_bit(172, input_dev->keybit);	//tianhongwei modfiy home key 2014/10/14
...
//decide need upgrade_firmware
    dev_info(&client->dev, "HW ID = 0x%x(NOW is ALTO4GL)\n", reg_value);
    printk(KERN_INFO "HW ID = 0x%x(NOW is ALTO4GL)\n", reg_value);
	printk(KERN_INFO "before fts_ctpm_auto_upgrade");
