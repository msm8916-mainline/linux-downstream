...
retry_init:
	...
	/* initialize */
	if (write_reg(client, ZT7548_X_RESOLUTION, (u16)pdata->x_resolution + ABS_PT_OFFSET) != I2C_SUCCESS)
		goto fail_init;

	if (write_reg(client, ZT7548_Y_RESOLUTION, (u16)pdata->y_resolution + ABS_PT_OFFSET) != I2C_SUCCESS)
		goto fail_init;
...
static void run_hfdnd_read(void *device_data)
{
	...
	for (i = 0; i < x_num; i++) {
		pr_info("%s: hfdnd_data[%d] : ", client->name, i);
		for (j = 0; j < y_num; j++) {
			offset = (i * y_num) + j;
