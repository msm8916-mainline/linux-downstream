...
static void cyttsp5_ta_cb(struct tsp_callbacks *cb, int ta_status)
{
	...
	if (cd->hw_power_state)
		gpio_set_value(cd->pdata->core_pdata->ta_gpio, ta_status);
}
...
int cyttsp5_core_suspend(struct device *dev)
{
	...
	if (cd->pdata->core_pdata->ta_gpio)
...
int cyttsp5_core_resume(struct device *dev)
{
	...
	if (cd->pdata->core_pdata->ta_gpio)
		gpio_set_value(cd->pdata->core_pdata->ta_gpio, cd->ta_status);
...
static void cyttsp5_gpio_request(struct cyttsp5_core_platform_data *pdata)
{
	gpio_request(pdata->irq_gpio, "TSP_INT");
	gpio_request(pdata->pwr_1p8, "TSP_1p8_GPIO");
	gpio_request(pdata->pwr_3p3, "TSP_3p3_GPIO");
}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cyttsp5_gpio_request(cd->cpdata);
