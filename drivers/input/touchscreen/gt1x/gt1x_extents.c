...
#define MXT_INFO_MAX_LEN		40
/* add by yong.bo on 2017/07/08  add tp information*/
static ssize_t gt1x_ts_info_show(struct class *class,
...
static CLASS_ATTR(ts_info, S_IRUSR, gt1x_ts_info_show, NULL);



static ssize_t gt1x_wakeup_gesture_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	if(gesture_enabled) {
		strncpy(buf, "1", 1);
...
static ssize_t gt1x_wakeup_gesture_store(struct class *class, struct class_attribute *attr,
			     const char *buf, size_t count)
{
	...
	GTP_DEBUG("Gesture debug %s", gesture_enabled ? "on":"off");
	return count;
}

static CLASS_ATTR(en_gesture, S_IRWXUGO/*S_IWUSR | S_IRUSR*/, gt1x_wakeup_gesture_show,
			gt1x_wakeup_gesture_store);

void gt1x_touchscreen_info(void)
{
	int error = 0;
	u8 cfg_data=0;
	touchscreen_class = class_create(THIS_MODULE, "touchscreen");
	if (IS_ERR(touchscreen_class)) {
		GTP_DEBUG("%s: create gt1x touchscreen class error!\n", __func__);
	}

	error = class_create_file(touchscreen_class, &class_attr_ts_info);
	if (error < 0) {
		GTP_DEBUG("%s gt1x touchscreen class_create_file ts_info failed!\n", __func__);
	}

	error = gt1x_i2c_read(GTP_REG_CONFIG_DATA, &cfg_data, 1);

	if(error < 0)
	{
		printk(" gt1x_i2c_read read error error_num= %d \n",error);
	}

	ts_info = kmalloc( MXT_INFO_MAX_LEN, GFP_KERNEL);
	snprintf(ts_info, MXT_INFO_MAX_LEN,
			"GTP,cfg_version:%02X\n", cfg_data&0x7F);

	error = class_create_file(touchscreen_class, &class_attr_en_gesture);
	if (error < 0) {
		GTP_DEBUG("%s gt1x touchscreen class_create_file en gesture failed!\n", __func__);
	}
}
/*end*/
...
bool is_touch_key(void)
{
	u8 coor_value=0;

	gt1x_i2c_read(GTP_READ_COOR_ADDR+2, &coor_value, 1);
	if((coor_value==0x01)||(coor_value==0x02)||(coor_value==0x04))
	{
		return true;
	}
	else {
