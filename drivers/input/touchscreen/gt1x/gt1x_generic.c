...
u8 gt1x_config_clear[GTP_CONFIG_MAX_LENGTH] = { 0 };
u32 gt1x_cfg_clear_length = GTP_CONFIG_MAX_LENGTH;
...
s32 gt1x_send_cfg(u8 * config, int cfg_len)
{
	...
	const u8 cfg_clear[] = GTP_CFG_CLEAR;
	...
	gt1x_cfg_clear_length =CFG_GROUP_LEN(cfg_clear);
	memset(gt1x_config_clear, 0, sizeof(gt1x_config_clear));
	memcpy(gt1x_config_clear, cfg_clear, gt1x_cfg_clear_length);

	while (retry++ < 5) {
		ret = gt1x_i2c_write(GTP_REG_CONFIG_DATA, gt1x_config_clear, gt1x_cfg_clear_length);
		if (!ret) {
			msleep(200);	/* at least 200ms, wait for storing config into flash. */
			GTP_DEBUG("Clear firmware successfully!");
			break;
		}
	}
	if(retry == 5) {
		GTP_ERROR("Clear firmware failed!");
	}
