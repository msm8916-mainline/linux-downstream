...
*/
	...
	if (!gpio_is_valid(power_ldo_gpio)) {
		GTP_ERROR("Invalid GPIO, power-gpio:%d \n",power_ldo_gpio);
        	return -EINVAL;
	}
...
int gt1x_power_switch(int on)
{
    ...
    if (!client || (!gpio_is_valid(gt1x_int_gpio)) || !vcc_i2c)
        return -1;
	...
	*/
		gpio_set_value(power_ldo_gpio, 1);
		udelay(2);
		...
		gpio_set_value(power_ldo_gpio, 0);
	}
	return ret;
...
static s32 gt1x_request_io_port(void)
{
	...
	ret = gpio_request(GTP_POWER_PORT, "GTP_POWER_PORT");
	if (ret < 0) {
		GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", (s32) GTP_POWER_PORT, ret);
		ret = -ENODEV;
	}
	GTP_GPIO_OUTPUT(GTP_POWER_PORT,1);
