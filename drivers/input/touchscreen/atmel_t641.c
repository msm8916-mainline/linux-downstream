...
//HMOON #include <linux/mfd/pm8xxx/cradle.h>
...
#ifdef TOUCH_INFO_MSG
#undef TOUCH_INFO_MSG
#endif
#define TOUCH_INFO_MSG(args...) \
    do { \
        if (global_mxt_data != NULL && global_mxt_data->msg_info == 1)\
            printk(KERN_INFO  "[Touch I] " args); \
    } while (0)

#ifdef TOUCH_DEBUG_MSG
#undef TOUCH_DEBUG_MSG
#endif
#define TOUCH_DEBUG_MSG(args...) \
    do { \
        if (global_mxt_data != NULL && global_mxt_data->msg_debug == 1)\
            printk(KERN_DEBUG  "[Touch D] " args); \
    } while (0)
...
// HMOON
	...
	//HMOON kobject_uevent_env(&lge_touch_sys_device.kobj, KOBJ_CHANGE, string);
	TOUCH_ERR_MSG("uevent[%s]\n", string[0]);
}
...
static int card_detect_solution(struct mxt_data *data, int finger_id)
{
	...
	if (data->incoming_call == INCOMING_CALL_RINGING && data->ts_data.total_num >= 2 ) {
		TOUCH_ERR_MSG("incoming_call num = %d \n", data->ts_data.total_num);
		goto ghost_err;
	}
...
static int mxt_lookup_bootloader_address(struct mxt_data *data, u8 retry)
{
	...
	TOUCH_DEBUG_MSG("%s appmode=0x%x family id =0x%x\n", __func__, appmode, family_id);
	...
	TOUCH_DEBUG_MSG("%s bootloader_addr=0x%x family id 0x%x\n", __func__, bootloader, family_id);
	return 0;
}
...
struct mxt_object *mxt_get_object(struct mxt_data *data, u8 type)
{
	...
	TOUCH_DEBUG_MSG("Invalid object type T%u\n", type);
	return NULL;
}
EXPORT_SYMBOL(mxt_get_object);
...
/*
	if (offset >= mxt_obj_size(object) * mxt_obj_instances(object)) {
		TOUCH_ERR_MSG("Tried to write outside object T%d offset:%d, size:%d\n",
				type, offset, mxt_obj_size(object));

		return -EINVAL;
	}

	TOUCH_ERR_MSG("mxt_write_object T[%d:%X] offset[%d] val[%d]\n",
...
EXPORT_SYMBOL(mxt_write_object);
...
static int mxt_set_diagnostic_mode(struct mxt_data *data, u8 dbg_mode)
{
	...
	if (dbg_mode & MXT_DIAG_MODE_MASK) {
		...
		TOUCH_DEBUG_MSG("current dianostic chip mode is %d\n", cur_mode);
	}
...
static void mxt_prepare_debug_data(struct mxt_data *data)
{
	...
	if (!rawdata->reference) {
		TOUCH_ERR_MSG("Fail to alloc reference of rawdata\n");
		error = -ENOMEM;
	...
	if (!rawdata->delta) {
		TOUCH_ERR_MSG("Fail to alloc delta of fdata\n");
		error = -ENOMEM;
...
//	u16 diff_butt_ref[4] = {0};
	...
	TOUCH_DEBUG_MSG("Write T71 CRC [%X]\n", data->fw_info.cfg_crc);
//	ret = mxt_write_mem(data, object->start_address + 196, 4, (u8*)&data->fw_info.cfg_crc);
	...
	if(cap_diff != NULL) {
		for(y = data->channel_size.start_y; y < data->channel_size.start_y + data->channel_size.size_y; y++) {
			...
			TOUCH_DEBUG_MSG("cap_diff[%d] %d\n",y ,cap_diff[y]);
		}
	}

	if (object!=NULL) {
		for( y = data->channel_size.start_y ; y < data->channel_size.start_y + data->channel_size.size_y; y++) {
			TOUCH_DEBUG_MSG("write cap:diff Y%d Line, %d\n", y, cap_diff[y]);
	...
	if (object!=NULL) {
		for (i = 0; i < data->pdata->t15_num_keys-1; i++) {
			...
			TOUCH_DEBUG_MSG("diff_butt_ref(%d) %d prev[%d] cur[%d]\n", i, b_diff,
			...
			if (ret) {
				TOUCH_ERR_MSG("button diff mem write fail\n");
			}
		}
	}
#endif
	...
	TOUCH_DEBUG_MSG("backup return:%d\n", ret);
...
static void mxt_proc_t6_messages(struct mxt_data *data, u8 *msg)
{
	...
	if (crc != data->config_crc) {
		data->config_crc = crc;
		TOUCH_DEBUG_MSG("T6 Config Checksum: 0x%06X\n", crc);
	...
	if (status != data->t6_status)
		TOUCH_DEBUG_MSG("T6 Status 0x%02X%s%s%s%s%s%s%s\n",
			status,
			(status == 0) ? " OK" : "",
...
int mxt_get_self_delta_chk(struct mxt_data *data)
{
	...
	s16 curr_delta;
      ...
      if(process_chk == 1)
      {
        TOUCH_DEBUG_MSG("Self Delta read Processing ..\n");
        return 0;
      }
      else
      {
	...
	dbg_object = mxt_get_object(data, MXT_DEBUG_DIAGNOSTIC_T37);
	if (!dbg_object) {
		TOUCH_DEBUG_MSG("Failed to get object_info\n");
		process_chk = 0;
		return 1;
	}
	...
	for(i=0; i<7; i++)
	{
		...
		if (comms_chk[0] == MXT_COMMAND_SELF_DELTA && comms_chk[1] == 0) {
			TOUCH_DEBUG_MSG("%s Enter success in Self delta mode\n", __func__);
			break;
		}
		else if(i == 4)
		{
			TOUCH_DEBUG_MSG("%s Enter fail in Self delta mode\n", __func__);
			process_chk = 0;
	...
	for (read_page = 0 ; read_page < 3; read_page++) {
		...
		TOUCH_DEBUG_MSG("CURR_DELTA read page %u :", read_page);
		for(i=0; i<6; i++)
		{
		...
		for (i = 0; i < NODE_PER_PAGE; i++) {
			curr_delta =( ((u16)ret_buf[i*2+1] << 8) + (u16)ret_buf[i*2] ); // =?? ????

			//TOUCH_DEBUG_MSG("%6hd", curr_delta);
			...
			if(read_page == 0)
			{
				...
				if(num < 19)
				{
					...
					else
					{
        				    ...
        				    {
								TOUCH_DEBUG_MSG("self touch anti fail Y%d Val(%d) Threhold(%d) curr cnt(%d):\n", num, curr_delta, (global_mxt_data->self_delta_chk[2] *  8 * -1), touch_anti_tot_cnt+1);
        				        touch_anti_tot_cnt++;
        				    }
					}
				...
				else if(num >= 20 && num < 50)
				{
					...
					else
					{
        				    if(curr_delta < (global_mxt_data->self_delta_chk[2] *  8 * -1))
        				    {
								TOUCH_DEBUG_MSG("self touch anti fail X%d Val(%d) Threhold(%d) curr cnt(%d):\n", num-20, curr_delta, (global_mxt_data->self_delta_chk[2] *  8 * -1), touch_anti_tot_cnt+1);
        				        touch_anti_tot_cnt++;
        				    }
					}
				}
			...
			else if (read_page == 2)
			{
				...
				if(num < 19)
				{
					...
					else
					{
        				    ...
        				    {
								TOUCH_DEBUG_MSG("self proc anti fail Y%d Val(%d) Threhold(%d) curr cnt(%d):\n", num, curr_delta, (global_mxt_data->self_delta_chk[12] *  4 * -1), proc_anti_tot_cnt+1);
        				        proc_anti_tot_cnt++;
        				    }
					}
				}
				else if(num >= 20 && num < 50)
				{
					...
					else
					{
        				    if(curr_delta < (global_mxt_data->self_delta_chk[12] *  4 * -1))
        				    {
								TOUCH_DEBUG_MSG("self proc anti fail X%d Val(%d) Threhold(%d) curr cnt(%d):\n", num, curr_delta, (global_mxt_data->self_delta_chk[12] *  4 * -1), proc_anti_tot_cnt+1);
        				        proc_anti_tot_cnt++;
        				    }
					}
		...
		ret = mxt_set_diagnostic_mode(data, MXT_DIAG_PAGE_UP);
		if (ret) {
			TOUCH_DEBUG_MSG("Failed to set self delta mode!\n");
			process_chk = 0;
			return 0; // Don't check self delta no more!!
		}
...
int mxt_get_reference_chk(struct mxt_data *data)
{
	...
	TOUCH_ERR_MSG("half_err_cnt(%d) ref_err_cnt(%d)\n", half_err_cnt, data->ref_limit.ref_err_cnt);
	...
	data->ref_chk = 0;
	//TOUCH_DEBUG_MSG("Err Cnt is %d, limit Cnt is %d\n",err_cnt, data->ref_limit.ref_err_cnt);
		...
		if (err_cnt < half_err_cnt) {
			...
			for (i=0; i < data->pdata->t15_num_keys - 1; i++) {
				...
				if (diff_b_v > 4000 || diff_b_v < -4000) {
					TOUCH_DEBUG_MSG("damaged key channel no calibration\n");
					err_chk = 0;
					retry_chk = 0;
					return 0;
				}
				...
				if (diff_b_v > (data->ref_limit.butt_dif[i] + data->ref_limit.err_weight) * 8 ||
					...
					TOUCH_DEBUG_MSG("%s Button Hole Occur!\n", get_touch_button_string(data->pdata->t15_keymap[i]));
					mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
					return 1;
				}
			}
...
static void mxt_proc_t9_message(struct mxt_data *data, u8 *message)
{
	...
	if (unlikely( id < 0 || id >= MXT_MAX_FINGER)) {
		TOUCH_ERR_MSG("%s wrong id:%d \n", __func__, id);
		return;
	}

	if (status & MXT_T9_SUPPRESS) {
		if (touched_finger_count) {
			TOUCH_DEBUG_MSG(" MXT_T9_SUPPRESS\n");
			mxt_reset_slots(data);
		}
		if (data->palm) {
		...
		if (touched_finger_count && data->ts_data.prev_data[id].status < MXT_STATE_PRESS) {
			for (i = MXT_MAX_FINGER - 1; i >= 0; i--) {
				if (data->ts_data.prev_data[i].status>= MXT_STATE_PRESS) {
					TOUCH_DEBUG_MSG("finger id changed <%d> -> <%d> \n", id, i);
					id = i;
...
static void mxt_proc_t72_messages(struct mxt_data *data, u8 *message)
{
	u8 state = 0;
	u8 peak_noise_level = 0;
	u8 noise_level = 0;

	state = (message[2] & 0x07);
	peak_noise_level = message[4];
	noise_level = message[5];

	TOUCH_INFO_MSG("MXT_PROCG_NOISESUPPRESSION_T72 state : %s peak_noise_level : %d noise_level : %d\n",
		(state == 1) ? "OFF" : (state == 2) ? "STABLE" : (state == 3) ? "NOISY" : (state == 4) ? "VERY_NOISY" : "Reserved",
		peak_noise_level, noise_level);
}
...
static int mxt_proc_t25_message(struct mxt_data *data, u8 *message)
{
	...
	if ( status == 0xFE ) {
		TOUCH_DEBUG_MSG("[SUCCESS] All tests passed\n");
...
static int mxt_proc_t37_message(struct mxt_data *data, u8 *msg_buf)
{
	...
	#if 0
	TOUCH_DEBUG_MSG("t37: %d, %d, %d, %d, %d, %d, %d, %d, %d\n",
...
retry:
	...
	if (data->g_tap_cnt != tap_num && data->mxt_multi_tap_enable) {
	    TOUCH_ERR_MSG("Tab count dismatch\n");
	    goto error;
	} else {
...
//HMOON
		...
		//HMOON kobject_uevent_env(&lge_touch_sys_device.kobj, KOBJ_CHANGE, knockon_event);
	} else {
...
/*
	if(scr_status & MXT_T100_FRIST_ID_SUPPRESSION) {
		TOUCH_DEBUG_MSG("T100_message First Report id has SUP!!! %02X Release all\n", scr_status);
		mxt_reset_slots(data);
		return;
	}
*/
		...
		* happened */
		...
		if ((status & MXT_T100_STATUS_MASK) == MXT_T100_RELEASE || (status & MXT_T100_STATUS_MASK) == MXT_T100_SUPPRESSION) {
			...
			if((status & MXT_T100_STATUS_MASK) == MXT_T100_SUPPRESSION)
				TOUCH_DEBUG_MSG(  "T100_message[%u] ###DETECT && SUPPRESSION (%02X)\n", id, status);
		}
...
///		}
	...
	if (data->debug_enabled) {
		TOUCH_DEBUG_MSG( "T100_message[%u] %s%s%s%s%s%s%s%s%s %s%s%s%s%s (0x%02X) x:%u y:%u z:%u area:%u amp:%u vec:%u h:%u w:%u\n",
			id,
...
{
	TOUCH_DEBUG_MSG("USB trigger USB_type: %d \n", usb_type);
	...
	if (global_mxt_data && global_mxt_data->patch.event_cnt) {
		...
		if (global_mxt_data->regulator_status == 0) {
			TOUCH_DEBUG_MSG("IC Regulator Disabled. Do nothing\n");
			if (usb_type == 0) {
		...
		if (global_mxt_data->mfts_enable && global_mxt_data->pdata->use_mfts) {
			TOUCH_DEBUG_MSG("MFTS : Not support USB trigger \n");
			return;
		}
...
static irqreturn_t mxt_process_messages_t44(struct mxt_data *data)
{
	...
	if (!regulator_is_enabled(data->vcc_i2c)) {
		TOUCH_ERR_MSG( "I2C Regulator Already Disabled.\n");
		return IRQ_NONE;
	}
...
//		TOUCH_DEBUG_MSG("Interrupt triggered but zero messages\n");
		return IRQ_HANDLED;
	} else if (count > data->max_reportid) {
		TOUCH_DEBUG_MSG("T44 count %d exceeded max report id\n", count);
	...
	if (ret < 0) {
		TOUCH_DEBUG_MSG( "Unexpected invalid message\n");
		return IRQ_NONE;
	}
...
static int mxt_init_t93_tab_count(struct mxt_data *data)
{
	...
	if (object) {
		...
		TOUCH_DEBUG_MSG("%s data->g_tap_cnt : %d\n", __func__, data->g_tap_cnt);
		return 0;
	} else {
		data->g_tap_cnt = 0;
		return 1;
...
static int mxt_check_reg_init(struct mxt_data *data, const char *name)
{
	...
	if (data->config_crc == config_crc) {
		TOUCH_DEBUG_MSG("Config already applied \n");
		ret = 0;
		goto release_mem;
	}

	if (memcmp((char *)data->info, (char *)&cfg_info, sizeof(struct mxt_info)) != 0) {
		TOUCH_ERR_MSG("Compatibility Error. Could not apply\n");
	...
	TOUCH_DEBUG_MSG("Config written\n");
...
static int mxt_parse_object_table(struct mxt_data *data)
{
	...
	data->mem_size = 0;
//641t porting S
	TOUCH_DEBUG_MSG("mxt_parse_object_table\n");
//641t porting E
	for (i = 0; i < data->info->object_num; i++) {
		object = data->object_table + i;
...
//641t porting E
		...
		case MXT_TOUCH_MULTI_T9:
			...
			//TOUCH_DEBUG_MSG("T9_reportid_min:%d T9_reportid_max:%d\n", data->T9_reportid_min, data->T9_reportid_max);
			break;
		case MXT_TOUCH_KEYARRAY_T15:
...
static int mxt_read_info_block(struct mxt_data *data)
{
	...
	if (data->info_crc != calculated_crc) {
		TOUCH_DEBUG_MSG("*************** Info Block CRC error calculated=0x%06X read=0x%06X\n",
	...
	TOUCH_DEBUG_MSG("Family:%02X Variant:%02X Binary:%u.%u.%02X TX:%d RX:%d Objects:%d\n",
...
static int mxt_read_t9_resolution(struct mxt_data *data)
{
	...
	TOUCH_DEBUG_MSG("Touchscreen size X:%u Y:%u\n", data->max_x, data->max_y);
...
static void mxt_regulator_enable(struct mxt_data *data)
{
	...
	/* vcc_dig enable */
	gpio_set_value(data->pdata->gpio_ldo3, 1);
	...
	if (data->vcc_i2c) {
		...
		if (error < 0) {
			TOUCH_ERR_MSG("vcc_i2c regulator enable fail\n");
			return;
		}
	}
...
static void mxt_regulator_disable(struct mxt_data *data)
{
	...
	/* vdd_ana enable */
	gpio_set_value(data->pdata->gpio_ldo3, 0);
	...
	if (data->vcc_i2c) {
		...
		if (error < 0) {
			TOUCH_ERR_MSG("vcc_i2c regulator disable fail\n");
			return;
		}
	}
...
static void mxt_probe_regulators(struct mxt_data *data)
{
	...
	if (!data->pdata->gpio_reset) {
		TOUCH_ERR_MSG("Must have reset GPIO to use regulator support\n");
		goto fail;
	}
	...
	if (error < 0) {
		TOUCH_ERR_MSG("Error %d cannot control i2c regulator\n", error);
		goto fail;
	}
...
static int mxt_configure_objects(struct mxt_data *data)
{
	...
	if (data->T9_reportid_min) {
		...
		TOUCH_DEBUG_MSG("No touch object detected\n");
	}
	return 0;
}
...
static void mxt_read_fw_version(struct mxt_data *data)
{
	...
	TOUCH_DEBUG_MSG("Family Id        = 0x%02X \n", data->info->family_id);
	TOUCH_DEBUG_MSG("Variant          = 0x%02X \n", data->info->variant_id);
	TOUCH_DEBUG_MSG("==================================\n");
}
...
static ssize_t mxt_object_control(struct mxt_data *data, const char *buf, size_t count)
{
	...
	if (!strncmp(command, "read", 4)) {	/*read*/
		...
		TOUCH_DEBUG_MSG("Object Write Success. Execute Read Object and Check Value.\n");
	}else{
...
static int mxt_check_firmware_format(struct device *dev, const struct firmware *fw)
{
	...
	TOUCH_DEBUG_MSG("Aborting: firmware file must be in binary format\n");
...
static int mxt_load_bin(struct device *dev, const char *name)
{
	...
	if (ret) {
		...
		TOUCH_DEBUG_MSG("Unlocking bootloader\n");
	...
	if (ret)
		goto disable_irq;
	TOUCH_DEBUG_MSG("Sent %d frames, %zd bytes\n", frame, pos);
...
static ssize_t mxt_update_bin_store(struct mxt_data *data, const char *buf, size_t count)
{
	...
	if (error) {
		...
		TOUCH_ERR_MSG("Need to update proper Configuration(RAW) \n");
	}
	return count;
}
...
static ssize_t mxt_update_raw_store(struct mxt_data *data, const char *buf, size_t count)
{
	...
	sscanf(buf, "%d", &value);
	TOUCH_ERR_MSG("Update mxt Configuration.\n");
...
static ssize_t mxt_patch_debug_enable_show(struct mxt_data *data, char *buf)
{
	...
	if (data->patch.patch == NULL) {
		TOUCH_DEBUG_MSG("patch not support \n");
		return count;
	}
...
static ssize_t mxt_power_control_show(struct mxt_data *data, char *buf)
{
	...
	ret += sprintf(buf+ret, "  T7 cfg ACTV %d  \n", data->t7_cfg.active);
	ret += sprintf(buf+ret, "  T7 cfg IDLE %d  \n", data->t7_cfg.idle);

	TOUCH_DEBUG_MSG("T7 cfg ACTV %d, IDLE %d \n", data->t7_cfg.active, data->t7_cfg.idle);
	return ret;
}
...
static ssize_t mxt_knock_on_store(struct mxt_data *data, const char *buf, size_t size)
{
	...
	if (data->suspended) {
		...
		if (value) {
			...
			if (mxt_patchevent_get(PATCH_EVENT_KNOCKON)) {
				TOUCH_DEBUG_MSG("Suspend : Knock On already enabled \n");
			} else {
				mxt_patchevent_set(PATCH_EVENT_KNOCKON);
				if (mxt_patchevent_get(PATCH_EVENT_TA)) {
					TOUCH_DEBUG_MSG("Suspend : Knock On Enabled(TA) \n");
					mxt_patch_event(global_mxt_data, CHARGER_KNOCKON_SLEEP);
				} else {
					TOUCH_DEBUG_MSG("Suspend : Knock On Enabled \n");
					mxt_patch_event(global_mxt_data, NOCHARGER_KNOCKON_SLEEP);
				}
			}
			...
			if (mxt_patchevent_get(PATCH_EVENT_KNOCKON)) {
				TOUCH_DEBUG_MSG("Suspend : Knock On Disabled \n");
				mxt_set_t7_power_cfg(data, MXT_POWER_CFG_DEEPSLEEP);
				mxt_regulator_disable(data);
				mxt_patchevent_unset(PATCH_EVENT_KNOCKON);
			} else {
				TOUCH_DEBUG_MSG("Suspend : Knock On already disabled \n");
			}
...
#ifdef TOUCH_HALL_IC_SUPPORT
	if (data->suspended && gpio_get_value(HALL_IC_GPIO)) {
		TOUCH_ERR_MSG("%s : Wake Up from Quick Cover.\n", __func__);
		return;
	}
#endif
...
//			mxt_set_t7_power_cfg(data, MXT_POWER_CFG_DEEPSLEEP);
		...
		if (value == LPWG_NONE) {
			data->mxt_knock_on_enable= false;
			data->mxt_multi_tap_enable= false;
			TOUCH_DEBUG_MSG("KnockOn/Multitap Gesture Disable\n");
		} else {
			TOUCH_DEBUG_MSG("Unknown Value. Not Setting\n");
			return;
		}
	}
}
...
static void mxt_lpwg_control(struct mxt_data *data, u32 value, bool onoff)
{
	...
	if (data->in_bootloader){
		TOUCH_ERR_MSG("%s : Fw upgrade mode.\n", __func__);
		return;
	}
...
static void lpwg_double_tap_check(struct mxt_data *data, u32 value)
{
	...
	TOUCH_DEBUG_MSG("%s Double Tap check\n", __func__);
...
static void atmel_ts_lpwg_update_all(struct i2c_client* client, u32 code, u32* value)
{
	...
	TOUCH_ERR_MSG("%s mode:%d, Scree:%d Proximity:%d(FAR:1, NEAR:0) QCover:%d(OPEN:0, CLOSE:1) \n",
			__func__, data->lpwg_mode, data->screen, data->sensor, data->qcover);

	if(data->screen) { // Screen On
		if(data->suspended){
			TOUCH_DEBUG_MSG("Screen On required and suspended status\n");
			touch_disable_irq_wake(data->irq);
			touch_disable_irq(data->irq);
			data->enable_reporting = false;
		}else{
			TOUCH_DEBUG_MSG("Screen On required but no suspended status. Skipped\n");
		}
	}
...
err_t atmel_ts_lpwg(struct i2c_client* client, u32 code, u32 value, struct point *tci_point)
{
	struct mxt_data *data = i2c_get_clientdata(client);
	int i;
	TOUCH_DEBUG_MSG("%s Code: %d Value: %d\n", __func__, code, value);
...
static ssize_t mxt_msg_error_show(struct mxt_data *data, char *buf)
{
	int len = 0;
	len += snprintf(buf + len, PAGE_SIZE - len, "%d\n", data->msg_error);
	return len;
}
static ssize_t mxt_msg_error_store(struct mxt_data *data, const char *buf, size_t count)
{
	int value = 0;
	sscanf(buf, "%d", &value);
	data->msg_error = value;
	TOUCH_TEST_MSG("%s : %d \n", __func__, value);
	return count;
}

static ssize_t mxt_msg_info_show(struct mxt_data *data, char *buf)
{
	int len = 0;
	len += snprintf(buf + len, PAGE_SIZE - len, "%d\n", data->msg_info);
	return len;
}
static ssize_t mxt_msg_info_store(struct mxt_data *data, const char *buf, size_t count)
{
	int value = 0;
	sscanf(buf, "%d", &value);
	data->msg_info = value;
...
static ssize_t mxt_msg_debug_show(struct mxt_data *data, char *buf)
{
	int len = 0;
	len += snprintf(buf + len, PAGE_SIZE - len, "%d\n", data->msg_debug);
	return len;
}
static ssize_t mxt_msg_debug_store(struct mxt_data *data, const char *buf, size_t count)
{
	int value = 0;
	sscanf(buf, "%d", &value);
	data->msg_debug = value;
	TOUCH_TEST_MSG("%s : %d \n", __func__, value);
	return count;
}
...
static LGE_TOUCH_ATTR(msg_error, S_IWUSR | S_IRUSR, mxt_msg_error_show, mxt_msg_error_store);
static LGE_TOUCH_ATTR(msg_info, S_IWUSR | S_IRUSR, mxt_msg_info_show, mxt_msg_info_store);
static LGE_TOUCH_ATTR(msg_debug, S_IWUSR | S_IRUSR, mxt_msg_debug_show, mxt_msg_debug_store);
...
static LGE_TOUCH_ATTR(ts_noise, S_IRUGO, mxt_info_show, NULL);
static LGE_TOUCH_ATTR(ts_noise_log_enable, S_IRUGO, mxt_info_show, NULL);
...
static struct attribute *lge_touch_attribute_list[] = {
	...
	&lge_touch_attr_msg_error.attr,
	&lge_touch_attr_msg_info.attr,
	&lge_touch_attr_msg_debug.attr,
...
static void mxt_reset_slots(struct mxt_data *data)
{
	...
	TOUCH_DEBUG_MSG("Release all event \n");
...
static void mxt_stop(struct mxt_data *data)
{
	...
	if (data->mxt_knock_on_enable) {
		...
		TOUCH_DEBUG_MSG("%s MXT_POWER_CFG_DEEPSLEEP\n", __func__);
		// mxt_set_t7_power_cfg(data, MXT_POWER_CFG_DEEPSLEEP);
		mxt_regulator_disable(data);
	}
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
	...
	if (rc) {
		TOUCH_DEBUG_MSG("DT : Unable to read numtouch\n");
	} else {
	...
	if (rc) {
		TOUCH_DEBUG_MSG("DT : Unable to read auto_fw_update\n");
	} else {
		pdata->auto_fw_update = temp_val;
		TOUCH_DEBUG_MSG("DT : auto_fw_update = %d\n", pdata->auto_fw_update);
	}
	...
	if (rc) {
		...
		TOUCH_DEBUG_MSG("DT : panel_check = %d\n", pdata->panel_check);
	}
	...
	if (pdata->panel_check) {
		...
		if (rc && (rc != -EINVAL)) {
			TOUCH_DEBUG_MSG("DT : atmel,extra_fw_name error \n");
			pdata->extra_fw_name = NULL;
		}
		else
			TOUCH_DEBUG_MSG("DT : extra_fw_name : %s \n", pdata->extra_fw_name);
	} else {
		pdata->extra_fw_name = NULL;
	}
	...
	if (rc) {
		TOUCH_DEBUG_MSG("DT : Unable to read knock_on_type - set as 0\n" );
	...
	if (rc) {
		TOUCH_DEBUG_MSG("DT : Unable to read global_access_pixel - set as 0\n" );
	...
	if (rc) {
		TOUCH_DEBUG_MSG("DT : Unable to read use_mfts - set as false \n" );
		pdata->use_mfts = 0;
	} else {
	...
	if (rc && (rc != -EINVAL)) {
		TOUCH_DEBUG_MSG( "DT : Unable to read lcd_x\n");
		pdata->lcd_x = 540;
	} else {
	...
	if (rc && (rc != -EINVAL)) {
		TOUCH_DEBUG_MSG( "DT : Unable to read lcd_y\n");
		pdata->lcd_y = 960;
	} else {
	...
	if (rc && (rc != -EINVAL)) {
		TOUCH_DEBUG_MSG( "DT : Unable to read ghost_detection_enable\n");
		pdata->ghost_detection_enable = 0;
	} else {
		pdata->ghost_detection_enable = temp_val;
	}
	TOUCH_DEBUG_MSG("DT : ghost_detection_enable: %d\n",pdata->ghost_detection_enable);
	...
	if (rc && (rc != -EINVAL)) {
		TOUCH_DEBUG_MSG( "DT : Unable to read butt_check_enable\n");
...
*/
	...
	// HMOON error = input_mt_init_slots(input_dev, data->num_touchids);
	if (error) {
...
int mxt_initialize_t9_input_device(struct mxt_data *data)
{
	...
	// HMOON error = input_mt_init_slots(input_dev, num_mt_slots);
	if (error) {
	...
	if (error) {
		...
		TOUCH_DEBUG_MSG("input_register_device done\n");
	}
...
int mxt_verify_fw(struct mxt_fw_info *fw_info, const struct firmware *fw)
{
	...
	if (le32_to_cpu(fw_img->magic_code) != MXT_FW_MAGIC) {
		...
		TOUCH_DEBUG_MSG("Firmware file only consist of raw firmware\n");
			...
			if (ppheader->magic == MXT_PATCH_MAGIC) {
				...
				if (ppheader->size) {
					...
					if (patch) {
						...
						TOUCH_DEBUG_MSG("%s Patch Updated \n", __func__);
					} else {
						data->patch.patch = NULL;
...
static int mxt_get_object_table(struct mxt_data *data)
{
	...
	//TOUCH_DEBUG_MSG("%s data=[0x%X] \n", __func__, (int)data);
	//TOUCH_DEBUG_MSG("%s data->object_table=[0x%X] \n", __func__, (int)data->object_table);
...
static int mxt_flash_fw(struct mxt_fw_info *fw_info)
{
	...
	u8 *fw_data = NULL; //fw_info->fw_raw_data;
#else
...
#if 1  // FW_DATA KZALLOC
	fw_data = kzalloc((fw_size + 2), GFP_KERNEL);
	if (!fw_data) {
		TOUCH_ERR_MSG("%s firmware data is Null\n", __func__);
		return -ENOMEM;
	} else {
		TOUCH_ERR_MSG("kzalloc() allocated size %d",(fw_size + 2));
	}
    memset(fw_data, 0x0, (fw_size+2));
	...
	while (pos < fw_size) {
		...
		*/
		...
		ret = mxt_check_bootloader(data, MXT_FRAME_CRC_PASS);
		if (ret) {
			TOUCH_ERR_MSG("Fail updating firmware. frame_crc err\n");
			goto out;
		}
...
static int mxt_flash_fw_on_probe(struct mxt_fw_info *fw_info)
{
	...
	if (data->info->version == fw_info->bin_ver && data->info->build == fw_info->build_ver) {
		TOUCH_DEBUG_MSG("Binary Version is same \n");
		goto out;
	}
...
/*
	...
	if(object != NULL)
	{
		...
		TOUCH_DEBUG_MSG("data->t71_diff_using %d \n", data->t71_diff_using);
		if (ret) {
			TOUCH_ERR_MSG("t71 read fail before firmware change\n");
			goto out;
		}

		ret = mxt_read_mem(data, object->start_address + 60, 14, data->t71_diff_val);
		TOUCH_DEBUG_MSG("data->t71_diff_val %d %d %d %d %d %d \n", data->t71_diff_val[0],data->t71_diff_val[1],data->t71_diff_val[2],data->t71_diff_val[3],data->t71_diff_val[4],data->t71_diff_val[5]);
		if (ret) {
			TOUCH_ERR_MSG("Reference Diff value Read fail before firmware change\n");
			goto out;
		}
	}
*/
...
load_fw:
	...
	if (error)
		...
		TOUCH_DEBUG_MSG("succeeded updating firmware\n");
out:
	return error;
}
...
static int mxt_table_initialize(struct mxt_data *data)
{
	...
	if (read_info_crc != calc_info_crc) {
		TOUCH_ERR_MSG("Infomation CRC error :[CRC 0x%06X!=0x%06X]\n",
...
static int mxt_write_config(struct mxt_fw_info *fw_info)
{
	...
	if (!fw_info->cfg_raw_data) {
		TOUCH_ERR_MSG("No cfg data in file\n");
		return ret;
	}
	...
	if (ret) {
		TOUCH_ERR_MSG("fail to read config crc\n");
		return ret;
	}
	...
	TOUCH_DEBUG_MSG("Writing Config:[FW:%06X] [IC:%06X]\n", fw_info->cfg_crc, current_crc);

	/* Saved diff value */
	if (buf_crc_t71[0] == 1) {
		TOUCH_DEBUG_MSG("buf_crc_t71[0] == 1\n");
		ret = mxt_read_mem(data, object->start_address + 60, 14 + data->pdata->t15_num_keys, &data->t71_diff_val);
		if (ret) {
			TOUCH_ERR_MSG("Reference Diff value Read fail\n");
			return ret;
		}
	}
	...
	for (index = 0; index < fw_info->cfg_len;) {
		if (index + sizeof(struct mxt_cfg_data) >= fw_info->cfg_len) {
			TOUCH_ERR_MSG("index(%d) of cfg_data exceeded total size(%d)!!\n",
		...
		if (index > fw_info->cfg_len) {
			TOUCH_ERR_MSG("index(%d) of cfg_data exceeded total size(%d) in T%d object!!\n",
		...
		//TOUCH_DEBUG_MSG("Type[%2d] Size[%3d] Instance[%d] Len[%3d] \n", cfg_data->type, object->size_minus_one, cfg_data->instance, fw_info->cfg_len);
	}

	TOUCH_DEBUG_MSG("Configuration Updated \n");

	/* Restore diff value */
	if (buf_crc_t71[0] == 1) {
		TOUCH_DEBUG_MSG("Restore diff value \n");
		object = mxt_get_object(data, MXT_SPT_DYNAMICCONFIGURATIONCONTAINER_T71);
		...
		if (ret) {
			TOUCH_ERR_MSG("Reference Diff Restore fail\n");
			return ret;
		}
...
int mxt_request_firmware_work(const struct firmware *fw, void *context)
{
	...
	if (error){
		TOUCH_ERR_MSG("Failed to mxt_flash_fw_on_probe\n");
		goto out;
	}
...
int mxt_update_firmware(struct mxt_data *data, const char *fwname)
{
	...
	if( error ){
		TOUCH_ERR_MSG("mxt_request_firmware_work with error %d \n", error);
		return 1;
	}
...
static int mxt_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#if defined(CONFIG_TOUCHSCREEN_LGE_LPWG)
	TOUCH_DEBUG_MSG("Use LPWG feature\n");
	...
	/* request touch_ldo_en2 pin for vdd_ana */
	if (data->pdata->gpio_ldo3> 0) {
		error = gpio_request(data->pdata->gpio_ldo3, "touch_ldo3_en");
		if (error < 0) {
			TOUCH_ERR_MSG("FAIL: touch_ldo2_en gpio_request\n");
			goto err_interrupt_failed;
		}
		gpio_direction_output(data->pdata->gpio_ldo3, 0);
	}
...
//	mxt_hw_reset(data);
	...
	if (data->pdata->panel_check && data->pdata->extra_fw_name) {
		TOUCH_DEBUG_MSG("%s panel check start \n", __func__);
		...
		if (data->rawdata->reference[0] < 1000 || data->rawdata->reference[3] > 50000) {
			TOUCH_DEBUG_MSG("Extra Pattern panel detected. Config need to change \n");
			...
			TOUCH_DEBUG_MSG("No need to change Config \n");
		}
		TOUCH_DEBUG_MSG("%s panel check done \n", __func__);
	}
...
// HMOON
	...
	if (global_mxt_data) {
		...
		TOUCH_DEBUG_MSG("%s global_mxt_data is NULL \n", __func__);
	}
	data->is_probing = false;

	TOUCH_ERR_MSG("%s success \n", __func__);
...
// HMOON static int __devexit mxt_remove(struct i2c_client *client)
{
	struct mxt_data *data = i2c_get_clientdata(client);
...
static int mxt_fb_resume(struct mxt_data *data)
{
	...
	if (data->pdata->use_mfts && data->mfts_enable) {
		TOUCH_DEBUG_MSG("MFTS : IC Init start \n");
...
//		mxt_hw_reset(data);
		...
		TOUCH_DEBUG_MSG("MFTS : IC Init complete \n");
...
static int __init mxt_init(void)
{
	...
	if(lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO){
		TOUCH_ERR_MSG("chargerlog mxt_init do nothing\n");
		return -EMLINK;
	}
#endif
	...
	if (!touch_multi_tap_wq) {
		TOUCH_ERR_MSG("touch_multi_tap_wq error\n");
		return -ENOMEM;
	}
#endif
...
#ifdef CONFIG_MACH_LGE
	if(lge_get_boot_mode() == LGE_BOOT_MODE_CHARGERLOGO){
		TOUCH_INFO_MSG("chargerlog mxt_exit do nothing\n");
		return;
	}
#endif
