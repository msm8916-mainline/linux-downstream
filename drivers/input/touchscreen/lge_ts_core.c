...
#if defined(CONFIG_MACH_MSM8926_VFP_KR) || defined(CONFIG_MACH_MSM8926_VFPV_KR)
	u8 mfts_enable;
#endif
...
static int touch_ic_init(struct lge_touch_data *ts)
{
	/*
	 * TODO
	 * - ghost, jitter, accuracy enable 설정에 따른 초기화
	 */
...
#if defined(CONFIG_MACH_MSM8926_VFP_KR) || defined(CONFIG_MACH_MSM8926_VFPV_KR)
   if(cradle_state == 1){
	   TOUCH_INFO_MSG("Touch event on Folder Close state. Reset!!\n");
	   release_all_ts_event(ts);
	   memset(&ts->ts_data, 0, sizeof(ts->ts_data));
	   goto err_out_critical;
   }
...
#if defined(CONFIG_MACH_MSM8926_VFP_KR) || defined(CONFIG_MACH_MSM8926_VFPV_KR)
static ssize_t mfts_enable_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct lge_touch_data *ts = dev_get_drvdata(dev);
	int len = 0;
	len = sprintf(buf, "%d\n", ts->mfts_enable);
	TOUCH_INFO_MSG("%s = %d \n", __func__, ts->mfts_enable);
	return len;
}
#endif
#if defined(CONFIG_MACH_MSM8926_VFP_KR) || defined(CONFIG_MACH_MSM8926_VFPV_KR)
static ssize_t mfts_enable_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct lge_touch_data *ts = dev_get_drvdata(dev);
	int value = 0;
	sscanf(buf, "%d", &value);
	TOUCH_INFO_MSG("%s = %d \n", __func__, value);
	ts->mfts_enable = value;
	on_mfts = ts->mfts_enable;
	return count;
}
#endif
...
#endif
#if defined(CONFIG_MACH_MSM8926_VFP_KR) || defined(CONFIG_MACH_MSM8926_VFPV_KR)
static DEVICE_ATTR(mfts, S_IWUSR | S_IRUSR, mfts_enable_show, mfts_enable_store);
#endif
