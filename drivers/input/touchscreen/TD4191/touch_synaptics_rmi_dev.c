...
static struct device_attribute attrs[] = {
	__ATTR(open, S_IWUGO, rmidev_show_error, rmidev_sysfs_open_store),
	__ATTR(release, S_IWUGO,rmidev_show_error, rmidev_sysfs_release_store),
	__ATTR(attn_state, S_IRUGO, rmidev_sysfs_attn_state_show, rmidev_store_error),
	__ATTR(pid, S_IRUGO | S_IWUGO, rmidev_sysfs_pid_show, rmidev_sysfs_pid_store),
	__ATTR(term, S_IWUGO, rmidev_show_error, rmidev_sysfs_term_store),
	__ATTR(intr_mask, S_IRUGO | S_IWUGO, rmidev_sysfs_intr_mask_show, rmidev_sysfs_intr_mask_store),
};
...
static int rmidev_reset_device(struct synaptics_ts_data *ts)
{
	...
	retval = rmidev_i2c_write(ts, ts->common_fc.dsc.command_base, &command, 1);
	...
	for (reg_read_cnt = 1; reg_read_cnt <= 10; reg_read_cnt++) {
		retval = rmidev_i2c_read(ts, ts->common_fc.dsc.query_base, &manufacturer_ID, 1);
	...
	retval = rmidev_i2c_write(ts, ts->common_fc.dsc.control_base + 1, &interrupt_enable_0, 1);
	...
	retval = rmidev_i2c_read(ts, ts->common_fc.dsc.control_base, &device_ctrl, 1);
	...
	retval = rmidev_i2c_write(ts, ts->common_fc.dsc.control_base, &device_ctrl, 1);
...
static int rmidev_irq_enable(struct synaptics_ts_data *ts, bool enable)
{
	...
	if (enable) {
		...
		retval = rmidev_i2c_read(ts, ts->common_fc.dsc.data_base + 1, &intr_status, 1);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(atomic_read(&lge_ts->state.power_state) == POWER_SLEEP) {
		TOUCH_RMIDEV_MSG("Power is sleeping\n");
		return -ENODEV;
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmidev->pid) {
		...
		if (!rmidev->task) {
			TOUCH_RMIDEV_MSG("Failed to locate PID of data logging tool\n");
			return -EINVAL;
		}
	}
...
static void rmidev_attn(unsigned char intr_mask)
{
	TOUCH_RMIDEV_MSG("\n");
