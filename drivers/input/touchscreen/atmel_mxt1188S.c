...
#include "atmel_mxt1188s_config.h"
#define DEBUG_ABS	1
...
static char *knockon_event[2] = { "TOUCH_GESTURE_WAKEUP=WAKEUP", NULL };
static char *lpwg_event[2] = { "TOUCH_GESTURE_WAKEUP=PASSWORD", NULL };
...
static struct tci_abs g_tci_press[MAX_POINT_SIZE_FOR_LPWG];
static struct tci_abs g_tci_report[MAX_POINT_SIZE_FOR_LPWG];
...
recheck:
	...
	error = __mxt_read_reg(data->client, data->T7_address,
				(u16)sizeof(data->t7_cfg), &data->t7_cfg);
	if (error)
		return error;
...
static int mxt_read_info_block(struct mxt_data *data)
{
	...
	size = (u16)sizeof(struct mxt_info);
...
//	int error = -1;
	...
	dev_info(&data->client->dev,"%s enable: %d\n", __func__, enable);
	if(enable)
	{ 
		...
		if(gpio_is_valid(data->pdata->gpio_ldo2)) {
			...
			printk("%s() LDO2 line:%dPIN ERROR\n",__func__, __LINE__);
#if 0	
		/* vcc_dig enable */
		if (data->vcc_dig) {
			error = regulator_enable(data->vcc_dig);
			if (error < 0) {
				printk("%s() vcc_dig regulator enable fail\n", __func__);
				return;
			}
		}
#endif
		...
		if(gpio_is_valid(data->pdata->gpio_ldo1)) 
			...
			printk("%s() line:%d LDO1 PIN ERROR\n",__func__, __LINE__);
		...
		if(gpio_is_valid(data->pdata->gpio_reset)) 
			...
			printk("%s() line:%dreset PIN ERROR\n",__func__, __LINE__);
	...
	else
	{
		...
		if(gpio_is_valid(data->pdata->gpio_reset)) 
			...
			printk("%s() RST PIN ERROR\n",__func__);
#if 0
		/* vcc_dig disable */
		if (data->vcc_dig) {
			error = regulator_disable(data->vcc_dig);
			if (error < 0) {
				printk("%s() vcc_dig regulator disable fail\n", __func__);
				return;
			}
		}
#endif
		...
		if(gpio_is_valid(data->pdata->gpio_ldo2)) 
			...
			printk("%s() LDO2 PIN ERROR\n",__func__);

		/* vcc_i2c disable */
		if(gpio_is_valid(data->pdata->gpio_ldo1)) 
			gpio_direction_output(data->pdata->gpio_ldo1, 0);
		else
			printk("%s() LDO1 PIN ERROR\n",__func__);
...
//	int error;
	...
	if (!data->pdata->gpio_reset) {
		printk("%s() Must have reset GPIO to use regulator support\n", __func__);
		goto fail;
	}
	...
	if (IS_ERR(data->vcc_dig)) {
		...
		printk("%s() Error %d getting ana regulator\n", __func__, error);
		goto fail;
	}

	error = regulator_set_voltage(data->vcc_dig, 3300000, 3300000);
	if (error < 0) {
		printk("%s() Error %d cannot control DVDD regulator\n", __func__, error);
		goto fail;
	}
#endif
...
{
	return (ssize_t)scnprintf(buf, PAGE_SIZE, "%u.%u\n",
...
#ifdef FIRMUP_ON_PROBE
    if (from_header) {
        fw_from_header = devm_kzalloc(dev, sizeof(struct firmware), GFP_KERNEL);
	...
	struct bin_attribute *bin_attr, char *buf, loff_t off, size_t count)
{
	...
	return (ssize_t)ret == 0 ? count : ret;
}
...
static err_t atmel_ts_lpwg(struct i2c_client* client, u32 code, u32 value, struct point *tci_point)
{
...
static void mxt_start(struct mxt_data *data)
{
	...
	dev_info(&data->client->dev, "%s, rst_pin:%d\n", __func__, gpio_get_value(data->pdata->gpio_reset));
...
static int mxt_parse_dt(struct device *dev, struct mxt_platform_data *pdata)
{
    ...
    rc = of_property_read_u32(node, "atmel,accuracy_filter_enable", &temp_val);
	if (rc && (rc != -EINVAL)) {
		printk( "DT : Unable to read ghost_detection_invalid_amplitude_enable\n");
		pdata->accuracy_filter_enable = 0;
	} else {
		pdata->accuracy_filter_enable = temp_val;
	}
	printk("DT : accuracy_filter_enable: %d\n",pdata->accuracy_filter_enable);

    rc = of_property_read_u32(node, "atmel,ghost_detection_enable", &temp_val);
	if (rc && (rc != -EINVAL)) {
		printk( "DT : Unable to read ghost_detection_enable\n");
		pdata->ghost_detection_enable = 0;
	} else {
		pdata->ghost_detection_enable = temp_val;
	}
	printk("DT : ghost_detection_enable: %d\n",pdata->ghost_detection_enable);
...
//	touch_thermal_mode = 0;
	...
	if (client->dev.of_node) {
		...
		if (!data->pdata) {
			printk("Failed to allocate pdata memory\n");
			error = -ENOMEM;
			goto err_free_mem;
		}

// dts parse, config copy from mxt_platform_data
		error = mxt_parse_dt(&client->dev, data->pdata);
		if (error)
			goto err_free_mem;
	}
	data->pdata->config = mxt1188S_config;
	data->pdata->charger_config = mxt1188S_charger_config;
	data->pdata->restore_config = mxt1188S_restore_config;
	data->pdata->pen_config = mxt1188S_pen_config;
	data->pdata->pen_high_speed_config = mxt1188S_pen_high_speed_config;
	data->pdata->pen_low_speed_config = mxt1188S_pen_low_speed_config;
	data->pdata->unpen_config = mxt1188S_unpen_config;
	data->pdata->sus_config = mxt1188S_sus_config;
	data->pdata->sus_charger_config = mxt1188s_sus_charger_config;
	data->pdata->act_config = mxt1188S_act_config;
	data->pdata->sus_udf_config = mxt1188S_udf_sus_config;
	data->pdata->sus_udf_charger_config = mxt1188s_udf_sus_charger_config;
	data->pdata->act_udf_config = mxt1188S_udf_act_config;
	data->pdata->irqflags = IRQF_TRIGGER_FALLING;
	...
	if (data->ts_pinctrl) {
		...
		if (IS_ERR(data->ts_pinset_state_active))
			printk("cannot get ts pinctrl active state\n");
		...
		if (data->ts_pinset_state_active) {
			...
			if (error)
				printk("cannot set ts pinctrl active state \n");
		} else {
			printk("pinctrl active == NULL \n");
		}
	}
	printk("End pinctrl \n");
	...
	error = gpio_request(data->pdata->gpio_ldo1, "touch_ldo1");
	if (error < 0) {
		dev_err(&client->dev, "FAIL: touch_lod1 gpio_request\n");
		goto err_interrupt_failed;
	}
	gpio_direction_output(data->pdata->gpio_ldo1, 0);
	
	error = gpio_request(data->pdata->gpio_ldo2, "touch_ldo2");
	if (error < 0) {
		dev_err(&client->dev, "FAIL: touch_ldo2 gpio_request\n");
		goto err_interrupt_failed;
	}
	gpio_direction_output(data->pdata->gpio_ldo2, 0);
