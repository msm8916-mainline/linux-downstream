...
#if defined(CONFIG_DISABLE_KEYS_AT_SUSPEND)  //disable T97 will cause current leak issue sometimes
/*
...
*/
/*Reduce T100 THR for improving Dwakeup sensitivity by susan; turn off Dynamic configure ;Reduced T7 idle interval*/
	{.reg = MXT_TOUCH_MULTITOUCHSCREEN_T100,
		.offset = 30,.buf = {30}, .len = 1, .mask = 0,.flag = BIT_MASK(P_COMMON)},
	...
	{.reg = MXT_GEN_POWERCONFIG_T7,
		.offset = 0,.buf = {40,10}, .len = 2, .mask = 0,.flag = BIT_MASK(OP_SET)},
...
static int init_pi_object(struct plugin_proci *p)
{
	...
	dev_info(dev, "%s: alloc mem %d, each %zu\n", 
