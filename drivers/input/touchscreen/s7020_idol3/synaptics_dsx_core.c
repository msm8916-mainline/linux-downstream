		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("%s %d and %d ",__func__,rmi4_data->enable_wakeup_gesture,input);

	if (rmi4_data->f11_wakeup_gesture || rmi4_data->f12_wakeup_gesture)
		rmi4_data->enable_wakeup_gesture = input;
pr_err("sb=====enable_wakeup_gesture==gesture=%d========\n",rmi4_data->enable_wakeup_gesture);
	if (input>0)
		device_init_wakeup(&client->dev, 1);
	else
		device_init_wakeup(&client->dev, 0);
...
//		printk("%s,retval=%d,detected_gestures=%d\n",retval,detected_gestures);
		if (retval < 0)
			return 0;
...
//end
	...
	if (retval < 0)
		return 0;
#if 0
	printk("subin---%s-----: 1551max x = %d max y = %d\n",
		__func__,
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F12:
				...
				break;
			case SYNAPTICS_RMI4_F1A:
			//wxc modify for temp from Synap
				break;
