...
static int RunAutoScan(struct synaptics_ts_data *ts)
{
	...
	if (cnt >= 3) {
		...
		WRITE_BUFFER(f54_wlog_buf, rst_size, "Timeout -- Not supported Report Type in FW\n");
...
static int ReadTRexShort(struct synaptics_ts_data *ts)
{
	...
	WRITE_BUFFER(f54_wlog_buf, rst_size, "TRex Short Test\n");
	...
	for (i = 0; i < 7; i++) {
		...
		for (j = 0; j < 8; j++) {
			if ((value & mask) == 1) {
				...
				WRITE_BUFFER(buf, short_num, "%d ", (i * 8 + (7 - j)));
			}

			value >>= 1;
		}

		WRITE_BUFFER(f54_wlog_buf, rst_size, "TRex-TRex Short Test Data = %#x", cap_data[i]);

		if (short_num) {
			WRITE_BUFFER(f54_wlog_buf, rst_size, " (Short TRx Number: ");
			WRITE_BUFFER(f54_wlog_buf, rst_size, buf);
			WRITE_BUFFER(f54_wlog_buf, rst_size, ")\n");
		}
...
static int ReadHighResistance(struct synaptics_ts_data *ts)
{
	...
	WRITE_BUFFER(f54_wlog_buf, rst_size, "High Resistance Test\n");
	WRITE_BUFFER(f54_wlog_buf, rst_size, "Max Rx Offset(pF) = %d\n", maxRxpF);
	WRITE_BUFFER(f54_wlog_buf, rst_size, "Max Tx Offset(pF) = %d\n", maxTxpF);
	WRITE_BUFFER(f54_wlog_buf, rst_size, "Min(pF) = %d\n", minpF);
	WRITE_BUFFER(f54_wlog_buf, rst_size, "\n=====================================================\n");
	WRITE_BUFFER(f54_wlog_buf, rst_size, "\tHigh Resistance Test\n");
	WRITE_BUFFER(f54_wlog_buf, rst_size, "=====================================================\n");
	WRITE_BUFFER(f54_wlog_buf, rst_size, " Parameters: ");
	WRITE_BUFFER(f54_wlog_buf, rst_size, "%5d %5d %5d ", maxRxpF, maxTxpF, minpF);
	WRITE_BUFFER(f54_wlog_buf, rst_size, "\n\n Limits(+) : ");

	for (i = 0; i < 3; i++)
		WRITE_BUFFER(f54_wlog_buf, rst_size, "%5d ", HighResistanceUpperLimit[i]);

	WRITE_BUFFER(f54_wlog_buf, rst_size, "\n Limits(-) : ");

	for (i = 0; i < 3; i++)
		WRITE_BUFFER(f54_wlog_buf, rst_size, "%5d ", HighResistanceLowerLimit[i]);
	...
	if (result == TOUCH_FAIL)
		WRITE_BUFFER(f54_wlog_buf, rst_size, "HighResistance Test failed.\n\n");
	else
		WRITE_BUFFER(f54_wlog_buf, rst_size, "HighResistance Test passed.\n\n");
...
static int ReadRawData(struct synaptics_ts_data *ts)
{
	...
	WRITE_BUFFER(f54_wlog_buf, rst_size, "Full Raw Capacitance Test\n");
	WRITE_BUFFER(f54_wlog_buf, rst_size, "\nInfo: Tx = %d Rx = %d \n", TxChNum, RxChNum);
	WRITE_BUFFER(f54_wlog_buf, rst_size, "Image Data : \n");
	WRITE_BUFFER(f54_wlog_buf, rst_size, "==========================================================================================================\n         :");

	for (i = 0; i < RxChNum; i++) {
		WRITE_BUFFER(f54_wlog_buf, rst_size, "%5d ", i);
	}
	...
	for (i = 0; i < TxChNum; i++) {
		...
		for (j = 0; j < RxChNum; j++) {
			...
			WRITE_BUFFER(f54_wlog_buf, rst_size, "%5d ", full_raw_data[i][j]);
