...
//static  u16 nDriveLineNum = 0, nSenseLineNum = 0;
...
static void _DrvMpTestItoTestGetMutualOneShotRawIir(u16 wszResultData[][MAX_CHANNEL_DRV], u16 nDriveLineNum, u16 nSenseLineNum)
{
    ...
    IicWriteData(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0], 3);
    j = IicReadData(SLAVE_I2C_ID_DBBUS, &szShotData2[i], 128); //(3+8*14+13)*2/2
    ...
    mdelay(100);
    szDbBusTxData[0] = 0x10;
    szDbBusTxData[1] = 0x20; //bank:fir2, addr:h0040
    szDbBusTxData[2] = 0x80;
    IicWriteData(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0], 3);
    j = IicReadData(SLAVE_I2C_ID_DBBUS, &szShotData2[i], 128); //(3+8*14+13)*2/2
    ...
    mdelay(100);
    szDbBusTxData[0] = 0x10;
    szDbBusTxData[1] = 0x21; //bank:fir3, addr:h0000
    szDbBusTxData[2] = 0x00;
    IicWriteData(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0], 3);
    j = IicReadData(SLAVE_I2C_ID_DBBUS, &szShotData2[i], 128); //(1+9*14+1)*2/2
    ...
    mdelay(100);
    szDbBusTxData[0] = 0x10;
    szDbBusTxData[1] = 0x21; //bank:fir3, addr:h0040
    szDbBusTxData[2] = 0x80;
    IicWriteData(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0], 3);
    j = IicReadData(SLAVE_I2C_ID_DBBUS, &szShotData2[i], 128); //(1+9*14+1)*2/2
    ...
    mdelay(100);
    szDbBusTxData[0] = 0x10;
    szDbBusTxData[1] = 0x22; //bank:fir4, addr:h0000
    szDbBusTxData[2] = 0x00;
    IicWriteData(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0], 3);
    j = IicReadData(SLAVE_I2C_ID_DBBUS, &szShotData2[i], 82); //(13+2*14)*2
...
//            DBG("%d  %d  %d\t", _gResult[i * _gDriveLineNum + j], 4464*_gMode[i * _gDriveLineNum + j], _gDeltaC[i * _gDriveLineNum + j]);
    ...
    for (j = 0; j < (_gDriveLineNum); j ++)	//_gDriveLineNum-1
    {
        for (i = 0; i < _gSenseLineNum; i ++)
        {
