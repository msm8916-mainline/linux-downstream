...
//#include "IDOL3_V1.16_20150307.h" 
//#include "IDOL3_V1.14_20150205.h" 
//#include "IDOL3_V1.13_20150130.h" 
//nclude "IDOL3_V1.11_20150113.h" 
//#include "IDOL3_V1.10_20150105.h" 
//#include "IDOL3_V1.07_20141211.h" 
//#include "IDOL3_V1.05_20141119.h"
//#include "IDOL3_V1.04_20141112.h"
//#include "IDOL3_V1.03_20141106.h"
//#include "IDOL3_V1.02_20141031.h"
...
struct input_dev *msc_device = NULL;  //add by furong to support TP Gesture
#endif //CONFIG_ENABLE_GESTURE_WAKEUP
...
static s32 _DrvFwCtrlParsePacket(u8 *pPacket, u16 nLength, TouchInfo_t *pInfo)
{
   ...
   if(proximity_enable&&
   (pPacket[nLength-2]==0x80||pPacket[nLength-2]==0x40))
{
    if(ps_data_state[0]==1&&pPacket[nLength-2]==0x80)
    {
        ps_data_state[0]=0;
        printk("[Fu]pPacket[%d-2]=%x, closing \n",nLength,pPacket[nLength-2]);
		if(vps->vps_enabled)
		{
    ...
    else if(ps_data_state[0]==0&&pPacket[nLength-2]==0x40)
    {
        ps_data_state[0]=1;
        printk("[Fu]pPacket[%d-2]=%x, leaving \n",nLength,pPacket[nLength-2]);
		if(vps->vps_enabled)
		{
...
//rong.fu add begin, 2015.03.02, pr969425, when in call, and face close, and screen off, tp should not report coordinate event...
	if (atomic_read(&tp_suspended) && proximity_enable) {
		return -1;
	}
//rong.fu add end
    ...
    {
        DBG("received raw data from touch panel as following[0x%x]:\n", _gGestureWakeupValue);
        DBG("pPacket[0]=%x \n pPacket[1]=%x pPacket[2]=%x pPacket[3]=%x pPacket[4]=%x pPacket[5]=%x \n", \
                pPacket[0], pPacket[1], pPacket[2], pPacket[3], pPacket[4], pPacket[5]);

        if (pPacket[0] == 0xA7 && pPacket[1] == 0x00 && pPacket[2] == 0x06 && pPacket[3] == PACKET_TYPE_GESTURE_WAKEUP && (_gGestureWakeupValue == 0))
        {
            ...
            switch (nWakeupMode)
            {
                case 0x58:
                    ...
                    input_sync(g_InputDevice);
                    //input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_UNLOCK);  
                    //input_sync(msc_device);
                    break;		
                case 0x60:
...
//                    input_report_key(g_InputDevice, KEY_UP, 0);
                    ...
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_UP_GESTURE);  
                    input_sync(msc_device);
                    break;		
                case 0x61:
...
//                    input_report_key(g_InputDevice, KEY_DOWN, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_DOWN_GESTURE);  
                    input_sync(msc_device);                 
                    break;		
                case 0x62:
...
//                    input_report_key(g_InputDevice, KEY_LEFT, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_LEFT_GESTURE);  
                    input_sync(msc_device);                      
                    break;		
                case 0x63:
...
//                    input_report_key(g_InputDevice, KEY_RIGHT, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_RIGHT_GESTURE);  
                    input_sync(msc_device);                       
                    break;		
                case 0x64:
...
//                    input_report_key(g_InputDevice, KEY_M, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_M_ALPHABET_GESTURE);  
                    input_sync(msc_device);                     
                    break;		
                case 0x65:
...
//                    input_report_key(g_InputDevice, KEY_W, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_W_ALPHABET_GESTURE);  
                    input_sync(msc_device);                   
                    break;		
                case 0x66:
...
//                    input_report_key(g_InputDevice, KEY_C, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_C_ALPHABET_GESTURE);  
                    input_sync(msc_device);                    
                    break;
                case 0x67:
...
//                    input_report_key(g_InputDevice, KEY_E, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_E_ALPHABET_GESTURE);  
                    input_sync(msc_device);                    
                    break;
                case 0x68:
...
//                    input_report_key(g_InputDevice, KEY_V, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_V_ALPHABET_GESTURE);  
                    input_sync(msc_device);                      
                    break;
                case 0x69:
...
//                    input_report_key(g_InputDevice, KEY_O, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_O_ALPHABET_GESTURE);  
                    input_sync(msc_device);                    
                    break;
                case 0x6A:
...
//                    input_report_key(g_InputDevice, KEY_S, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_S_ALPHABET_GESTURE);  
                    input_sync(msc_device);                     
                    break;
                case 0x6B:
...
//                    input_report_key(g_InputDevice, KEY_Z, 0);
                    //input_report_key(g_InputDevice, KEY_POWER, 0);
                    //input_sync(g_InputDevice);
                    input_event(msc_device, EV_MSC, MSC_GESTURE, KEY_Z_ALPHABET_GESTURE);  
                    input_sync(msc_device);                      
                    break;
                default:
...
void DrvFwCtrlOpenGestureWakeup(u16 nMode)
{
	...
	for (j = 0; j < 5; j++) {
	    ...
	    szDbBusTxData[1] = (nMode >> 8) & 0xFF;//0
	    szDbBusTxData[2] = nMode & 0xFF;// 1
		mutex_lock(&g_Mutex);
	    while (i < 3)
	    {
	    	DBG("[Fu] i = %d, szDbBusTxData[1] = %d, szDbBusTxData[2] = %d\n", i, szDbBusTxData[1], szDbBusTxData[2]);
	        rc = IicWriteData(SLAVE_I2C_ID_DWI2C, &szDbBusTxData[0], 3);
	        
	        if (rc > 0)
	        {
	            DBG("gesture wakeup write success #1, rc = %d \n", rc);
	            break;
	        }
	        i++;
	    }
	    ...
	    if (i == 3)
	    {
	        printk("gesture wakeup write failed #1, rc = %d \n", rc);		
	    }

		//read
		szDbBusTxData[3] = 0x53;
		szDbBusTxData[4] = 0x10;
		szDbBusTxData[5] = 0x58;	 
		//mutex_lock(&g_Mutex);
		IicWriteData(SLAVE_I2C_ID_DWI2C, &szDbBusTxData[3], 3);
		rc = IicReadData(SLAVE_I2C_ID_DWI2C, &szDbBusRxData[0], 2);
		//mutex_unlock(&g_Mutex);
		DBG("[%s] rc = %d, Data[0] = %d, Data[1]=%d\n", __func__, rc, szDbBusRxData[0], szDbBusRxData[1]);//Data[0] = 1, Data[1] = 0

		if ((szDbBusRxData[0] == szDbBusTxData[2]) && (szDbBusRxData[1] == szDbBusTxData[1])) {
			printk("[Fu] gesture open sucess\n");
			break;
		} else if (j < 2) {
			mdelay(50);
		} else {
...
//---------------------------------------///
#ifdef CONFIG_ENABLE_CHARGER_STATUS
void DrvFwCtrlChargerInput(void)
{
	u8 szDbBusTxData[6] = {0};
	s32 rc;
...
void DrvFwCtrlChargerOutput(void)
{
	u8 szDbBusTxData[6] = {0};
	s32 rc;
	...
	//write
	szDbBusTxData[0] = 0x58;
	szDbBusTxData[1] = 0x56;
	szDbBusTxData[2] = 0x90;
...
u32 _s_nCrcMainA, _s_nCrcInfoA, _s_nCrcMainB, _s_nCrcInfoB;
Msg26xxmSwId_e _s_eSwId = MSG26XXM_SW_ID_UNDEFINED;

static void _DrvFwCtrlCalculateAllCrc(void)
{
    //u32 nRetVal = 0;
    u16 nRegData = 0;
    ...
    _s_nCrcMainA = RegGet16BitValue(0x3C80);
    _s_nCrcMainA = (_s_nCrcMainA << 16) | RegGet16BitValue(0x3C82);
    DBG("Main Block CRC = 0x%x\n", _s_nCrcMainA);

    _s_nCrcInfoA = RegGet16BitValue(0x3CA0);
    _s_nCrcInfoA = (_s_nCrcInfoA << 16) | RegGet16BitValue(0x3CA2);
...
static void _DrvFwCtrlReadAllBinFinfo(void)
{
    //u32 nRetVal = 0;
    u16 nRegData = 0;
    ...
    _s_nCrcMainB = szDbBusRxData[3];
    _s_nCrcMainB = (_s_nCrcMainB << 8) | szDbBusRxData[2];
    _s_nCrcMainB = (_s_nCrcMainB << 8) | szDbBusRxData[1];
    _s_nCrcMainB = (_s_nCrcMainB << 8) | szDbBusRxData[0];
    DBG("_s_nCrcMainB = 0x%x\n", _s_nCrcMainB);
    ...
    IicWriteData(SLAVE_I2C_ID_DWI2C, &szDbBusTxData[0], 5);
    IicReadData(SLAVE_I2C_ID_DWI2C, &szDbBusRxData[0], 4);
    _s_nCrcInfoB = szDbBusRxData[3];
    _s_nCrcInfoB = (_s_nCrcInfoB << 8) | szDbBusRxData[2];
    _s_nCrcInfoB = (_s_nCrcInfoB << 8) | szDbBusRxData[1];
    _s_nCrcInfoB = (_s_nCrcInfoB << 8) | szDbBusRxData[0];
    DBG("_s_nCrcInfoB = 0x%x\n", _s_nCrcInfoB);
    ...
    IicWriteData(SLAVE_I2C_ID_DWI2C, &szDbBusTxData[0], 5);
    IicReadData(SLAVE_I2C_ID_DWI2C, &szDbBusRxData[0], 4);
    _s_eSwId = szDbBusRxData[1];
    _s_eSwId = (_s_eSwId << 8) | szDbBusRxData[0];
    DBG("_s_eSwId = 0x%x\n", _s_eSwId);
...
//add for firmware can't update stuck in do while  FAE add 2014/10/11
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(void)
{
    ...
    while ((_gUpdateRetryCount > 0) && (nRetVal != 0))
    {
        _gUpdateRetryCount --;
        nRetVal = _DrvFwCtrlUpdateFirmwareBySwId();
        DBG("*** _gUpdateRetryCount = %d, update firmware by sw id result = %d ***\n", _gUpdateRetryCount, nRetVal);
    }
...
void DrvFwCtrlCheckFirmwareUpdateBySwId(void)
{
    ...
    _DrvFwCtrlCalculateAllCrc();
    _DrvFwCtrlReadAllBinFinfo();
    ...
    DBG("_s_nCrcMainA=0x%x, _s_nCrcInfoA=0x%x, _s_nCrcMainB=0x%x, _s_nCrcInfoB=0x%x\n", _s_nCrcMainA, _s_nCrcInfoA, _s_nCrcMainB, _s_nCrcInfoB);

    if (_s_nCrcMainA == _s_nCrcMainB && _s_nCrcInfoA == _s_nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
        //eSwId = _DrvFwCtrlGetSwId(EMEM_MAIN);

        if (_s_eSwId == MSG26XXM_SW_ID_XXXX)
        {
        ...
        else if (_s_eSwId == MSG26XXM_SW_ID_YYYY)
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY // By two dimensional array
        ...
        {
            DBG("_s_eSwId = 0x%x is an undefined SW ID.\n", _s_eSwId);

            _s_eSwId = MSG26XXM_SW_ID_UNDEFINED;
            nUpdateBinMajor = 0;
            nUpdateBinMinor = 0;
        }

        DBG("_s_eSwId=0x%x, nMajor=%d, nMinor=%d, nUpdateBinMajor=%d, nUpdateBinMinor=%d\n", _s_eSwId, nMajor, nMinor, nUpdateBinMajor, nUpdateBinMinor);
...
#endif
        {
            if (_s_eSwId < MSG26XXM_SW_ID_UNDEFINED && _s_eSwId != 0x0000 && _s_eSwId != 0xFFFF)
            {
                if (_s_eSwId == MSG26XXM_SW_ID_XXXX)
                {
    ...
    else if (_s_nCrcMainA == _s_nCrcMainB && _s_nCrcInfoA != _s_nCrcInfoB) // Case 2. Main Block:OK, Info Block:FAIL
    {
        //eSwId = _DrvFwCtrlGetSwId(EMEM_MAIN);
    ...
    {
        _s_nCrcInfoA = _DrvFwCtrlRetrieveInfoCrcFromInfoBlock();
        _s_nCrcInfoB = _DrvFwCtrlCalculateInfoCrcByDeviceDriver();

        DBG("8K-4 : _s_nCrcInfoA=0x%x, _s_nCrcInfoB=0x%x\n", _s_nCrcInfoA, _s_nCrcInfoB);

        if (_s_nCrcInfoA == _s_nCrcInfoB) // Check if info block is actually OK.
        {
            _s_eSwId = _DrvFwCtrlGetSwId(EMEM_INFO);
...
//    DBG("*** %s() ***\n", __func__);
	...
	DBG("[TP] %s packet[0] = 0x%x, packet[21] = 0x%x\n ", __func__, pPacket[0], pPacket[21]);//packet[21] -- proximity sensor
    ...
    if (0 == _DrvFwCtrlParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        if (tInfo.nKeyCode != 0xFF)   //key touch pressed
        {
            ...
            if (tp_debug_on)
            {
                printk(KERN_ERR "tInfo.nKeyCode=%x, nLastKeyCode=%x, g_TpVirtualKey[%d]=%d\n", tInfo.nKeyCode, nLastKeyCode, tInfo.nKeyCode, g_TpVirtualKey[tInfo.nKeyCode]);
            }
        #endif
