...
****************************************************************************/
static ssize_t show_upgrade(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
...
#if defined(CONFIG_LGE_MODULE_DETECT)
static ssize_t show_module_id(TouchDriverData *pDriverData, char *buf)
{
	int ret = 0;
	TOUCH_LOG("Module id is MIT300\n");
	ret = snprintf(buf, PAGE_SIZE, "%d\n", MIT300);
...
#if defined(CONFIG_LGE_MODULE_DETECT)
	if (idx != MIT300) {
		TOUCH_LOG("This panel is not MIT300, idx = %d\n", idx);
		goto exit_async_touch_init_fail;
	}
	TOUCH_LOG("This panel is MIT300\n");
#endif /* CONFIG_LGE_MODULE_DETECT */
