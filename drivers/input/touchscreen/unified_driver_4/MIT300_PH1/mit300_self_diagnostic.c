...
static int  MIT300_PrintData(struct i2c_client *client, char *buf, int* result, int type)
{
	...
	TOUCH_FUNC();
		...
		TOUCH_LOG("DDIC Revision Status : %s\n", (lge_get_board_revno() == HW_REV_1_0 || lge_get_db7400_cut() >= CUT6) ? "CUT.6" : "CUT.5");
#endif
...
#if defined(CONFIG_LGD_PH1DONGBU_INCELL_VIDEO_HD_PANEL)
		if (lge_get_board_revno() == HW_REV_1_0 || lge_get_db7400_cut() >= CUT6) {
