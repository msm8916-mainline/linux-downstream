...
extern bool flag_knock_on_ctrl_during_panel_on;
/****************************************************************************
...
#if defined(CONFIG_LGD_PH1DONGBU_INCELL_VIDEO_HD_PANEL)
	if ((newState == STATE_KNOCK_ON_ONLY) || (newState == STATE_KNOCK_ON_CODE)  || (newState == STATE_OFF))
	{
		if (flag_knock_on_ctrl_during_panel_on)
		{
			TOUCH_LOG("Change to set lpwg mode during LCD on sequence.\n");
			ret = lpwg_control(client, newState);
		}
		else {
			TOUCH_LOG("Skip to set lpwg mode during LCD on sequence.\n");
			return TOUCH_FAIL;
		}
	} else {
		ret = lpwg_control(client, newState);
