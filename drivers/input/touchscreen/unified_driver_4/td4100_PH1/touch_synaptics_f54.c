...
#include <linux/input/unified_driver_4/lgtp_platform_api.h>
#include <linux/input/unified_driver_4/lgtp_model_config.h>
...
static int ReadChCount(struct synaptics_ts_data *ts)
{
	...
	/*Read channel count*/
	ret = TouchReadReg(ts->client, RX_CH_CNT_REG, &rx_max_cnt, 1);
	if( ret < 0) {
		return TOUCH_FAIL;
	}
	
	ret = TouchReadReg(ts->client, TX_CH_CNT_REG, &tx_max_cnt, 1);
	if( ret < 0) {
		return TOUCH_FAIL;
	}

	ret = TouchWriteByteReg(ts->client, PAGE_SELECT_REG, ts->sensor_fc.function_page);
	if( ret < 0) {
		return TOUCH_FAIL;
	}

	ret = TouchReadReg(ts->client, ts->sensor_fc.dsc.data_base+1, data, rx_max_cnt);
	if( ret < 0) {
		return TOUCH_FAIL;
	}
	...
	ret = TouchReadReg(ts->client, ts->sensor_fc.dsc.data_base+2, data, tx_max_cnt);
	if( ret < 0) {
		return TOUCH_FAIL;
	}
	...
	ret = TouchWriteByteReg(ts->client, PAGE_SELECT_REG, ts->analog_fc.function_page);
	if( ret < 0) {
		return TOUCH_FAIL;
	}
...
static int TestPreparation(struct synaptics_ts_data *ts)
{
	...
	/*Turn off CBC.*/
	ret = TouchReadReg(ts->client, ANALOG_CONTROL_REG, &data, 1);
	data = data & 0xDF;
	ret = TouchWriteReg(ts->client, ANALOG_CONTROL_REG, &data, 1);

	/*Apply ForceUpdate.*/
	ret = TouchReadReg(ts->client, ANALOG_COMMAND_REG, &data, 1);
	data = data | 0x04;
	ret = TouchWriteReg(ts->client, ANALOG_COMMAND_REG, &data, 1);
	msleep(WAIT_TIME);
...
static int ReadTRexShort(struct synaptics_ts_data *ts)
{
	...
	ret = TouchReadReg(ts->client, REPORT_DATA_REG, cap_data, MaxArrayLength);
...
static int TRexShortTest(struct synaptics_ts_data *ts)
{
	...
	data = 0x1A;
	ret = TouchWriteReg(ts->client, REPORT_TYPE_REG, &data, 1);
...
int F54TestHandle(struct synaptics_ts_data *ts, int input, int mode, char *buf)
{
	...
	data = 0x00;
	ret = TouchWriteReg(ts->client, REPORT_LSB_REG, &data, 1);
	ret = TouchWriteReg(ts->client, REPORT_MSB_REG, &data, 1);
