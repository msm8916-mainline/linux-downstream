		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	/* page read */
	retval = TouchReadReg( PAGE_SELECT_REG, &page_old, sizeof(page_old));
	...
	if (page_old == page_new) {
		...
		retval = TouchWriteByteReg( PAGE_SELECT_REG, page_new);
	...
	/* read register */
	retval = TouchReadReg((unsigned char)(addr & ~(MASK_8BIT << 8)), data, length);
	...
	/* page restore */
	if (page_changed) {
		retval = TouchWriteByteReg( PAGE_SELECT_REG, page_old);
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	/* write register */
	retval = TouchWriteReg( (unsigned char)(addr & ~(MASK_8BIT << 8)), data, length);
...
static int rmidev_init_device(struct synaptics_ts_data *ts)
{
	...
	rmidev->sysfs_dir = kobject_create_and_add(SYSFS_FOLDER_NAME, NULL);  //td4100 for kernel crash
