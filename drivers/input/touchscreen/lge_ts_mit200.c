...
static int mit_fw_version_check(struct mms_data *ts, struct touch_fw_info *info)
{
	...
	if (strcmp(ts->pdata->fw_product,
				ts->module.product_code) != 0) {
...
static int mit_isc_page_write(struct mms_data *ts, const u8 *wdata, int addr)
{
	...
	if (i2c_master_send(client, cmd, FW_BLOCK_SIZE + 6) !=
			FW_BLOCK_SIZE + 6) {
...
static int mit_flash_section(struct mms_data *ts,
		struct touch_fw_info *info, bool compare)
{
	int addr = 0;
	...
	for (addr = ((int)info->fw->size) - FW_BLOCK_SIZE;
			addr >= 0; addr -= FW_BLOCK_SIZE) {
		...
		if (compare) {
			...
			if (memcmp(&info->fw->data[addr],
						cmpdata, FW_BLOCK_SIZE)) {
				TOUCH_INFO_MSG(
					"Addr[0x%06x] Verify failed\n", addr);
...
static int get_intensity(struct mms_data *ts)
{
	...
	for (col = 0; col < ts->dev.col_num; col++) {
		...
		if (nLength > sizeof(read_buf)) {
			TOUCH_ERR_MSG(
				"stack overflow - nLength: %d > read_buf size:%d\n",
...
static int  print_intensity(struct mms_data *ts, char *buf)
{
	...
	for (row = 0; row < MAX_ROW; row++) {
		pr_info("[Touch] [%2d]  ", row);
		ret += sprintf(buf + ret, "[%2d]  ", row);
		for (col = 0; col < MAX_COL; col++) {
			ret += sprintf(buf + ret, "%4d ",
				ts->intensity_data[row][col]);
			pr_info("%4d ", ts->intensity_data[row][col]);
		}
		pr_info("\n");
		ret += sprintf(buf + ret, "\n");
	}
...
static int mit_enter_test(struct mms_data *ts)
{
	...
	do {
		...
		if (i2c_transfer(client->adapter, msg, ARRAY_SIZE(msg)) !=
				ARRAY_SIZE(msg)) {
			TOUCH_ERR_MSG(
				"ENTER Test-Mode MIT_REGL_EVENT_PKT_SZ ERROR!\n");
			ret = -1;
			goto TESTOUT;
		}
		...
		if (i2c_transfer(client->adapter, msg, ARRAY_SIZE(msg)) !=
				ARRAY_SIZE(msg)) {
			TOUCH_ERR_MSG(
				"ENTER Test-Mode MIT_REGL_INPUT_EVENT ERROR!\n");
			ret = -1;
			goto TESTOUT;
		}
		...
		if (count) {
			...
			ret = -1;
			TOUCH_ERR_MSG(
				"failed retry count over to make marker 0x0C\n");
			goto TESTOUT;
		}
	} while (marker != 0x0C);
...
static int mit_exit_test(struct mms_data *ts)
{
	...
	do {
		...
		if (i2c_transfer(client->adapter, msg, ARRAY_SIZE(msg)) !=
				ARRAY_SIZE(msg)) {
			TOUCH_ERR_MSG(
				"EXIT Test-Mode MIT_REGL_EVENT_PKT_SZ ERROR!\n");
			ret = -1;
			goto TESTEXITOUT;
		}
		cmd[1] = MIT_REGL_INPUT_EVENT;
		...
		if (i2c_transfer(client->adapter, msg, ARRAY_SIZE(msg)) !=
				ARRAY_SIZE(msg)) {
			TOUCH_ERR_MSG(
				"EXIT Test-Mode MIT_REGL_INPUT_EVENT ERROR!\n");
			ret = -1;
			goto TESTEXITOUT;
		}
...
static int mit_select_test(struct mms_data *ts)
{
	...
	do {
		...
		if (i2c_transfer(client->adapter, msg, ARRAY_SIZE(msg)) !=
				ARRAY_SIZE(msg)) {
			TOUCH_ERR_MSG(
				"SELECT Test-Mode MIT_REGL_EVENT_PKT_SZ ERROR!\n");
			ret = -1;
			goto TESTOUT;
		}
		...
		if (i2c_transfer(client->adapter, msg, ARRAY_SIZE(msg)) !=
				ARRAY_SIZE(msg)) {
			TOUCH_ERR_MSG(
				"SELECT Test-Mode MIT_REGL_INPUT_EVENT ERROR!\n");
			ret = -1;
			goto TESTOUT;
		}
...
static int get_rawdata(struct mms_data *ts)
{
	...
	if (ts->module.otp == OTP_APPLIED) {
		limit_upper = ts->pdata->limit->raw_data_otp_max +
			ts->pdata->limit->raw_data_margin;
		limit_lower = ts->pdata->limit->raw_data_otp_min -
			ts->pdata->limit->raw_data_margin;
	} else {
		limit_upper = ts->pdata->limit->raw_data_max +
			ts->pdata->limit->raw_data_margin;
		limit_lower = ts->pdata->limit->raw_data_min -
			ts->pdata->limit->raw_data_margin;
	}
...
static int print_rawdata(struct mms_data *ts, char *buf, int type)
{
	...
	for (row = 0; row < MAX_ROW; row++) {
		...
		for (col = 0; col < MAX_COL; col++) {
			...
			ret += sprintf(buf + ret,
				"%5d ", ts->mit_data[row][col]);
			pr_info("%5d ", ts->mit_data[row][col]);
			if (type == RAW_DATA_STORE)
				ret += sprintf(buf + ret, ",");

			ts->r_min = (ts->r_min > ts->mit_data[row][col]) ?
				ts->mit_data[row][col] : ts->r_min;
			ts->r_max = (ts->r_max < ts->mit_data[row][col]) ?
				ts->mit_data[row][col] : ts->r_max;
...
static int  print_openshort_data(struct mms_data *ts, char *buf, int type)
{
	...
	for (row = 0; row < MAX_ROW; row++) {
		...
		for (col = 0; col < MAX_COL; col++) {
			...
			ts->o_min = (ts->o_min > ts->mit_data[row][col]) ?
				ts->mit_data[row][col] : ts->o_min;
			ts->o_max = (ts->o_max < ts->mit_data[row][col]) ?
				ts->mit_data[row][col] : ts->o_max;
	...
	if (type == OPENSHORT) {
		...
		TOUCH_INFO_MSG(
			"OPEN / SHORT TEST SPEC(LOWER : %d)\n",
			ts->pdata->limit->open_short_min);
		ret += sprintf(buf + ret,
			"OPEN / SHORT TEST SPEC(LOWER : %d)\n",
			ts->pdata->limit->open_short_min);

		if (ts->pdata->selfdiagnostic_state[SD_OPENSHORT] == 0 ||
				ret == 0) {
			for (row = 0; row < MAX_ROW; row++) {
				pr_info("[Touch] [%2d]  ", row);
				ret += sprintf(buf + ret, "[%2d]  ", row);
...
static int  check_slope_data(struct mms_data *ts, char *buf)
{
	...
	for (col = 0; col < MAX_COL; col++) {
		for (row = 1; row < MAX_ROW - 1; row++) {
			denominator = (ts->mit_data[row - 1][col] +
					ts->mit_data[row + 1][col]) / 2;
			...
			ts->s_min = (ts->s_min > get_data[row][col]) ?
				get_data[row][col] : ts->s_min;
			ts->s_max = (ts->s_max < get_data[row][col]) ?
				get_data[row][col] : ts->s_max;
...
RETRY:
	...
	if (ts->test_mode == RAW_DATA_SHOW ||
			ts->test_mode == RAW_DATA_STORE ||
			ts->test_mode == SLOPE) {
		if (get_rawdata(ts) == -1) {
