...
#define BMM_VDD_MIN_UV       1620000
#define BMM_VDD_MAX_UV       3600000
#define BMM_VIO_MIN_UV       120000
#define BMM_VIO_MAX_UV       3600000
		...
		const char *buf, size_t count)
{
	...
	if (data != client_data->enable) {
		if (data) {
			...
			if(bmm_power_ctl(client_data, true)) {
				LDBG("Enable Power Fail !\n");
			}
			...
			LDBG("Disable E-compass\n");
			cancel_delayed_work_sync(&client_data->work);
			if(bmm_power_ctl(client_data, false)) {
				LDBG("Disable Power Fail !\n");
			}
		}
...
/* ATTR For control GPIO */
static ssize_t bma2x2_gpio_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	irq_gpio_value = gpio_get_value(client_data->irq_gpio);

	return sprintf(buf, "E-compass irq_gpio_value = %s\n", (irq_gpio_value>0? "High":"Low"));

}

static ssize_t bma2x2_gpio_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int irq_gpio_value=0;
	...
	sscanf(buf, "%d\n", &irq_gpio_value);
	gpio_set_value(client_data->irq_gpio, (irq_gpio_value>0? 1:0));

	LDBG("set Ecompass irq_gpio (%d)  = %s\n",client_data->irq_gpio, (irq_gpio_value>0? "High":"Low"));
...
static DEVICE_ATTR(gpio_ctl, S_IRUGO|S_IWUSR|S_IWGRP|S_IWOTH,
		bma2x2_gpio_show, bma2x2_gpio_store);
...
static int bmm_power_init(struct bmm_client_data *data)
{
	int ret;

	data->vdd = regulator_get(&data->client->dev, "vdd");
	if (IS_ERR(data->vdd)) {
		PERR("Regulator get failed vdd \n");
		return -1;
	}
	...
	if (regulator_count_voltages(data->vdd) > 0) {
		ret = regulator_set_voltage(data->vdd,
				...
				BMM_VDD_MAX_UV);
		if (ret) {
			PERR("Regulator set failed vdd ret=%d\n",ret);
			goto reg_vdd_put;
		}
	}
	...
	if (regulator_count_voltages(data->vio) > 0) {
		ret = regulator_set_voltage(data->vio,
				...
				BMM_VIO_MAX_UV);
		if (ret) {
			PERR("Regulator set failed vio ret=%d\n", ret);
			goto reg_vio_put;
		}
	}
...
reg_vdd_set:
	if (regulator_count_voltages(data->vdd) > 0)
		regulator_set_voltage(data->vdd, 0, BMM_VDD_MAX_UV);
reg_vdd_put:
...
static int bmm_power_deinit(struct bmm_client_data *data)
{
	if (regulator_count_voltages(data->vdd) > 0)
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	client_data->irq_gpio = 69 + 902;
	err = gpio_request(client_data->irq_gpio, "E-compass-irq");
	if (err) {
		LDBG("gpio_request fail\n");
	}
	err = gpio_direction_input(client_data->irq_gpio);
	//err = gpio_direction_output(client_data->irq_gpio,0);
	if (err) {
		LDBG("gpio_direction_input fail\n");
	}
	LDBG("IRQ GPIO = %d \n", client_data->irq_gpio);
	...
	err = bmm_power_ctl(client_data, false);

	printk("===== E-commpass Probe End =====\n");
	return 0;

deinit_power_exit:
	bmm_power_deinit(client_data);
