/*
 * JP1081_android one chip USB 1.1 ethernet devices
 *
...
/* ------------------------------------------------------------------------------------------ */
/* jp1081_android mac and phy operations */
...
static int qf_read(struct usbnet *dev, u8 reg, u16 length, void *data)
{
	...
	dev_dbg(&dev->udev->dev, "qf_read() reg=0x%02x length=%d", reg, length);
...
static int qf_write(struct usbnet *dev, u8 reg, u16 length, void *data)
{
	...
	dev_dbg(&dev->udev->dev, "qf_write() reg=0x%02x, length=%d", reg, length);
...
{
	dev_dbg(&dev->udev->dev, "qf_write_reg() reg=0x%02x, value=0x%02x", reg, value);
...
static void qf_write_async_helper(struct usbnet *dev, u8 reg, u8 value, u16 length, void *data)
{
	...
	if (!urb) {
		dev_err(&dev->udev->dev, "Error allocating URB in qf_write_async_helper!");
		return;
	}

	req = kmalloc(sizeof(struct usb_ctrlrequest), GFP_ATOMIC);
	if (!req) {
		dev_err(&dev->udev->dev, "Failed to allocate memory for control request");
		usb_free_urb(urb);
		return;
	}
	...
	if (status < 0) {
		dev_err(&dev->udev->dev, "Error submitting the control message: status=%d",
		       status);
		kfree(req);
		usb_free_urb(urb);
	}
...
{
	dev_dbg(&dev->udev->dev, "qf_write_async() reg=0x%02x length=%d", reg, length);
...
{
	dev_dbg(&dev->udev->dev, "qf_write_reg_async() reg=0x%02x value=0x%02x", reg, value);
...
static int qf_share_read_word(struct usbnet *dev, int phy, u8 reg, __le16 *value)
{
	...
	if (i >= QF_SHARE_TIMEOUT) {
		dev_err(&dev->udev->dev, "%s read timed out!", phy ? "phy" : "eeprom");
		ret = -EIO;
		goto out;
	}
	...
	dev_dbg(&dev->udev->dev, "read shared %d 0x%02x returned 0x%04x, %d",
	       phy, reg, *value, ret);
...
static int qf_share_write_word(struct usbnet *dev, int phy, u8 reg, __le16 value)
{
	...
	if (i >= QF_SHARE_TIMEOUT) {
		dev_err(&dev->udev->dev, "%s write timed out!", phy ? "phy" : "eeprom");
		ret = -EIO;
		goto out;
	}
...
static int jp1081_android_get_eeprom_len(struct net_device *dev)
{
	return QF_EEPROM_LEN;
}

/* get jp1081_android eeprom information */
static int jp1081_android_get_eeprom(struct net_device *net, struct ethtool_eeprom *eeprom, u8 * data)
{
...
static int jp1081_android_mdio_read(struct net_device *netdev, int phy_id, int loc)
{
	...
	if (phy_id) {
		dev_dbg(&dev->udev->dev, "Only internal phy supported");
		return 0;
	}
	...
	dev_dbg(&dev->udev->dev,
	       "jp1081_android_mdio_read() phy_id=0x%02x, loc=0x%02x, returns=0x%04x",
...
static void jp1081_android_mdio_write(struct net_device *netdev, int phy_id, int loc, int val)
{
	struct usbnet *dev = netdev_priv(netdev);
	...
	dev_dbg(&dev->udev->dev,"jp1081_android_mdio_write() phy_id=0x%02x, loc=0x%02x, val=0x%04x",
	       phy_id, loc, val);
...
static void jp1081_android_get_drvinfo(struct net_device *net, struct ethtool_drvinfo *info)
{
...
static u32 jp1081_android_get_link(struct net_device *net)
{
	struct usbnet *dev = netdev_priv(net);
...
static int jp1081_android_ioctl(struct net_device *net, struct ifreq *rq, int cmd)
{
	struct usbnet *dev = netdev_priv(net);
...
static struct ethtool_ops jp1081_android_ethtool_ops = {
	.get_drvinfo	= jp1081_android_get_drvinfo,
	.get_link	= jp1081_android_get_link,
	.get_msglevel	= usbnet_get_msglevel,
	.set_msglevel	= usbnet_set_msglevel,
	.get_eeprom_len	= jp1081_android_get_eeprom_len,
	.get_eeprom	= jp1081_android_get_eeprom,
...
#if 1
static const struct net_device_ops jp1081_netdev_ops = {
	...
	.ndo_do_ioctl = jp1081_android_ioctl,
	//.ndo_set_multicast_list = jp1081_android_set_multicast,
	.ndo_set_rx_mode = jp1081_android_set_multicast,
...
static int jp1081_android_bind(struct usbnet *dev, struct usb_interface *intf)
{
	...
	if (ret)
		goto out;
#if 0
	dev->net->netdev_ops->ndo_do_ioctl = jp1081_android_ioctl;
	dev->net->netdev_ops->ndo_set_multicast_list = jp1081_android_set_multicast;
#else
	dev->net->netdev_ops = &jp1081_netdev_ops;
#endif
	dev->net->ethtool_ops = &jp1081_android_ethtool_ops;
...
static int jp1081_android_rx_fixup(struct usbnet *dev, struct sk_buff *skb)
{
	u8 status;
	int len;
...
static struct sk_buff *jp1081_android_tx_fixup(struct usbnet *dev, struct sk_buff *skb, gfp_t flags)
{
	int len;
...
static void jp1081_android_status(struct usbnet *dev, struct urb *urb)
{
	int link;
	u8 *buf;
...
static int jp1081_android_link_reset(struct usbnet *dev)
{
	...
	dev_dbg(&dev->udev->dev, "link_reset() speed: %d duplex: %d",
...
static const struct driver_info jp1081_android_info = {
	...
	.rx_fixup	= jp1081_android_rx_fixup,
	.tx_fixup	= jp1081_android_tx_fixup,
	.status		= jp1081_android_status,
	.link_reset	= jp1081_android_link_reset,
	.reset		= jp1081_android_link_reset,
};

static const struct usb_device_id products[] = {
	{
	 USB_DEVICE(0x0fe6, 0x9700),	/* JP1081_ANDROID device */
...
MODULE_DESCRIPTION("JiaPeng NET JP1081 one chip USB 1.1 ethernet devices on android platform");
MODULE_LICENSE("GPL");
