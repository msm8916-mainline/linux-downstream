		...
		uint32_t *low_threshold, uint32_t *high_threshold)
{
	...
	pr_debug("mzy >>>> %s, warm_temp:%d and cool_temp:%d\n", __func__, param->high_temp,
				param->low_temp);
	...
	pr_debug("mzy >>>> %s, high_output:%lld\n", __func__, high_output);
	...
	*high_threshold = low_output;
	...
	pr_debug("mzy >>>> %s, high_volt:%d, low_volt:%d\n", __func__, *high_threshold,
				*low_threshold);
	return 0;
}
