...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	int rem = 0;
	int tasksize;
	int zswap_stored_pages_counter;
	int i;
	...
	for_each_process(tsk) {
		...
		if (atomic_read(&zswap_stored_pages)) {
			...
			zswap_stored_pages_counter = atomic_read(&zswap_stored_pages);
			tasksize += (int)zswap_pool_pages * get_mm_counter(p->mm, MM_SWAPENTS) / zswap_stored_pages_counter;
