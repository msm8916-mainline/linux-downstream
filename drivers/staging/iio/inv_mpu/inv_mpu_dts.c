...
{
#if 1 /* No need regulator setting in Odin */
	int err ;
...
int inv_parse_irqgpio(struct device *dev, struct mpu_platform_data *pdata)
{
	...
	if ((ret= of_get_named_gpio(np, "inven,irq-gpio", 0)) >= 0) {
		pdata->irq_gpio = ret;
		ret = 0;
	}
	else
	...
	//parse reset pin.
	if ((ret= of_get_named_gpio(np, "inven,secondary_reest", 0)) >= 0) {
		pdata->reset_gpio= ret;
		ret = 0;
	}
	else
	{
		printk(KERN_INFO "Fail read reset-gpio ret: %d\n", ret);	
		ret = -1;
		return ret;
	}
	return ret;
