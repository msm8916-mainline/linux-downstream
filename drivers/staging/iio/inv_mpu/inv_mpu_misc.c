...
/* The calculation Error limit of AccelX,Y and Z-axis:
* +-224(X/Y), +-298(Z) under considering all environmental coefficient
*/
#ifdef IKR_SELFTEST
		     ...
		     u32 len, u8 const *data)
{
...
#if 0 //CONFIG_DYNAMIC_DEBUG  
	{
		...
		pr_debug("%s WM%02X%02X%02X%s%s - %d\n", st->hw->name,
			 ...
			 len);
		printk(KERN_ERR"%s before_WM%02X%02X%02X%s%s - %d\n", st->hw->name,
			 mpu_addr, bank[1], addr[1],
			 wr_pr_debug_begin(data, len, write),
			 wr_pr_debug_end(write),
			 len);
...
#if 1//CONFIG_DYNAMIC_DEBUG  //debug_core
	{
		u8 data_debug;
		printk(KERN_INFO "debug_core after_WM\n");
		mpu_memory_read(st, mpu_addr, mem_addr, len, &data_debug);
		printk(KERN_INFO "debug_core  after_WM2\n");
	}
#endif
		return 0;
	}
#else
	for (i=0;i<3;i++) {
		res = i2c_transfer(st->sl_handle, msgs+i, 1);
		if (res != 1) {
			if (res >= 0){
				res = -EIO;
			}
	...
	{
		u8 data_debug;
		printk(KERN_INFO "[sx9500] debug_core after_WM\n");
		mpu_memory_read(st, mpu_addr, mem_addr, len, &data_debug);
		printk(KERN_INFO "[sx9500]debug_core  after_WM2\n");
	}
#endif
	return res;
#endif /*invensense original */
}
		    ...
		    u32 len, u8 *data)
{
...
#if 0 //CONFIG_DYNAMIC_DEBUG
	{
		...
		printk(KERN_ERR"%s RM%02X%02X%02X%02X - %s%s\n", st->hw->name,
			 mpu_addr, bank[1], addr[1], len,
			 wr_pr_debug_begin(data, len, read),
			 wr_pr_debug_end(read));
	}
...
#ifdef IKR_SELFTEST
	for (j = 0; j < 3; j++) {
        if(j != 2) { /* X, Y bias check */
		    if ((abs(accel_bias_regular[j])) > (ACCEL_XY_BIAS_THRESH * DEF_SELFTEST_ACCL_SENS)) {
	...
	u8 *data, int size)
{
	...
	//	printk(KERN_ERR"[sx9500]debug_core load_firmware remain size: %d",size);
		result = mem_w(memaddr, write_size, data);
	...
	u8 *data, int size)
{
	...
	memset(firmware, 0, sizeof(u8) * MPU_MEM_BANK_SIZE);
	/* Write and verify memory */
	write_size = MPU_MEM_BANK_SIZE - MPU_DMP_LOAD_START;
	size -= write_size;
	data += write_size;
