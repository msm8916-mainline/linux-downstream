...
static struct inv_mpu_state *mpudebug; /* To debug stability issue */
...
#ifdef LGE_LIGER_SELF_TEST_FLAG
		pr_debug("set self test flag true...\n");
		st->self_test_exec_state = true;
#endif
		mutex_unlock(&indio_dev->mlock);
		return sprintf(buf, "%d\n", result);
...
/* LGE_CHANGE_E */
	...
	result= gpio_direction_input(st->plat_data.irq_gpio);
	if (result) {
		dev_err(&client->dev,
...
/* LGE_CHANGE_E */
	...
	if (result) {
		dev_err(&client->dev, "unable to request inv_reset gpio\n");
	}
...
//msleep(100); /* Power-On RESET time */
	...
	*(struct mpu_platform_data *)dev_get_platdata(&client->dev);
	...
	pr_info("name : %s, type : %d\n", id->name, st->chip_type);
	...
	mpudebug = st; /* To debug stability issue */
...
static int inv_mpu_resume(struct device *dev)
{
	...
	if (st->chip_config.dmp_on && st->chip_config.enable) {
		...
		/* Prevent interrupt wakeup when entering suspend mode */
		result  = inv_set_interrupt_on_gesture_event(st, false);
	} else if (st->chip_config.enable) {
		result = st->set_power_state(st, true);
	}

	/* use latch bit 1 on INT_PIN_CFG register to wake up AP */
	result = inv_i2c_single_write(st, REG_INT_PIN_CFG, 0);
	if(result) {
		pr_err("failed to write REG_INT_PIN_CFG on inv_mpu_resume\n");
	}
...
static int inv_mpu_suspend(struct device *dev)
{
	...
	if (st->chip_config.dmp_on && st->chip_config.enable) {
		...
		if ((!st->batch.on) && (!st->chip_config.smd_enable) &&
			...
			result |= st->set_power_state(st, false);
		/* Prevent interrupt wakeup when entering suspend mode */
		result  = inv_set_interrupt_on_gesture_event(st, true);
	} else if (st->chip_config.enable) {
	...
	if(result) {
		pr_err("failed to write REG_INT_PIN_CFG on inv_mpu_suspend\n");
	}
...
static struct of_device_id inv_mpu_match_table[] = {
...
static struct i2c_driver inv_mpu_driver = {
	...
	.driver = {
		...
		.name	=	"inv-mpu-iio",
		.pm     =   INV_MPU_PMOPS,
        .of_match_table = inv_mpu_match_table,
	},
	.address_list = normal_i2c,
};

static void inv_mpu_init_async(void *data, async_cookie_t cookie)
{
...
{
	async_schedule(inv_mpu_init_async, NULL);
