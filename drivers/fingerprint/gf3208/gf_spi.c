...
struct gf_key_map key_map[] =
{
      ...
      {  "LEFT" ,  KEY_LEFT   },
      {  "RIGHT",  KEY_RIGHT  },
	  {  "FPWAKE", KEY_FINGERPRINT_WAKE },  //weitianlei add for wake up LCM
      {  "CAMERA", KEY_CAMERA },
      {  "FORCE",  KEY_F9     },
      {  "CLICK",  KEY_F19    },
};
...
*/
	...
	pr_info("goodix gd minor = %ld \n",minor);
	if (minor < N_SPI_MINORS) {
		struct device *dev;
		pr_info("goodix gd 111 status = %d \n",status);
		gf_dev->devt = MKDEV(SPIDEV_MAJOR, minor);
		dev = device_create(gf_class, &gf_dev->spi->dev, gf_dev->devt,
				    gf_dev, GF_DEV_NAME);
		status = IS_ERR(dev) ? PTR_ERR(dev) : 0;
		pr_info("goodix gd 222 status = %d \n",status);
	} else {
