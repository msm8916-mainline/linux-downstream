...
#define SPIDEV_MAJOR		211	/* assigned */
...
#define GF_KEY_WAKE		KEY_FINGERPRINT_WAKE
#define GF_KEY_CAMERA	KEY_CAMERA
/**************************debug******************************/
#define GF_DEBUG   1
/*#undef  GF_DEBUG*/
...
static void spi_clock_set(struct gf_dev *gf_dev, int speed)
{
	...
	if (rate < 0) {
		pr_info("%s: no match found for requested clock frequency:%d",__func__, speed);
		return;
	}
...
static void gf_spi_clk_open(struct gf_dev *gf_dev)
{
	...
	clk_prepare_enable(gf_dev->core_clk);

	//open iface clk
	clk_prepare_enable(gf_dev->iface_clk);
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
	...
	__set_bit(GF_KEY_WAKE, gf_dev->input->keybit);
	__set_bit(GF_KEY_CAMERA, gf_dev->input->keybit);
...
#endif
{
	...
	vreg = regulator_get(&spi->dev,"vcc_spi");
	if (!vreg) {
		dev_err(&spi->dev, "Unable to get vcc_spi\n");
//		return -1;
	}
	...
	if (regulator_count_voltages(vreg) > 0) {
		ret = regulator_set_voltage(vreg, 2800000,2800000);
			if (ret){
				dev_err(&spi->dev,"Unable to set voltage on vcc_spi");
//				return -1;
			}
	}
	ret = regulator_enable(vreg);
	if (ret) {
		dev_err(&spi->dev, "error enabling vdd_ana %d\n",ret);
		regulator_put(vreg);
		vreg = NULL;
	}
	dev_dbg(&spi->dev,"Set voltage on vcc_spi for goodix fingerprint");
...
static int __init gf_init(void)
{
    ...
    */
    ...
    if (get_fp_id() != 2)
    {
        printk("%s(): wrong fp id, not gf driver, exit\n", __func__);
        return 0;
    }
#endif         
