...
/*
static void fpc_gpio_reset(struct fpc1020_data *fpc1020)
{
	int error = 0;
	int counter = 3;
	...
	while (counter) {
		...
		if (!error) {
			...
			printk(KERN_ERR "%s timed out,retrying ...\n",
				__func__);
...
*/
	...
	if (!strncmp(buf, "enable", strlen("enable")))
		...
		rc = vreg_setup(fpc1020, "vcc_spi", false);
	else
		return -EINVAL;
	return rc ? rc : count;
...
*/
	...
	gpio_set_value(fpc1020->rst_gpio, 1);
	udelay(FPC1020_RESET_HIGH2_US);
	irq_gpio = gpio_get_value(fpc1020->irq_gpio) ? 0 : -EIO;
	pr_info("fpc:%s after reset irq gpio = %d\n", __func__,irq_gpio);
	if (irq_gpio) {
		dev_dbg(dev, "%s IRQ after reset ERROR !\n", __func__);
	}
	return irq_gpio;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "enable", strlen("enable")))
		fpc1020->isSleep = true;
	else if (!strncmp(buf, "disable", strlen("disable")))
		fpc1020->isSleep = false;
	else
		return -EINVAL;
	return count;
}
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if( *buf == '1' )
	{
		...
		input_report_key(fpc1020->idev_key, KEY_FINGERPRINT_WAKE, 1);
		input_report_key(fpc1020->idev_key, KEY_FINGERPRINT_WAKE, 0);
		input_sync(fpc1020->idev_key);
	}
	else
	{
		return -EINVAL;
...
if (!strncmp(buf, "enable", strlen("enable")))
	{
	//	wake_lock_timeout(&fpc1020->ttw_wl, msecs_to_jiffies(500));
		queue_work(fpc1020->workqueue, &fpc1020->work);
	}
	else if (!strncmp(buf, "disable", strlen("disable")))
	{
...
static void fpc1020_work_func(struct work_struct *work)
{

	struct fpc1020_data *fpc1020 = container_of(work, struct fpc1020_data, work);
	wake_lock_timeout(&fpc1020->ttw_wl, msecs_to_jiffies(1500));
...
//	if(fpc1020->wakeup_enabled && fpc1020->isSleep) {
/*	if(fpc1020->isSleep) {
		input_report_key(fpc1020->idev, KEY_FINGERPRINT_WAKE, 1);
		input_report_key(fpc1020->idev, KEY_FINGERPRINT_WAKE, 0);
		input_sync(fpc1020->idev);
		dev_dbg(fpc1020->dev, "%s report wakeup key\n", __func__);
	};
*/	return IRQ_HANDLED;
}
...
*/
	...
	fpc1020->idev_key = devm_input_allocate_device(dev);
    if (!fpc1020->idev_key) {
		dev_err(dev, "failed to allocate input device\n");
		rc = -ENOMEM;
		goto exit;
	}
    set_bit(EV_KEY,	fpc1020->idev_key->evbit);
	set_bit(KEY_FINGERPRINT_WAKE, fpc1020->idev_key->keybit);
    snprintf(fpc1020->idev_key_name, sizeof(fpc1020->idev_key_name),
			"finger%s", "_key");
    fpc1020->idev_key->name = fpc1020->idev_key_name;
	rc = input_register_device(fpc1020->idev_key);
...
//	fpc_gpio_reset(fpc1020);
	fpc1020->workqueue = NULL;
	fpc1020->workqueue = create_workqueue("fpc1020_wake_up_workqueue");
	INIT_WORK(&fpc1020->work,fpc1020_work_func);
#if defined(CONFIG_FB)
    fpc1020->fb_notifier.notifier_call = fb_notifier_suspend;
...
static int fpc1020_remove(struct spi_device *spi)
{
	...
	if(fpc1020->workqueue)
		destroy_workqueue(fpc1020->workqueue);
//	(void)vreg_setup(fpc1020, "vdd_io", false);
//	(void)vreg_setup(fpc1020, "vcc_spi", false);
//	(void)vreg_setup(fpc1020, "vdd_ana", false);
	dev_dbg(&spi->dev, "%s\n", __func__);
...
*/
#if defined(CONFIG_FB)
static int fb_notifier_suspend(struct notifier_block *self,
				unsigned long event, void *data)
{
	...
	printk(KERN_WARNING "***enter fpc1020 fb_notifier_suspend event=%lu ****\n",event);
	
	if (evdata && evdata->data && event == FB_EARLY_EVENT_BLANK && fpc1020) {
		blank = (evdata->data);
	...
	printk(KERN_WARNING "*** leaved fpc1020 fb_notifier_suspend ****\n");
	return 0;
...
static int __init fpc1020_init(void)
{
	int rc , fp_id;
	fp_id = get_fp_id();
	printk("fpc1020_driver_init enter,fp_id=%d!\n",fp_id);
	if(1!=fp_id)
	{
		printk("fpc1020_init: wrong fp id, exit\n");
		return 0;
	}
	rc = spi_register_driver(&fpc1020_driver);
	if (!rc)
		pr_info("%s OK\n", __func__);
	else
		pr_err("%s %d\n", __func__, rc);
