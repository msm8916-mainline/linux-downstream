...
void ssflash_led_turn_on_temp(void);
void ssflash_led_turn_off_temp(void);
...
void ssflash_led_turn_on_temp()
{
	flash_request_gpio(pdata);

	pr_err("ssflash_led_turn_on_temp \n");
	...
	lm3632_write_byte(info->lm3632, 0x06, 0x00);
	lm3632_write_byte(info->lm3632, 0x09, 0x51);
	lm3632_write_byte(info->lm3632, 0x0A, 0x1B);
...
}EXPORT_SYMBOL_GPL(ssflash_led_turn_on_temp);

void ssflash_led_turn_off_temp()
{
	flash_request_gpio(pdata);

	pr_err("ssflash_led_turn_off_temp \n");
...
}EXPORT_SYMBOL_GPL(ssflash_led_turn_off_temp);
		...
		const char *buf, size_t count)
{
	...
	switch(nValue) {
		...
		case 1:
			...
			lm3632_write_byte(ffl_info->lm3632, 0x06, 0x00);
		...
		case 100:
			...
			lm3632_write_byte(ffl_info->lm3632, 0x06, 0x30);
			lm3632_write_byte(ffl_info->lm3632, 0x09, 0x51);
			lm3632_write_byte(ffl_info->lm3632, 0x0A, 0x1B);
