...
error:
	if (CLK_GPIO == flash_led->gpio_type[FLASH_EN] &&
		IS_ERR(flash_led->flash_en_clk))
		devm_clk_put(&pdev->dev, flash_led->flash_en_clk);
	else if (NORMAL_GPIO == flash_led->gpio_type[FLASH_EN] &&
		flash_led->flash_en)
		gpio_free(flash_led->flash_en);

	if (CLK_GPIO == flash_led->gpio_type[FLASH_NOW] &&
		IS_ERR(flash_led->flash_now_clk))
		devm_clk_put(&pdev->dev, flash_led->flash_now_clk);
	else if (NORMAL_GPIO == flash_led->gpio_type[FLASH_NOW] &&
		flash_led->flash_now)
