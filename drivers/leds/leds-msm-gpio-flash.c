...
int number_pin=3; //add for detect whether the forth pin of the gpio is specified for led
...
int led_gpio_flash_probe(struct platform_device *pdev)
{
	...
	/*add for whether property is  specified */
	if ((of_find_property(node, "qcom,flash-front-enable", NULL))&&(of_find_property(node, "qcom,flash-rear-enable", NULL)))
	{
	 number_pin=4;//add for detect whether the forth pin of the gpio is specified
	...
	if (flash_led->front_enable < 0) {
		...
		goto error;
	/*+++ add for led flash use two gpios */
	...
	}/*end if  add for whether property is specified */
