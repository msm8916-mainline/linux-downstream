...
#define Bre_Imax          0x72   //LED最大电流配置,0x00=omA,0x01=5mA,0x02=10mA,0x03=15mA,
#define Rise_t  0x02   //LED呼吸上升时间,0x00=0.13s,0x01=0.26s,0x02=0.52s,0x03=1.04s,0x04=2.08s,0x05=4.16s,0x06=8.32s,0x07=16.64s
#define Fall_t   0x02   //LED呼吸下降时间,0x00=0.13s,0x01=0.26s,0x02=0.52s,0x03=1.04s,0x04=2.08s,0x05=4.16s,0x06=8.32s,0x07=16.64s
#define Hold_time   0x04   //LED呼吸到最亮时的保持时间0x00=0.13s,0x01=0.26s,0x02=0.52s,0x03=1.04s,0x04=2.08s,0x05=4.16s
#define Off_time      0x04   //LED呼吸到灭时的保持时间0x00=0.13s,0x01=0.26s,0x02=0.52s,0x03=1.04s,0x04=2.08s,0x05=4.16s,0x06=8.32s,0x07=16.64s
...
struct i2c_board_info aw2013_info = {
			.type = "aw2013",
			.addr = 0x45,//aw2013 i2c slave address
			.platform_data = &aw2013_pdata,
		};


static int aw2013_has_inited = 0;
//static unsigned char aw2013_red_level=0x00,aw2013_green_level=0x00,aw2013_blue_level=0x00,aw2013_white_level=0x00;
//static unsigned char aw2013_yellow_level=0x00,aw2013_cyan_level=0x00,aw2013_purple_level=0x00;
...
int aw2013_set_color_singlecolor(struct aw2013_dev_data *aw2013)
{
	...
	if(aw2013->leds[LED_RED].brightness)red_on = 1;
	if(aw2013->leds[LED_GREEN].brightness)green_on = 1;
	if(aw2013->leds[LED_BLUE].brightness)blue_on = 1;
		
	if(aw2013->leds[LED_RED].brightness > MAX_BRIGHTNESS_RED)aw2013->leds[LED_RED].brightness = MAX_BRIGHTNESS_RED;
	if(aw2013->leds[LED_GREEN].brightness > MAX_BRIGHTNESS_GREEN)aw2013->leds[LED_GREEN].brightness = MAX_BRIGHTNESS_GREEN;
	if(aw2013->leds[LED_BLUE].brightness > MAX_BRIGHTNESS_BLUE)aw2013->leds[LED_BLUE].brightness = MAX_BRIGHTNESS_BLUE;		

	//only support one leds blink, we have bug on multi-leds blink 
	...
	if(red_on || green_on || blue_on)
	{
		if(0 == aw2013_has_inited){
			aw2013_i2c_write(0x0,0x55);//reset all register
			aw2013_i2c_write(0x01,0x1);//enable led module
			mdelay(1);
			//aw2013_has_inited = 1;
		}
		
		aw2013_i2c_write(0x01, 0xe1);		// enable LED 		

		if(red_on)aw2013_i2c_write(0x31, blink_flag|0x61);
		if(green_on)aw2013_i2c_write(0x32, blink_flag|0x62);	
		if(blue_on)aw2013_i2c_write(0x33, blink_flag|0x63);	
		
		if(red_on)aw2013_i2c_write(0x34, aw2013->leds[LED_RED].brightness);	// LED0 level,
		if(green_on)aw2013_i2c_write(0x35, aw2013->leds[LED_GREEN].brightness);	// LED1 level,
		if(blue_on)aw2013_i2c_write(0x36, aw2013->leds[LED_BLUE].brightness);	// LED2 level,
		...
		aw2013_i2c_write(0x01,0);//shut down led module
		mdelay(1);
...
int aw2013_set_color_multicolor(struct aw2013_dev_data *aw2013, enum led_colors color)
{
	...
	if(aw2013->leds[color].brightness)
	{
		...
		switch (color)
		{
				...
				aw2013_i2c_write(0x01,0xe1);//enable led module
				aw2013_i2c_write(0x31, blink_flag|0x61);
				aw2013_i2c_write(0x32, blink_flag|0x62);
				aw2013_i2c_write(0x33, blink_flag|0x62);
				aw2013_i2c_write(0x34, MAX_BRIGHTNESS_RED);	// LED0 level,
				aw2013_i2c_write(0x35, MAX_BRIGHTNESS_GREEN);	// LED1 level,
				aw2013_i2c_write(0x36, MAX_BRIGHTNESS_BLUE);	// LED2 level,
				...
				aw2013_i2c_write(0x34, MAX_BRIGHTNESS_RED);	// LED0 level,
				aw2013_i2c_write(0x35, MAX_BRIGHTNESS_GREEN);	// LED1 level,
				aw2013_i2c_write(0x36, 0);	// LED2 level,
				...
				aw2013_i2c_write(0x33, blink_flag|0x62);
				aw2013_i2c_write(0x34, MAX_BRIGHTNESS_RED);	// LED0 level,
				aw2013_i2c_write(0x35, 0);	// LED1 level,
				aw2013_i2c_write(0x36, MAX_BRIGHTNESS_BLUE);	// LED2 level,
				...
				aw2013_i2c_write(0x32, blink_flag|0x62);
				aw2013_i2c_write(0x33, blink_flag|0x62);
				aw2013_i2c_write(0x34, 0);	// LED0 level,
				aw2013_i2c_write(0x35, MAX_BRIGHTNESS_GREEN);	// LED1 level,
				aw2013_i2c_write(0x36, MAX_BRIGHTNESS_BLUE);	// LED2 level,
	...
	struct i2c_client *client)
{
	...
	//AW2013_DEBUG("jin blink shut down led module\n");
...
//hoper atteched device atter show fortake off  linux boot warning,stack error
