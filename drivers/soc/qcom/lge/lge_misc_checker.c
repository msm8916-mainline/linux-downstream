...
#define MISC_PATH "/dev/block/platform/7824900.sdhci/by-name/misc"
...
static unsigned int g_index_r;
static unsigned int g_index_w;
...
static int get_ftm_items(int size, void *out)
{
	struct file *filp;
	int ret = 0;
	int len;
...
static int get_ftm_item_read(char *buffer, const struct kernel_param *kp)
{
	...
	item = get_ftm_item_entry(g_index_r);
	if (item == NULL) {
		pr_err("invalid index : %d\n", g_index_r);
		return -1;
	}
	...
	get_ftm_item(g_index_r, item->size, value);
	value[item->size] = '\0';
...
static int set_ftm_item_write(const char *val, struct kernel_param *kp)
{
	...
	if (2 == sscanf(val, "%9s %511s", index_str, value_str)) {
		ret = kstrtoint(index_str, 10, &index);
		if (ret == 0) {
			if (check_index(index) != 0) {
				pr_err("invalid index : %d\n", index);
				...
				g_index_w = index;
				item = get_ftm_item_entry(g_index_w);
				if (item == NULL) {
					pr_err("invalid index : %d\n", g_index_w);
					return -1;
				}
		...
		if (ret == 0) {
			num = (char)value;
			g_status = FTM_VALUE_BYTE;
			set_ftm_item(index, item->size, &num);
		} else {
			g_status = FTM_VALUE_STRING;
...
static int get_ftm_item_write(char *buffer, const struct kernel_param *kp)
{
	...
	get_ftm_item(g_index_w, item->size, value);
	value[item->size] = '\0';

	if (g_status == FTM_VALUE_BYTE)
		return sprintf(buffer, "%s = %d", item->name, (char)value[0]);
	else
		return sprintf(buffer, "%s = %s", item->name, value);
}
...
static int misc_checker_probe(struct platform_device *pdev)
{
	...
	of_property_read_u32(node, "lge,misc_checker_index", (u32 *)&misc_checker_index);
