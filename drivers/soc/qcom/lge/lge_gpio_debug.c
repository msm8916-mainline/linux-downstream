...
#define GPIO_DIR_CONN_INTR(tlmm, intr) (tlmm->base + 0x106000 + (0x4 * (intr)))
...
static int gpio_debug_probe(struct platform_device *pdev)
{
	...
	debugfs_create_file("address", 0600, debugfs_base, NULL, &gpio_address_fops);
	debugfs_create_file("data", 0600, debugfs_base, NULL, &gpio_data_fops);
	debugfs_create_file("data_all", 0600, debugfs_base, NULL, &gpio_all_fops);
