...
#define BBK_LARGE_OBJ_SUPPRESSION
#define BBK_GLOVES_MODE
#elif defined(PD1302) || defined(PD1302F) || defined(PD1302F_EX) || defined(PD1302LG4)
#define BBK_DCLICK_WAKE
#define BBK_LARGE_OBJ_SUPPRESSION
#define BBK_GLOVES_MODE
#elif  defined(TD1404) 
...
struct rmi_device_platform_data {
	...
	struct regulator *v2p8_gyro;
	int attn_gpio;
	u32 irq_gpio_flags;
	int rst_gpio;
	u32 reset_gpio_flags;
	#if defined(PD1401F)||defined(PD1401F_EX) || defined(PD1401V)||defined(PD1401CL)||defined(PD1401CLG4)
	int ldo_gpio;
	u32 ldo_gpio_flags;
	#endif
	...
	int  dclick_dimension_min_x;
	int  dclick_dimension_max_x;
	int  dclick_dimension_min_y;
	int  dclick_dimension_max_y;
	...
	int (*gpio_config)(struct rmi_device_platform_data *pdata,bool configure);
...
#if (defined(CONFIG_HAS_EARLYSUSPEND) && !defined(CONFIG_RMI4_SPECIAL_EARLYSUSPEND)) || defined(CONFIG_FB)
...
struct rmi_driver {
	...
	int (*irq_handler)(struct rmi_device *rmi_dev);	/* wyl delete the irq request*/
			...
			const char *buf, size_t count)
{
	dev_warn(dev,
		 ...
		 "attribute %s.", (int)count, attr->attr.name);
	return -EPERM;
}
