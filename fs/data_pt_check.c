...
void vfs_set_data_pt_check(int flag)
{
	atomic_set(&g_data_pt_check, flag);
}
int vfs_get_data_pt_check(void)
{
	return atomic_read(&g_data_pt_check);
}

#ifdef CONFIG_ODM_SDCARD_PT_CHECK
void vfs_set_sdcard_pt_check(int flag)
{
	atomic_set(&g_sdcard_pt_check, flag);
}
int vfs_get_sdcard_pt_check(void)
{
	return atomic_read(&g_sdcard_pt_check);
}

void vfs_set_sdcard_pt_limit_size(u64 size)
{
	atomic64_set(&g_sdcard_pt_limit_size, size);
}
u64 vfs_get_sdcard_pt_limit_size(void)
{
...
void vfs_set_data_pt_size(u64 size)
{
	atomic64_set(&g_data_pt_size, size);
}
u64 vfs_get_data_pt_size(void)
{
	return atomic64_read(&g_data_pt_size);
}

void vfs_set_data_pt_limit_size(u64 size)
{
	atomic64_set(&g_data_pt_limit_size, size);
}
u64 vfs_get_data_pt_limit_size(void)
{
