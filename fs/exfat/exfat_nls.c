...
static const u16 bad_dos_chars[] = {
	/* + , ; = [ ] */
...
static const u16 bad_uni_chars[] = {
	/* " * / : < > ? \ | */
	...
	while (i < DOS_NAME_LENGTH) {
		...
		if (*uniname == (u16) '\0') {
			...
			if (uniname < last_period)
				...
				i = 8;
		} else if (nls_wstrchr((u16 *)bad_dos_chars, *uniname)) {
			lossy = TRUE;
			*(dosname+i) = '_';
			i++;
		} else {
	...
	*(++end_of_name) = '\0';
	...
	if (nls == NULL) {
		...
		while (j < (MAX_NAME_LENGTH-1)) {
			...
			if ((*uniname < 0x0020) || nls_wstrchr((u16 *)bad_uni_chars, *uniname))
				lossy = TRUE;
