...
INT32 ffsMountVol(struct super_block *sb, INT32 drv)
{
...
#if EXFAT_CONFIG_FS_TYPE_CHECK
	if (!memcmp(p_pbr->oem_name, "EXFAT   ", 8)) {
		TIME_START();
		ret = exfat_mount(sb, p_pbr);
		TIME_END();
		PRINT_TIME(__LINE__);
	} else {
		PRINTK("Error, No exFAT\n");
		ret = FFS_FORMATERR;
	}
#else
	if (i < 53) {
...
#if EXFAT_CONFIG_READAHEAD
	{
		...
		struct backing_dev_info *bdi =
			bd_inode->i_mapping->backing_dev_info;
		struct exfat_sb_info *sbi = EXFAT_SB(sb);

		if (sbi->options.readahead_kb) {
			sbi->ra_pages = bdi->ra_pages;
			bdi->ra_pages = (sbi->options.readahead_kb /
				PAGE_CACHE_SIZE);
		}
	}
#endif
...
INT32 ffsUmountVol(struct super_block *sb)
{
...
#if EXFAT_CONFIG_READAHEAD
	{
		...
		if (sbi->options.readahead_kb)
			bdi->ra_pages = sbi->ra_pages;//restore
	}
#endif
	fs_sync(sb, 0);
...
INT32 ffsReadDir(struct inode *inode, DIR_ENTRY_T *dir_entry)
{
	...
	while (clu.dir != CLUSTER_32(~0)) {
		...
		if (dir.dir == CLUSTER_32(0))
        {
            if (dentries_per_clu == 0)
            {
                printk("[exfat] blocking kernel crash\n");
                break;
            }
...
INT32 clr_alloc_bitmap(struct super_block *sb, UINT32 clu)
{
	...
	int ret;
	UINT32 discard_clu = clu + 2;//Start # of cluster: 0(bitmap) and 2(LBA)
#endif
	FS_INFO_T *p_fs = &(EXFAT_SB(sb)->fs_info);
	BD_INFO_T *p_bd = &(EXFAT_SB(sb)->bd_info);
...
#if EXFAT_CONFIG_DISCARD
	if (opts->discard) {
			PRINTK("[DISCARD] clu#=%u, discard_clu#=%u, sectors=%u\n", clu,
				discard_clu, (1 << p_fs->sectors_per_clu_bits));
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,37)
		ret = sb_issue_discard(sb, START_SECTOR(discard_clu), (1 << p_fs->sectors_per_clu_bits));
		//ret = sb_issue_discard(sb, START_SECTOR(clu), (1 << p_fs->sectors_per_clu_bits));
#else
		ret = sb_issue_discard(sb, START_SECTOR(discard_clu), (1 << p_fs->sectors_per_clu_bits), GFP_NOFS, 0);
...
#ifdef EXFAT_CONFIG_FIX_NAME_ENTRY_PADDING
	memset(ep, 0x0, sizeof(NAME_DENTRY_T));
#endif
...
ENTRY_SET_CACHE_T *get_entry_set_in_dir (struct super_block *sb, CHAIN_T *p_dir, INT32 entry, UINT32 type, DENTRY_T **file_ep)
{
	...
	PRINTK("trying to malloc %zx bytes for %d entries\n",
		(offsetof(ENTRY_SET_CACHE_T, __buf) + (num_entries) * sizeof(DENTRY_T)),
			num_entries);
...
#ifdef EXFAT_CONFIG_SUPPORT_COMPAT64
	byte_offset += ((long)ep - (long)&(es->__buf)) + es->offset;
#else
...
#if EXFAT_CONFIG_FIX_FILE_NAME_TOO_LONG_ERROR_TYPE
	if (lossy == FFS_NAMETOOLONG) {
		return (FFS_NAMETOOLONG);
	} else if (lossy)
		return(FFS_INVALIDPATH);
#else
...
INT32 exfat_mount(struct super_block *sb, PBR_SECTOR_T *p_pbr)
{
	...
	struct exfat_sb_info *sbi = EXFAT_SB(sb);
	UINT8 shift = sbi->shift;
#endif

#if EXFAT_CONFIG_PAGESIZE_ALIGNED_BLOCK
	//p_bpb->sector_size_bits = p_bpb->sector_size_bits + 3;//4096
	//p_bpb->sectors_per_clu_bits = p_bpb->sectors_per_clu_bits - 3; //s/8
	//the number of total sectors
	//SET64(p_bpb->vol_length, GET64(p_bpb->vol_length) >> 3);
	//SET32(p_bpb->fat_length, GET32(p_bpb->fat_length) >> 3);
	//SET32(p_bpb->fat_offset, GET32(p_bpb->fat_offset) >> 3);
	//SET32(p_bpb->clu_offset, GET32(p_bpb->clu_offset) >> 3);
#endif
	PRINTK("Cluster heap offset: %u\n", GET32(p_bpb->clu_offset));
	PRINTK("Cluster      length: %llu\n", GET64(p_bpb->vol_length));
	PRINTK("FAT          offset: %u\n", GET32(p_bpb->fat_offset));
	PRINTK("FAT          length: %u\n", GET32(p_bpb->fat_length));

	if (p_bpb->num_fats == 0)
		return FFS_FORMATERR;

#if EXFAT_CONFIG_PAGESIZE_ALIGNED_BLOCK
	p_fs->sectors_per_clu = 1 << (p_bpb->sectors_per_clu_bits - shift);
	p_fs->sectors_per_clu_bits = (p_bpb->sectors_per_clu_bits - shift);
#else
...
#if EXFAT_CONFIG_PAGESIZE_ALIGNED_BLOCK
	p_fs->num_FAT_sectors = GET32(p_bpb->fat_length) >> shift;
#else
	p_fs->num_FAT_sectors = GET32(p_bpb->fat_length);
#endif

#if EXFAT_CONFIG_PAGESIZE_ALIGNED_BLOCK
	p_fs->FAT1_start_sector = p_fs->PBR_sector
		+ (GET32(p_bpb->fat_offset) >> shift);
#else
...
#if EXFAT_CONFIG_PAGESIZE_ALIGNED_BLOCK
	p_fs->root_start_sector = p_fs->PBR_sector
	       	+ (GET32(p_bpb->clu_offset) >> shift);
#else
...
#if EXFAT_CONFIG_PAGESIZE_ALIGNED_BLOCK
	p_fs->num_sectors = GET64(p_bpb->vol_length) >> shift;
#else
	...
	PRINTK("p_fs->sectors_per_clu: 		%u\n", p_fs->sectors_per_clu);
	PRINTK("p_fs->sectors_per_clu_bits: 	%u\n", p_fs->sectors_per_clu_bits);
	PRINTK("p_fs->cluster_size_bits: 	%u\n", p_fs->cluster_size_bits);
	PRINTK("p_fs->cluster_size: 		%u\n", p_fs->cluster_size);
	PRINTK("p_fs->num_FAT_sectors: 		%u\n", p_fs->num_FAT_sectors);
	PRINTK("p_fs->FAT1_start_sector:	%u\n", p_fs->FAT1_start_sector);
	PRINTK("cluster_size_bits: %d\n", p_fs->cluster_size_bits);
