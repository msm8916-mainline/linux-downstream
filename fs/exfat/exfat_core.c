...
s32 ffsMountVol(struct super_block *sb)
{
	...
	sm_init(&p_fs->v_sem);
#if defined(CONFIG_EXFAT_FS_MODULE)
	p_fs->drv = (typeof(p_fs->drv))g_drv_id;
	if (++g_drv_id == (u32)(-1))
		g_drv_id = 0;
#endif
...
s32 ffsRemoveFile(struct inode *inode, FILE_ID_T *fid)
{
	...
	/* (1) update the directory entry */
	exfat_remove_file(inode, &dir, dentry);
...
/*
bool is_sb_nls_disk_loaded(struct super_block *sb)
{
	return ((EXFAT_SB(sb)->nls_disk) ? true : false);
}
*/
...
s32 ffsGetStat(struct inode *inode, DIR_ENTRY_T *info)
{
	...
	*(uni_name.name) = 0x0;
	...
	//refer to exfat_fill_super() in exfat_super.c, only load exfat_sb_info.nls_disk for non-EXFAT volume
	if ((*(uni_name.name) == 0x0) && (p_fs->vol_type != EXFAT)/*is_sb_nls_disk_loaded(sb)*/)
...
void exfat_fs_sync(struct super_block *sb, s32 do_sync)
{
	if (do_sync)
		bdev_sync(sb);
} /* end of exfat_fs_sync */
					...
					if (is_feasible_entry) {
						...
						else
						{
							if (uniname >= p_uniname->name)
								...
								//order is not used in extract_uni_name_from_name_entry()
								if (order < 2)
								{
									order = 2;
								}
...
void exfat_remove_file(struct inode *inode, CHAIN_T *p_dir, s32 entry)
{
	s32 num_entries;
	u32 sector;
	DENTRY_T *ep;
