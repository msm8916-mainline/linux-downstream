...
#if 0
	PRINTK("cluster: %u, clu_offset: %d, sec_offset: %d\n", cluster,
			clu_offset, sec_offset);
#endif
...
static int exfat_statfs(struct dentry *dentry, struct kstatfs *buf)
{
	...
	if (p_fs->used_clusters == (UINT32) ~0) {
		...
		if (p_fs->dev_ejected)
			printk("[EXFAT] device is ejected\n");
	}
...
static int exfat_show_options(struct seq_file *m, struct vfsmount *mnt)
{
	...
	if (opts->readahead_kb != 0)
		seq_printf(m, ",readahead_kb=%u", (opts->readahead_kb / 1024));
#endif
	if (p_fs->dev_ejected)
		seq_puts(m, ",ejected");
	return 0;
...
#endif
#if EXFAT_CONFIG_READAHEAD
	opts->readahead_kb = DEFAULT_READAHEAD_SIZE;
#endif
	*debug = 0;
...
static void setup_d_op(struct super_block *sb)
{
...
static int exfat_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	if (sbi->options.discard) {
		...
		if (!blk_queue_discard(q)) {
			printk(KERN_WARNING "[EXFAT] Device does not support discard.\n");
