...
static const time_t accum_days_in_year[] = {
...
static void exfat_free_super(struct exfat_sb_info *sbi)
{
	...
	if (!list_empty(&sbi->fs_info.fs_info_list))
	{
		list_del_init(&sbi->fs_info.fs_info_list);
	}
	sbi->fs_info.owner_sb = NULL;
#endif
	if (sbi->nls_disk)
...
#endif
#if defined(CONFIG_EXFAT_FS_MODULE) && defined(CONFIG_EXFAT_DRIVE_DYNAMIC_LIST)
	INIT_LIST_HEAD(&sbi->fs_info.fs_info_list);
	sbi->fs_info.owner_sb = sb;
#endif
	sb->s_fs_info = sbi;
