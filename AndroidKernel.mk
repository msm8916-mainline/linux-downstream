...
ifeq ($(PRODUCT_SUPPORT_NTFS), y)
	...
	@sh tuxera_update.sh --target target/lg.d/mobile-msm8909-arm32-4.8 --use-cache --latest --max-cache-entries 2 --source-dir $(ANDROID_BUILD_TOP)/kernel --output-dir $(ANDROID_BUILD_TOP)/$(KERNEL_OUT) -a --user lg-mobile --pass AumlTsj0ou
	@tar -xzf tuxera-file-systems*.tgz
	@mkdir -p $(TARGET_OUT_EXECUTABLES)
	@cp $(ANDROID_BUILD_TOP)/tuxera-file-systems*/ntfs/kernel-module/tntfs.ko $(ANDROID_BUILD_TOP)/$(TARGET_OUT_EXECUTABLES)/../lib/modules/
	@cp $(ANDROID_BUILD_TOP)/tuxera-file-systems*/ntfs/tools/* $(TARGET_OUT_EXECUTABLES)
	@rm -f kheaders*.tar.bz2
	@rm -f tuxera-file-systems*.tgz
	@rm -rf tuxera-file-systems*
...
ifeq ($(TARGET_BUILD_VARIANT), user)
	...
	$(ANDROID_BUILD_TOP)/device/lge/$(MYDEVICE)/$(TARGET_PRODUCT)/products/operator/apps/LGUPlus_Iwlan/module/user/mwlan_aarp.ko \
	$(ANDROID_BUILD_TOP)/$(KERNEL_MODULES_OUT)/mwlan_aarp.ko
	perl $(ANDROID_BUILD_TOP)/kernel/scripts/sign-file sha1 $(ANDROID_BUILD_TOP)/$(KERNEL_OUT)/signing_key.priv $(ANDROID_BUILD_TOP)/$(KERNEL_OUT)/signing_key.x509 \
	$(ANDROID_BUILD_TOP)/device/lge/$(MYDEVICE)/$(TARGET_PRODUCT)/products/operator/apps/LGUPlus_Iwlan/module/user/ipsecdrvtl.ko \
	$(ANDROID_BUILD_TOP)/$(KERNEL_MODULES_OUT)/ipsecdrvtl.ko
else
	perl $(ANDROID_BUILD_TOP)/kernel/scripts/sign-file sha1 $(ANDROID_BUILD_TOP)/$(KERNEL_OUT)/signing_key.priv $(ANDROID_BUILD_TOP)/$(KERNEL_OUT)/signing_key.x509 \
	$(ANDROID_BUILD_TOP)/device/lge/$(MYDEVICE)/$(TARGET_PRODUCT)/products/operator/apps/LGUPlus_Iwlan/module/debug/mwlan_aarp.ko \
	$(ANDROID_BUILD_TOP)/$(KERNEL_MODULES_OUT)/mwlan_aarp.ko
	perl $(ANDROID_BUILD_TOP)/kernel/scripts/sign-file sha1 $(ANDROID_BUILD_TOP)/$(KERNEL_OUT)/signing_key.priv $(ANDROID_BUILD_TOP)/$(KERNEL_OUT)/signing_key.x509 \
	$(ANDROID_BUILD_TOP)/device/lge/$(MYDEVICE)/$(TARGET_PRODUCT)/products/operator/apps/LGUPlus_Iwlan/module/debug/ipsecdrvtl.ko \
	$(ANDROID_BUILD_TOP)/$(KERNEL_MODULES_OUT)/ipsecdrvtl.ko
endif
endif
	$(mv-modules)
