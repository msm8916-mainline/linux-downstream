 ...
 * --------------------------------------------------------------------------------------
 * 14/11/19      Hu Jin       		new it
 * 14/11/25      Hu Jin       		optimize it
...
struct akm_pdm_pinctrl_info {
	struct pinctrl *pinctrl;
    struct pinctrl_state *akm_pwdn_active;
    struct pinctrl_state *akm_pwdn_suspend;
    struct pinctrl_state *akm_mclk_active;
    struct pinctrl_state *akm_mclk_suspend;
};

static struct akm_pdm_pinctrl_info pinctrl_info;

static struct snd_soc_codec *ak4375_codec;
static struct ak4375_priv *ak4375_data;

#define AKM_ENABLE 1
#define AKM_DISABLE 0
static uint8_t akm_enable = AKM_DISABLE;
				   ...
				   struct device_attribute *attr, char *buf)
{
	...
	int ret,  fpt;
		regs = 0x00;
		rege = 0x15;
        if(ak4375_codec==NULL)
            return 0;
		for ( i = regs ; i <= rege ; i++ ){
			value = snd_soc_read(ak4375_codec, i);
			//value=i;
			rx[i]=value;
			printk("***AK4375 Addr,Reg=(%x, %x)\n", i, value);
		}
		value = snd_soc_read(ak4375_codec, 0x24);
		i++;
		rx[i]=value;
		printk("***AK4375 Addr,Reg=(%x, %x)\n", 0x24, value);
					...
					const char *buf, size_t count)
{
	...
	for (i = 0; i < pt_count; i+=2) {
		snd_soc_write(ak4375_codec, val[i], val[i+1]);
		printk("%s: write add=%d, val=%d.\n", __func__, val[i], val[i+1]);
	}
...
static int enable_akm_pwdn(void)
{
	...
	ret = pinctrl_select_state(pinctrl_info.pinctrl,
			pinctrl_info.akm_pwdn_active);
	if (ret < 0) {
		pr_err("hujinpwdn failed to configure the gpio\n");
		return ret;
	}
	gpio_set_value(ak4375_data->akm_pwdn_gpio, 1);
	ret = pinctrl_select_state(pinctrl_info.pinctrl,
				pinctrl_info.akm_pwdn_active);
	if (ret < 0) {
		pr_err("hujinpwdn failed to configure the gpio\n");
...
#if 0
static int enable_akm_mclk(void)
{
    int ret;

	ret = pinctrl_select_state(pinctrl_info.pinctrl,
			pinctrl_info.akm_mclk_active);
	if (ret < 0) {
		pr_err("hujinmclk failed to configure the gpio\n");
		return ret;
	}
	gpio_set_value(ak4375_data->akm_mclk_gpio, 1);
	ret = pinctrl_select_state(pinctrl_info.pinctrl,
				pinctrl_info.akm_mclk_active);
	if (ret < 0) {
		pr_err("hujinmclk failed to configure the gpio\n");
...
#endif
#if 1
static int disable_akm_pwdn(void)
{
    int ret;

	ret = pinctrl_select_state(pinctrl_info.pinctrl,
			pinctrl_info.akm_pwdn_suspend);
	if (ret < 0) {
		pr_err("hujinpwdn failed to configure the gpio\n");
		return ret;
	}
	gpio_set_value(ak4375_data->akm_pwdn_gpio, 0);
	ret = pinctrl_select_state(pinctrl_info.pinctrl,
				pinctrl_info.akm_pwdn_suspend);
	if (ret < 0) {
		pr_err("hujinpwdn failed to configure the gpio\n");
...
static int get_akpwdn(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value  *ucontrol)
{
    return 0;
}

static int set_akpwdn(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value  *ucontrol)
{
    pr_debug("hujin ---[%d]\n", ucontrol->value.enumerated.item[0]);
#if 1
	switch(ucontrol->value.enumerated.item[0]){
		case 1:
            pr_debug("hujin enable\n");
            akm_enable = AKM_ENABLE;
            //enable_akm_pwdn();
            break;
		case 0:
            pr_debug("hujin disable\n");
			akm_enable = AKM_DISABLE;
            //disable_akm_pwdn();
            break;
		default:
...
static int ak4375_dac_event2(struct snd_soc_codec *codec, int event) 
{
	...
	case SND_SOC_DAPM_PRE_PMU:	/* before widget power up */
        ak4375_writeMask(codec, AK4375_00_POWER_MANAGEMENT1, 0x01,0x01);	//PMCP1=1
	...
	case SND_SOC_DAPM_POST_PMD:	/* after widget power down */
		...
		ak4375_writeMask(codec, AK4375_00_POWER_MANAGEMENT1, 0x01,0x00);	//PMCP1=1
...
static int ak4375_set_pllblock(struct snd_soc_codec *codec, int fs)
{
	...
	ak4375_writeMask(codec, AK4375_0E_PLL_CLK_SOURCE_SELECT, 0x01, 0x00);	//PLS=1(BICK)
    #else
	snd_soc_write(codec, AK4375_13_SRC_CLK_SOURCE, (nTemp|0x01));		//DIV=0or1,SRCCKS=1(SRC Clock Select=PLL) set
...
{
	//ak4375_set_bias_level(codec, SND_SOC_BIAS_OFF);
	if(akm_enable == AKM_ENABLE){
		pr_debug("hujin AKM enable, so ignore suspend\n");
        //done none
    }else if(akm_enable == AKM_DISABLE){
		pr_debug("hujin AKM disable, so enter suspend\n");
		disable_akm_pwdn();
    }
...
static int setup_akm_pwdn(struct i2c_client *i2c)
{
	struct pinctrl *pinctrl;
	int akm_pwdn_gpio;

	akm_pwdn_gpio = of_get_named_gpio(i2c->dev.of_node,
	                "akm,cdc-pwdn-gpios", 0);
	if ((akm_pwdn_gpio < 0)) {
	    pr_err("hujinpwdn akm gpio error\n");
	} else {
	    if ((!gpio_is_valid(akm_pwdn_gpio))){
	        pr_err("hujinpwdn %s: Invalid akm gpio", __func__);
	        return -EINVAL;
	    }
	    pr_debug("hujinpwdn akm_pwdn_gpio=[%d]\n", akm_pwdn_gpio);
        ak4375_data->akm_pwdn_gpio = akm_pwdn_gpio;
	    pinctrl = devm_pinctrl_get(&i2c->dev);
	    if (IS_ERR(pinctrl)) {
	        pr_err("hujinpwdn %s: Unable to get pinctrl handle\n", __func__);
	        return -EINVAL;
	    }
	    pinctrl_info.pinctrl = pinctrl;
	    /* get pinctrl handle for cross det pin*/
	    pinctrl_info.akm_pwdn_active = pinctrl_lookup_state(pinctrl,
	                        "akm_pwdn_active");
	    if (IS_ERR(pinctrl_info.akm_pwdn_active)) {
	        pr_err("hujinpwdn %s: Unable to get pinctrl active handle\n",
	                             __func__);
	        return -EINVAL;
	    }
	    pinctrl_info.akm_pwdn_suspend = pinctrl_lookup_state(pinctrl,
	                        "akm_pwdn_suspend");
	    if (IS_ERR(pinctrl_info.akm_pwdn_suspend)) {
	        pr_err("hujinpwdn %s: Unable to get pinctrl active handle\n",
	                             __func__);
	        return -EINVAL;
	    }
...
#if 0
static int setup_akm_mclk(struct i2c_client *i2c)
{ 
	struct pinctrl *pinctrl;
	int akm_mclk_gpio;

	akm_mclk_gpio = of_get_named_gpio(i2c->dev.of_node,
	                "akm,cdc-mclk-gpios", 0);
	if ((akm_mclk_gpio < 0)) {
	    pr_err("hujinmclk akm gpio error\n");
	} else {
	    if ((!gpio_is_valid(akm_mclk_gpio))){
	        pr_err("hujinmclk %s: Invalid akm gpio", __func__);
	        return -EINVAL;
	    }
	    pr_debug("hujinmclk akm_mclk_gpio=[%d]\n", akm_mclk_gpio);
        ak4375_data->akm_mclk_gpio = akm_mclk_gpio;
	    pinctrl = devm_pinctrl_get(&i2c->dev);
	    if (IS_ERR(pinctrl)) {
	        pr_err("hujinmclk %s: Unable to get pinctrl handle\n", __func__);
	        return -EINVAL;
	    }
	    pinctrl_info.pinctrl = pinctrl;
	    /* get pinctrl handle for cross det pin*/
	    pinctrl_info.akm_mclk_active = pinctrl_lookup_state(pinctrl,
	                        "akm_mclk_active");
	    if (IS_ERR(pinctrl_info.akm_mclk_active)) {
	        pr_err("hujinmclk %s: Unable to get pinctrl active handle\n",
	                             __func__);
	        return -EINVAL;
	    }
	    pinctrl_info.akm_mclk_suspend = pinctrl_lookup_state(pinctrl,
	                        "akm_mclk_suspend");
	    if (IS_ERR(pinctrl_info.akm_mclk_suspend)) {
	        pr_err("hujinmclk %s: Unable to get pinctrl active handle\n",
	                             __func__);
	        return -EINVAL;
	    }
...
static struct of_device_id akm4375_match_table[] = {
...
static struct i2c_driver ak4375_i2c_driver = {
	.driver = {
		.name = "ak4375",
		.owner = THIS_MODULE,
		.of_match_table = akm4375_match_table,
	},
