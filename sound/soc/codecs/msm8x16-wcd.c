	...
	case SND_SOC_DAPM_POST_PMU:
		...
		if (strnstr(w->name, "MIC BIAS2 Power External", 30)) { /* for power external option */
			...
			if (++msm8x16_wcd->micb_2_ref_cnt == 1) {
				snd_soc_update_bits(codec, w->reg, 0x80, 0x80);
#if defined(CONFIG_SEC_MILLETVE_PROJECT) || defined (CONFIG_SEC_MATISSEVE_PROJECT)
...
#ifdef CONFIG_SAMSUNG_JACK
#if defined(CONFIG_SEC_E7_PROJECT) || defined(CONFIG_SEC_E5_PROJECT)
			snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_MICB_1_EN,
				0x05, 0x00);
#else
			snd_soc_update_bits(codec, MSM8X16_WCD_A_ANALOG_MICB_1_EN,
				0x45, 0x00);
#endif /* CONFIG_MACH_E7_EUR_OPEN */
...
#endif
#ifdef CONFIG_SEC_MILLETVE_PROJECT
