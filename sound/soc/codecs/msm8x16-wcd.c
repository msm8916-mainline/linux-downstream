...
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
static void msm8x16_wcd_set_switch_hptype(struct snd_soc_codec *codec,
		uint32_t zl, int jack_type, int state);
#endif
...
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
	.set_switch_hptype = msm8x16_wcd_set_switch_hptype,
#endif
...
#ifdef CONFIG_MACH_MSM8909_LGPS22
static int msm8909_wcd_ext_spk_boost_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct msm8x16_wcd_priv *msm8x16_wcd = snd_soc_codec_get_drvdata(codec);

	if (msm8x16_wcd->ext_spk_boost_5v_set == false)
		ucontrol->value.integer.value[0] = 0;
	else
		ucontrol->value.integer.value[0] = 1;

	dev_dbg(codec->dev, "%s: msm8x16_wcd->ext_spk_boost_5v_set = %d\n",
				__func__, msm8x16_wcd->ext_spk_boost_5v_set);
	return 0;
}

static int msm8909_wcd_ext_spk_boost_set(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct msm8x16_wcd_priv *msm8x16_wcd = snd_soc_codec_get_drvdata(codec);
	...
	switch (ucontrol->value.integer.value[0]) {
	case 0:
		msm8x16_wcd->ext_spk_boost_5v_set = false;
		gpio_set_value(msm8x16_wcd->boost_spk_enable_gpio, 0);
		break;
	case 1:
		msm8x16_wcd->ext_spk_boost_5v_set = true;
		gpio_set_value(msm8x16_wcd->boost_spk_enable_gpio, 1);
		break;
	default:
		return -EINVAL;
	}
	dev_dbg(codec->dev, "%s: msm8x16_wcd->spk_boost_5v_set = %d\n",
		__func__, msm8x16_wcd->ext_spk_boost_5v_set);
	return 0;
}
#else /* qti original */
...
static const struct snd_kcontrol_new msm8x16_wcd_snd_controls[] = {
	...
	SOC_ENUM_EXT("Ext Spk Boost", msm8x16_wcd_ext_spk_boost_ctl_enum[0],
		msm8909_wcd_ext_spk_boost_get, msm8909_wcd_ext_spk_boost_set),
#else /* qti original */
	SOC_ENUM_EXT("Ext Spk Boost", msm8x16_wcd_ext_spk_boost_ctl_enum[0],
...
#ifdef CONFIG_MACH_MSM8909_LGPS22
static int msm8909_setup_boost_spk(struct snd_soc_codec *codec)
{
	int ret = 0;
	struct snd_soc_card *card = codec->card;
	struct msm8x16_wcd_priv *msm8x16_wcd_priv =
				snd_soc_codec_get_drvdata(codec);
	const char *prop_name = "qcom,boost-spk-enable-gpio";

	msm8x16_wcd_priv->boost_spk_enable_gpio = of_get_named_gpio(
		card->dev->of_node, prop_name, 0);

	if (msm8x16_wcd_priv->boost_spk_enable_gpio < 0) {
		pr_err("property %s in node %s not found %d\n",
			   prop_name,
			   card->dev->of_node->full_name,
			   msm8x16_wcd_priv->boost_spk_enable_gpio);
		return -EINVAL;
	} else {
		pr_info("%s: boost-spk-enable-gpio : %d\n", __func__,
				msm8x16_wcd_priv->boost_spk_enable_gpio);
		ret = gpio_request(msm8x16_wcd_priv->boost_spk_enable_gpio,
						   "boost-spk-enable-gpio");
		if (ret) {
			pr_err("%s: gpio_request failed for boost-spk-enable-gpio.\n",
				   __func__);
			return -EINVAL;
		}
		gpio_direction_output(msm8x16_wcd_priv->boost_spk_enable_gpio,
							  0);
		gpio_set_value(msm8x16_wcd_priv->boost_spk_enable_gpio, 0);
	}
...
#ifndef CONFIG_AUDIO_LGE_NO_USE_HSD_3TYPE
			if (zl > 4000) {
				/* L impedance is higher than 400 ohm */
				msm8x16_wcd_priv->sdev.name = "h2w_aux";
			} else if (zl < 1000 && zl > 4000) {
				/* L impedance is 100 ~ 400 ohm */
				msm8x16_wcd_priv->sdev.name = "h2w_advanced";
			} else
#endif
			{
				/* L impedance is under 100 ohm */
				msm8x16_wcd_priv->sdev.name = "h2w";
			}
#ifdef CONFIG_LGE_EARJACK_DEBUGGER
			msm_serial_set_uart_console(0);
#endif
			switch_set_state(&msm8x16_wcd_priv->sdev,
						(jack_type == SND_JACK_HEADPHONE)? 2 : 1);
			dev_dbg(codec->dev, "%s : switch %s \n", __func__,
								msm8x16_wcd_priv->sdev.name);
		}
	} else {
#ifdef CONFIG_LGE_EARJACK_DEBUGGER
			msm_serial_set_uart_console(1);
#endif
		switch_set_state(&msm8x16_wcd_priv->sdev, 0);
		dev_dbg(codec->dev, "%s : switch no headset \n", __func__);
	}
}
...
#ifdef CONFIG_MACH_MSM8909_LGPS22
	ret = msm8909_setup_boost_spk(codec);
	if (ret < 0)
		pr_err("%s: msm8909_setup_boost_spk failed.\n", __func__);
#endif
...
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
	msm8x16_wcd_priv->sdev.name = "h2w";
	msm8x16_wcd_priv->sdev.print_state = msm8x16_hsd_print_state;
	ret = switch_dev_register(&msm8x16_wcd_priv->sdev);
	if (ret < 0) {
		dev_err(codec->dev, "%s Failed to register switch device %d\n", __func__, ret);
		switch_dev_unregister(&msm8x16_wcd_priv->sdev);
	}
#endif
#ifdef CONFIG_LGE_EARJACK_DEBUGGER
	msm_serial_set_uart_console(1);
...
#endif
#ifdef CONFIG_MACH_MSM8909_LGPS22
	if (gpio_is_valid(msm8x16_wcd_priv->boost_spk_enable_gpio))
		gpio_free(msm8x16_wcd_priv->boost_spk_enable_gpio);
#endif
	msm8x16_wcd = codec->control_data;
