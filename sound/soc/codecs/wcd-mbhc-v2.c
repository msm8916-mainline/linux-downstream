...
#include <linux/debugfs.h>//Added by lichuangchuang for fastmmi headset status (8916) SW00000000 2014/06/18
...
//Modified by chaofubang to reduce iphone headset detect time HQ01339669 20150825 start
#define HS_DETECT_PLUG_TIME_MS (1000)
#define SPECIAL_HS_DETECT_TIME_MS (2 * 1000)
#define MBHC_BUTTON_PRESS_THRESHOLD_MIN 250
#define GND_MIC_SWAP_THRESHOLD 2
//Modified by chaofubang to reduce iphone headset detect time HQ01339669 20150825 end
	...
	if (!insertion) {
		...
		if (mbhc->impedance_detect)
			wcd_mbhc_calc_impedance(mbhc,
					&mbhc->zl, &mbhc->zr);
		pr_err("%s: Reporting insertion %d(%x)\n", __func__,//Modify by lichuangchuang for headset no button report in spk call mode (8916) SW00000000 2014-09-01
			 jack_type, mbhc->hph_status);
		wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
				    mbhc->hph_status, WCD_MBHC_JACK_MASK);
...
static void wcd_correct_swch_plug(struct work_struct *work)
{
	...
	codec = mbhc->codec;
	//Added by chaofubang to cancel button press report in accessary detection time HQ00000000 20150825 start
	mbhc->in_swch_irq_handler = true;
	//Added by chaofubang to cancel button press report in accessary detection time HQ00000000 20150825 end
	...
	while (!time_after(jiffies, timeout)) {
		...
		if (result2 == 1) {
			...
			//Modified by chaofubang to fix iphone OMTP headset detect HQ01331432 20150815 start		
			...
			//Modified by chaofubang to fix iphone OMTP headset detect HQ01331432 20150815 end
...
static void wcd_mbhc_detect_plug_type(struct wcd_mbhc *mbhc)
{
	...
	wcd_enable_curr_micbias(mbhc, WCD_MBHC_EN_MB);
	//Added by chaofubang to fix OMTP detect err as playing back music HQ01279612 20150805 start.
	/*
	...
	if (test_bit(WCD_MBHC_EVENT_PA_HPHL,
		&mbhc->event_state))
		wcd_mbhc_set_and_turnoff_hph_padac(mbhc);
	//Added by chaofubang to fix OMTP detect err as playing back music HQ01279612 20150805 end.
...
exit:
	...
	if (plug_type == MBHC_PLUG_TYPE_HEADSET ||
			plug_type == MBHC_PLUG_TYPE_HEADPHONE) {
		//Deleted by chaofubang to fix pop noise as pluging headset HQ01339816 start
		//wcd_mbhc_find_plug_and_report(mbhc, plug_type);
		//Deleted by chaofubang to fix pop noise as pluging headset HQ01339816 end
...
//Added by lichuangchuang for fastmmi headset status (8916) SW00000000 2014/06/18 begin
		      ...
		      bool impedance_det_en)
{
	...
	if (mbhc->headset_jack.jack == NULL) {
		...
		//Modify by chaofubang for some headset no button report (8916) SW00146801 2015-06-10 start
		...
		//Modify by chaofubang for some headset no button report (8916) SW00146801 2015-06-10 end
	...
	wcd9xxx_init_debugfs_user(mbhc);//Added by lichuangchuang for fastmmi headset status (8916) SW00000000 2014/06/18
...
void wcd_mbhc_deinit(struct wcd_mbhc *mbhc)
{
	...
	msm8x16_unregister_notifier(codec, &mbhc->nblock);
	wcd9xxx_cleanup_debugfs_user(mbhc);//Added by lichuangchuang for fastmmi headset status (8916) SW00000000 2014/06/18 begin
	mutex_destroy(&mbhc->codec_resource_lock);
}
