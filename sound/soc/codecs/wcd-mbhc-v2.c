				...
				struct snd_soc_jack *jack, int status, int mask)
{
	...
	/*                                                               */
	/* exchaned from SND_JACK_LINEOUT to SND_JACK_HEADSET */
	if (status & SND_JACK_LINEOUT) {
				...
				enum snd_jack_types jack_type)
{
	...
	if (!insertion) {
		...
		wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
				mbhc->hph_status, WCD_MBHC_JACK_MASK);
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
		if (mbhc->mbhc_cb->set_switch_hptype)
			mbhc->mbhc_cb->set_switch_hptype(codec, 0, 0, 0);
#endif
...
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
		if (mbhc->mbhc_cb->set_switch_hptype)
			mbhc->mbhc_cb->set_switch_hptype(codec, mbhc->zl, jack_type, 1);
#endif
		wcd_mbhc_jack_report(mbhc, &mbhc->headset_jack,
				    mbhc->hph_status, WCD_MBHC_JACK_MASK);
...
#if CONFIG_MACH_LGE 	/*                                                                                   */
	if (plug_type == MBHC_PLUG_TYPE_HIGH_HPH && (!det_extn_cable_en)) {
...
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
			/* a particular headset is reported as unsupported headset */
			wcd_mbhc_report_plug(mbhc, 0, SND_JACK_HEADSET);
#endif
...
static irqreturn_t wcd_mbhc_mech_plug_detect_irq(int irq, void *data)
{
	...
	if (unlikely(wcd9xxx_spmi_lock_sleep() == false)) {
		pr_warn("[LGE MBHC] %s: failed to hold suspend\n", __func__);
		r = IRQ_NONE;
	} else {
		/* Call handler */
...
static int wcd_mbhc_get_button_mask(u16 btn)
{
	...
	switch (btn) {
	case 0:
		pr_info("SND_JACK_BTN_0 : Hook-Key\n");
		mask = SND_JACK_BTN_0;
		break;
	case 1:
		mask = SND_JACK_BTN_1;
		pr_info("SND_JACK_BTN_1 : Voice Rec\n");
		break;
	case 3:
		mask = SND_JACK_BTN_2;
		pr_info("SND_JACK_BTN_2 : Volume Up\n");
		break;
	case 7:
		mask = SND_JACK_BTN_3;
		pr_info("SND_JACK_BTN_3 : Volume Dn\n");
		break;
	case 15:
		pr_debug("SND_JACK_BTN_4 - ignore this BTN.\n");
		/* mask = SND_JACK_BTN_4; */
		break;
	default:
		pr_debug("SND_JACK_BTN_default\n");
		break;
	}
	return mask;
}
...
irqreturn_t wcd_mbhc_btn_press_handler(int irq, void *data)
{
	...
	if (mbhc->current_plug != MBHC_PLUG_TYPE_HEADSET) {
		pr_info("[LGE MBHC] %s: Plug isn't headset, ignore button press\n",
				__func__);
		goto done;
	}
	result1 = snd_soc_read(codec, MSM8X16_WCD_A_ANALOG_MBHC_BTN_RESULT);
...
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
	/* Do not report btn press except for BTN 0,1,2 */
	if (!mask) {
		pr_info("[LGE MBHC] %s: Unsupported button event, ignore button press.\n",
				__func__);
		goto done;
	}
#endif
	...
	if (schedule_delayed_work(&mbhc->mbhc_btn_dwork,
		...
		WARN(1, "[LGE MBHC] Button pressed twice without release event\n");
		wcd9xxx_spmi_unlock_sleep();
	}
done:
	pr_debug("%s: leave\n", __func__);
...
static irqreturn_t wcd_mbhc_release_handler(int irq, void *data)
{
	...
	if (mbhc->btn_press_intr) {
		...
		pr_info("[LGE MBHC] %s: This release is for fake btn press\n", __func__);
		goto exit;
	}
	...
	if (mbhc->current_plug == MBHC_PLUG_TYPE_HEADPHONE) {
#ifndef CONFIG_AUDIO_LGE_CODE_REMOVAL
		/* intergration for our framework
		because when headphones is pluged, headset event is ignored.
		should report unplug event of a headphone.
		*/
		wcd_mbhc_report_plug(mbhc, 0, SND_JACK_HEADPHONE);
		mbhc->current_plug = MBHC_PLUG_TYPE_HEADSET;
#endif
	...
	if (mbhc->buttons_pressed & WCD_MBHC_JACK_BUTTON_MASK) {
		...
		if (ret == 0) {
			pr_debug("%s: Reporting long button release event\n",
				 __func__);
			pr_info("[LGE MBHC] Headset LONG Button : %s is released.\n",
						mbhc->buttons_pressed==0x800000?"Volume Down":
						button_name[(mbhc->buttons_pressed>>24)*-1/2+2]);
			wcd_mbhc_jack_report(mbhc, &mbhc->button_jack,
			...
			if (mbhc->in_swch_irq_handler) {
				pr_info("[LGE MBHC] %s: Switch irq kicked in, ignore\n",
					__func__);
			} else {
				pr_debug("%s: Reporting btn press\n",
					 __func__);
				pr_info("[LGE MBHC] Headset Button : %s is pressed.\n",
							mbhc->buttons_pressed==0x800000?"Volume Down":
							button_name[(mbhc->buttons_pressed>>24)*-1/2+2]);
				...
				pr_debug("%s: Reporting btn release\n",
					 __func__);
				pr_info("[LGE MBHC] Headset Button : %s is released.\n",
							mbhc->buttons_pressed==0x800000?"Volume Down":
							button_name[(mbhc->buttons_pressed>>24)*-1/2+2]);
				wcd_mbhc_jack_report(mbhc,
						&mbhc->button_jack,
...
#ifndef FEATURE_AUDIO_LGE_4BUTTON_REMOVAL
		/* to support 4 button */
		ret = snd_jack_set_key(mbhc->button_jack.jack,
				       SND_JACK_BTN_1,
				       KEY_VOICECOMMAND);
