...
#if 1
  rc = of_property_read_string(escore->dev->of_node, "qcom,clk-src", &escore->clk_src_name);
	printk("%s  rc = 0x%x %s \n",__func__,rc,escore->clk_src_name);
	//if(strcmp(escore->clk_src_name, "BBCLK2"))
	{
		escore->s_clk = clk_get(&i2c->dev, "ref_clk");
	   //rc = clk_prepare_enable(escore->s_clk);
		//printk("%s  set clk rc = 0x%x\n",__func__,rc);
	}
