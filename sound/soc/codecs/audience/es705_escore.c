...
int escore_opalum_set_path(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	int value = ucontrol->value.enumerated.item[0];
	int rc = 0;
	dev_dbg(escore_priv.dev, "%s(): Opalum Set Path to %s\n",
		__func__, (value) ? "Voice" : "Media");

	escore_priv.es_opalum_path = value;
	/* 0 = media, 1 = voice 2=calibreate 3=dolbymedia*/
	switch(value) {
		case OPALUM_PATH_MEDIA:
			rc = escore_write_locked(NULL, ES705_OPALUM_PATH,
			OPALUM_PATH_MEDIA);
			break;
		case OPALUM_PATH_VOICE:
			rc = escore_write_locked(NULL, ES705_OPALUM_PATH,
			OPALUM_PATH_VOICE);
			break;
		case OPALUM_PATH_CALIBRATE:
			rc = escore_write_locked(NULL, ES705_OPALUM_PATH,
			OPALUM_PATH_CALIBRATE);
			break;
		case OPALUM_PATH_DOLBYMEDIA:
			rc = escore_write_locked(NULL, ES705_OPALUM_PATH,
			OPALUM_PATH_DOLBYMEDIA);
			break;
	}
	return rc;
}

int escore_opalum_get_path(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.enumerated.item[0] = escore_priv.es_opalum_path;
...
static const char * const es705_opalum_path_texts[] = {
	"media", "voice", "calibrate", "dolbymedia",
};
static const struct soc_enum es705_opalum_path_enum =
	SOC_ENUM_SINGLE(SND_SOC_NOPM, 0,
			ARRAY_SIZE(es705_opalum_path_texts),
			es705_opalum_path_texts);
...
static struct snd_kcontrol_new es705_digital_ext_snd_controls[] = {
	...
	SOC_ENUM_EXT("Opalum path", es705_opalum_path_enum,
			   escore_opalum_get_path,
			   escore_opalum_set_path),
...
/* API Interrupt registration */
	...
	of_property_read_u32(dev->of_node, "qcom,enable_hs_uart_intf", &val);
	if (val == 1)
		pdata->enable_hs_uart_intf = true;
	else
		pdata->enable_hs_uart_intf = false;

	if (of_property_read_u32(dev->of_node, "qcom,gpio_b_irq_type", &val)) {
...
int es705_enable_ext_clk(int i)
{
	int rc;
	printk(" %s i %d \n",__func__,i);
	if(i)
		rc = clk_prepare_enable(escore_priv.s_clk);
	else
		clk_disable_unprepare(escore_priv.s_clk);
