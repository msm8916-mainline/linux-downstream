static ssize_t tfa9897_reg_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    g_tfa9897_sys->client1->addr = 0x34;
    g_tfa9897_sys->client2->addr = 0x36;

    reg = 0x03;
    sprintf(buf, "%s chip addr=0x%02x [reg 0x%02x] = 0x%02x\r\n", buf,
            g_tfa9897_sys->client1->addr,  reg, tfa9897_read(g_tfa9897_sys->client1, reg));

    sprintf(buf, "%s chip addr=0x%02x [reg 0x%02x] = 0x%02x\r\n", buf,
            g_tfa9897_sys->client2->addr,  reg, tfa9897_read(g_tfa9897_sys->client2, reg));
    ...
    g_tfa9897_sys->client1->addr = 0x38;
    g_tfa9897_sys->client2->addr = 0x40;
        ...
        const char *buf, size_t size)
{
     ...
     * Bruce FIXME one2all
     * */
    tfa9897_write_word(g_tfa9897_sys->client1, reg, data);
    tfa9897_write_word(g_tfa9897_sys->client2, reg, data);
...
static DEVICE_ATTR(tfa9897_reg, 0666, tfa9897_reg_show, tfa9897_reg_store);
...
void enable_top_amp(int on)
{
    ...
    g_tfa9897_sys->client1->addr = 0x34;
    old = tfa9897_read(g_tfa9897_sys->client1, 0x09);
    new = (old & ~(1 << 3)) | (on<<3);
    g_tfa9897_sys->client1->addr = 0x38;

    printk("old=0x%x new=0x%x\n", old, new);

    tfa9897_trans_word(g_tfa9897_sys->client1, 0x09, new);
}
...
void enable_btm_amp(int on)
{
    ...
    g_tfa9897_sys->client2->addr = 0x36;
    old = tfa9897_read(g_tfa9897_sys->client2, 0x09);
    new = (old & ~(1 << 3)) | (on<<3);
    g_tfa9897_sys->client2->addr = 0x40;

    printk("old=0x%x new=0x%x\n", old, new);

    tfa9897_trans_word(g_tfa9897_sys->client2, 0x09, new);
}
...
static DEVICE_ATTR(stereo_mono_mode, S_IRUGO | S_IWUGO, tfa9897_sm_show, tfa9897_sm_store);


int tfa9897_debug_init(struct i2c_client *i2c)
{
    int rc = 0;

    rc = device_create_file(&(i2c->dev), &dev_attr_tfa9897_reg);
    if (rc) {
