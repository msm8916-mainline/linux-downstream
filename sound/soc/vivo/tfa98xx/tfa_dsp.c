...
static int tfa9890_specific(struct tfa98xx *tfa98xx)
{
	...
	snd_soc_write(codec, 0x47, 0x7BE1);
...
#ifdef MTP_BACKUP
/************************************************
	This is for backup mtp value to store in /data/SmartPAMtp.txt
	when device first bootup, after that, if it detect that the file is exist will
	no need to backup again. To do this is to pick out the ic which
	mtp is damaged.
*************************************************/
static int tfa98xx_StoreMtpSettings(struct tfa98xx *tfa98xx, const char* fileName)
{
	struct snd_soc_codec *codec = tfa98xx->codec;
	struct file *fp;
	mm_segment_t fs;
	...
	memset(buf,0,500);
	 ...
	 if(IS_ERR(fp)){
	 	//no such file.
	 	//tfa98xx->mtp_backup=0;//shijianxing :need init in probe
	 }else{
	 	//already exist.
	 	filp_close(fp,NULL);
	 	tfa98xx->mtp_backup=1;
		return 0;
	 }
	
       err = snd_soc_write(codec, 0x40, 0x5a6b);
       if (err){
              printk("0x40 write err is %d\n", err);
		return err;//shijianxing:delete SmartPAMtp.txt file
	}
	
	readValue = snd_soc_read(codec, 0x81);
	if(readValue < 0){
		goto err;
	
	}
       count+=sprintf(buf+count, "0x81:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x82);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x82:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x83);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x83:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x84);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x84:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x85);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x85:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x86);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x86:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x87);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x87:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x8a);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x8a:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x8b);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x8b:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x8c);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x8c:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x8d);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x8d:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x8e);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x8e:%d\n", readValue);

        readValue = snd_soc_read(codec, 0x8f);
	if(readValue < 0){
		goto err;
	
	}
	count+=sprintf(buf+count, "0x8f:%d\n", readValue);
	printk("%s: count=%d.\n",__func__,count);	
	...
	printk("%s: success to open file.\n",__func__);
	 ...
	 vfs_write(fp,buf,count,&pos);        
        filp_close(fp,NULL);
	 set_fs(fs);
	 err = snd_soc_write(codec, 0x40, 0x0);	
        if (err)
            printk("0x40 write err is %d\n", err);
	 tfa98xx->mtp_backup = 1;//shijianxing
	 return 0;
err:
	//shijianxing :need delete SmartPAMtp.txt file
	printk("%s: count=%d.\n",__func__,count);
	err = snd_soc_write(codec, 0x40, 0x0);	
       if (err)
    		printk("0x40 write err is %d\n", err);	
...
#ifdef MTP_BACKUP
	if(!tfa98xx->mtp_backup){
		tfa98xx_StoreMtpSettings(tfa98xx, "/data/SmartPAMtp.txt");
		/*  TODO  */
		/*  TODO: if return error, then need to delete /data/MTP.txt  from memory  */
	//	tfa98xx->mtp_backup = 1;//shijianxing add
	}
#endif
...
int tfa98xx_is_pwdn(struct tfa98xx *tfa98xx)
{
	...
	pr_info("tfa98xx System Control Reg:0x%x Status Reg:0x%x\n",
				snd_soc_read(codec, TFA98XX_SYS_CTRL),status);

	return (status & TFA98XX_STATUSREG_AREFS) == 0;
}
int tfa98xx_is_configured(struct tfa98xx *tfa98xx)
{
	struct snd_soc_codec *codec = tfa98xx->codec;
	u16 reg_val;
	/* check if SBSL bit is clear by looking at DSP */
	reg_val = snd_soc_read(codec, TFA98XX_SYS_CTRL);

	pr_info("tfa98xx system control reg 0x%x\n", reg_val);

	return (reg_val & TFA98XX_SYS_CTRL_SBSL) != 0;
}
					 ...
					 const u8 *data)
{
	...
	if (ret)
		return ret;
	if(tfa98xx->rev != REV_TFA9890){//shijianxing M:TFA9890 can't support DRC feature
		ret = tfa98xx_dsp_support_drc(tfa98xx, &has_drc);
		if (ret)
			return ret;
...
int tfa98xx_dsp_get_calibration_impedance(struct tfa98xx *tfa98xx, u32 *re25)
{
	...
	*re25 = TO_FIXED(data[0])*100 / (1 << (23 - SPKRBST_TEMPERATURE_EXP));
...
int tfaRunSpeakerBoost(struct tfa98xx *tfa98xx, int force)
{
	...
	//tfa98xx_unmute(tfa98xx);//shijianxing M
...
int tfa98xx_dsp_start(struct tfa98xx *tfa98xx, int next_profile, int vstep)
{
	...
	pr_info("%s Read System Ctrl Reg value 0x%x\n", __func__,
			snd_soc_read(codec, TFA98XX_SYS_CTRL));
	pr_info("%s Read Audio Reg value 0x%x\n", __func__,
			snd_soc_read(codec, TFA98XX_AUDIOREG));
	pr_info("%s Read Status Reg value 0x%x\n", __func__,
	...
	if (forcecoldboot || tfa98xx_is_coldboot(tfa98xx)) {
		...
		if (next_profile != active_profile) {
			...
			if(!tfa98xx_is_configured(tfa98xx)){
				pr_err("tfa98xx SBSL bit error for recover dsp.\n");
				/* enable I2S output */
				ret = tfa98xx_aec_output(tfa98xx, 1);
				if (ret)
					return ret;
				ret = tfaRunSpeakerBoost(tfa98xx, 1);

			}
			else if (tfa98xx_is_pwdn(tfa98xx)) {
				/* enable I2S output */
				ret = tfa98xx_aec_output(tfa98xx, 1);
				if (ret)
					return ret;
