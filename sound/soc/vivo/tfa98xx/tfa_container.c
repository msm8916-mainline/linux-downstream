...
char *tfa98xx_filename = "tfa98xx/tfa98xx.cnt";

module_param(tfa98xx_filename, charp, 0);
...
int tfa98xx_cnt_loadfile(struct tfa98xx *tfa98xx, int index)
{
	...
	ret = request_firmware(&container, tfa98xx_filename, codec->dev);
	if (ret) {
		pr_err("Failed to read %s", tfa98xx_filename);
		return ret;
	}

	pr_err("loaded %s - size: %d\n", tfa98xx_filename, container ? (int)container->size : 0);
