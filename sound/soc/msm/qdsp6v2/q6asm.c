...
int send_asm_custom_topology(struct audio_client *ac)
{
	...
	if (!result) {
		...
		pr_debug("%s: Set topologies failed after timedout payload = 0x%pa\n",
			__func__, &cal_block->cal_data.paddr);
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] send_asm_custom_topology error");
#endif
		result = -ETIMEDOUT;
		goto unmap;
	}
		...
		uint32_t format, uint16_t bits_per_sample)
{
	...
	if (!rc) {
		pr_err("%s: timeout. waited for open read\n",
				__func__);
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio]__q6asm_open_read error");
#endif
		goto fail_cmd;
	}
				...
				uint32_t passthrough_flag)
{
	...
	if (!rc) {
		pr_err("%s: timeout. waited for OPEN_WRITE_COMPR rc[%d]\n",
			__func__, rc);
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] q6asm_open_write_compressed error");
#endif
		rc = -ETIMEDOUT;
		goto fail_cmd;
	}
...
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] __q6sam_open_write error");
#endif
		goto fail_cmd;
	}
	if (atomic_read(&ac->cmd_state) < 0) {
				   ...
				   bool overwrite_topology, int topology)
{
	...
	if (!rc) {
		pr_err("%s: timeout. waited for open read-write\n",
				__func__);
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] __q6asm_open_read_write error");
#endif
		goto fail_cmd;
	}
	if (atomic_read(&ac->cmd_state) < 0) {
...
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] __q6asm_enc_cfg_blk_pcm error");
#endif
		goto fail_cmd;
	}
	if (atomic_read(&ac->cmd_state) < 0) {
...
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] q6asm_enable_sbrps error");
#endif
		goto fail_cmd;
	}
	if (atomic_read(&ac->cmd_state) < 0) {
				...
				uint32_t bufsz, uint32_t bufcnt)
{
	...
	if (!rc) {
		...
		rc = -EINVAL;
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] q6asm_memory_map error");
#endif
		kfree(buffer_node);
		goto fail_cmd;
	}
...
int q6asm_memory_unmap(struct audio_client *ac, phys_addr_t buf_add, int dir)
{
	...
	if (!rc) {
		...
		rc = -ETIMEDOUT;
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] q6asm_memory_unmap error");
#endif
		goto fail_cmd;
				...
				bool is_contiguous)
{
	...
	if (!rc) {
		...
		kfree(buffer_node);
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] q6asm_memory_map_regions error");
#endif
		goto fail_cmd;
	}
	if (atomic_read(&ac->mem_state) < 0) {
...
static int q6asm_memory_unmap_regions(struct audio_client *ac, int dir)
{
	...
	if (!rc) {
		...
		rc = -ETIMEDOUT;
#ifdef CONFIG_AUDIO_DEBUG
		panic("[Audio] q6asm_memory_unmap_regions error");
#endif
		goto fail_cmd;
	} else if (atomic_read(&ac->mem_state) < 0) {
		pr_err("%s: DSP returned error[%d]\n",
