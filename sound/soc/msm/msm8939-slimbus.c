...
//#define HS_STARTWORK_TIMEOUT		1000
...
static int msm8939_spk_control = 0;
static int vdd_spkr_gpio = -1;
static int clk_users;

static int msm8939_ext_spk_pamp;
...
module_param(mbhc_ready_state, int,
			S_IRUGO | S_IWUSR | S_IWGRP); // var/type/permission
MODULE_PARM_DESC(mbhc_ready_state, "let self-d know wheteher mbhc is in ready state 1 or not 0");
#endif
...
static void msm8939_ext_spk_power_amp_on(u32 spk)
{
...
if (!flag_mute_spk_for_swirrc){
	...
	if (spk & (LO_1_SPK_AMP | LO_2_SPK_AMP)) {
		...
		if (gpio_is_valid(ext_spk_amp_gpio)) {
			...
			msm8939_ext_spk_pamp |= spk;
			if ((msm8939_ext_spk_pamp & LO_1_SPK_AMP)&&(msm8939_ext_spk_pamp & LO_2_SPK_AMP)){
				if (ext_spk_amp_gpio >= 0) {
					pr_debug("%s  enable power", __func__);
					//msm8939_ext_spk_power_amp_enable(1);
				}
			}
		}
#endif
...
static void msm8939_ext_spk_power_amp_off(u32 spk)
{
	...
	if (gpio_is_valid(ext_spk_amp_gpio)) {
		pr_debug("%s:Disable left and right speakers case spk = 0x%x\n",
			__func__, spk);
		msm8939_ext_spk_pamp &= ~spk;
		if (!msm8939_ext_spk_pamp) {
			if (ext_spk_amp_gpio >= 0) {
				pr_debug("%s  disable power", __func__);
				//msm8939_ext_spk_power_amp_enable(0);
			}
			msm8939_ext_spk_pamp = 0;
		}
	}
#endif
	} else  {
...
static int msm8939_ext_spkramp_event(struct snd_soc_dapm_widget *w,
			struct snd_kcontrol *k, int event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		if (!strncmp(w->name, "Lineout_1 amp", 14))
			msm8939_ext_spk_power_amp_on(LO_1_SPK_AMP);
		else if (!strncmp(w->name, "Lineout_2 amp", 14))
			msm8939_ext_spk_power_amp_on(LO_2_SPK_AMP);
		else {
		...
		if (!strncmp(w->name, "Lineout_1 amp", 14))
			msm8939_ext_spk_power_amp_off(LO_1_SPK_AMP);
		else if (!strncmp(w->name, "Lineout_2 amp", 14))
			msm8939_ext_spk_power_amp_off(LO_2_SPK_AMP);
		else {
			pr_err("%s() Invalid Speaker Widget = %s\n",
				__func__, w->name);
			return -EINVAL;
		       ...
		       struct snd_ctl_elem_value *ucontrol)
{
	...
	ucontrol->value.integer.value[0] = (msm8939_ext_spk_pamp)? 1: 0;
	return 0;
}
