...
static int msm_q6_enable_mi2s_clocks(bool enable)
{
	...
	if(enable) {
		...
		if (IS_ERR_VALUE(rc)){
			pr_err("[Liu]%s: fail to open AFE port\n", __func__);
			return -EINVAL;
		}
	}
	else {
         ...
         struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("[Liu]+%s: enable=%d\n",
		__func__, enable);

	switch (enable) {
	case 1:
		ret = quat_mi2s_sclk_enable(1);
		break;
	case 0:
		ret = quat_mi2s_sclk_enable(0);
		break;
	default:
		pr_err("%s: Unexpected input value\n", __func__);
		break;
...
static const char *const quat_sclk_text[] = {"DISABLE", "ENABLE"};
static const struct soc_enum quat_sclk_enum[] = {
	SOC_ENUM_SINGLE_EXT(2, quat_sclk_text),
};
...
static const struct snd_kcontrol_new msm_snd_controls[] = {
	...
	SOC_ENUM_EXT("QUAT_MI2S SCLK", quat_sclk_enum[0],
