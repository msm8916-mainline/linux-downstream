...
/* 2015.06.12 enable primary and quat i2s start*/
static atomic_t quat_mi2s_clk_ref;
/* 2015.06.12 enable primary and quat i2s end*/
...
static struct snd_soc_dai_link msm8x16_dai[] = {
	...
	},
	{
		...
		//.codec_dai_name = "snd-soc-dummy-dai",
		//.codec_name = "snd-soc-dummy",
		.codec_name = "tas2552-codec.1-0040",
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (!strcmp(ptr, "external")) {
		...
		pr_debug("%s: ext_pa = %d ext_pa_str %s \n", __func__, pdata->ext_pa,ext_pa_str);
		pinctrl = devm_pinctrl_get(&pdev->dev);
		if (IS_ERR(pinctrl)) {
			pr_err("%s: Unable to get pinctrl handle\n",
					__func__);
